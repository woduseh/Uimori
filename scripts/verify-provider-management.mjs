...
async function main() {
  ...
  const runId = `provider-management-${newId()}`,
  ...
  const requiredScreenshots = [
    ...
    ...[0, 1, 2, 3, 4].map((index) => `provider-parameters-${index}-mobile.png`),
  ...
  const summary = {
    ...
    limitations: [
      'Synthetic loopback provider only; no live provider, paid generation, user DB or deployment.',
  ...
  try {
    ...
    if (final.buildId !== summary.identity.buildId || (await fingerprint()).hash !== initial.hash)
      throw new Error('Source/build changed during provider management UI verification');
    ...
    const errors = [],
      terminated = [];
    if (fixture) {
      summary.fixture = fixture.stats();
      if (summary.fixture.errors.length) errors.push(...summary.fixture.errors);
      try {
        await fixture.close();
      } catch (error) {
