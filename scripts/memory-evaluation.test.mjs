...
const oracle = () =>
  corpus.questions.map((q) => ({
    id: q.id,
    text: q.expected.join('; '),
    evidence: structuredClone(q.evidence),
  }));
...
test('all questions require their distinct gold citations; duplicate citation cannot cover another source', () => {
  const answers = oracle();
  const a = answers.find((a) => a.id === 'uncertainty');
  a.evidence = [a.evidence[0], a.evidence[0]];
...
test('preflight is offline and blocks absent authorization, fake token scope and insufficient call budgets', () => {
  ...
  const config = {
    ...
    maxCalls: 1000,
    maxUsd: 1000,
    maxInputTokensPerCall: 200000,
    maxOutputTokensPerCall: 1000,
    tokenCount: {
      corpusHash: corpus.corpusHash,
      model: 'synthetic-model',
      tokens: 180000,
      method: 'test attestation only',
    },
    pricing: {
  ...
  const valid = preflight(corpus, config);
  assert.equal(valid.status, 'READY_FOR_REVIEW');
  assert.equal(valid.executeSupported, false);
  assert.equal(valid.networkCalls, 0);
  assert.equal(valid.costUsd, null);
  for (const patch of [
    ...
    { tokenCount: { ...config.tokenCount, model: 'other-model' } },
    { tokenCount: { ...config.tokenCount, tokens: 200001 } },
    { pricing: null },
  ])
    assert.equal(preflight(corpus, { ...config, ...patch }).status, 'BLOCKED');
});
