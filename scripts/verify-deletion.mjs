...
let environmentBlocked = false,
  fixture;
const cancel = (signal) => {
  failures.push(`${signal}: redesign UI cancelled`);
...
try {
  ...
  const env = {
    ...
    NR_DB: path.join(runtime, 'deletion.sqlite'),
    NR_PORT: '0',
    NR_INSTANCE: runId,
  ...
  for (const id of ['DEL01', 'DEL02', 'DEL03', 'DEL04', 'DEL05', 'DEL06', 'ORG01'])
  ...
  if (
    ...
    throw new Error('Build/source identity changed during redesign UI verification');
