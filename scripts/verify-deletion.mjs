...
if (process.argv.length > 2) throw new Error('verify-deletion accepts no arguments');
const runId = `deletion-${newId()}`, directory = path.join(artifactRoot, runId), runtime = path.join(directory, 'runtime');
await mkdir(runtime, { recursive: true });
const children = new Set(), failures = [];
const summary = { runId, status: 'FAIL', startedAt: new Date().toISOString(), scope: 'Deletion workflows and related synthetic browser regression', failures, limitations: [
...
let environmentBlocked = false, fixture;
const cancel = signal => { failures.push(`${signal}: redesign UI cancelled`); for (const child of children) void killOwned(child).catch(error => failures.push(error.message)); };
...
try {
  ...
  const env = { NR_REGISTRATION_FIXTURE_URL: fixture.url, NR_DB: path.join(runtime, 'deletion.sqlite'), NR_PORT: '0', NR_INSTANCE: runId, NR_BUILD_ID: identity.buildId, NR_TEST_MODE: '1', NR_ACCESS_TOKEN: '', NR_PROVIDER_ORIGINS: new URL(fixture.url).origin, NR_ARTIFACT_DIR: directory, NR_BROWSER_OUTPUT: path.join(directory, 'browser'), NR_SECRET_CANARY: canary, NR_BROWSER_PATH: browser, TEMP: temp, TMP: temp };
  ...
  const result = await command(['node_modules/@playwright/test/cli.js', 'test', 'tests/deletion-browser.spec.ts', 'tests/organization-browser.spec.ts', 'tests/prompt-unified-browser.spec.ts', '--reporter=json'], { env: { ...env, PLAYWRIGHT_JSON_OUTPUT_NAME: reporter }, timeout: 600_000, log: path.join(directory, 'playwright.log'), children });
  ...
  for (const id of ['DEL01','DEL02','DEL03','DEL04','DEL05','DEL06','ORG01']) if (!summary.report.tests.some(test => test.status === 'passed' && test.title.includes(id))) throw new Error(`Missing ${id} evidence`);
  if ((await assertBuild()).buildId !== identity.buildId || (await fingerprint()).hash !== before.hash) throw new Error('Build/source identity changed during redesign UI verification');
  ...
  const cleanupErrors = [];
  if (fixture) try { summary.registrationFixture = fixture.stats(); await fixture.close(); } catch (error) { cleanupErrors.push(error.message); }
