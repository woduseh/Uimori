...
for (let index = 2; index < process.argv.length; index++) {
  ...
  if (arg === '--help') { console.log('node scripts/verify-live-retry.mjs --source <closed live evidence directory> --job <translation job ID> [--preflight | --execute]\nDefault is metadata/read-only preflight. --execute copies the closed evidence DB, preserves its attempts and sources, then submits one explicit failed-chunk retry. Per-run call, timeout and output limits remain in force. NR_VERTEX_REQUEST_TIER=flex is required. Credentials remain environment references.'); process.exit(0); }
...
let build; let baseline; let sourceSummary; let sourceOwner;
const capture = file => {
  ...
  try {
    ...
    return { job, source, run, chunks, attempts, activeRuns, activeJobs,
      requestCount: attempts.filter(item => JSON.parse(item.request).protocol === 'vertex-gemini-v1').length };
  } finally { db.close(); }
};
...
try {
  ...
  summary.sourceDbSha256Before = await fileHash(sourceDb); summary.sourceWalBefore = await walIdentity(sourceDb + '-wal'); summary.sourceShmBefore = await walIdentity(sourceDb + '-shm'); summary.beforeRequestCount = baseline.requestCount;
...
async function executeRetry() {
  ...
  try {
    ...
    check(copied.requestCount === baseline.requestCount && JSON.stringify(copied.attempts) === JSON.stringify(baseline.attempts), 'LIVE_RETRY_COPY_LEDGER_MISMATCH');
    ...
    while (true) {
      ...
      if (Date.now() - lastLog > 30_000) { await log('retry.waiting', { totalAdmittedRequests: current.requestCount, additionalRequests: fresh.length, jobStatus: job.status }); lastLog = Date.now(); }
      await delay(100);
    }
  ...
  } catch (error) { summary.failures.push(code(error)); summary.status = 'FAIL'; }
  finally {
    const cleanupErrors = [];
    if (child) try { await killOwned(child); } catch { cleanupErrors.push('LIVE_RETRY_SERVER_CLEANUP_FAILED'); }
    try {
      const final = capture(dbPath); const fresh = final.attempts.slice(baseline.attempts.length);
      summary.additionalRequestCount = fresh.length; summary.totalRequestCount = final.requestCount;
      summary.newAttempts = fresh.map(item => ({ id: item.id, jobId: item.job_id, role: item.role, modelId: item.model_id, status: item.status, inputTokens: item.input_tokens, outputTokens: item.output_tokens,
        costUsd: item.cost_usd, rawUsage: item.raw_usage === null ? null : JSON.parse(item.raw_usage), priceRevision: item.price_revision, error: item.error }));
