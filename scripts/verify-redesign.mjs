...
if (process.argv.length > 2) throw new Error('verify-redesign accepts no arguments');
const runId = `redesign-${newId()}`, directory = path.join(artifactRoot, runId), runtime = path.join(directory, 'runtime');
await mkdir(runtime, { recursive: true });
const children = new Set(), failures = [];
const summary = { runId, status: 'FAIL', startedAt: new Date().toISOString(), scope: 'Bot workspace and package redesign plus existing synthetic browser regression', failures, limitations: [
...
try {
  ...
  const result = await command(['node_modules/@playwright/test/cli.js', 'test', '--reporter=json'], { env: { ...env, PLAYWRIGHT_JSON_OUTPUT_NAME: reporter }, timeout: 600_000, log: path.join(directory, 'playwright.log'), children });
  ...
  for (const id of ['ORG01', 'PKUI01', 'PRUI01', 'NUI01', 'NUI02', 'NUI03', 'PMUI05', 'PMUI06', 'BUI01', 'BUI02', 'BUI03', 'BUI04', 'PKUI02']) if (!summary.report.tests.some(test => test.status === 'passed' && test.title.includes(id))) throw new Error(`Missing ${id} evidence`);
