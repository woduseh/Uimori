...
import { root, artifactRoot, newId, json, assertBuild, fingerprint, browserPath, command, readReport, requireCommand, killOwned, removeOwned, artifactScan, canary } from './lib.mjs';

if (process.argv.length > 2) throw new Error('verify-self-host accepts no arguments');
const runId = `self-host-${newId()}`, directory = path.join(artifactRoot, runId), runtime = path.join(directory, 'runtime');
await mkdir(runtime, { recursive: true });
const children = new Set(), failures = [];
const summary = { runId, status: 'FAIL', startedAt: new Date().toISOString(), scope: 'Personal self-host over an actual loopback HTTPS reverse proxy and independent desktop/mobile Chromium contexts', failures, limitations: [
...
let environmentBlocked = false, proxy;
const cancel = signal => { failures.push(`${signal}: self-host smoke cancelled`); for (const child of children) void killOwned(child).catch(error => failures.push(error.message)); };
...
async function startProxy() {
  ...
  let target;
  const server = createHttpsServer({ key: await readFile(path.join(tlsDirectory, 'test-only-key.pem')), cert: await readFile(path.join(tlsDirectory, 'test-only-cert.pem')) }, (request, response) => {
    stats.requests++;
    if (!target) { response.writeHead(503).end(); return; }
    const upstream = httpRequest(new URL(request.url ?? '/', target), { method: request.method, headers: { ...request.headers, connection: 'close' } }, incoming => {
      response.writeHead(incoming.statusCode ?? 502, incoming.headers); response.flushHeaders();
      if (String(incoming.headers['content-type']).startsWith('text/event-stream')) {
        stats.eventStreams++; incoming.on('data', data => { stats.eventBytes += data.length; });
      }
    ...
    upstream.on('error', () => { if (response.destroyed) return; stats.upstreamErrors++; if (!response.headersSent) response.writeHead(502); response.end(); });
  ...
  if (!address || typeof address === 'string') throw new Error('HTTPS proxy did not bind an isolated port');
  return { origin: `https://127.0.0.1:${address.port}`, setTarget: value => { target = value; }, stats: () => ({ ...stats }), close: async () => {
...
function secureRequest(origin, pathname, { method = 'GET', headers = {}, body } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const request = httpsRequest(new URL(pathname, origin), { method, rejectUnauthorized: false, headers: { ...headers, ...(encoded === undefined ? {} : { 'content-type': 'application/json', 'content-length': Buffer.byteLength(encoded) }) } }, response => {
      const parts = []; response.on('data', part => parts.push(part)); response.once('error', reject);
      response.once('end', () => resolve({ status: response.statusCode, headers: response.headers, body: Buffer.concat(parts).toString('utf8') }));
    });
    request.setTimeout(5000, () => request.destroy(new Error('HTTPS identity request timeout'))); request.once('error', reject); request.end(encoded);
  });
}

async function startApp(env) {
  const child = spawn(process.execPath, ['dist/server/index.js'], { cwd: root, env: { ...process.env, ...env }, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
  children.add(child);
  let output = '', pending = '';
  try {
    const ready = await new Promise((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('App ready timeout (15000ms)')), 15000);
      const fail = value => { clearTimeout(timer); reject(new Error(`App exited before ready (${value}): ${output.slice(-1000)}`)); };
      ...
      child.stdout.on('data', value => {
        ...
        for (const line of lines) {
          ...
          if (event.event === 'ready') { clearTimeout(timer); child.removeListener('exit', fail); child.removeListener('error', fail); resolve(event); }
        }
      });
    });
    if (!/^http:\/\/127\.0\.0\.1:\d+$/u.test(ready.url)) throw new Error('Unexpected internal ready URL');
    return { child, ready, log: () => writeFile(path.join(directory, 'server.log'), output) };
  } catch (error) { await writeFile(path.join(directory, 'server.log'), output); throw error; }
}
...
try {
  ...
  const env = { NR_DB: path.join(runtime, 'self-host.sqlite'), NR_PORT: '0', NR_HOST: '127.0.0.1', NR_INSTANCE: runId, NR_BUILD_ID: identity.buildId, NR_TEST_MODE: '', NR_PUBLIC_ORIGIN: proxy.origin, NR_ACCESS_TOKEN: randomBytes(32).toString('hex'), NR_ACCESS_TOKEN_FILE: '', NR_PROVIDER_ORIGINS: '', NR_ARTIFACT_DIR: directory, NR_BROWSER_OUTPUT: path.join(directory, 'browser'), NR_SECRET_CANARY: canary, NR_BROWSER_PATH: browser, NR_SELF_HOST_BROWSER: '1', NR_BASE_URL: proxy.origin, TEMP: temp, TMP: temp };
  server = await startApp(env); proxy.setTarget(server.ready.url); summary.server = server.ready; summary.publicOrigin = proxy.origin;
  ...
  const login = await secureRequest(proxy.origin, '/api/session', { method: 'POST', headers: { Origin: proxy.origin }, body: { token: env.NR_ACCESS_TOKEN } });
  ...
  for (const [key, expected] of Object.entries({ buildId: env.NR_BUILD_ID, instanceId: env.NR_INSTANCE, dbPath: env.NR_DB })) if (actual[key] !== expected) throw new Error(`HTTPS health identity mismatch: ${key}`);
  const logout = await secureRequest(proxy.origin, '/api/session', { method: 'DELETE', headers: { Origin: proxy.origin, Cookie: cookie }, body: {} });
  ...
  const result = await command(['node_modules/@playwright/test/cli.js', 'test', 'tests/self-host-browser.spec.ts', '--reporter=json'], { env: { ...env, PLAYWRIGHT_JSON_OUTPUT_NAME: reporter }, timeout: 180_000, log: path.join(directory, 'playwright.log'), children });
  ...
  for (const id of ['SHUI01', 'SHUI02']) if (!summary.report.tests.some(test => test.status === 'passed' && test.title.includes(id))) throw new Error(`Missing ${id} evidence`);
  if (!proxy.stats().eventStreams || !proxy.stats().eventBytes) throw new Error('No actual HTTPS event stream bytes observed');
  if ((await assertBuild()).buildId !== identity.buildId || (await fingerprint()).hash !== before.hash) throw new Error('Build/source identity changed during self-host verification');
  ...
  const cleanupErrors = [];
  if (proxy) try { summary.proxy = proxy.stats(); await proxy.close(); } catch (error) { cleanupErrors.push(error.message); }
  for (const child of children) try { await killOwned(child); } catch (error) { cleanupErrors.push(error.message); }
  if (server) try { await server.log(); } catch (error) { cleanupErrors.push(error.message); }
