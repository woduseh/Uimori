...
const directory=path.join(artifactRoot,`composer-more-${newId()}`), children=new Set();
const summary={status:'FAIL',scope:'Synthetic composer more menu, one-shot lore exclusion and uncertain request recovery',limitations:['Fresh isolated SQLite with synthetic prompts; no provider calls or personal data.'],cleanup:'NOT_RUN'};
try {
  ...
  const result=await command(['node_modules/@playwright/test/cli.js','test','tests/lore-context-browser.spec.ts','tests/run-retry-browser.spec.ts','--grep','LCUI0[234]|failed request','--reporter=json'],{env:{...env,NR_BASE_URL:server.ready.url,PLAYWRIGHT_JSON_OUTPUT_NAME:report},children,log:path.join(directory,'playwright.log')});
  summary.report=await readReport(report,since,'playwright');
  summary.status=result.code===0&&summary.report.passed===4&&summary.report.executed===4?'PASS':'FAIL';
