...
const directory=path.join(artifactRoot,`run-retry-${newId()}`), children=new Set();
const summary={status:'FAIL',scope:'Synthetic failed-card UI with real fixture admission and lost-response recovery',limitations:['Failure card injected into reader response; no live provider calls.'],cleanup:'NOT_RUN'};
try {
  ...
  const result=await command(['node_modules/@playwright/test/cli.js','test','tests/run-retry-browser.spec.ts','--reporter=json'],{env:{...env,NR_BASE_URL:server.ready.url,PLAYWRIGHT_JSON_OUTPUT_NAME:report},children,log:path.join(directory,'playwright.log')});
