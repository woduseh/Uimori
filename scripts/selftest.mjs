...
async function childCase(name, directory) {
  ...
  async function run(args, timeout = 20000) {
    const result = await command(args, {
      timeout,
      log: path.join(directory, `child-${commands.length}.log`),
    });
  ...
  try {
    if (['assertion', 'zero', 'skip', 'stale-report'].includes(name)) {
      ...
      await writeFile(
        config,
        `export default { test: { include: [${JSON.stringify(fixture.replaceAll('\\', '/'))}], fileParallelism: false } };\n`
      );
      const args = [
      ...
      if (name === 'assertion') {
        ...
        if (
          ...
          assertions.length !== 1 ||
          assertions[0].fullName !== 'required fixture' ||
          assertions[0].status !== 'failed' ||
          !assertions[0].failureMessages?.some((message) =>
            /AssertionError: expected 1 to be 2/.test(message)
          )
        )
          throw new Error(
            'Deliberate assertion fixture was not reached; startup/environment failure is not assertion evidence'
          );
        throw new Error('Injected assertion detected in fresh Vitest report');
      }
      requireCommand(result);
      if (name === 'stale-report') {
        const old = new Date(started - 60000);
        await utimes(reportFile, old, old);
      }
      await readReport(
        reportFile,
        started,
        'vitest',
        name === 'zero' ? /no-such-test-name/ : undefined
      );
      ...
      await assertHealth(`http://127.0.0.1:${server.address().port}`, {
      ...
      requireCommand(
        await run(['-e', 'console.error("original assertion failure");process.exitCode=1'])
      );
      ...
      const result = await run([
        'scripts/verify.mjs',
        name === 'unknown-case' ? '--case' : '--milestone',
        'nonexistent',
      ]);
    ...
    if (name === 'cleanup') {
      ...
      try {
        await rm(path.join(directory, 'missing-cleanup-target'), { force: false });
      } catch (error) {
        errors.push(`Cleanup: ${error.code}`);
      }
    }
  }
  const result = {
...
async function selftest(directory) {
  ...
  for (const [name, pattern] of Object.entries(expected)) {
    ...
    const result = await command(['scripts/selftest.mjs', '--child', name, '--output', childDir], {
      timeout: 30000,
      log: path.join(directory, `${name}.log`),
    });
    let inner;
    try {
      inner = JSON.parse(await readFile(path.join(childDir, 'inner-result.json'), 'utf8'));
    } catch {
      inner = { errors: ['inner result missing'] };
    }
    ...
    const detected =
      ...
      (name !== 'cleanup' ||
        (inner.errors.length === 2 && /^Cleanup: ENOENT/.test(inner.errors[1])));
    results.push({
      name,
      status: detected ? 'PASS' : 'FAIL',
      meaning: 'Expected inner failure detected',
      childExitCode: result.code,
      childResult: path.relative(directory, path.join(childDir, 'inner-result.json')),
      primaryError: primary,
      cleanupErrors: inner.errors?.slice(1) || [],
    });
  }
  const report = {
    status: results.every((result) => result.status === 'PASS') ? 'PASS' : 'FAIL',
    completedAt: new Date().toISOString(),
    scope: 'Verifier failure detection only; these are not product PASS tests.',
    results,
  };
  await json(path.join(directory, 'selftest.json'), report);
  console.log(JSON.stringify(report, null, 2));
  if (report.status !== 'PASS') process.exitCode = 1;
}
const args = process.argv.slice(2);
try {
  if (args[0] === '--child' && args[2] === '--output' && args.length === 4)
    await childCase(args[1], path.resolve(args[3]));
  else if (args.length === 0 || (args[0] === '--output' && args.length === 2))
    await selftest(
      args.length ? path.resolve(args[1]) : path.join(artifactRoot, `selftest-${newId()}`)
    );
