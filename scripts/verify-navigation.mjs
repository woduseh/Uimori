...
if (process.argv.length > 2) throw new Error('verify-navigation accepts no arguments');
const runId = `navigation-${newId()}`,
...
let environmentBlocked = false,
  fixture;
const cancel = (signal) => {
  failures.push(`${signal}: navigation UI cancelled`);
...
try {
  ...
  const env = {
    ...
    NR_DB: path.join(runtime, 'navigation.sqlite'),
    NR_PORT: '0',
    NR_INSTANCE: runId,
  ...
  for (const id of ['ORG01', 'ORG02', 'ORG03', 'ORG04', 'DEL03'])
  ...
  if (
    ...
    throw new Error('Build/source identity changed during navigation UI verification');
