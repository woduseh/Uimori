...
export async function doctor(directory = path.join(artifactRoot, `doctor-${newId()}`)) {
  ...
  try {
    if (Number(process.versions.node.split('.')[0]) !== 24)
      throw new Error('This checkout requires Node 24.x (node:sqlite).');
    ...
    try {
      ...
      await removeOwned(directory, temp);
      result.cleanup = {
        status: 'PASS',
        removed: [temp],
        ownedLocalServerClosed: true,
        browserClosed: true,
      };
    } catch (error) {
      result.cleanup = { status: 'FAIL', error: error.message };
      result.status = 'FAIL';
    }
    result.finishedAt = new Date().toISOString();
    await json(path.join(directory, 'doctor.json'), result);
  }
  return result;
}
