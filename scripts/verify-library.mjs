...
if (process.argv.length > 2) throw new Error('verify-library accepts no arguments');
const runId = `library-${newId()}`,
...
let environmentBlocked = false,
  fixture;
const cancel = (signal) => {
  failures.push(`${signal}: library UI cancelled`);
...
try {
  ...
  const env = {
    NR_REGISTRATION_FIXTURE_URL: fixture.url,
    NR_DB: path.join(runtime, 'library.sqlite'),
    NR_PORT: '0',
    NR_INSTANCE: runId,
    NR_BUILD_ID: identity.buildId,
    NR_TEST_MODE: '1',
    NR_ACCESS_TOKEN: '',
    NR_PROVIDER_ORIGINS: new URL(fixture.url).origin,
  ...
  const result = await command(
    [
      ...
      'library-images-browser.spec.ts',
      'package-features-browser.spec.ts',
      'shared-package-browser.spec.ts',
      'deletion-browser.spec.ts',
      '--reporter=json',
    ],
    {
  ...
  if (
    ...
    throw new Error('Build/source identity changed during library UI verification');
