...
async function main() {
  ...
  try {
    ...
    const result=await command(['node_modules/@playwright/test/cli.js','test','tests/loading-browser.spec.ts','--reporter=json'],{env:{...env,PLAYWRIGHT_JSON_OUTPUT_NAME:reportFile},timeout:180000,children,log:path.join(directory,'playwright.log')});
    ...
    const final=await assertBuild(); if(final.buildId!==summary.identity.buildId||(await fingerprint()).hash!==initial.hash) throw new Error('Source/build changed during loading UI verification');
