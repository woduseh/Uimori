...
if (process.argv.length > 2) throw new Error('verify-package-ui accepts no arguments');
const runId = `package-ui-${newId()}`,
...
const summary = {
  ...
  failures,
  limitations: [
    'Synthetic prompts, common packages and source text only; no private materials are copied into these tests.',
...
const cancel = (signal) => {
  failures.push(`${signal}: package UI cancelled`);
...
try {
  ...
  const env = {
    NR_DB: path.join(runtime, 'package.sqlite'),
    NR_PORT: '0',
    NR_INSTANCE: runId,
  ...
  if (
    ...
    throw new Error('Build/source identity changed during native UI verification');
