...
export function NewStory({ library, initialBot, initialFolder, onCreated }: { library: Library; initialBot?: Content; initialFolder?: ChatFolder; onCreated: (chat: Chat) => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const items = selected.map(key => [initialBot, ...library.contents].find(item => item && refValue(item) === key)).filter((item): item is Content => !!item);
    setPackageLoading(true);
    void Promise.all(items.map(async item => {
      const key = refValue(item);
      if (loadedContents[key]) return loadedContents[key];
      if (item === initialBot || !library.contentBodiesOmitted && (!item.hasPackage || item.package)) return item;
      return api<Content>(`/revisions/content/${item.id}/${item.revision}`);
    })).then(items => { if (current) setLoadedContents(previous => ({ ...previous, ...Object.fromEntries(items.map(item => [refValue(item), item])) })); }).catch(caught => { if (current) setError((caught as Error).message); }).finally(() => { if (current) setPackageLoading(false); });
  ...
  function captureSelection(): StorySelection {
    ...
    const selectedBot = bot ? loadedContents[bot] ?? [initialBot,...library.contents].find(item => item && refValue(item) === bot) : null;
    const selectedPersona = persona ? loadedContents[persona] ?? library.contents.find(item => (item.kind === 'persona'||item.hasPackage||item.package) && refValue(item) === persona) : null;
    ...
    if ([selectedBot, selectedPersona].some(item => item?.hasPackage && !item.package)) throw new Error('선택한 패키지 본문을 불러오지 못했어요. 자료를 다시 골라 주세요.');
    const values = Object.fromEntries(([['bot', selectedBot], ['persona', selectedPersona]] as const).flatMap(([role, item]) => item?.package ? [[`${refValue(item)}:${role}`, resolvePromptValues({ version: 1, controls: item.package.controls, blocks: [] }, packageValues[`${refValue(item)}:${role}`])]] : []));
    const opening = start && selectedBot.package ? resolvePackageStart(selectedBot.package, start, values[`${refValue(selectedBot)}:bot`]) : null;
    ...
    return structuredClone({
      ...
      profile: selectedBot || selectedPersona || selectedPrompt || main || translation ? {
        ...
        packageAttachments: ([['bot', selectedBot], ['persona', selectedPersona]] as const).flatMap(([role, item]) => item && (item.package || item.hasPackage) ? [{ id: item.id, revision: item.revision, role }] : []),
        ...(Object.keys(values).length ? { packageValues: values } : {}),
        ...(opening ? { packageStart: { packageId: opening.packageId, packageRevision: opening.packageRevision, startId: opening.startId, idempotencyKey: crypto.randomUUID() } } : {}),
    ...
    try {
      ...
      if (selection.profile) {
        if (!profileRecorded.current) {
          ...
          savePendingStoryProfile(chat.id, { ...selection.profile, ...(selection.profile.packageStart ? { packageStart: { ...selection.profile.packageStart, expectedSettingsRevision: chat.settingsRevision } } : {}) });
  ...
  const bots = contents.filter(item => (initialBot?item.id===initialBot.id:item.kind === 'bot' || item.hasPackage || item.package) && `${item.title} ${item.description}`.toLowerCase().includes(search.toLowerCase()));
  const activeBot = frozen?.bot ?? loadedContents[bot] ?? (initialBot && refValue(initialBot) === bot ? initialBot : contents.find(item => refValue(item) === bot));
  const activePersona = frozen?.persona ?? loadedContents[persona];
  let opening: PackageStartSnapshot | null = null, openingError = '';
  if (start && activeBot?.package) { try { opening = resolvePackageStart(activeBot.package, start, packageValues[`${refValue(activeBot)}:bot`]); } catch (caught) { openingError = (caught as Error).message; } }
  return <form className="new-story" onSubmit={event => { event.preventDefault(); void create(); }}>
    ...
    {!!activeBot?.package?.starts?.length && <fieldset><legend>첫 장면</legend><label>사용할 시작<select aria-label="사용할 시작" value={start} disabled={locked || packageLoading} onChange={event => {
      const id = event.target.value; setStart(id);
      const pkg = activeBot.package!;
      const values = id ? resolvePackageStart(pkg, id).values : resolvePromptValues({ version: 1, controls: pkg.controls, blocks: [] });
      setPackageValues(previous => ({ ...previous, [`${refValue(activeBot)}:bot`]: values }));
    }}><option value="">직접 첫 장면 요청하기</option>{activeBot.package.starts.map(item => <option key={item.id} value={item.id}>{item.title} · {item.mode === 'authored' ? '작성된 도입문' : '모델 생성'}</option>)}</select></label>
      {opening && <><p className="muted">{activeBot.package.starts.find(item => item.id === start)?.description}</p><div className="source-text" aria-label="시작 미리보기" style={{ whiteSpace: 'pre-wrap', maxHeight: 260, overflow: 'auto' }}>{opening.text}</div><p className="muted">{opening.mode === 'authored' ? '확정하면 이 도입문을 원문 그대로 저장해요. 모델을 호출하지 않아요.' : '확정하면 이 요청으로 선택한 본문 모델을 호출해요.'}{opening.initialAction && ' 초기 행동은 확정할 때 한 번 실행해요.'}</p></>}
      {openingError && <p className="error" role="alert">{openingError}</p>}
    </fieldset>}
    {([['bot', activeBot, '봇'], ['persona', activePersona, '페르소나']] as const).map(([role, item, label]) => item?.package?.controls.length ? <fieldset key={`${refValue(item)}:${role}`}><legend>이번 채팅의 {label} 설정</legend><PackageControlValues controls={item.package.controls} values={packageValues[`${refValue(item)}:${role}`]} labelPrefix={`시작 ${label} 옵션`} disabled={locked || packageLoading} onChange={values => setPackageValues(previous => ({ ...previous, [`${refValue(item)}:${role}`]: values }))}/></fieldset> : null)}
    ...
    <button disabled={busy || uncertain.current || !bot || packageLoading || !!openingError}>{busy ? '채팅을 준비하는 중…' : created.current ? '설정 저장 다시 시도' : opening ? opening.mode === 'authored' ? '도입문 확정하고 채팅 만들기' : '첫 장면 생성하고 채팅 만들기' : '채팅 만들기'}</button>
  </form>;
}
