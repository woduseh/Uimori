...
export function Dialog({ open, title, onClose, children, wide = false, className = '' }: { open: boolean; title: string; onClose: () => void; children: ReactNode; wide?: boolean; className?: string }) {
  ...
  return <dialog ref={ref} className={`app-dialog ${wide ? 'wide' : ''} ${className}`} aria-labelledby={id} onKeyDown={event => {
    ...
    <header className="dialog-header"><h2 id={id}>{title}</h2><button className="icon-button secondary" aria-label={`${title} 닫기`} onClick={onClose}><X size={20}/></button></header><div className="dialog-body">{children}</div>
  </dialog>;
}
