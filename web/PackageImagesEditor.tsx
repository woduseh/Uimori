...
export function PackageImagesEditor({ value, onChange, onDirtyChange }: Props) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: A package revision switch aborts uploads and resets its local image editor state.
  useLayoutEffect(() => {
    generation.current++;
    setBusy(false);
    setMessage('');
  ...
  const change = (images: PackageImage[]) => {
    ...
    if (
      next.portraitImageId &&
      !images.some((image) => image.id === next.portraitImageId && image.allowedUse !== 'inline')
    )
      delete next.portraitImageId;
    latest.current.value = next;
    latest.current.onChange(next);
  };
  const edit = (id: string, patch: Partial<PackageImage>) =>
    change(
      (latest.current.value.images ?? []).map((image) =>
  ...
  const currentPage = Math.min(page, Math.max(0, Math.ceil(found.length / pageSize) - 1)),
    visible = found.slice(currentPage * pageSize, (currentPage + 1) * pageSize);
  const active = images.find((image) => image.id === selected),
    removing = images.find((image) => image.id === remove);
  ...
  async function addFiles(files: File[]) {
    ...
    setBusy(true);
    setError('');
    setMessage(`0 / ${files.length}개 업로드`);
    ...
    try {
      ...
      if (current()) {
        setMessage(`${added}개 이미지를 자료에 추가했어요. 자료를 저장하면 적용돼요.`);
        setError(
  ...
  return (
    <section className="package-images" aria-label="자료 이미지">
      ...
      <p role="status" aria-live="polite">
        {message || `${images.length}개 이미지`}
        {busy && ' · 업로드가 끝난 뒤 자료를 저장해 주세요.'}
      </p>
      {error && (
        <p role="alert" className="error package-images-error">
          {error}
        </p>
      )}
      <div className={`package-images-layout${images.length ? '' : ' is-empty'}`}>
        <div>
          <div className="package-images-list" role="list" aria-label="등록한 이미지">
            {visible.map((image) => (
              <div role="listitem" key={image.id}>
                <button
                  type="button"
                  className={`secondary package-image-choice${selected === image.id ? ' is-selected' : ''}`}
          ...
          {!found.length &&
            (images.length ? (
              ...
              <p className="package-images-empty">
                <strong>아직 등록한 이미지가 없어요.</strong>
                <small>위에서 이미지 파일을 추가한 뒤 이름과 설명을 편집해요.</small>
              </p>
            ))}
          {found.length > pageSize && (
            <nav className="package-images-pages" aria-label="자료 이미지 페이지">
              <button
                type="button"
                className="secondary"
              ...
              <span>
                {currentPage * pageSize + 1}–{Math.min((currentPage + 1) * pageSize, found.length)}{' '}
                / {found.length}
              </span>
              <button
                type="button"
        ...
        {active ? (
          <div className="package-image-detail" aria-label="선택한 이미지 편집">
            <img
            ...
            <label>
              이미지 이름
              <input
                aria-label="선택한 이미지 이름"
                required
                maxLength={200}
                value={active.title}
                onChange={(event) => edit(active.id, { title: event.target.value })}
              />
            </label>
            <label>
              설명 <small>선택 사항</small>
              <textarea
                aria-label="선택한 이미지 설명"
                maxLength={2000}
                rows={3}
                value={active.description}
                placeholder="이름만으로 구분하기 어려울 때 적어요."
                onChange={(event) => edit(active.id, { description: event.target.value })}
              />
            </label>
            <label>
              사용 위치
              <select
                aria-label="선택한 이미지 사용 위치"
                value={active.allowedUse}
                onChange={(event) =>
                  edit(active.id, { allowedUse: event.target.value as PackageImage['allowedUse'] })
                }
              >
                <option value="both">대표 이미지와 본문</option>
                <option value="profile">대표 이미지</option>
                <option value="inline">본문용</option>
              </select>
            </label>
            <label className="check">
              <Switch
                ...
                onChange={(event) => {
                  ...
                  if (event.target.checked) next.portraitImageId = active.id;
                  else delete next.portraitImageId;
                  latest.current.value = next;
                  latest.current.onChange(next);
                }}
              />
          ...
          images.length > 0 && (
            <p className="muted">이미지를 선택하면 이름과 설명을 편집할 수 있어요.</p>
          )
        )}
      </div>
    </section>
