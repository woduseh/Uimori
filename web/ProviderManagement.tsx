  ...
  return <section className="connection-editor" data-testid="connection-editor" aria-label="연결과 모델">
    ...
    <form hidden={screen!=='connection'} ref={connectionForm} className="editor-grid provider-management-form" aria-label="연결 편집 양식" onSubmit={event=>{event.preventDefault();const body={...connectionPayload(connection),...(editingConnection?{expectedRevision:editingConnection.revision}:{})};if(editingConnection?.enabled&&!connection.enabled){setConfirmation({kind:'connection',id:editingConnection.id,title:connection.title,body,fromForm:true});return;}void perform(()=>saveConnection(body,editingConnection?.id),'connection');}}>
      ...
      <fieldset className="editor-fields full" disabled={busy||!!confirmation}>
        ...
        <label className="full" hidden={vertex&&connection.credentialEnv.startsWith('NARRATIVE_PROVIDER_VERTEX_FILE_')}>서버 환경변수 이름<input aria-label="서버 환경변수 이름" autoComplete="off" required={official} pattern="NARRATIVE_PROVIDER_[A-Z0-9_]+" placeholder={vertex?'비우면 서버 ADC 사용':'NARRATIVE_PROVIDER_NAME'} value={connection.credentialEnv} onChange={event=>setConnection({...connection,credentialEnv:event.target.value})}/></label>
    ...
    <form hidden={screen!=='model'} ref={modelForm} className="editor-grid provider-management-form" aria-label="모델 편집 양식" noValidate onSubmit={event=>{event.preventDefault();const invalid=event.currentTarget.querySelector<HTMLInputElement|HTMLSelectElement>('input:invalid,select:invalid,textarea:invalid');if(invalid){const section=invalid.closest<HTMLElement>('[data-model-section]')?.dataset.modelSection;setModelSection((section as typeof modelSection)||'basic');for(let parent:HTMLElement|null=invalid.parentElement;parent&&parent!==event.currentTarget;parent=parent.parentElement)if(parent instanceof HTMLDetailsElement)parent.open=true;requestAnimationFrame(()=>{invalid.focus();invalid.reportValidity();});return;}if(!chosen)return;const body={...modelPayload(model,chosen),...(editingModel?{expectedRevision:editingModel.revision}:{})};if(editingModel&&editingModel.enabled!==false&&!model.enabled){setConfirmation({kind:'model',id:editingModel.id,title:model.title,body,fromForm:true});return;}void perform(()=>saveModel(body,editingModel?.id),'model');}}>
      ...
      </fieldset><div className="provider-actions full"><button disabled={busy||!!confirmation||!chosen}>{editingModel?'모델 변경 저장':'모델 프리셋 등록'}</button><button type="button" className="secondary" disabled={busy} onClick={()=>navigate('models')}>모델 편집 끝내기</button></div>
    </form>
