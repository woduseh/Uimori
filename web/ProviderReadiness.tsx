...
export function ProviderReadiness({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Explicit refresh rereads server readiness for the same connection revision.
  useEffect(() => {
    if (connection.protocol === 'codex-app-server-v1') return;
    let current = true;
    setState({ key });
    void api<Readiness>(`/provider-management/connections/${connection.id}/readiness`)
      .then((value) => {
  ...
  }, [connection.id, connection.protocol, key, refresh]);
    ...
    return (
      <section className="provider-readiness full" aria-label="선택한 연결 준비 상태">
        ...
        {connection.catalogError && (
          <p className="error">
            모델 목록 조회 실패 · 마지막 저장 목록과 수동 입력을 사용할 수 있어요.
          </p>
        )}
        <div className="provider-actions">
          ...
          <small>로그인한 공식 Codex 실행기에 사용 가능한 모델 목록을 요청해요.</small>
        </div>
      </section>
    );
  return (
    <section className="provider-readiness full" aria-label="선택한 연결 준비 상태">
      ...
      {state?.key === key && state.error ? (
        ...
        <p role="status">서버 설정을 확인하고 있어요…</p>
      ) : (
        <>
