...
export function PackageTransfer({
  ...
  async function save() {
    ...
    try {
      ...
      const data = pkg.images?.length
        ? await api('/package-bundles/export', { package: pkg })
        : pkg;
      ...
      link.href = url;
      link.download = `${pkg.title || 'package'}.uimori.json`;
      link.click();
  ...
  async function load(file: File) {
    ...
    try {
      if (file.size > 64 * 1024 * 1024)
        throw new Error('이미지 포함 패키지 파일은 64 MB 이하여야 해요.');
  ...
  return (
    <div className="library-package-transfer">
      ...
      <section className="library-transfer-card">
        <h3>파일에서 가져오기</h3>
        <p className="muted">내용을 확인한 뒤 현재 초안에 적용해요.</p>
        <label>
          <span className="sr-only">패키지 JSON 가져오기</span>
          <input
            type="file"
            aria-label="패키지 JSON 가져오기"
            disabled={disabled || busy}
