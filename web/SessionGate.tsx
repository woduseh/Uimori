...
export function SessionGate({ children }: { children: ReactNode }) {
    ...
    const refresh = async (expired = false) => {
      ...
      try {
        ...
        if (alive && revision.current === expected) {
          setSession(next);
          setError(
            expired && next.required && !next.authenticated
              ? '접속이 만료됐어요. 토큰을 입력해 다시 연결해 주세요.'
              : ''
          );
        }
      } catch {
  ...
  return (
    <main className="login">
      ...
      <h1>{session?.required ? '개인 작업실에 연결' : '작업실 연결 확인 중'}</h1>
      {error && (
      ...
      {session?.required && (
        ...
        >
          ...
          <button disabled={busy}>작업실 연결</button>
          <small>토큰은 이 입력창에서만 사용하고 브라우저 저장소에 보관하지 않아요.</small>
        </form>
      )}
      {!session && (
