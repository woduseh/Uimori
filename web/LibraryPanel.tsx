...
type LibraryTab = PrimaryLibraryTab;
const libraryTabs: { id: LibraryTab; title: string }[] = [{ id: 'bot', title: '봇' }, { id: 'persona', title: '페르소나' }, { id: 'module', title: '모듈' }, { id: 'prompts', title: '프롬프트' }];
...
export function LibraryPanel({ library, reload, onError, onStartStory, initialTab = 'bot', onTabChange, onDirtyChange }: { library: Library | null; reload: () => Promise<void>; onError: (error: string) => void; onStartStory?: (bot: Content) => void; initialTab?: PrimaryLibraryTab; onTabChange?: (tab: PrimaryLibraryTab) => void; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  function switchTab(next:LibraryTab, closeOnly=false) { cancelOpening(); setEditing(null); setDirty(false); if (!closeOnly) { setTab(next); externalTab.current=next;onTabChange?.(next); } }
  ...
  function openNew() { cancelOpening(); if (tab !== 'prompts') setEditing({ kind: tab, item: null }); }
  return <section className="library-page" data-testid="library-panel" aria-label="서재">
    ...
    {pendingNavigation && <div role="alertdialog" aria-label="미저장 자료 확인"><p>저장하지 않은 편집 내용이 있어요.</p><button type="button" className="secondary" onClick={()=>{setPendingNavigation(null);if (["bot","persona","module","prompts"].includes(tab)) {externalTab.current=tab as PrimaryLibraryTab;onTabChange?.(tab as PrimaryLibraryTab);}}}>계속 편집</button><button type="button" onClick={()=>{switchTab(pendingNavigation.tab,pendingNavigation.closeOnly);setPendingNavigation(null);}}>초안 버리고 이동</button></div>}
    {loading && <p role="status">자료 본문을 불러오는 중이에요…</p>}
    {!library ? <p role="status">서재를 불러오는 중이에요…</p> : editing ? <ContentEditor key={editing.item ? refValue(editing.item) : `new-${editing.kind}`} library={library} reload={reload} onError={onError} initial={editing.item} kind={editing.kind} onClose={() => navigate(tab,true)} onDirtyChange={setDirty} onStartStory={onStartStory}/> : <>
      ...
      <div id="library-results" role="tabpanel" aria-labelledby={`library-tab-${tab}`} className={tab === 'prompts' ? 'library-prompt-editor' : 'library-cards'}>
        {tab === 'prompts' ? <PromptEditor library={library} reload={reload} onError={onError} onDirtyChange={setDirty}/> : filtered.map(item => <article className="library-card" key={refValue(item)}><div className="library-avatar" aria-hidden="true">{Array.from(item.title.trim())[0] ?? '·'}</div><h2><button type="button" className="library-title-button" onClick={() => void openContent(item)} aria-label={`${item.title} 자료 편집`}>{item.title}</button></h2><p>{item.description || '아직 설명이 없어요.'}</p><small>{item.loading === 'pinned' ? '항상 포함' : '모델이 필요할 때 읽기'}</small>{item.kind === 'bot' && onStartStory ? <button type="button" className="secondary" onClick={() => void openContent(item, true)} aria-label={`${item.title} 봇으로 시작`}>채팅 시작</button> : <button type="button" className="secondary" onClick={() => void openContent(item)}>자료 살펴보기</button>}</article>)}
        {tab !== 'prompts' && filtered.length === 0 && <div className="library-empty"><h2>{query ? '찾는 자료가 없어요' : `아직 ${contentLabels[tab]} 자료가 없어요`}</h2><p>{query ? '다른 이름이나 설명으로 찾아보세요.' : '직접 만든 설정을 보관하고 여러 이야기에서 함께 사용할 수 있어요.'}</p><button type="button" className="secondary" onClick={openNew}>새로 만들기</button></div>}
      </div>
    </>}
  </section>;
}

function ContentEditor({ library, reload, onError, initial, kind, onClose, onStartStory, onDirtyChange }: EditorProps & { initial: Content | null; kind: ContentKind; onClose: () => void; onStartStory?: (bot: Content) => void; onDirtyChange: (dirty:boolean)=>void }) {
  ...
  async function saveContent(copyKind?: 'bot'|'persona'|'module') {
    if(behaviorDraftDirty){setError('상태와 행동의 초안을 먼저 검증하고 적용해 주세요.');return;}
  ...
  return <section className="library-detail" aria-label="자료 상세">
    <div className="library-detail-heading"><button type="button" className="secondary" disabled={busy} onClick={onClose}>← 서재 목록</button><h2>{selected ? selected.title : '새 자료'}</h2>{selected?.kind === 'bot' && onStartStory && <button type="button" className="secondary" disabled={busy || dirty || !!importedPackage || behaviorDraftDirty} onClick={() => onStartStory(selected)}>채팅 시작</button>}</div>
    <form className="editor-grid" onSubmit={async event => {
      ...
      <fieldset className="editor-fields full" disabled={busy}>
        ...
        <details className="full"><summary>패키지 가져오기·내보내기와 역할 사본</summary><p className="muted">로어와 지침은 사본에 함께 보존해요. 역할을 바꿔도 인물 관점과 역할별 지침을 자동 각색하지 않아요.</p><div className="form-actions">{value.package && <button type="button" className="secondary" onClick={()=>{try{saveDownload((value.title||'package')+'.uimori.json',packageSnapshot());}catch(caught){setError((caught as Error).message);}}}>패키지 JSON 내보내기</button>}<label>패키지 JSON 가져오기<input type="file" aria-label="패키지 JSON 가져오기" accept=".json,application/json" onChange={async event=>{const file=event.target.files?.[0];event.target.value='';if(!file)return;try{if(file.size>1500000)throw new Error('패키지 JSON은 1.5 MB 이하여야 해요.');const parsed=JSON.parse(await file.text());setImportedPackage(validateContentPackage(parsed.package??parsed));setError('');}catch(caught){setError((caught as Error).message);}}}/></label></div>{importedPackage && <div><p>{importedPackage.title} · 로어 {importedPackage.lore.length}개 · 지침 {importedPackage.instructions.length}개</p><button type="button" onClick={()=>{setValue({...value,title:importedPackage.title,description:importedPackage.description,text:importedPackage.body??'',package:importedPackage});setImportedPackage(null);}}>가져온 패키지로 초안 바꾸기</button><button type="button" className="ghost" onClick={()=>setImportedPackage(null)}>가져오기 취소</button></div>}<div className="form-actions">{(['bot','persona','module'] as const).filter(role=>role!==value.kind).map(role=><button type="button" className="secondary" key={role} disabled={!value.title.trim()} onClick={()=>void saveContent(role)}>{contentLabels[role]}로 사본 만들기</button>)}</div></details>
      ...
      {behaviorDraftDirty&&<p className="full muted">상태와 행동에 미적용 초안이 있어요. 검증 후 적용하면 자료를 저장할 수 있어요.</p>}
      <div className="form-actions full"><button disabled={busy || !!importedPackage || behaviorDraftDirty}>{busy ? '저장 중…' : selected ? '새 revision 저장' : '자료 등록'}</button><span role="status">{saved}</span></div>
