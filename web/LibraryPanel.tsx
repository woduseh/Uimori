...
const freshContent = (kind: ContentKind): Omit<Content, 'id' | 'revision'> => ({
  ...
  description: '',
  text: '',
  loading: ['bot', 'persona', 'module', 'canon'].includes(kind) ? 'pinned' : 'discoverable',
  relatedIds: [],
  ...(['bot', 'persona', 'module'].includes(kind)
...
const libraryTabs: { id: LibraryTab; title: string }[] = [
  { id: 'bot', title: '봇' },
  { id: 'persona', title: '페르소나' },
  { id: 'module', title: '모듈' },
  { id: 'prompts', title: '프롬프트' },
  { id: 'other', title: '기타 자료' },
];
...
export function LibraryPanel({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Only an external tab change requests navigation; local edits keep their pending confirmation.
  useEffect(() => {
  ...
  function switchTab(next: LibraryTab, closeOnly = false) {
    cancelOpening();
    setEditing(null);
    setDirty(false);
    if (!closeOnly) {
      ...
      if (next !== 'other') onTabChange?.(next);
    }
  }
  function navigate(next: LibraryTab, closeOnly = false) {
    if (next === tab && !closeOnly && !editing) return;
  ...
  async function openContent(item: Content, start = false) {
    ...
    try {
      ...
      if (start) onStartStory?.(full);
      else setEditing({ kind: full.kind, item: full });
    } catch (error) {
  ...
  const filtered =
    library?.contents.filter(
      (item) =>
        (tab === 'other' ? !['bot', 'persona', 'module'].includes(item.kind) : item.kind === tab) &&
        `${item.title} ${item.description}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())
    ) ?? [];
  function openNew() {
    cancelOpening();
    if (tab !== 'prompts') setEditing({ kind: tab === 'other' ? 'lore' : tab, item: null });
  }
  function continueEditing() {
    setPendingNavigation(null);
    externalTab.current = tab;
    if (tab !== 'other') onTabChange?.(tab);
  }
  return (
    <section className="library-page" data-testid="library-panel" aria-label="서재">
      <header className="library-heading">
        <div>
          <h1>서재</h1>
          <p className="muted">봇·페르소나·모듈에 인물과 세계, 로어와 지침을 담아요.</p>
        </div>
        {!editing && tab !== 'prompts' && (
      ...
      {!library ? (
        ...
        <>
          ...
          <div
            ...
            className={tab === 'prompts' ? 'library-prompt-editor' : 'library-cards'}
          >
            {tab === 'prompts' ? (
              <PromptEditor
                library={library}
              ...
              filtered.map((item) => (
                <article className="library-card" key={refValue(item)}>
                  ...
                  <h2>
                    <button
                      ...
                      aria-label={`${item.title} 자료 편집`}
                    >
                      {item.title}
                    </button>
                  </h2>
                  <p>{item.description || '아직 설명이 없어요.'}</p>
                  <small>{item.loading === 'pinned' ? '항상 포함' : '모델이 필요할 때 읽기'}</small>
                  {(item.kind === 'bot' || item.package || item.hasPackage) && onStartStory ? (
                    <button
                      type="button"
                      className="secondary"
                      onClick={() => void openContent(item, true)}
                      aria-label={`${item.title} 봇으로 시작`}
                    >
                      {item.kind === 'bot' ? '채팅 시작' : '이 자료를 봇으로 시작'}
                    </button>
                  ) : (
                    <button
                      type="button"
            ...
            {tab !== 'prompts' && filtered.length === 0 && (
              <div className="library-empty">
                <h2>
                  {query
                    ? '찾는 자료가 없어요'
                    : `아직 ${tab === 'other' ? '기타' : contentLabels[tab]} 자료가 없어요`}
                </h2>
                <p>
                  {query
                    ? '다른 이름이나 설명으로 찾아보세요.'
                    : '직접 만든 설정을 보관하고 여러 이야기에서 함께 사용할 수 있어요.'}
                </p>
...
function ContentEditor({
  ...
  useEffect(() => {
    onDirtyChange(dirty || !!importedPackage || behaviorDraftDirty);
  }, [dirty, importedPackage, behaviorDraftDirty, onDirtyChange]);
    ...
    <section className="library-detail" aria-label="자료 상세">
      <div className="library-detail-heading">
        ...
        {selected && (selected.kind === 'bot' || selected.package) && onStartStory && (
          <button
            type="button"
            className="secondary"
            disabled={busy || dirty || !!importedPackage || behaviorDraftDirty}
      ...
      >
        ...
        <div className="library-savebar form-actions full">
          <button disabled={busy || !!importedPackage || behaviorDraftDirty}>
