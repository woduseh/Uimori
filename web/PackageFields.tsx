...
export function PackageFields({
  ...
  return (
    <section className="package-fields full" aria-label="패키지 구성">
      <div className="package-tabs" role="group" aria-label="패키지 편집 분류">
        {(
          [
            ['lore', '로어'],
            ['images', '이미지'],
      ...
      <div hidden={tab !== 'lore'}>
        <LoreEditor key={value.id} value={value} onChange={update} onDraftChange={setLoreDirty} />
      </div>
                ...
                update({
                  stateView: {
                    ...
                    fields: [
                      ...
                      { key: `field_${(value.stateView?.fields.length ?? 0) + 1}`, label: '상태' },
                    ],
                  },
                })
              }
