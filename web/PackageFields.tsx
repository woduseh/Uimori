...
export function PackageFields({value,onChange,onBehaviorDraftChange}:{value:ContentPackage;onChange:(v:ContentPackage)=>void;onBehaviorDraftChange?:(dirty:boolean)=>void}){
  const [tab,setTab]=useState<'lore'|'instructions'|'options'|'display'|'roles'|'behavior'|'images'|'starts'|'features'>('lore');
  const [behaviorDirty,setBehaviorDirty]=useState(false),[controlsDirty,setControlsDirty]=useState(false),[instructionsDirty,setInstructionsDirty]=useState(false);
  const [imagesDirty,setImagesDirty]=useState(false),[startsDirty,setStartsDirty]=useState(false),[featuresDirty,setFeaturesDirty]=useState(false),[loreDirty,setLoreDirty]=useState(false);
  useEffect(()=>{onBehaviorDraftChange?.(behaviorDirty||controlsDirty||instructionsDirty||imagesDirty||startsDirty||featuresDirty||loreDirty);},[behaviorDirty,controlsDirty,instructionsDirty,imagesDirty,startsDirty,featuresDirty,loreDirty,onBehaviorDraftChange]);
  ...
  return <section className="package-fields full" aria-label="패키지 구성">
    <div className="package-tabs" role="group" aria-label="패키지 편집 분류">{([['lore','로어'],['images','이미지'],['starts','시작'],['instructions','지침'],['options','옵션'],['features','연결과 기능'],['display','표현'],['roles','역할별 지침'],['behavior','상태와 행동']] as const).map(([id,title])=><button type="button" className="secondary" aria-pressed={tab===id} key={id} onClick={()=>setTab(id)}>{title}</button>)}</div>
    <div hidden={tab!=='images'}><PackageImagesEditor key={value.id} value={value} onChange={onChange} onDirtyChange={setImagesDirty}/></div>
    <div hidden={tab!=='starts'}><PackageStartsEditor key={value.id} value={value} onChange={onChange} onDraftChange={setStartsDirty}/></div>
    <div hidden={tab!=='features'}><PackageFeaturesEditor key={value.id} value={value} onChange={onChange} onDirtyChange={setFeaturesDirty}/></div>
    <div hidden={tab!=='behavior'}><BehaviorEditor key={value.id} value={(value as ContentPackage & {behavior?:PackageBehavior}).behavior} onChange={behavior=>update({behavior} as Partial<ContentPackage>)} onDirtyChange={setBehaviorDirty}/></div>
    <div hidden={tab!=='lore'}><LoreEditor key={value.id} value={value} onChange={update} onDraftChange={setLoreDirty}/></div>
    <div hidden={tab!=='instructions'}><PackageInstructionsEditor key={value.id} value={value} onChange={instructions=>update({instructions})} onDirtyChange={setInstructionsDirty}/></div>
    <div hidden={tab!=='options'}><PackageControlsEditor key={value.id} value={value} onChange={controls=>update({controls})} onDirtyChange={setControlsDirty}/></div>
