...
const sections: { id: ProfileSection; title: string }[] = [{ id: 'characters', title: '봇·페르소나·모듈' }, { id: 'prompts', title: '프롬프트·창작 프리셋' }, { id: 'models', title: '모델' }];

export function ProfileEditor({ profile, library, onSaved, onError, onDirtyChange, onLibraryChanged, initialTab = 'characters', branchId, ownerBotId, nextRequest='', loreContextReset=false }: { ownerBotId?:string; branchId?: string; profile: ChatProfile; library: Library; onSaved: () => Promise<void>; onError: (error: string) => void; onDirtyChange?: (dirty: boolean) => void; onLibraryChanged?: () => Promise<void>; initialTab?: ProfileSection; nextRequest?:string; loreContextReset?:boolean }) {
  const [value, setValue] = useState(profile);
  const [dirty, setDirty] = useState(false);
  const [promptDirty, setPromptDirty] = useState(false);
  const [lorePending,setLorePending]=useState(false),[loreResetVersion,setLoreResetVersion]=useState(0);
  ...
  useEffect(() => { if (!dirty&&!lorePending) setValue(current => profile.chatId !== current.chatId || profile.revision >= current.revision ? profile : current); }, [profile, dirty,lorePending]);
  useEffect(() => { onDirtyChange?.(dirty || promptDirty||lorePending); }, [dirty, promptDirty,lorePending, onDirtyChange]);
  ...
  async function save(work: () => Promise<ChatProfile>, message = '채팅 설정을 저장했어요.') {
    if(lorePending){setError('로어 문맥 정책의 숫자 초안을 먼저 확인해 주세요.');return false;}
  ...
  const profileBody = (next: ChatProfile) => ({ expectedRevision: next.revision, attachments: next.attachments, creative: next.creative, routes: next.routes, image: next.image, ...(next.prompts ? { prompts: next.prompts } : {}), ...(next.promptControls ? {promptControls:next.promptControls}:{}), ...(next.hiddenStory?{hiddenStory:next.hiddenStory}:{}), ...(next.packageAttachments?{packageAttachments:next.packageAttachments}:{}), ...(next.packageValues?{packageValues:next.packageValues}:{}),...(next.loreContext?{loreContext:next.loreContext}:{}) });
  return <section className="profile-editor" data-testid="profile-editor" aria-label="콘텐츠와 창작 제어">
    ...
    <div className="profile-tabs" role="tablist" aria-label="채팅 설정 분류" onKeyDown={event => {
    ...
    <form onSubmit={event => { event.preventDefault(); void save(() => api<ChatProfile>(`/chats/${profile.chatId}/profile`, profileBody(value), 'PUT')); }}>
      <div id="profile-fields" role="tabpanel" aria-labelledby={`profile-tab-${tab}`}><fieldset className="profile-fields" disabled={saving}>
        ...
        <div hidden={tab!=='characters'}><LoreContextPolicyEditor key={`${profile.chatId}:${loreResetVersion}`} value={value.loreContext} onChange={loreContext=>change({...value,loreContext})} onPendingChange={setLorePending} chatId={profile.chatId} branchId={branchId} profileRevision={value.revision} request={nextRequest} reset={loreContextReset}/></div>
      ...
      <div className="profile-savebar form-actions"><button disabled={saving || !dirty||lorePending}>{saving ? '저장 중…' : '콘텐츠와 제어 저장'}</button>{(dirty||lorePending) && <button type="button" className="secondary" disabled={saving} onClick={() => { setValue(profile); setDirty(false);setLorePending(false);setLoreResetVersion(version=>version+1); setError(''); setStatus('최신 설정을 불러왔어요.'); onError(''); }}>장착 설정 다시 불러오기</button>}<span role="status">{status || (dirty||lorePending ? '저장하지 않은 변경이 있어요.' : '')}</span></div>
