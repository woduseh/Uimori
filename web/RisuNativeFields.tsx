...
function CardLoreKeywords({
  ...
  return (
    <input
      ...
      onChange={(e) => {
        // Preserve the separator while typing; authored keys still update immediately.
    ...
    <div className="native-editor native-content-editor full">
      ...
      <fieldset disabled={!!draft} className="native-panel-fields">
        ...
        <div {...panel('lore')}>
          <NativeCollectionEditor
            label="로어"
            items={lore.map((entry, i) => ({
              title: string(entry.comment ?? entry.name) || '로어 ' + (i + 1),
              subtitle:
                entry.enabled === false
                  ? '사용 안 함'
                  : entry.alwaysActive || entry.constant
                    ? '항상 활성'
                    : '조건부 활성',
            }))}
            onAdd={() =>
                ...
                moduleLore
                  ...
                  : {
                      id:
                        Math.max(
                          -1,
                          ...lore.map((entry) => (typeof entry.id === 'number' ? entry.id : -1))
                        ) + 1,
                      name: '새 로어',
                      keys: [],
                      content: '',
            ...
            onRemove={(i) => writeLore(lore.filter((_, n) => n !== i))}
          >
            {(i) => {
              const entry = lore[i];
              const edit = (patch: Record<string, unknown>) =>
                writeLore(lore.map((item, n) => (n === i ? { ...item, ...patch } : item)));
              return (
                <>
                  <div className="native-field-row">
                    <label>
                      ...
                      <input
                        aria-label="로어 이름"
                        value={string(entry.comment ?? entry.name)}
                        onChange={(e) =>
                          edit({
                            [moduleLore || Object.hasOwn(entry, 'comment') ? 'comment' : 'name']:
                              e.target.value,
                          })
                        }
                      />
                    ...
                    <label>
                      ...
                      {moduleLore ? (
                        ...
                        <CardLoreKeywords
                          key={value.id + ':' + i + ':' + String(entry.id ?? '')}
                          value={Array.isArray(entry.keys) ? entry.keys : []}
                  ...
                  <div className="native-field-row">
                    ...
                    <label className="check">
                      <input
                        type="checkbox"
                        checked={moduleLore ? entry.alwaysActive === true : entry.constant === true}
                        onChange={(e) =>
                          edit({ [moduleLore ? 'alwaysActive' : 'constant']: e.target.checked })
                        }
                      />
                      항상 활성
                    </label>
                  ...
                  <details className="native-advanced">
                    ...
                    <label>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          edit({
                            [moduleLore ? 'insertorder' : 'insertion_order']: Number(
                              e.target.value
                            ),
                          })
                        }
                    ...
                    <p className="muted">
                      추가 조건·장식자·폴더 정보는 원문에 유지돼요. 고급 설정의 원문에서 편집할 수
                      있어요.
                    </p>
                  </details>
                </>
