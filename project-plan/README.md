...
| 파일 | 내용 |
| --- | --- |
| [코드 지도](CODE-MAP.md) · [개발 지침 정리 근거](AGENT-GUIDANCE-2026-09-13.md) | 관련 계약·진입점을 골라 읽는 탐색 지도와 GPT-6 Astra 가이드 적용 근거 |
...
| [설정·서재 화면 설계](../docs/UI-SCREEN-DESIGN.md) | 데스크톱·모바일 시안, 프로바이더·모델·데이터 관리, 메뉴·빈 화면·편집 상태와 아이콘 명세 |
| [UI 설계 v2 · 원고가 화면이다](../docs/UI-DESIGN-V2.md) | 2026-09-08 실행 화면 검토 뒤의 제안 설계. 채팅 리더·입력창·탐색·서재의 세 층 모델, 수치 목표, 뒤집는 결정과 적용 순서. 구현·검증 전 |
| [공통 UI·아이콘 구현 결과](UI-COMPACT-IMPLEMENTATION.md) | 설정 목록·상세, 서재·모델·프로바이더·데이터, 초안·뒤로가기 보호와 반응형 검증 |
...
| [검사 시점](../docs/QUALITY.md#실행-시점) · [코드 품질 결과](QUALITY-RESULTS.md) | 현재 작은 커밋·사용 흐름 완료·릴리스 검사 기준과 당시 실행 결과 |
...
새 작업은 현재 요청을 기준으로 [코드 지도](CODE-MAP.md)에서 관련 계약·진입점만 찾아요. 진행 판단이 필요한 베타 작업은 BETA-PLAN, 기능 계약의 요약은 CURRENT를 참고하며 전체 문서를 먼저 읽지는 않아요. 인수 목록의 모든 향후 항목을 한 번에 구현할 필요는 없어요. 현재 사용자 요청을 우선하며 파일 간 실질적 충돌은 결정과 영향을 기록해요.
