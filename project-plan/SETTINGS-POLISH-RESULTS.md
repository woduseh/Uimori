...
구현 커밋 `3b39148`을 main의 `c6111af`에 통합했어요. 기존 서재·프로바이더 UI 변경을 보존하고, Codex 도움말과 화면 이동의 명칭을 main의 `프로바이더와 모델`에 맞췄어요.

- 통합 main의 서식·lint·타입·tooling 27·빌드 PASS.
- 전체 Vitest: 1,709 PASS / 1 FAIL / opt-in 1 SKIP. 실패는 `comfyui-client.test.ts`의 stalled view response body deadline 검사 15초 timeout이었어요. 소스 변경 없이 해당 파일 단독 재실행은 16/16 PASS(866ms)예요. 전체 최초 실행의 실패를 전체 PASS로 바꾸지 않아요.
- 관련 브라우저 15/15 PASS, cleanup PASS. 증거: `output/playwright/settings-polish-2026-09-09T17-22-11-564Z-5760faae/summary.json`.
- 최초 승인 검토는 준비 단계의 반대 방향 merge를 거부했어요. 요청한 main 대상 직접 병합은 승인되어 수행했어요. 원격 push·배포는 하지 않았어요.
