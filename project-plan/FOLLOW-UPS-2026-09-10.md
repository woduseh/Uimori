...
`verify:redesign` 전체 실행에서 남은 실패는 여섯 건이에요. 아래 두 건 외 `CSUI04`·`PRUI01`·`SIDENAV01`·`UI03 UI12 new story retry`는 [일관성 정리 기록](UIUX-CONSISTENCY-2026-09-10.md)에 성격을 정리했어요. 이 실행은 macOS·Edge 152이고 저장소 CI는 Windows·Node 24.14예요. 같은 환경의 A/B 비교는 유효하지만 **남은 실패의 절대 개수는 Windows에서 다시 확인해야 해요.**
...
**미확정.** 누적 순서에서 누가 두 control을 남기는지는 아직 몰라요. `/api/prompt-workspace`에 직접 쓰는 spec 일곱 개는 단독 실행 후 0으로 정리되는 것을 확인했고, `/api/prompt-workspace/apply`를 쓰는 spec 열 개가 남은 후보예요.

**수정 방향 세 가지.** 원인 spec이 뒷정리를 하거나, 검사가 전역 상태에 의존하지 않게 바꾸거나, 전체 실행에서 spec 사이의 전역 작업본을 격리해요. 세 번째가 근본이지만 검증 하네스 계약 변경이라 별도 판단이 필요해요.
...
**보존된 실행 DB를 먼저 읽어요.** 하네스가 실행 종료 시점의 SQLite를 `output/playwright/<runId>/evidence-db/app.sqlite`에 남겨요. 2번의 원인은 이 DB의 한 번 조회로 찾았어요. 전역 상태가 의심되면 56개 spec을 재생하기 전에 이 DB를 열어요.
