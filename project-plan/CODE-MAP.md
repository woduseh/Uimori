...
| 공유 패키지·로어·시작·이미지 | [패키지](../docs/PACKAGES.md), [로어 문맥](../docs/LORE-CONTEXT.md), [키워드 활성화](../docs/LORE-CONTEXT.md#키워드-활성화) | `core/content-package.ts`, `package-context.ts`, `package-start.ts`, `lore-activation.ts`; `server/product-store.ts`, `package-start.ts`, `package-images.ts`, `compat/risu/lore-activation.ts`; `third_party/risuai/cad8595a/lore-activation.ts` |
