...
2026-09-08 서재를 봇·페르소나·모듈의 카드/목록과 분류별 한 단계 폴더로 개편하고 프롬프트 관리를 분리했어요. 대표 이미지 업로드·선택·해제와 주요 자료 선택창의 이미지 표시, 다중 이동·CAS·다른 탭 갱신·고정 개정 보존을 연결하고 독립 기타 자료 종류를 제거했어요. 현재 schema/archive는 **v12**이며 사용자 DB는 열거나 초기화하지 않았어요. 병행 UI 변경을 보존한 전체 단위·통합 **1,248 PASS·1 opt-in skip**, 최종 타입·빌드·전체 브라우저 **120/120 PASS**, 별도 이미지 로드 실패/복구·source/build 일치·cleanup을 확인했어요. 마지막 모바일 편집 복귀 수정은 타입·빌드·전체 브라우저로 재검증했어요. [사용·저장 계약](../docs/LIBRARY.md) · [변경·최종 검증·초기 실패 기록](LIBRARY-RESULTS.md). 아래 버전과 수치는 각 시점의 기록이에요.
