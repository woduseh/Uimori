...
- **응답 후 코드 처리**는 `after-turn` 코드 행동으로 완성 응답을 읽어 자기 상태를 갱신해요. 기존 파서 뒤 상태를 transaction 밖에서 계산하고 원문 저장 때 영수증·CAS로 게시하며 원문은 변경하지 않아요. 후처리 실패는 유효한 이전 상태와 원문을 보존하고, 후보는 새 응답으로 재계산하며 백업/포크는 결과만 보존해요. 전용 영수증을 모델 입력과 분리하고 화면에는 진행/실패 metadata만 전달해요. 후처리 모델 호출과 건너뛰기를 제공하며 HTTP 호출은 후속 범위예요. [응답 후 코드](../docs/EXTENSION-PROGRAMS.md#응답-후-코드-처리)
