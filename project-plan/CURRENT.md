...
> 2026-09-14: v0.1.0 준비의 제품 방향은 [베타 결정](../docs/DECISIONS-2026-09-12-BETA.md), 실행과 검증 상태는 [베타 계획](BETA-PLAN.md)을 따라요. 기본 Risu 가져오기와 사용자·모델·생성 전/후 JavaScript·Lua 계산, 현재 분기 대화 읽기를 구현했어요. 남은 입력·출력·표시 훅과 동적 화면은 [새 세션 인계](BETA-PLAN.md#새-세션에서-이어갈-작업--2026-09-14)에서 이어가며 전체 호환 실행·확장 SDK와는 구분해요.
...
- **분기 공유 변수**는 사용자 명시 편집과 같은 기본값 resolver로 지침·템플릿·패널에 반영하고 Run 예약·분기·포크·candidate·백업에서 보존해요. 제작자 행동의 공유 읽기·쓰기는 단계별 임시 계산과 명시 권한으로 연결했고 Risu Lua 변수·JSON 상태 API도 같은 Host를 사용해요. 혼합 트리거·CBS 쓰기와 입력/표시 이벤트는 남아 있어요. [공유 변수 계약](../docs/PROMPT-RUNTIME.md#분기-공유-변수) · [Lua 지원 범위](../docs/RISU-IMPORT.md#lua-콜백-가져오기)
