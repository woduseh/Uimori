...
2026-09-08 실제 실행 화면(1440×900, 390×844 합성 뷰포트)을 검토한 뒤 [UI 설계 v2](../docs/UI-DESIGN-V2.md)를 제안으로 작성했어요. 채팅 리더의 상시 크롬을 세 층으로 재배치하고 입력창·탐색·서재 상세를 단순화하는 안이며, 응답별 작업 현황·장면 탐색 바·입력창 모델 select·헤더 포크 아이콘 등 기존 결정 일부를 뒤집어요. 시안은 `ui-redesign/v2/`에 있고 아직 구현·검증 전이에요. 아래 "현재 구현"은 v2 이전의 현행 동작이에요.
