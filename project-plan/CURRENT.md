...
- **응답 후 코드 처리**는 `after-turn` 코드 행동으로 완성 응답을 읽어 자기 상태를 갱신해요. 기존 파서 뒤 상태를 transaction 밖에서 계산하고 원문 저장 때 영수증·CAS로 게시하며 원문은 변경하지 않아요. 후처리 실패는 유효한 이전 상태와 원문을 보존하고, 후보는 새 응답으로 재계산하며 백업/포크는 결과만 보존해요. 전용 영수증을 모델 입력과 분리하고 화면에는 진행/실패 metadata만 전달해요. 후처리 모델·HTTP 호출은 후속 범위예요. [응답 후 코드](../docs/EXTENSION-PROGRAMS.md#응답-후-코드-처리)

- **확장 코드의 추가 모델 요청**은 생성 전 자동 준비와 모델이 호출한 행동에서 `model.generate`를 사용해요. 전역 확장 모델 선택과 자료 개정에 묶인 채팅별 허용이 필요하며, 준비·문맥 정리·본문은 Run 전체 예산·전송 전 attempt·취소·사용량 경로를 공유해요. 마지막 본문 호출을 남기고 원문은 main만 저장해요. 자동 준비 건너뛰기는 늦은 결과를 닫고 전송한 호출의 정산 후 같은 요청을 계속해요. 후보는 준비를 재실행하거나 기존 사용량을 중복 계산하지 않아요. 일반 채팅 백업의 과거 실행/허가 기록은 보존하되 새 채팅의 호출 권한은 다시 선택해요. 사용자 버튼/응답 후의 추가 호출과 일반 HTTP는 후속 범위예요. [확장 모델 계약](../docs/EXTENSION-PROGRAMS.md#모델-호출-host-api와-명시-권한)
