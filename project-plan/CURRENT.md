...
UI 설계 v2 2단계의 헤더·입력창을 적용했어요. 본문 모델은 헤더의 모델 칩(기존 `빠른 본문 모델` select)이고, 채팅 메뉴(⋯)에 여기까지 새 채팅으로 복사·읽기 설정·집중 읽기(모바일)·작업 현황·보관된 전개를 모았어요. 채팅 설정은 헤더 아이콘으로 남겼어요. 입력창은 한 줄 `⋯·글·↑`에서 두 줄 이상이면 글 아래로 버튼이 내려가고, 모델·단축키 캡션 줄은 없앴어요. 빈 입력창 데스크톱 **56px**·모바일 **52px**, 모바일 첫 화면 본문 **70%**예요. `quality`·빌드 통과, 영향 받는 브라우저 검사 **14건 통과**(SCUI03은 검사 쪽 타이밍으로 1회 실패 뒤 재실행 통과). 전체 재실행은 단계 완료 시점에 돌려요. [적용 기록](../docs/UI-DESIGN-V2.md#적용-기록). 아래는 이 변경 이전 기록이에요.
