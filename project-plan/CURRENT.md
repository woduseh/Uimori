...
> 2026-09-14: v0.1.0 준비의 제품 방향은 [베타 결정](../docs/DECISIONS-2026-09-12-BETA.md), 실행과 검증 상태는 [베타 계획](BETA-PLAN.md)을 따라요. 기본 Risu 가져오기와 사용자·모델·생성 전/후 JavaScript·Lua 계산, 현재 분기 대화 읽기, Risu 콜백의 입력·요청·출력·표시 편집 훅과 선언형 `setvar` 트리거, 유지보수 게이트와 운영자 CLI controller, 대용량 staged 가져오기를 구현했어요. 2026-09-14 검토 뒤의 현재 작업(동작 버그·Risu 스냅샷 경계·중복 정리·문서·호환 기능)은 [검토 후속 계획](BETA-PLAN.md#현재-작업-검토-후속-계획--2026-09-14)이, 확정한 방향은 [2026-09-14 후속 선택](../docs/DECISIONS-2026-09-12-BETA.md#호환-실행의-범위--2026-09-14-후속-선택)이 소유해요. 전체 호환 실행·확장 SDK와는 구분해요.
...
- **DB·archive·채팅 백업 버전**은 [DATA-MIGRATIONS의 현재 버전](../docs/DATA-MIGRATIONS.md#현재-버전)이 소유하고 다른 문서는 숫자를 다시 적지 않아요. 새 DB와 지원하는 이전 schema에서 migration을 순서대로 적용하고 내부 ledger를 기록해요. 원문·상태·이미지·snapshot은 보존하고 모든 과거 개발 DB·archive 이관을 의미하지 않아요. 운영 앱의 버전과는 별개예요. [운영 배포](../docs/ORACLE-RELEASE.md)
- **분기 공유 변수**는 사용자 명시 편집과 같은 기본값 resolver로 지침·템플릿·패널에 반영하고 Run 예약·분기·포크·candidate·백업에서 보존해요. 제작자 행동의 공유 읽기·쓰기는 단계별 임시 계산과 명시 권한으로 연결했고 Risu Lua 변수·JSON 상태 API도 같은 Host를 사용해요. 선언형 `setvar` 트리거와 스크립트가 첫 효과가 아닌 혼합 트리거도 같은 Host로 연결했어요. 호환 CBS 원문의 `setvar`류 쓰기도 예약 영수증을 거쳐 같은 허용으로 채택돼요. 나머지 트리거 효과 종류는 남아 있어요. [공유 변수 계약](../docs/PROMPT-RUNTIME.md#분기-공유-변수) · [Lua 지원 범위](../docs/RISU-IMPORT.md#lua-콜백-가져오기)
...
현재 워크트리에 `main`을 병합한 뒤 채팅별 고정 층과 공통 모델 역할을 구현했어요. 삭제·비활성 고정 대상의 실행 차단, 전역 복귀, 최신 저장본 추종, 옵션 소속 충돌과 과거 snapshot 불변을 검사해요. 지정한 Lua 봇 검토 결과는 [정적 검토 보고](LUA-PORTING-REVIEW-2026-09-10.md)에 있으며 당시에는 런타임 확장이 없었어요. 이후 [베타 계획](BETA-PLAN.md)에서 JavaScript·Lua 격리 실행과 Risu 콜백·트리거 연결을 구현했어요.

L01(공급자별 경로)·L02(실기기 접속)는 당시 3일 실사용으로 기본 경로만 확인한 기록이에요. 베타의 공식 지원 환경·실제 표본 인수는 [베타 계획](BETA-PLAN.md)에서 별도로 확인해요. 중단된 품질 실험이나 유료 실행은 새 승인 없이 재개하지 않아요. 2026-09-10 결정 5의 런타임 코드 영구 미지원 방향은 [베타 결정](../docs/DECISIONS-2026-09-12-BETA.md)으로 대체됐고 실행 기반은 그 뒤 구현됐어요.
