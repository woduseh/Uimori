...
여섯 폭(360~1440) 자동 점검으로 가로 넘침·잘린 제어가 없음을 확인하고, 모바일에서 44px 미만이던 헤더 모델 칩·헤더 아이콘·상태 줄 상세 버튼과 터치 화면의 사이드바 행 아이콘을 44px로 맞췼어요. `quality`·빌드 통과, 관련 검사 14건 통과. [적용 기록](../docs/UI-DESIGN-V2.md#적용-기록). 아래는 이 변경 이전 기록이에요.
