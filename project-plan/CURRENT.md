...
공급자별 생성 옵션·캐시·짧은 응답 테스트, 최신 모델·연결 설정 한 벌, 입력 한도·대화 자동 요약을 구현하고 통합 검증 중이에요. Google은 global 고정이고 누적 호출·금액 예산을 제거했어요. 현재 DB·JSON archive는 **v9**이며 사용자 DB를 이관·초기화하지 않아요. 입력 한도 기본 272,000은 로컬 추정치이고 출력 한도는 늘리지 않았어요. 2026-09-08 사용자가 공통 자료·로어 병행 작업과 함께 main 병합·커밋을 승인했어요. 합본의 최종 검증은 **PENDING**이며 아래 과거 PASS와 구분해요. [개편 결과](PROVIDER-PARAMETERS-RESULTS.md) · [컨텍스트 계약](../docs/CONTEXT-LIMITS.md)
