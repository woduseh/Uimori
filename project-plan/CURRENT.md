...
> 2026-09-13: v0.1.0 준비의 제품 방향은 [베타 결정](../docs/DECISIONS-2026-09-12-BETA.md), 실행과 검증 상태는 [베타 계획](BETA-PLAN.md)을 따라요. 기본 Risu 가져오기와 사용자·모델·생성 전 자동 행동의 코드 계산이 구현됐으며, 전체 호환 실행·확장 SDK와는 구분해요.
...
- **사용자 버튼의 추가 모델 요청**은 기존 actions API에서 영구 작업으로 접수하고 같은 `model.generate` Host API·product attempt·상태 채택 경로를 사용해요. 화면 이동 뒤 진행·취소·결과 조회를 제공하고 원문은 바꾸지 않아요. 충돌 시 계산 결과를 미반영으로 보존하며 재시작·복원에서 자동 재실행하지 않아요. DB v18에 기록하고 archive15/chat-backup1 버전과 복원한 채팅의 허가 재선택을 유지해요. [사용자 작업 계약](../docs/EXTENSION-PROGRAMS.md#사용자-버튼의-모델-작업)
...
- **분기 공유 변수**는 사용자 명시 편집과 같은 기본값 resolver로 지침·템플릿·패널에 반영하고 Run 예약·분기·포크·candidate·백업에서 보존해요. 제작자 행동의 공유 읽기·쓰기는 단계별 임시 계산과 명시 권한으로 연결했으며 가져온 trigger·Lua 어댑터는 남아 있어요. [공유 변수 계약](../docs/PROMPT-RUNTIME.md#분기-공유-변수)
