...
2026-09-10 결정 3과 2026-09-11 후속 범위에 따라 검증을 나눠요. 명령 설명과 영역 대응은 [검사 안내](../docs/QUALITY.md)와 [검사 운영 정리](../docs/TESTING-AUDIT.md)를 따라요.
...
전체 회귀는 작은 변경마다 반복하지 않지만 필요한 릴리스의 실패를 무시하거나 영역을 줄여 통과로 바꾸지 않아요. BLOCKED와 미실행 검사를 PASS로 해석하지 않으며, 실제 모델 품질·청구 비용·휴대폰·Linux 동작은 합성 검증으로 보증하지 않아요.
