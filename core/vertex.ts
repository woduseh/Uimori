...
const emptyResult = (): ProviderResult => ({ status: 'error', text: '', toolCalls: [], refusal: null, error: null,
...
function scrub(value: Json, token: string): Json {
  ...
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, item]) =>
    [key, /^(authorization|api[_-]?key|credential|secret|password|access[_-]?token)$/i.test(key) ? '[REDACTED]' : scrub(item, token)]));
  return value;
}

/** Strict SSE framing: byte boundaries, CRLF, comments and multiline data are independent of JSON. */
export async function consumeSse(reader: ReadableStreamDefaultReader<Uint8Array>, onData: (value: unknown) => void, signal: AbortSignal, allowDone = false): Promise<void> {
  const decoder = new TextDecoder('utf-8', { fatal: true });
  let buffer = ''; let data: string[] = []; let dataBytes = 0; let totalBytes = 0;
  const dispatch = () => {
    if (!data.length) return;
    const payload = data.join('\n'); data = []; dataBytes = 0;
    let value: unknown;
    try { value = allowDone && payload === '[DONE]' ? payload : JSON.parse(payload); } catch { throw new ProviderContractError('INVALID_EVENT'); }
    onData(value);
  };
  const line = (raw: string) => {
    ...
    if (!value) dispatch();
      ...
      const part = value.slice(5).replace(/^ /u, ''); data.push(part); dataBytes += part.length;
  ...
  try {
    while (true) {
      ...
      signal.throwIfAborted();
      if (next.done) {
        try { buffer += decoder.decode(); } catch { throw new ProviderContractError('INVALID_UTF8'); }
      ...
      try { buffer += decoder.decode(next.value, { stream: true }); } catch { throw new ProviderContractError('INVALID_UTF8'); }
      let newline: number;
...
export async function executeVertexProvider(connectionValue: ProviderConnection, requestValue: unknown, options: ProviderExecutionOptions): Promise<ProviderResult> {
  ...
  try {
    ...
    if (!response.headers.get('content-type')?.toLowerCase().startsWith('text/event-stream') || !response.body) {
      await response.body?.cancel(); return failure('INVALID_CONTENT_TYPE');
    }
    ...
    if (tier === 'flex' && ['completed', 'tool_calls'].includes(result.status) && (!raw || typeof raw !== 'object' || Array.isArray(raw) || raw.trafficType !== 'ON_DEMAND_FLEX')) return { ...result, status: 'error', error: { code: 'FLEX_NOT_CONFIRMED' } };
    return result;
  } catch (error) {
    return failure(error instanceof ProviderContractError || error instanceof VertexProtocolError ? error.code : transportFailureCode(error));
  } finally {
    try { await reader?.cancel(); } catch { /* A closed remote stream must not erase observed usage. */ }
  }
}
