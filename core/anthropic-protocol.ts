...
function reject(code: string): never { throw new AnthropicProtocolError(code); }
...
function isJson(value: unknown, depth = 0): value is Json {
  ...
  return object(value) && [Object.prototype, null].includes(Object.getPrototypeOf(value)) && Object.values(value).every(item => isJson(item, depth + 1));
}
...
function readTurn(value: Json): AnthropicTurn {
  ...
  for (const item of value.toolNames) {
    if (!object(item) || !nonempty(item.name) || !nonempty(item.wireName) || !/^[A-Za-z0-9_-]{1,64}$/u.test(item.wireName) || names.has(item.name) || aliases.has(item.wireName)) reject('INVALID_ANTHROPIC_CONTINUATION');
    names.add(item.name); aliases.add(item.wireName);
  }
...
export function encodeAnthropic(request: ProviderRequest): { body: Json; context: AnthropicTurn } {
  ...
  const toolNames: ToolName[] = request.stable.tools.map((tool, index) => {
    if (!nonempty(tool.name) || tool.name.length > 200 || names.has(tool.name) || !nonempty(tool.description) || !object(tool.inputSchema)) reject('INVALID_TOOLS');
    names.add(tool.name);
    return { name: tool.name, wireName: 'tool_' + index + '_' + tool.name.replace(/[^A-Za-z0-9_-]/gu, '_').slice(0, 48) };
  });
  ...
  if (nativeJson) {
    try { schema = translationJsonSchema(input.source); } catch { reject('INVALID_ANTHROPIC_TRANSLATION_SOURCE'); }
  }
  ...
  if (request.opaqueState !== undefined && request.opaqueState !== null) {
    ...
    if (previous.modelId !== request.modelId || previous.bindingHash !== bindingHash || canonical(previous.toolNames as unknown as Json) !== canonical(toolNames as unknown as Json)) reject('ANTHROPIC_CONTINUATION_MISMATCH');
    if (previous.phase !== 'tool_calls' || !previous.pending.length || results.length !== previous.completedResults.length + previous.pending.length ||
        canonical(results.slice(0, previous.completedResults.length)) !== canonical(previous.completedResults)) reject('TOOL_RESULT_MISMATCH');
    const fresh = new Map<string, Record<string, Json>>();
    for (const item of results.slice(previous.completedResults.length)) {
      if (!object(item) || !nonempty(item.callId) || !nonempty(item.name) || !Object.hasOwn(item, 'result') || fresh.has(item.callId) ||
          (item.denied !== undefined && typeof item.denied !== 'boolean')) reject('TOOL_RESULT_MISMATCH');
      fresh.set(item.callId, item);
    }
    const assistant = previous.messages.at(-1);
    if (!object(assistant) || assistant.role !== 'assistant' || !Array.isArray(assistant.content) ||
        assistant.content.filter(part => object(part) && part.type === 'tool_use').length !== previous.pending.length) reject('INVALID_ANTHROPIC_CONTINUATION');
    const responses = previous.pending.map(call => {
      if (!nonempty(call.id) || !nonempty(call.name) || !nonempty(call.wireName) || !Number.isSafeInteger(call.index)) reject('INVALID_ANTHROPIC_CONTINUATION');
      const part = (assistant.content as Json[])[call.index];
      if (!object(part) || part.type !== 'tool_use' || part.id !== call.id || part.name !== call.wireName || !object(part.input)) reject('INVALID_ANTHROPIC_CONTINUATION');
      const item = fresh.get(call.id);
      if (!item || item.name !== call.name || (item.args !== undefined && canonical(item.args) !== canonical(part.input))) reject('TOOL_RESULT_MISMATCH');
      fresh.delete(call.id);
      return { type: 'tool_result', tool_use_id: call.id, content: JSON.stringify(item.result), ...(item.denied === true ? { is_error: true } : {}) };
    });
...
function token(value: Json | undefined): number | null {
  ...
  if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0) reject('INVALID_USAGE');
  return value;
}
function mergedUsage(previous: ProviderUsage, incoming: Json): ProviderUsage {
  ...
  for (const name of ['input_tokens', 'output_tokens', 'cache_creation_input_tokens', 'cache_read_input_tokens']) {
    const next = token(incoming[name]); const prior = token(old[name]);
  ...
  if (incoming.cache_creation !== undefined && incoming.cache_creation !== null) {
    ...
    for (const name of ['ephemeral_5m_input_tokens', 'ephemeral_1h_input_tokens']) token(incoming.cache_creation[name]);
  }
  ...
  const cacheCreation = raw.cache_creation_input_tokens === undefined ? 0 : token(raw.cache_creation_input_tokens);
  const cacheRead = raw.cache_read_input_tokens === undefined ? 0 : token(raw.cache_read_input_tokens);
  const inputTokens = base === null || cacheCreation === null || cacheRead === null ? null : base + cacheCreation + cacheRead;
  if (inputTokens !== null && !Number.isSafeInteger(inputTokens)) reject('INVALID_USAGE');
  return { inputTokens, outputTokens: token(raw.output_tokens), costUsd: null, raw: structuredClone(raw), priceRevision: null };
}

type StreamBlock = { content: Record<string, Json>; open: boolean; json: string; hasJsonDelta: boolean; invalid: string | null };
const truncated = new Set(['max_tokens', 'model_context_window_exceeded']);
const reasons = new Set(['end_turn', 'tool_use', 'max_tokens', 'stop_sequence', 'pause_turn', 'refusal', 'model_context_window_exceeded']);
...
export class AnthropicDecoder {
  ...
  accept(value: unknown): void {
    ...
    try { this.acceptEvent(copy(value)); }
    catch (error) {
  ...
  private acceptEvent(event: Json): void {
    ...
    if (event.type === 'message_start') {
      if (this.started || !object(event.message) || event.message.type !== 'message' || event.message.role !== 'assistant' || !nonempty(event.message.id) ||
          !Array.isArray(event.message.content) || event.message.content.length || (event.message.stop_reason !== undefined && event.message.stop_reason !== null)) reject('ANTHROPIC_INVALID_EVENT_ORDER');
      if (event.message.usage !== undefined) this.observedUsage = mergedUsage(this.observedUsage, event.message.usage);
      this.started = true; return;
    }
    // The API may introduce informational event types. They cannot modify content or mark completion.
    if (!['content_block_start', 'content_block_delta', 'content_block_stop', 'message_delta', 'message_stop'].includes(event.type)) return;
    if (!this.started) reject('ANTHROPIC_INVALID_EVENT_ORDER');
    if (event.type === 'message_delta') {
      if (event.usage !== undefined) this.observedUsage = mergedUsage(this.observedUsage, event.usage);
      ...
      if (event.delta.stop_reason !== undefined && event.delta.stop_reason !== null) {
        ...
        if (!nonempty(event.delta.stop_reason) || !reasons.has(event.delta.stop_reason)) reject('ANTHROPIC_UNKNOWN_STOP_REASON');
    ...
    if (event.type === 'message_stop') {
      ...
      if (!truncated.has(this.stopReason) && this.stopReason !== 'refusal' && !this.explicitRefusal) {
        ...
        if ((this.stopReason === 'tool_use') !== (this.calls.length > 0)) reject('ANTHROPIC_TOOL_TERMINAL_MISMATCH');
      }
      this.stopped = true; return;
    }
    if (this.stopReason) reject('ANTHROPIC_CONTENT_AFTER_TERMINAL');
    if (!Number.isSafeInteger(event.index) || (event.index as number) < 0 || (event.index as number) >= 1024) reject('INVALID_ANTHROPIC_BLOCK_INDEX');
    const index = event.index as number;
    if (event.type === 'content_block_start') {
      if (index !== this.blocks.length || !object(event.content_block) || !nonempty(event.content_block.type)) reject('ANTHROPIC_INVALID_EVENT_ORDER');
      const content = structuredClone(event.content_block);
      if (content.type === 'text' && typeof content.text !== 'string') reject('INVALID_ANTHROPIC_CONTENT_BLOCK');
      if (content.type === 'thinking') {
        if (typeof content.thinking !== 'string' || (content.signature !== undefined && typeof content.signature !== 'string')) reject('INVALID_ANTHROPIC_CONTENT_BLOCK');
      }
      if (content.type === 'redacted_thinking' && !nonempty(content.data)) reject('INVALID_ANTHROPIC_CONTENT_BLOCK');
      if (content.type === 'tool_use') {
        if (!nonempty(content.id) || !nonempty(content.name) || !object(content.input)) reject('INVALID_TOOL_ARGUMENTS');
        if (this.ids.has(content.id)) reject('DUPLICATE_TOOL_ID');
        if (!this.context.toolNames.some(tool => tool.wireName === content.name)) reject('ANTHROPIC_UNKNOWN_TOOL');
        if (this.blocks.filter(block => block.content.type === 'tool_use').length >= 32) reject('INVALID_TOOLS');
        this.ids.add(content.id);
      }
      this.blocks.push({ content, open: true, json: '', hasJsonDelta: false, invalid: null }); return;
    }
    ...
    if (event.type === 'content_block_delta') {
      ...
      if (delta.type === 'text_delta' && block.content.type === 'text' && typeof delta.text === 'string') block.content.text = (block.content.text as string) + delta.text;
      else if (delta.type === 'thinking_delta' && block.content.type === 'thinking' && typeof delta.thinking === 'string') block.content.thinking = (block.content.thinking as string) + delta.thinking;
      else if (delta.type === 'signature_delta' && block.content.type === 'thinking' && typeof delta.signature === 'string') block.content.signature = (block.content.signature as string ?? '') + delta.signature;
      else if (delta.type === 'input_json_delta' && block.content.type === 'tool_use' && typeof delta.partial_json === 'string') {
        if (Object.keys(block.content.input as Record<string, Json>).length) reject('ANTHROPIC_TOOL_INPUT_CHANGED');
        block.json += delta.partial_json; block.hasJsonDelta = true;
        if (block.json.length > 1_000_000) reject('EVENT_TOO_LARGE');
      } else if (delta.type === 'citations_delta' && block.content.type === 'text' && object(delta.citation)) {
        if (block.content.citations !== undefined && !Array.isArray(block.content.citations)) reject('INVALID_ANTHROPIC_CONTENT_BLOCK');
        block.content.citations = [...(block.content.citations as Json[] ?? []), structuredClone(delta.citation)];
      } else reject('ANTHROPIC_UNSUPPORTED_CONTENT_DELTA');
      return;
    }
    block.open = false;
    if (block.content.type === 'thinking' && !nonempty(block.content.signature)) block.invalid = 'ANTHROPIC_INCOMPLETE_THINKING';
    if (block.content.type === 'tool_use') {
      if (block.hasJsonDelta) {
        let parsed: unknown;
        try { parsed = JSON.parse(block.json); } catch { block.invalid = 'INVALID_TOOL_ARGUMENTS'; return; }
        if (!object(parsed) || !isJson(parsed)) { block.invalid = 'INVALID_TOOL_ARGUMENTS'; return; }
        block.content.input = structuredClone(parsed);
      }
      const tool = this.context.toolNames.find(item => item.wireName === block.content.name)!;
      this.calls.push({ id: block.content.id as string, name: tool.name, arguments: structuredClone(block.content.input as Record<string, Json>) });
      this.pending.push({ id: block.content.id as string, name: tool.name, wireName: tool.wireName, index });
    }
  }
  snapshot(): ProviderResult {
    const text = this.blocks.filter(block => block.content.type === 'text').map(block => block.content.text as string).join('');
    ...
    if (!this.fault && this.stopped) {
      if (this.stopReason === 'refusal' || this.explicitRefusal) { status = 'refused'; refusal = 'ANTHROPIC_REFUSAL'; }
        ...
        status = 'partial';
        error = { code: this.stopReason === 'max_tokens' ? 'MAX_TOKENS' : this.stopReason === 'model_context_window_exceeded' ? 'CONTEXT_WINDOW_EXCEEDED' : this.stopReason === 'pause_turn' ? 'ANTHROPIC_PAUSE_TURN' : 'ANTHROPIC_STOP_SEQUENCE' };
      }
    }
    const messages = this.blocks.length ? [...this.context.messages, { role: 'assistant', content: this.blocks.map(block => block.content) }] : this.context.messages;
    const { stateHash: _stateHash, ...base } = this.context;
    return structuredClone({
      status, text, toolCalls: this.calls, refusal, error, usage: this.observedUsage,
      opaqueState: seal({ ...base, phase: status, messages, pending: this.pending, usedIds: [...this.ids] }),
    }) as ProviderResult;
  }
  finish(): ProviderResult {
    if (this.fault) reject(this.fault);
    if (!this.stopped) { this.fault = 'UNEXPECTED_EOF'; reject(this.fault); }
...
export function diagnosticAnthropicBody(value: Json): Json {
  ...
  return Object.fromEntries(Object.entries(value).map(([key, item]) => [key,
    /^(signature|thoughtSignature|thought_signature)$/iu.test(key) ? '[provider signature withheld]'
      : (value.type === 'thinking' && key === 'thinking') || (value.type === 'redacted_thinking' && key === 'data') ? '[provider thinking withheld]'
