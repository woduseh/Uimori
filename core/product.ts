...
export type PromptPreset = ContentRef & { title: string; role: PromptRole; text: string };
export type TaskRole = 'main' | 'translation' | 'status' | 'image';
export type Connection = ContentRef & { title: string; protocol: ProviderProtocol; endpoint: string; credentialEnv?: string; requestTier?: VertexRequestTier; enabled: boolean; catalog: { id: string; name: string; capabilities: Record<string, boolean | null>; priceRevision: string | null }[]; catalogError: string | null };
export type ModelPreset = ContentRef & ModelGeneration & { title: string; connectionId: string; connectionRevision: number; modelId: string; timeoutMs?: number };
export type ChatProfile = { chatId: string; revision: number; attachments: ContentRef[]; creative: CreativeControls; routes: Record<TaskRole, ContentRef | null>; image: boolean; prompts?: Partial<Record<PromptRole,ContentRef | null>> };
...
export type Library = { promptPresets?: PromptPreset[]; contents: Content[]; presets: CreativePreset[]; connections: Connection[]; models: ModelPreset[]; assets: Asset[] };
