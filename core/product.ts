...
export type ModelGeneration = { maxOutputTokens: number; temperature: number | null; thinkingLevel?: 'LOW' | 'MEDIUM' | 'HIGH'; structuredOutput?: boolean; reasoningEffort?: 'none' | 'minimal' | 'low' | 'medium' | 'high' | 'xhigh' | 'max'; outputEffort?: 'low' | 'medium' | 'high' | 'xhigh' | 'max'; thinkingMode?: 'disabled' | 'enabled' | 'adaptive'; thinkingBudgetTokens?: number; verbosity?: 'low' | 'medium' | 'high'; reasoningMode?: 'standard' | 'pro'; reasoningContext?: 'auto' | 'all_turns' | 'current_turn'; topP?: number; stopSequences?: string[]; serviceTier?: string; cacheMode?: 'disabled' | 'explicit' | 'automatic'; cacheTtl?: '5m' | '30m' | '1h' };
...
export type Content = ContentRef & { coverImage?: {url:string;title:string}; kind: ContentKind; title: string; description: string; text: string; loading: 'pinned' | 'discoverable'; relatedIds: string[]; package?: import('./content-package.js').ContentPackage; hasPackage?: boolean };
...
export type Connection = ContentRef & { title: string; protocol: ProviderProtocol; endpoint: string; credentialEnv?: string; enabled: boolean; catalog: { id: string; name: string; capabilities: Record<string, boolean | null>; priceRevision: string | null }[]; catalogError: string | null; catalogUpdatedAt?: string | null };
export type ModelPreset = ContentRef & ModelGeneration & { title: string; connectionId: string; modelId: string; inputTokenLimit?: number; capabilityProtocol?: ProviderProtocol; capabilityRevision?: string; timeoutMs?: number; enabled?: boolean; evaluationTools?: EvaluationToolOptions; userOverrides?: { tools: boolean | null; structuredOutput: boolean | null; note: string }; source?: { kind: 'catalog' | 'manual'; catalogUpdatedAt: string | null } };
export type ModelSnapshot = ModelPreset & { connection: Connection };
export type ChatProfile = { loreContext?: import('./lore-context.js').LoreContextPolicy; hiddenStory?: import('./hidden-story-package.js').HiddenStorySelection; chatId: string; revision: number; attachments: ContentRef[]; creative: CreativeControls; routes: Record<TaskRole, ModelRef | null>; image: boolean; prompts?: Partial<Record<PromptRole,ContentRef | null>>; promptControls?: Record<string,import('./prompt-program.js').ChatPromptControls>; packageAttachments?: import('./content-package.js').PackageAttachment[]; packageValues?: Record<string,Record<string,import('./prompt-program.js').PromptValue>> };
...
export type Asset = { packageOwner?: { id: string; revision: number; role: import("./content-package.js").PackageRole; title: string; imageId: string }; id: string; chatId: string; revision: number; title: string; mime: string; hash: string; description: string; actor: string; outfit: string; location: string; allowedUse: 'profile' | 'inline' | 'both'; url: string };
