...
export type RegistrationConnectionDraft = Pick<
  Connection,
  'title' | 'protocol' | 'endpoint' | 'credentialEnv' | 'enabled'
>;
export type RegistrationModelDraft = Omit<
  ModelPreset,
  'id' | 'revision' | 'connectionId' | 'source' | 'userOverrides' | 'capabilityRevision'
>;
export type RegistrationPlan = {
  connection:
    | { kind: 'existing'; id: string; revision: number }
    | { kind: 'new'; draft: RegistrationConnectionDraft };
...
export type RegistrationView = Omit<
  RegistrationRun,
  'intentHash' | 'attempts' | 'targetSnapshot' | 'planConnectionSnapshot' | 'appliedSnapshot'
> & {
  modelCalls: number;
  usage: ProviderUsage | null;
};
