...
export function validateContextBudget(value: unknown): ContextBudget {
  if (!value || typeof value !== 'object' || Array.isArray(value) ||
      ...
      Object.keys(value).length !== 2 || !Object.hasOwn(value, 'inputTokenLimit') || !Object.hasOwn(value, 'estimator')) {
    throw new ProviderContractError('INVALID_CONTEXT_BUDGET');
  }
  const budget = value as ContextBudget;
  if (budget.estimator !== CONTEXT_ESTIMATOR || !Number.isSafeInteger(budget.inputTokenLimit) ||
      budget.inputTokenLimit < MIN_INPUT_TOKEN_LIMIT || budget.inputTokenLimit > MAX_INPUT_TOKEN_LIMIT) {
    throw new ProviderContractError('INVALID_CONTEXT_BUDGET');
  }
  return { inputTokenLimit: budget.inputTokenLimit, estimator: CONTEXT_ESTIMATOR };
}

export function contextBudgetForModel(model: { inputTokenLimit?: number; modelId?: string; maxOutputTokens?: number; connection?: { protocol: ProviderProtocol } }): ContextBudget {
  const configured = validateContextBudget({ inputTokenLimit: model.inputTokenLimit === undefined ? DEFAULT_INPUT_TOKEN_LIMIT : model.inputTokenLimit, estimator: CONTEXT_ESTIMATOR });
  const protocol = model.connection?.protocol;
  const capability = protocol && model.modelId ? modelCapability(protocol, model.modelId) : undefined;
  ...
  const window = capability?.protocol === 'vertex-gemini-v1' ? 1_048_576
    : capability?.protocol === 'anthropic-messages-v1' ? 1_000_000
    : capability?.protocol === 'openai-responses-v1' || capability?.protocol === 'openai-chat-v1' ? 1_050_000 : undefined;
  if (window === undefined) return configured;
  const reservedOutput = model.maxOutputTokens ?? capability!.maxOutputTokens;
  if (!Number.isSafeInteger(reservedOutput) || reservedOutput < 1 || reservedOutput > capability!.maxOutputTokens) throw new ProviderContractError('INVALID_CONTEXT_BUDGET');
  return validateContextBudget({ ...configured, inputTokenLimit: Math.min(configured.inputTokenLimit, window - reservedOutput, MAX_INPUT_TOKEN_LIMIT) });
}
...
export function estimateContextTokens(value: unknown): number {
  ...
  try { serialized = JSON.stringify(value); } catch { throw new ProviderContractError('INVALID_CONTEXT_INPUT'); }
  ...
  const pieces=new Map<string,number>();let tokens=0;
  for(let start=0;start<serialized.length;){
    let end=Math.min(serialized.length,start+4096);
    if(end<serialized.length&&/[\uD800-\uDBFF]/u.test(serialized[end-1])&&/[\uDC00-\uDFFF]/u.test(serialized[end]))end--;
    const piece=serialized.slice(start,end);let count=pieces.get(piece);
    if(count===undefined){count=tokenizer.encode(piece,[],[]).length;pieces.set(piece,count);}
    tokens+=count;start=end;
  }
...
export function assertContextBudget(body: unknown, budget?: ContextBudget): void {
  ...
  if (estimateContextTokens(body) > checked.inputTokenLimit) throw new ProviderContractError('INPUT_CONTEXT_LIMIT_EXCEEDED');
}
