...
export const PROVIDER_DEFINITIONS: readonly ProviderDefinition[] = Object.freeze([
  ...
  definition({ id: 'vertex-gemini-v1', label: 'Google Agent Platform', endpointDefault: '', credentialEnvDefault: '', auth: 'adc-or-bearer', catalog: 'local-support',
    optionKeys: ['maxOutputTokens', 'temperature', 'timeoutMs', 'thinkingLevel', 'topP', 'stopSequences', 'serviceTier'], reference: 'core/vertex.ts#executeVertexProvider',
    limitations: ['Gemini 모델 연결 · Gemini Enterprise Agent Platform의 global 프로젝트 endpoint를 사용해요.', '모델별 지원 명세에 있는 Gemini를 실행해요. 로컬 지원 목록은 계정의 원격 가용성을 보장하지 않아요.', 'ADC는 서버의 GOOGLE_APPLICATION_CREDENTIALS 파일을 사용해요. Flex는 모델의 Service Tier에서 선택해요. 실제 청구액은 미확인이에요.'] }),
  definition({ id: 'openai-responses-v1', label: 'OpenAI · Responses', endpointDefault: 'https://api.openai.com/v1', credentialEnvDefault: 'OPENAI_API_KEY', auth: 'bearer', catalog: 'remote',
    optionKeys: [...responsesOptions,'verbosity','reasoningMode','reasoningContext','serviceTier','cacheMode','cacheTtl'], reference: 'core/openai-protocol.ts#encodeResponses',
  ...
  definition({ id: 'anthropic-messages-v1', label: 'Anthropic · Messages', endpointDefault: 'https://api.anthropic.com/v1', credentialEnvDefault: 'ANTHROPIC_API_KEY', auth: 'api-key', catalog: 'remote',
    optionKeys: ['maxOutputTokens','timeoutMs','structuredOutput','outputEffort','thinkingMode','stopSequences','serviceTier','cacheMode','cacheTtl'], reference: 'core/anthropic-protocol.ts#encodeAnthropic',
    limitations: ['공식 API 기본 주소와 x-api-key 인증을 사용해요.', 'Claude Opus 5의 Output Effort와 Thinking을 모델별 조합에 맞게 사용해요.', '모델 기본값은 해당 API 필드를 생략해요. 실제 외부 실행은 사용자가 확인해요.'] }),
