...
import { sourceMemoryEntryAllowed, sourceReadRange, sourceRequestView } from './source-context.js';
...
function provenance(entry: MemoryEntry, sourceOffset = 0) {
  ...
  for (const ref of sources.slice(sourceOffset, sourceOffset + 12)) {
    ...
    if (bytes(base) > PROVENANCE_MAX_BYTES) {
      refs.pop();
      break;
    }
  }
  ...
  return {
    ...
    sourceOffset,
    provenanceTruncated: next < sources.length,
    sourceContinuation: next < sources.length ? { id: entry.id, sourceOffset: next } : null,
  };
}
...
export function executeStoryRead(
  ...
  if ((!snapshot.story?.memory && !allowWithoutMemory) || !STORY_READ_NAMES.includes(action.name))
    return denied('TOOL_NOT_ALLOWED');
  const scope = { chatId: snapshot.chatId, history: snapshot.history };
  const memory = snapshot.story?.memory;
    ...
    Object.keys(args).some(
      (key) =>
        !(
          search
            ? ['query', 'offset', 'limit']
            : action.name === 'memory.read'
              ? ['id', 'offset', 'limit', 'sourceOffset']
              : ['id', 'offset', 'limit']
        ).includes(key)
    )
  )
  ...
  if (
    ...
    (args.limit !== undefined && (!Number.isSafeInteger(args.limit) || Number(args.limit) < 1))
  )
    return denied('INVALID_ARGUMENTS');
  ...
  try {
    const entries = visibleMemoryEntries(scope, memory?.entries ?? []).filter((entry) =>
      sourceMemoryEntryAllowed(snapshot, entry)
    );
    let result: unknown;
    if (search) {
      ...
      if (action.name === 'story.search') {
        const found = snapshot.sourceSegments
          ? searchHiddenSources(snapshot, args.query, offset, limit)
          : searchMemorySources(scope, { query: args.query, offset, limit });
        ...
        result = boundedSearch(
          found.total,
          offset,
          found.results.map((entry) => {
            const { entry: metadata, ...page } = provenance(entry);
            return {
              ...metadata,
              ...page,
              excerpt: entry.text.slice(0, 240),
      ...
      if (action.name === 'story.read') {
        ...
        if (
          args.sourceOffset !== undefined &&
          (!Number.isSafeInteger(args.sourceOffset) || Number(args.sourceOffset) < 0)
        )
          return denied('INVALID_ARGUMENTS');
        const metadata = provenance(
          entry,
          args.sourceOffset === undefined ? 0 : Number(args.sourceOffset)
        );
        result = boundedRead(offset, Math.min(entry.text.length, offset + limit), (end) => ({
          ...metadata,
