...
export function planNativeMessages(
  ...
  const system: Json[] = [];
  const messages: Json[] = [];
  const cache = planProviderCache(request, protocol);
  const options = cache.options;
  const reject = (code: string, m: LogicalMessage, index: number): never => {
    throw new ProviderContractError(
      `${code}:block=${m.provenance.blockId}:index=${index}:protocol=${protocol}:model=${request.modelId}`
    );
  };
  const note = (
    code: string,
    m: LogicalMessage,
    index: number,
    status: MessageDiagnostic['status'] = 'mapped'
  ) =>
    diagnostics.push({
      code,
      blockId: m.provenance.blockId,
      logicalIndex: index,
      protocol,
  ...
  for (const [index, message] of prompt.messages.entries()) {
    ...
    if (
      ...
      reject('PROMPT_COMPLETED_ASSISTANT_AT_END_UNSUPPORTED', message, index);
    ...
    if (anthropic && role === 'system' && !wasLeading) {
      ...
      if (before?.role !== 'user' || (after && after.role !== 'assistant'))
        reject('PROMPT_MID_SYSTEM_PLACEMENT_UNSUPPORTED', message, index);
    }
    if (
      anthropic &&
      index > 0 &&
      prompt.messages[index - 1].role === message.role &&
      message.role !== 'system'
    )
    ...
    for (const anchor of prompt.cachePlan.filter((a) => a.afterMessageId === message.id)) {
      ...
      if (cache.disabled) {
        ...
        note('PROMPT_CACHE_DISABLED_BY_MODEL', message, index, 'not-applied');
        continue;
      }
      const supported = cache.breakpoint !== undefined;
      const available = cacheCount < cache.explicitLimit;
      if (!supported || !available) {
        if (anchor.policy === 'require')
          reject(supported ? 'PROMPT_CACHE_LIMIT' : 'PROMPT_CACHE_UNSUPPORTED', message, index);
        note(
          supported ? 'PROMPT_CACHE_LIMIT_NOT_APPLIED' : 'PROMPT_CACHE_NOT_APPLIED',
          message,
          index,
          'not-applied'
        );
    ...
    if ((anthropic || vertex) && message.role === 'system' && wasLeading) {
      system.push(...parts);
      note('LEADING_SYSTEM_TO_DEDICATED_FIELD', message, index);
    } else if (vertex) {
      const role = message.role === 'assistant' ? 'model' : 'user';
      const previous = messages.at(-1) as { role: string; parts: Json[] } | undefined;
      if (previous?.role === role) {
        previous.parts.push(...parts);
        note('CONSECUTIVE_ROLE_PARTS_COMBINED', message, index);
      } else messages.push({ role, parts });
    } else messages.push({ role, content: parts });
  }
  if ((anthropic || vertex) && messages.length === 0)
    throw new ProviderContractError('PROMPT_CONVERSATION_REQUIRED');
