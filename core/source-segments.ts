...
export type SegmentTranslationMarker = {
  range: SegmentRange;
  literal: string;
  segmentId: string;
  kind: 'open' | 'close' | 'scene' | 'portrait';
};
export function segmentTranslationMarkers(
  source: SegmentSource,
  policy?: SourceSegmentPolicy
...
function collectTranslationMarkers(
  ...
  const add = (
    ...
    range: SegmentRange,
    kind: SegmentTranslationMarker['kind']
  ) =>
    markers.push({
      range,
  ...
  for (const segment of document.segments) {
    for (const kind of ['open', 'close'] as const) {
      const range = segment.markers?.[kind];
      if (range) add(segment, range, kind);
    }
    if (segment.portrait) add(segment, segment.portrait.range, 'portrait');
    const scene = policy?.rules.find((rule) => rule.id === segment.ruleId)?.scene;
    if (segment.scene && scene) {
      ...
      add(
        segment,
        { start: range.start + open, end: range.start + open + scene.open.length },
        'scene'
      );
      for (let offset = open + scene.open.length; offset < close; ) {
        const found = raw.indexOf(scene.separator, offset);
        if (found < 0 || found >= close) break;
        add(
          segment,
          { start: range.start + found, end: range.start + found + scene.separator.length },
          'scene'
        );
        offset = found + scene.separator.length;
      }
      add(
        segment,
        { start: range.start + close, end: range.start + close + scene.close.length },
        'scene'
      );
    }
  }
...
export function validateSegmentTranslation(
  source: SegmentSource,
  text: string,
  policy?: SourceSegmentPolicy
): { ok: boolean; diagnostics: SegmentDiagnostic[] } {
  const original = parseSourceSegments(source, policy),
    translated = parseSourceSegments({ ...source, text }, policy),
    diagnostics = [...original.diagnostics, ...translated.diagnostics].filter(
      (d) => d.severity === 'error'
    );
  const shape = (doc: SegmentDocument, body: string) =>
    doc.segments
      .filter((s) => body.slice(s.range.start, s.range.end).trim())
