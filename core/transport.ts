...
export type ProviderRequest = {
  ...
  bootstrap?: { callId:string; name:string; args:Record<string,Json>; result:Json; denied:boolean }[];
...
export type ProviderUsage = { inputTokens: number | null; outputTokens: number | null; costUsd: number | null; raw: Json; priceRevision: string | null };
export type ProviderToolCall = { id: string; name: string; arguments: Record<string, Json>; recoveredFromTruncation?: boolean };
export type ProviderResult = {
  ...
  delivery?: { kind: 'evaluation-artifact'; noticeProvided: boolean; noticeCharacters: number; correctionCount: number };
};
export type WireRecord = {
  connectionId: string; protocol: ProviderConnection['protocol']; role: ProviderRole; modelId: string;
  method: 'POST' | 'RPC'; url: string; headers: Record<string, string>; body: Json; bodySha256: string; stablePrefixSha256: string;
};
export type CatalogModel = {
  id: string; label: string; capabilities: { tools: boolean | null; structuredOutput: boolean | null };
  pricing: { inputUsdPerMillion: number | null; outputUsdPerMillion: number | null; revision: string | null };
...
function keys(value: unknown, allowed: readonly string[]): asserts value is Record<string, unknown> {
  if (!object(value) || Object.keys(value).some(key => !allowed.includes(key))) reject('UNSUPPORTED_OPTIONS');
}
...
function numeric(value: unknown, integer = false): number | null {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || value < 0 || (integer && !Number.isSafeInteger(value))) reject('INVALID_USAGE');
  return value;
}
...
export function validateConnection(value: unknown, approvedOrigins: readonly string[]): ProviderConnection {
  ...
  if (value.protocol === 'codex-app-server-v1') {
    if (value.endpoint !== 'codex://local' || value.credentialEnv !== undefined || value.requestTier !== undefined) reject('INVALID_CODEX_CONNECTION');
...
function validateGeneration(value:unknown):asserts value is ModelGeneration{
  keys(value, ['maxOutputTokens', 'temperature', 'thinkingLevel', 'structuredOutput', 'reasoningEffort', 'thinkingMode', 'thinkingBudgetTokens']);
  if (value.structuredOutput !== undefined && typeof value.structuredOutput !== 'boolean') reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (value.reasoningEffort !== undefined && !['none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'].includes(value.reasoningEffort as string)) reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (value.thinkingMode !== undefined && !['disabled', 'enabled', 'adaptive'].includes(value.thinkingMode as string)) reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (value.thinkingBudgetTokens !== undefined && (!Number.isSafeInteger(value.thinkingBudgetTokens) || Number(value.thinkingBudgetTokens) < 1024 || Number(value.thinkingBudgetTokens) >= Number(value.maxOutputTokens))) reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (value.thinkingLevel !== undefined && !['LOW', 'MEDIUM', 'HIGH'].includes(value.thinkingLevel as string)) reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (!Number.isSafeInteger(value.maxOutputTokens) || (value.maxOutputTokens as number) < 1 || (value.maxOutputTokens as number) > 200_000) reject('UNSUPPORTED_GENERATION_OPTIONS');
  if (value.temperature !== null && (typeof value.temperature !== 'number' || !Number.isFinite(value.temperature) || value.temperature < 0 || value.temperature > 2)) reject('UNSUPPORTED_GENERATION_OPTIONS');
}

export function validateRequest(value: unknown): ProviderRequest {
  keys(value, ['role', 'modelId', 'stable', 'generation','generationBinding', 'input', 'opaqueState','prompt','bootstrap','toolChoice']);
  ...
  if(value.generationBinding!==undefined){
    validateGeneration(value.generationBinding);if(value.generation===undefined)reject('INVALID_GENERATION_BINDING');
    for(const key of ['temperature','thinkingLevel','structuredOutput','thinkingMode','thinkingBudgetTokens'] as const)if(value.generation[key]!==value.generationBinding[key])reject('INVALID_GENERATION_BINDING');
  }
  if(value.bootstrap!==undefined){
    if(!Array.isArray(value.bootstrap)||value.bootstrap.length>8)reject('INVALID_BOOTSTRAP');const ids=new Set<string>();
    for(const item of value.bootstrap){keys(item,['callId','name','args','result','denied']);string(item.callId);string(item.name);if(ids.has(item.callId)||!object(item.args)||typeof item.denied!=='boolean'||!Object.hasOwn(item,'result'))reject('INVALID_BOOTSTRAP');ids.add(item.callId);}
  }
  ...
  for (const tool of value.stable.tools) {
    keys(tool, ['name', 'description', 'inputSchema']); string(tool.name); string(tool.description, 4000);
    if (names.has(tool.name) || !object(tool.inputSchema)) reject('INVALID_TOOLS');
    names.add(tool.name);
  }
  if(value.toolChoice!==undefined){string(value.toolChoice);if(value.toolChoice!=='auto'&&!names.has(value.toolChoice))reject('INVALID_TOOL_CHOICE');}
  keys(value.input, ['task', 'controls', 'source', 'catalog', 'results', 'history']); string(value.input.task, 500_000);
  if (!object(value.input.controls) || Object.entries(value.input.controls).some(([key, val]) =>
    key.length > 100 || (val !== null && !['string', 'number', 'boolean'].includes(typeof val)) || (typeof val === 'number' && !Number.isFinite(val)))) reject('INVALID_CONTROLS');
  // JSON round-trip rejects circular/non-serializable application data before fetch.
  let encoded: string;
  try { encoded = JSON.stringify(value); } catch { return reject('INVALID_JSON'); }
...
export function parseCatalog(value: unknown): CatalogModel[] {
  ...
  return value.models.map(raw => {
    keys(raw, ['id', 'label', 'capabilities', 'pricing']); string(raw.id); string(raw.label, 400);
    if (ids.has(raw.id)) reject('DUPLICATE_MODEL');
    ids.add(raw.id);
    const cap = raw.capabilities ?? {};
    keys(cap, ['tools', 'structuredOutput']);
    if (Object.values(cap).some(v => v !== null && typeof v !== 'boolean')) reject('INVALID_CAPABILITY');
    ...
    return { id: raw.id, label: raw.label,
      capabilities: { tools: (cap.tools ?? null) as boolean | null, structuredOutput: (cap.structuredOutput ?? null) as boolean | null },
      pricing: { inputUsdPerMillion: numeric(price.inputUsdPerMillion), outputUsdPerMillion: numeric(price.outputUsdPerMillion), revision: (price.revision ?? null) as string | null },
      origin: 'catalog' };
  });
}
export function registerManualModel(id: string): CatalogModel {
  string(id);
  return { id, label: id, capabilities: { tools: null, structuredOutput: null }, pricing: { inputUsdPerMillion: null, outputUsdPerMillion: null, revision: null }, origin: 'manual' };
}
export function refreshCatalog(previous: readonly CatalogModel[], payload: unknown): { models: CatalogModel[]; error: string | null } {
  try {
    const incoming = parseCatalog(payload);
    return { models: [...incoming, ...previous.filter(model => model.origin === 'manual' && !incoming.some(item => item.id === model.id)).map(model => structuredClone(model))], error: null };
  } catch (error) { return { models: structuredClone([...previous]), error: error instanceof ProviderContractError ? error.code : 'CATALOG_UNAVAILABLE' }; }
}
...
function redact(value: Json, secret?: string): Json {
  ...
  if (object(value)) return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, /^(authorization|api[_-]?key|credential|secret|password|access[_-]?token)$/i.test(key) ? '[REDACTED]' : redact(item as Json, secret)]));
  return value;
}

/** Fetch + fatal UTF-8 decoder + SSE assembler. Never retries or follows redirects. */
export type ProviderExecutionOptions = {
  executeCodex?: (connection: ProviderConnection, request: ProviderRequest, options: ProviderExecutionOptions) => Promise<ProviderResult>;
...
export async function executeProvider(connectionValue: ProviderConnection, requestValue: ProviderRequest, options: ProviderExecutionOptions): Promise<ProviderResult> {
  if (connectionValue.protocol === 'codex-app-server-v1') {
    const connection = validateConnection(connectionValue, options.approvedOrigins), request = validateRequest(requestValue);
    if (request.generation && (request.generation.temperature !== null || request.generation.structuredOutput !== undefined || request.generation.thinkingLevel !== undefined || request.generation.thinkingMode !== undefined || request.generation.thinkingBudgetTokens !== undefined)) reject('UNSUPPORTED_GENERATION_OPTIONS');
    if (options.executeCodex) return options.executeCodex(connection, request, options);
    return { status: 'error', text: '', toolCalls: [], refusal: null, error: { code: 'CODEX_UNAVAILABLE' }, usage: { inputTokens: null, outputTokens: null, costUsd: null, raw: null, priceRevision: null }, opaqueState: null };
  }
  ...
  if (!Number.isSafeInteger(timeoutMs) || timeoutMs < 1 || timeoutMs > 600_000) reject('INVALID_TIMEOUT');
  const timeout = AbortSignal.timeout(timeoutMs);
  const signal = AbortSignal.any([options.signal, timeout]);
  let reader: ReadableStreamDefaultReader<Uint8Array> | undefined;
  let secret: string | undefined;
  const failure = (code: string) => {
    result.status = options.signal.aborted ? 'cancelled' : result.refusal ? 'refused' : (result.text || fragments.size ? 'partial' : 'error');
  ...
  try {
    ...
    const body = JSON.stringify({ protocol: connection.protocol, role: request.role, stable: request.stable, modelId: request.modelId, ...(request.generation ? { generation: request.generation } : {}), ...(request.bootstrap?{bootstrap:request.bootstrap}:{}), ...(request.toolChoice?{toolChoice:request.toolChoice}:{}), input: request.input, ...(request.prompt?{prompt:request.prompt}:{}), ...(request.opaqueState !== undefined ? { opaqueState: request.opaqueState } : {}) });
    ...
    await options.onWire?.({ connectionId: connection.id, protocol: connection.protocol, role: request.role, modelId: request.modelId,
      ...
      body: redact(JSON.parse(body) as Json, secret), bodySha256: sha(body), stablePrefixSha256: sha(stablePrefix) });
    if (signal.aborted) return failure('CANCELLED');
    const response = await fetch(connection.endpoint, { method: 'POST', headers, body, signal, redirect: 'error' });
    if (!response.ok) { await response.body?.cancel(); return failure(`HTTP_${response.status}`); }
    if (!response.headers.get('content-type')?.startsWith('text/event-stream') || !response.body) { await response.body?.cancel(); return failure('INVALID_CONTENT_TYPE'); }
    ...
    const dispatch = (): boolean => {
      ...
      try { const parsed: unknown = JSON.parse(data.join('\n')); if (!object(parsed)) reject('INVALID_EVENT'); event = parsed; }
      ...
      switch (event.type) {
        case 'text_delta':
          ...
          result.text += event.delta; break;
        case 'tool_delta': {
          if (!Number.isSafeInteger(event.index) || (event.index as number) < 0 || (event.index as number) > 31) reject('INVALID_TOOL_DELTA');
          const index = event.index as number;
          const fragment = fragments.get(index) ?? { id: '', name: '', args: '' };
          if (event.id !== undefined) { string(event.id); if (fragment.id && fragment.id !== event.id) reject('TOOL_ID_CHANGED'); fragment.id = event.id; }
          if (event.name !== undefined) { string(event.name); if (fragment.name && fragment.name !== event.name) reject('TOOL_NAME_CHANGED'); fragment.name = event.name; }
          if (event.argumentsDelta !== undefined) { if (typeof event.argumentsDelta !== 'string') reject('INVALID_TOOL_DELTA'); fragment.args += event.argumentsDelta; }
          fragments.set(index, fragment); break;
        }
        case 'usage': {
          keys(event, ['type', 'inputTokens', 'outputTokens', 'costUsd', 'raw', 'priceRevision']);
          if (event.priceRevision !== undefined && event.priceRevision !== null) string(event.priceRevision);
          result.usage = { inputTokens: numeric(event.inputTokens, true), outputTokens: numeric(event.outputTokens, true), costUsd: numeric(event.costUsd),
            raw: redact((event.raw ?? null) as Json, secret), priceRevision: (event.priceRevision ?? null) as string | null }; break;
        }
        case 'opaque_state': result.opaqueState = (event.state ?? null) as Json; break;
        case 'refusal':
          string(event.message, 10_000); result.refusal = event.message; break;
        case 'error': failure('PROVIDER_ERROR'); return true;
        case 'done': {
          if (!['stop', 'tool_calls', 'refusal'].includes(event.reason as string)) reject('INVALID_TERMINAL');
          if (result.refusal) { result.status = 'refused'; return true; }
          if (event.reason === 'refusal') reject('INVALID_TERMINAL');
          const ids = new Set<string>();
          for (const [, fragment] of [...fragments].sort(([a], [b]) => a - b)) {
            string(fragment.id); string(fragment.name);
            if (ids.has(fragment.id)) reject('DUPLICATE_TOOL_ID');
            ids.add(fragment.id);
            let args: unknown;
            try { args = JSON.parse(fragment.args); } catch { return reject('INVALID_TOOL_ARGUMENTS'); }
            if (!object(args)) reject('INVALID_TOOL_ARGUMENTS');
            result.toolCalls.push({ id: fragment.id, name: fragment.name, arguments: args as Record<string, Json> });
          }
          if ((event.reason === 'tool_calls') !== (result.toolCalls.length > 0)) reject('INVALID_TERMINAL');
          if (event.reason === 'stop' && !result.text.trim()) reject('EMPTY_COMPLETION');
          result.status = event.reason === 'stop' ? 'completed' : 'tool_calls'; return true;
        }
        default: reject('UNSUPPORTED_EVENT');
      }
      return false;
    };
    while (true) {
      const chunk = await reader.read();
      if (chunk.done) { decoder.decode(); return failure('UNEXPECTED_EOF'); }
      ...
      while ((newline = buffer.indexOf('\n')) >= 0) {
        ...
        if (line === '') { if (dispatch()) return result; }
        else if (line.startsWith('data:')) { const value = line.slice(5).replace(/^ /u, ''); data.push(value); dataLength += value.length; }
    ...
    try { await reader?.cancel(); } catch { /* Remote shutdown must not hide terminal evidence. */ }
  }
}
