...
function reject(code: string): never { throw new VertexProtocolError(code); }
...
export type VertexTurn = {
  ...
  contents: Json[]; completedResults: Json[]; pending: PendingCall[]; usedIds: string[]; stateHash: string;
};
...
export function encodeVertex(request: ProviderRequest): { body: Json; context: VertexTurn } {
  ...
  if (request.opaqueState !== undefined && request.opaqueState !== null) {
    ...
    if (previous.modelId !== request.modelId || previous.bindingHash !== bindingHash) reject('VERTEX_CONTINUATION_MISMATCH');
    if (previous.phase !== 'tool_calls' || !previous.pending.length || results.length !== previous.completedResults.length + previous.pending.length) reject('TOOL_RESULT_MISMATCH');
    if (canonical(results.slice(0, previous.completedResults.length)) !== canonical(previous.completedResults)) reject('TOOL_RESULT_MISMATCH');
    const fresh = results.slice(previous.completedResults.length);
    const indexed = new Map<string, Record<string, Json>>();
    for (const result of fresh) {
      if (!object(result) || !nonempty(result.callId) || !nonempty(result.name) || !Object.hasOwn(result, 'result') || indexed.has(result.callId)) reject('TOOL_RESULT_MISMATCH');
      indexed.set(result.callId, result);
    }
    const modelTurn = previous.contents.at(-1);
    if (!object(modelTurn) || modelTurn.role !== 'model' || !Array.isArray(modelTurn.parts)) reject('INVALID_VERTEX_CONTINUATION');
    const functionParts = modelTurn.parts.filter(part => object(part) && Object.hasOwn(part, 'functionCall'));
    ...
    const responses = previous.pending.map(call => {
      if (!nonempty(call.hostId) || !nonempty(call.name) || !Number.isSafeInteger(call.partIndex)) reject('INVALID_VERTEX_CONTINUATION');
      const part = (modelTurn.parts as Json[])[call.partIndex];
      if (!object(part) || !object(part.functionCall) || part.functionCall.name !== call.name || part.functionCall.id !== call.providerId) reject('INVALID_VERTEX_CONTINUATION');
      ...
      return { functionResponse: { ...(call.providerId !== undefined ? { id: call.providerId } : {}), name: call.name, response: object(result.result) ? result.result : { output: result.result } } };
    });
  ...
  const declarations = request.stable.tools.map(tool => {
    if (!/^[A-Za-z_][A-Za-z0-9_.:-]{0,127}$/u.test(tool.name) || names.has(tool.name) || !object(tool.inputSchema)) reject('INVALID_TOOLS');
    names.add(tool.name);
...
const refusalReasons = new Set(['SAFETY', 'RECITATION', 'BLOCKLIST', 'PROHIBITED_CONTENT', 'SPII', 'MODEL_ARMOR', 'IMAGE_SAFETY', 'IMAGE_PROHIBITED_CONTENT', 'IMAGE_RECITATION']);
const failureReasons = new Set(['OTHER', 'MALFORMED_FUNCTION_CALL', 'UNEXPECTED_TOOL_CALL', 'IMAGE_OTHER', 'NO_IMAGE']);
const promptReasons = new Set(['SAFETY', 'OTHER', 'BLOCKLIST', 'PROHIBITED_CONTENT', 'MODEL_ARMOR', 'IMAGE_SAFETY', 'JAILBREAK']);
...
function usage(raw: Json): ProviderUsage {
  ...
  for (const field of ['promptTokenCount', 'candidatesTokenCount', 'thoughtsTokenCount', 'totalTokenCount', 'toolUsePromptTokenCount', 'cachedContentTokenCount']) token(raw[field]);
  ...
  return { inputTokens: token(raw.promptTokenCount), outputTokens, costUsd: null, raw: structuredClone(raw), priceRevision: null };
}
...
export class VertexDecoder {
  ...
  accept(event: unknown): void {
    ...
    try { this.acceptEvent(copy(event)); }
    catch (error) {
  ...
  private acceptEvent(event: Json): void {
    ...
    if (event.promptFeedback !== undefined) {
      ...
      if (reason !== undefined && reason !== 'BLOCKED_REASON_UNSPECIFIED') {
        if (typeof reason !== 'string' || !promptReasons.has(reason)) reject('VERTEX_UNKNOWN_BLOCK_REASON');
        if (this.promptBlock || this.finishReason || this.parts.length) reject('VERTEX_DUPLICATE_TERMINAL');
    ...
    for (const candidate of event.candidates) {
      if (!object(candidate) || (candidate.index !== undefined && candidate.index !== 0)) reject('VERTEX_MULTIPLE_CANDIDATES');
      if (this.promptBlock) reject('VERTEX_DUPLICATE_TERMINAL');
      if (candidate.content !== undefined) {
        if (!object(candidate.content) || (candidate.content.role !== undefined && candidate.content.role !== 'model') || (candidate.content.parts !== undefined && !Array.isArray(candidate.content.parts))) reject('INVALID_VERTEX_EVENT');
        const parts = candidate.content.parts ?? [];
        if (this.finishReason && (parts as Json[]).length) reject('VERTEX_EVENT_AFTER_FINISH');
        for (const [key, value] of Object.entries(candidate.content)) if (key !== 'parts' && key !== 'role') this.contentMetadata[key] = value;
        for (const part of parts as Json[]) this.acceptPart(part);
      }
      if (candidate.finishReason !== undefined) {
        if (this.finishReason) reject('VERTEX_DUPLICATE_TERMINAL');
        const reason = candidate.finishReason;
        if (typeof reason !== 'string' || (!['STOP', 'MAX_TOKENS'].includes(reason) && !refusalReasons.has(reason) && !failureReasons.has(reason))) reject('VERTEX_UNKNOWN_FINISH_REASON');
        this.finishReason = reason;
      }
    }
  }
  private acceptPart(part: Json): void {
    if (!object(part) || (part.thought !== undefined && typeof part.thought !== 'boolean') || (part.text !== undefined && typeof part.text !== 'string') || (part.thoughtSignature !== undefined && typeof part.thoughtSignature !== 'string')) reject('INVALID_VERTEX_EVENT');
    if (['text', 'inlineData', 'fileData', 'functionCall', 'functionResponse', 'executableCode', 'codeExecutionResult'].filter(key => Object.hasOwn(part, key)).length > 1) reject('INVALID_VERTEX_PART');
    ...
    if (Object.hasOwn(call, 'partialArgs') || (call.willContinue !== undefined && call.willContinue !== false)) reject('VERTEX_STREAMED_TOOL_ARGUMENTS_UNSUPPORTED');
    ...
    if (!nonempty(call.name) || (call.id !== undefined && !nonempty(call.id)) || !object(args)) reject('INVALID_TOOL_ARGUMENTS');
    if (this.calls.length >= 32) reject('INVALID_TOOLS');
    const id = typeof call.id === 'string' ? call.id : `vertex-host-${this.context.bindingHash.slice(0, 16)}-${this.context.contents.length}-${partIndex}`;
    if (this.ids.has(id)) reject('DUPLICATE_TOOL_ID');
    this.ids.add(id);
    this.calls.push({ id, name: call.name, arguments: structuredClone(args) });
    this.pending.push({ hostId: id, name: call.name, partIndex, ...(typeof call.id === 'string' ? { providerId: call.id } : {}) });
  }
  snapshot(): ProviderResult {
    ...
    if (!this.fault) {
      ...
      } else if (this.finishReason === 'MAX_TOKENS') { status = 'partial'; error = { code: 'MAX_TOKENS' }; }
      else if (this.finishReason && refusalReasons.has(this.finishReason)) { status = 'refused'; refusal = this.finishReason; }
      else if (this.finishReason) { status = 'error'; error = { code: this.finishReason }; }
    }
    const contents = this.parts.length ? [...this.context.contents, { role: 'model', ...this.contentMetadata, parts: this.parts }] : this.context.contents;
    const { stateHash: _stateHash, ...base } = this.context;
    const opaqueState = seal({ ...base, phase: status, contents, pending: this.pending, usedIds: [...this.ids] });
    return structuredClone({ status, text: this.text, toolCalls: this.calls, refusal, error, usage: this.observedUsage, opaqueState }) as ProviderResult;
  }
...
export function diagnosticVertexBody(body: Json): Json {
  ...
  return Object.fromEntries(Object.entries(body).map(([key, value]) => [key,
    /^(thoughtSignature|thought_signature|signature)$/iu.test(key) ? '[provider signature withheld]'
