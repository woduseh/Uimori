...
function reject(code: string): never { throw new OpenAIProtocolError(code); }
...
function copy(value: unknown, code = 'INVALID_OPENAI_EVENT', depth = 0): Json {
  ...
  if (object(value) && [Object.prototype, null].includes(Object.getPrototypeOf(value))) return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, copy(item, code, depth + 1)]));
  return reject(code);
}
...
export type OpenAITurn = {
  version: 'openai-responses-turn-v1' | 'openai-chat-turn-v1'; modelId: string; bindingHash: string;
  phase: 'request' | ProviderResult['status']; input: Json[]; completedResults: Json[];
  pending: OpenAIPendingCall[]; usedIds: string[]; aliases: OpenAIToolAlias[]; stateHash: string;
};
function seal(value: Omit<OpenAITurn, 'stateHash'>): OpenAITurn { const { stateHash: _oldHash, ...body } = value as OpenAITurn; return { ...body, stateHash: hash(copy(body)) }; }
function readTurn(value: Json): OpenAITurn {
  ...
  if (hash(body) !== stateHash || !Array.isArray(value.input) || !Array.isArray(value.completedResults) || !Array.isArray(value.pending) || !Array.isArray(value.usedIds) || !Array.isArray(value.aliases)) return reject('INVALID_OPENAI_CONTINUATION');
...
function prepare(request: ProviderRequest, version: OpenAITurn['version']) {
  ...
  const bindingHash = hash(copy({ role: request.role, modelId: request.modelId, stable: request.stable, generation: generation ?? null, input }, 'INVALID_OPENAI_REQUEST'));
  ...
  const aliases = request.stable.tools.map((tool, index) => {
    ...
    names.add(tool.name);
    return { name: tool.name, providerName: `tool_${index}_${tool.name.replace(/[^A-Za-z0-9_-]/gu, '_').slice(0, 48)}` };
  });
  const format = request.role === 'translation' && (generation?.structuredOutput ?? version === 'openai-responses-turn-v1');
  let schema: Json | undefined;
  if (format) {
    try { schema = translationJsonSchema(input.source); } catch { return reject('INVALID_TRANSLATION_SOURCE'); }
  }
  ...
  if (request.opaqueState !== undefined && request.opaqueState !== null) {
    ...
    if (previous.version !== version || previous.modelId !== request.modelId || previous.bindingHash !== bindingHash || canonical(copy(previous.aliases)) !== canonical(copy(aliases))) reject('OPENAI_CONTINUATION_MISMATCH');
    if (previous.phase !== 'tool_calls' || !previous.pending.length || results.length !== previous.completedResults.length + previous.pending.length || canonical(results.slice(0, previous.completedResults.length)) !== canonical(previous.completedResults)) reject('TOOL_RESULT_MISMATCH');
    const index = new Map<string, Record<string, Json>>();
    for (const result of results.slice(previous.completedResults.length)) {
      if (!object(result) || !nonempty(result.callId) || !nonempty(result.name) || !Object.hasOwn(result, 'result') || index.has(result.callId)) reject('TOOL_RESULT_MISMATCH');
      index.set(result.callId, result);
    }
    fresh = previous.pending.map(call => {
      const result = index.get(call.id);
      if (!result || result.name !== call.name || aliases.find(alias => alias.providerName === call.providerName)?.name !== call.name) return reject('TOOL_RESULT_MISMATCH');
      index.delete(call.id); return result;
    });
  ...
  return { generation, results, bindingHash, aliases, schema, wireInput, previous, fresh, instructions };
}
...
function readUsage(value: Json, inputKey: string, outputKey: string): ProviderUsage {
  ...
  const count = (key: string) => {
    ...
    if (typeof number !== 'number' || !Number.isSafeInteger(number) || number < 0) return reject('INVALID_USAGE');
    return number;
  };
  count('total_tokens');
  // Both APIs already include reasoning tokens in output/completion_tokens; do not add them again.
  return { inputTokens: count(inputKey), outputTokens: count(outputKey), costUsd: null, raw: copy(value), priceRevision: null };
}
const empty = (): ProviderResult => ({ status: 'error', text: '', toolCalls: [], refusal: null, error: null,
...
function withState(result: ProviderResult, context: OpenAITurn, output: Json[]): ProviderResult {
  const pending = result.toolCalls.map(call => ({ id: call.id, name: call.name, providerName: context.aliases.find(alias => alias.name === call.name)!.providerName }));
...
export function encodeResponses(request: ProviderRequest): { body: Json; context: OpenAITurn } {
  const prepared = prepare(request, 'openai-responses-turn-v1');
  ...
  const input: Json[] = previous ? [...previous.input, ...previous.pending.map((call, index) => ({ type: 'function_call_output', call_id: call.id, output: JSON.stringify(fresh[index].result) }))]
    : [{ role: 'user', content: [{ type: 'input_text', text: 'Request data (JSON):\n' + JSON.stringify(prepared.wireInput) }] }];
  const body: Json = { model: request.modelId, instructions: prepared.instructions, input, stream: true, store: false,
...
export class ResponsesDecoder {
  ...
  constructor(context: OpenAITurn) { this.context = structuredClone(context); }
  private index(value: Json | undefined): number {
    if (typeof value !== 'number' || !Number.isSafeInteger(value) || value < 0 || value > 1024) return reject('INVALID_OUTPUT_INDEX');
    return value;
  }
  private state(event: Record<string, Json>): ItemState {
    const state = this.items.get(this.index(event.output_index));
    if (!state || !nonempty(event.item_id) || state.item.id !== event.item_id) return reject('OUTPUT_ITEM_MISMATCH');
    if (state.done) return reject('OUTPUT_ITEM_ALREADY_DONE');
    return state;
  }
  private mergeItem(index: number, item: Json, done: boolean): void {
    if (!object(item) || !nonempty(item.type) || (item.id !== undefined && !nonempty(item.id))) reject('INVALID_OUTPUT_ITEM');
    if (item.id !== undefined && this.context.input.some(previous => object(previous) && previous.id === item.id)) reject('DUPLICATE_OUTPUT_ITEM_ID');
    const old = this.items.get(index);
    if (old) {
      for (const key of ['id', 'type', 'role', 'call_id', 'name']) if (old.item[key] !== undefined && old.item[key] !== item[key]) reject('OUTPUT_ITEM_MISMATCH');
      if (typeof old.item.arguments === 'string' && (typeof item.arguments !== 'string' || !item.arguments.startsWith(old.item.arguments))) reject('TOOL_ARGUMENTS_MISMATCH');
      if (Array.isArray(old.item.content)) {
        if (!Array.isArray(item.content) || item.content.length < old.item.content.length) reject('OUTPUT_ITEM_MISMATCH');
        old.item.content.forEach((part, partIndex) => {
          const final = (item.content as Json[])[partIndex];
          if (!object(part) || !object(final) || part.type !== final.type) reject('OUTPUT_ITEM_MISMATCH');
          for (const key of ['text', 'refusal']) if (typeof part[key] === 'string' && (typeof final[key] !== 'string' || !(final[key] as string).startsWith(part[key] as string))) reject('OUTPUT_TEXT_MISMATCH');
        });
      }
    }
    if (item.id !== undefined && [...this.items].some(([otherIndex, other]) => otherIndex !== index && other.item.id === item.id)) reject('DUPLICATE_OUTPUT_ITEM_ID');
    this.items.set(index, { item: copy(item) as Record<string, Json>, done });
  }
  private refresh(strict: boolean): void {
    let text = ''; let refusal = ''; const calls: ProviderToolCall[] = []; const ids = new Set<string>();
    for (const [, state] of [...this.items].sort(([a], [b]) => a - b)) {
      const item = state.item;
      if (strict && item.status !== undefined && item.status !== null && item.status !== 'completed') reject('INCOMPLETE_OUTPUT_ITEM');
      if (item.type === 'message') {
        if (item.role !== 'assistant' || !Array.isArray(item.content)) { if (strict) reject('INVALID_OUTPUT_ITEM'); else continue; }
        for (const part of item.content as Json[]) {
          if (!object(part)) { if (strict) reject('INVALID_OUTPUT_ITEM'); else continue; }
      ...
      } else if (item.type !== 'function_call' && item.type !== 'reasoning' && strict) reject('UNSUPPORTED_OUTPUT_ITEM');
    }
  ...
  accept(value: unknown): void {
    ...
    if (event.sequence_number !== undefined) {
      ...
      if (typeof sequence !== 'number' || !Number.isSafeInteger(sequence) || sequence < 0 || (this.sequence !== undefined && sequence <= this.sequence)) reject('INVALID_EVENT_SEQUENCE');
      this.sequence = sequence;
    }
    const response = object(event.response) ? event.response : undefined;
    const responseId = event.response_id ?? response?.id;
    if (responseId !== undefined) {
      if (!nonempty(responseId) || (this.responseId !== undefined && this.responseId !== responseId)) reject('RESPONSE_ID_MISMATCH');
      this.responseId = responseId;
    }
    if (response?.usage !== undefined && response.usage !== null) this.result.usage = readUsage(response.usage, 'input_tokens', 'output_tokens');
    if (['response.created', 'response.in_progress', 'response.queued'].includes(event.type)) return;
    if (event.type === 'error' || object(event.error)) {
      this.terminal = true; this.result.status = this.result.text ? 'partial' : 'error'; this.result.error = { code: 'PROVIDER_ERROR' }; return;
    }
    if (event.type === 'response.output_item.added') {
      const index = this.index(event.output_index); if (this.items.has(index)) reject('DUPLICATE_OUTPUT_ITEM');
      this.mergeItem(index, event.item, false);
    } else if (event.type === 'response.output_item.done') {
      this.mergeItem(this.index(event.output_index), event.item, true);
    } else if (event.type === 'response.function_call_arguments.delta' || event.type === 'response.function_call_arguments.done') {
      const state = this.state(event);
      if (state.item.type !== 'function_call' || typeof state.item.arguments !== 'string') reject('OUTPUT_ITEM_MISMATCH');
      ...
      else { if (!data.startsWith(state.item.arguments)) reject('TOOL_ARGUMENTS_MISMATCH'); state.item.arguments = data; }
    } else if (event.type === 'response.content_part.added' || event.type === 'response.content_part.done') {
      const state = this.state(event); const index = this.index(event.content_index);
      if (state.item.type !== 'message' || !Array.isArray(state.item.content) || !object(event.part) || index > state.item.content.length) reject('OUTPUT_ITEM_MISMATCH');
      ...
      if (object(old)) for (const key of ['text', 'refusal']) if (typeof old[key] === 'string' && (typeof event.part[key] !== 'string' || !(event.part[key] as string).startsWith(old[key] as string))) reject('OUTPUT_TEXT_MISMATCH');
      state.item.content[index] = copy(event.part);
    } else if (/^response\.(output_text|refusal)\.(delta|done)$/u.test(event.type)) {
      const state = this.state(event); const index = this.index(event.content_index); const refusal = event.type.startsWith('response.refusal.');
      if (state.item.type !== 'message' || !Array.isArray(state.item.content) || !object(state.item.content[index])) reject('OUTPUT_ITEM_MISMATCH');
      const part = state.item.content[index] as Record<string, Json>; const field = refusal ? 'refusal' : 'text';
      if (part.type !== (refusal ? 'refusal' : 'output_text') || typeof part[field] !== 'string') reject('OUTPUT_ITEM_MISMATCH');
      ...
      else { if (!data.startsWith(part[field] as string)) reject('OUTPUT_TEXT_MISMATCH'); part[field] = data; }
      ...
      if (!response || response.status !== expected || !Array.isArray(response.output)) reject('INVALID_TERMINAL_EVENT');
      if (expected === 'completed' && [...this.items.keys()].some(index => index >= (response.output as Json[]).length)) reject('OUTPUT_ITEM_MISMATCH');
      response.output.forEach((item, index) => this.mergeItem(index, item, expected === 'completed'));
      ...
      return;
    } else if (event.type.startsWith('response.reasoning_') || event.type === 'response.output_text.annotation.added') {
      // Completed response.output is authoritative for opaque reasoning and annotations.
      return;
    } else reject('UNSUPPORTED_RESPONSES_EVENT');
    this.refresh(false);
  }
  snapshot(): ProviderResult { return withState(this.result, this.context, [...this.items].sort(([a], [b]) => a - b).map(([, state]) => state.item)); }
  finish(): ProviderResult { if (!this.terminal) reject('UNEXPECTED_EOF'); return this.snapshot(); }
}
