  ...
  for (const item of value) {
    if (
      ...
      Object.keys(item).some(
        (key) => !['id', 'title', 'description', 'blobHash', 'mime', 'allowedUse'].includes(key)
      )
    )
      throw new Error('PACKAGE_IMAGE_FIELDS');
    ...
    if (
      ...
      !image.title.trim() ||
      image.title.length > 200 ||
      typeof image.description !== 'string' ||
      image.description.length > 2000 ||
      typeof image.blobHash !== 'string' ||
      !/^[a-f0-9]{64}$/u.test(image.blobHash) ||
      !PACKAGE_IMAGE_MIMES.includes(image.mime) ||
      !['profile', 'inline', 'both'].includes(image.allowedUse)
    )
      throw new Error('PACKAGE_IMAGE_INVALID');
    ids.add(image.id);
  }
...
export function packageImages(
  profile: Pick<ProfileSnapshot, 'chatId' | 'packages' | 'packageAttachments'>
): Asset[] {
  return (profile.packageAttachments ?? []).flatMap((attachment) => {
    ...
    return validatePackageImages(pkg.images ?? []).map((image) => ({
      id: `package:${pkg.id}:${attachment.role}:${image.id}`,
