...
export function behaviorInputJsonSchema(schema: BehaviorSchema): Json {
  const label = { ...(schema.label ? { title: schema.label } : {}), ...(schema.description ? { description: schema.description } : {}) };
  switch (schema.type) {
    case 'number': return { ...label, type: schema.integer ? 'integer' : 'number', minimum: schema.min, maximum: schema.max };
    case 'string': return { ...label, type: 'string', maxLength: schema.maxLength };
    case 'boolean': return { ...label, type: 'boolean' };
    case 'enum': {
      const types = [...new Set(schema.values.map(value => typeof value))];
      return { ...label, ...(types.length === 1 ? { type: types[0] } : {}), enum: [...schema.values] };
    }
    case 'list': return { ...label, type: 'array', items: behaviorInputJsonSchema(schema.items), maxItems: schema.maxItems };
    case 'record': return { ...label, type: 'object', properties: Object.fromEntries(Object.entries(schema.properties).map(([key, child]) => [key, behaviorInputJsonSchema(child)])), required: Object.keys(schema.properties), additionalProperties: false };
  }
}
...
export function listBehaviorTools(snapshot: RunSnapshot): BehaviorToolBinding[] {
  const profile = snapshot.profile, bindings: BehaviorToolBinding[] = [], names = new Set<string>();
  for (const ref of profile?.packageAttachments ?? []) {
    const pkg = profile?.packages?.find(item => item.id === ref.id && item.revision === ref.revision);
    if (!pkg) throw new ContentPackageError('PACKAGE_SNAPSHOT_REVISION_MISSING', ref.id);
    validateContentPackage(pkg);
    if (ref.role === 'persona' && profile?.creative.personaReference === false) continue;
    for (const action of pkg.behavior?.actions ?? []) {
      if (!action.triggers?.includes('model')) continue;
      if (action.inputSchema.type !== 'record') throw new BehaviorError(400, 'BEHAVIOR_MODEL_INPUT_ROOT');
      const instanceId = packageInstanceId(ref), name = toolName(instanceId, action.id);
      if (names.has(name)) throw new BehaviorError(400, 'BEHAVIOR_TOOL_NAME_COLLISION');
      names.add(name);
      bindings.push({ instanceId, actionId: action.id, tool: {
        name,
      ...
      if (bindings.length > MAX_MODEL_BEHAVIOR_ACTIONS) throw new BehaviorError(400, 'BEHAVIOR_MODEL_ACTION_LIMIT');
    }
  }
  return bindings;
}
...
export function assertBehaviorToolCapability(snapshot: RunSnapshot, bindings = listBehaviorTools(snapshot)): void {
  if (!bindings.length) return;
  const target = snapshot.profile?.models.main;
  if (target && (target.userOverrides?.tools === false || target.connection.catalog.find(model => model.id === target.modelId)?.capabilities.tools === false)) {
