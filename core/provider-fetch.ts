...
export function providerFetchOptions(init: RequestInit & { signal: AbortSignal }): RequestInit & { dispatcher: Agent } {
  return { ...init, dispatcher: providerDispatcher };
}

const transportCodes = new Set(['UND_ERR_HEADERS_TIMEOUT', 'UND_ERR_BODY_TIMEOUT', 'UND_ERR_SOCKET', 'UND_ERR_CONNECT_TIMEOUT']);
...
export function transportFailureCode(error: unknown): string {
  ...
  try {
    for (let depth = 0; depth < 6 && current && typeof current === 'object' && !seen.has(current); depth++) {
      ...
      if (typeof candidate.code === 'string' && transportCodes.has(candidate.code)) return candidate.code;
      current = candidate.cause;
    }
  } catch { /* Unknown error properties are untrusted diagnostics. */ }
