...
export function imageCatalogPage(
  assets: readonly ImageMetadata[],
  query = '',
  offset = 0,
  limit = 20
) {
  if (
    ...
    throw new Error('ASSET_SEARCH_INVALID');
  const needle = query.toLocaleLowerCase();
  const found = assets.filter((asset) =>
    [asset.ref, asset.alt, asset.caption, asset.actorId, asset.clothing, asset.location].some(
  ...
  for (const asset of found.slice(offset, offset + limit)) {
    const item = imageMetadata(asset),
