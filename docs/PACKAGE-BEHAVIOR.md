...
상태창·선택기의 배치는 같은 패키지의 [커스텀 패널](PACKAGE-PANELS.md)로 구성할 수 있어요. 저장·입력·조건·추첨은 이 문서의 기존 행동 계약을 그대로 사용하며 패널이 별도 상태나 생성 권한을 갖지 않아요.

복잡한 알고리즘은 사용자·모델·생성 전 자동 행동의 [상태 계산 코드](EXTENSION-PROGRAMS.md)에 둘 수 있어요. `program`이 있는 행동은 해당 상태/입력으로 JavaScript를 실행하고 반환한 결과를 기존 schema·개정 검사 후 저장해요. 모델/자동 행동의 결과는 Run 임시 상태에 보관하고 성공 원문 저장 때 게시해요. 코드가 포함된 자동 준비 전체는 예약 후 순서대로 계산하며 실패·건너뛰기 때 부분 결과를 적용하지 않아요. 코드가 없는 선언형 자동 행동과 출력 파서는 계속 아래 계약을 사용해요.
...
`instructions`의 main 항목에 `position: "scene-state"`를 넣으면 메인 프롬프트의 동일한 이름을 가진 slot에 들어가요. 장착 순서, 패키지 내 지침 순서를 유지해요. 자동 host 지침에 중복 추가하지 않아요. 해당 slot이 프롬프트에 선언되지 않으면 그 지침만 제외하고 `PACKAGE_INSERTION_SLOT_MISSING` 경고를 기록해요. slot 자체의 조건은 프롬프트 작성자가 결정해요. 패키지 지침의 조건·계산 오류도 해당 지침에 한정해 제외하며, 여러 지침이 같은 실행 예산을 공유해요.
...
`behavior`는 `revision`, `schemaVersion`, `stateSchema`, `initialState`, `actions`, `outputParsers`를 가져요. `mode`는 기본 `authoritative`, 선택값 `annotation`이며 상태를 함께 확정하는 범위를 구분해요. 사용할 수 없는 상태·정의 변경·자동 준비 오류는 해당 Run의 `packageBehaviorUnavailable`에 기록하고 본문을 계속해요. 실패한 상태를 초깃값으로 위장하지 않으며 실행 문맥에는 `state: null`과 사용 불가 정보가 들어가요. 원래 정의·상태·추첨은 보존해 포크·백업/복원에서도 유실하지 않아요.
...
코드 행동이 공유 변수를 변경하면 `nextRequest`는 채택된 변경 후 `variables`를 읽어요. 해당 요청의 선택적 `variableContext`에는 변수값·개정·변수층 오류만 보존하고, 전체 profile이나 현재 DB 조회로 과거 표현을 다시 만들지 않아요. 복원은 당시 변수 쓰기 영수증의 개정·명시 set 값과 이 context를 대조한 뒤 같은 요청 계산식으로 문구를 검증해요. 변수·행동 상태·요청 예약은 함께 성공하거나 모두 취소되며, 같은 행동 명령의 재전송은 변수나 예약을 다시 만들지 않아요. 코드의 변수 권한과 임시 계산은 [공유 변수 Host API](EXTENSION-PROGRAMS.md#host-api로-분기-공유-변수-읽기와-쓰기)를 따라요.
...
모델 행동은 메인 요청 전체에서 최대 20개, 자동 선언은 최대 100개, 한 Run에서 실행하는 자동·모델 행동은 합계 최대 100개예요. Run/기회 journal 각각 JSON 4,000,000자 한도가 있어요. 도구 개수 초과나 공급자 catalog의 명시적 도구 미지원은 해당 behavior 인스턴스를 사용 불가로 기록하고 본문 요청을 유지해요. 기능 정보가 미확정인 프로바이더는 실행을 허용하지만 실제 공급자의 지원을 보증하지 않아요.

모델이 권한 없는 behavior 도구를 요청하거나 도구가 미등록·실행기 미연결인 경우에는 **그 작업만 실행하지 않고** 본문을 이어갈 기회를 남겨요. 실행기 없는 도구와 실패한 도구는 후속 요청에서 노출하지 않으며, 반복 호출도 다시 실행하지 않아요. 거절 영수증은 call ID·도구명·오류 코드와 미실행 안내만 가지며 인자는 `{}`예요. 미등록 이름은 `unapproved`로 표시하고, 호스트의 임의 진단 필드도 영수증·다음 모델 요청에 복사하지 않아요. 거절 판정 전에 저장하는 provider attempt 응답에서도 behavior 인자를 제외하며, 승인되어 성공한 인자는 실제 ToolEvent에 보존해요. 이 처리는 권한을 추가하거나 다른 도구로 우회하지 않아요.

전체 `maxCalls`와 평가 도구의 본문 호출 한도를 모두 지키고, 마지막 허용 요청에는 behavior 도구를 제외해요. 거절 때문에 예산을 추가하지 않으며, 모델이 계속 도구 호출만 반환하면 본문 완료를 보장할 수는 없어요. 연결 인증 실패·취소·원문/분기 소유권 상실과 분류되지 않은 호스트 거절은 본문 실행을 계속하지 않아요.
...
`scope`를 생략하거나 `"block"`으로 지정하면 기존 전체 텍스트 표식 규칙을 유지해요. `scope: "line"`은 앞뒤 공백을 제외한 한 줄 전체가 `start`로 시작하고 `end`로 끝나는 레코드를 읽어요. 여러 레코드가 같은 닫는 기호를 사용해도 다른 시작 표식의 줄과 충돌하지 않아요. 같은 시작 표식의 줄이 여러 개이거나 닫힘이 없으면 실패해요. line의 선택적 파서는 시작 표식 자체가 없을 때만 건너뛰며 다른 종류의 닫는 기호는 무시해요. JSON·구분자 해석 뒤의 상태 검증과 원자적 저장은 같은 경로를 사용해요.

구분자 형식의 `fieldCount`는 정확한 열 수 또는 허용하는 열 수 목록(예: `2` 또는 `[2, 3]`)이에요. 생략하면 기존의 필요한 경로만 읽는 동작을 유지해요. 필드의 `optional: true`는 입력 경로가 없을 때 해당 상태값을 보존해요. 존재하는 `null`·잘못된 타입·범위 초과를 누락으로 간주하지 않으며, 없는 중간 객체와 값이 잘못된 중간 객체도 구분해요.

이 옵션은 일반적인 레코드 추출과 상태 schema 적용을 위한 것이에요. 자료별 알고리즘·정규식 실행·임의 코드 해석을 이 파서의 옵션으로 계속 추가하지 않아요.

- 원문 500,000자 이하, 파서 최대 30개예요. 일반 정규식이나 실행 코드를 받지 않아요.
- 기본 block 범위에서는 `required: false`이며 시작·끝 표식이 **둘 다 없는 경우**만 건너뛰어요. 반쪽·중복 표식과 구조 손상은 실패예요. line 범위의 선택 규칙은 위 설명을 따라요.
- 자동·모델 행동과 authoritative 출력 파서는 여러 패키지에 걸쳐 한 묶음으로 적용해요. 파싱·값 계산이 실패하면 묶음 전체를 되돌리고 실패를 기록해요. 완료된 원문은 남으며 다음 본문은 사용할 수 없는 부가 상태를 제외하고 계속할 수 있어요.
...
- 순수 행동 입력·조건·효과·새 상태 값·출력 파서 평가의 오류만 `BehaviorEvaluationError`로 구분해 격리해요. HTTP 400이라는 이유만으로 복구 가능한 오류로 취급하지 않아요. 저장된 상태의 schema 검증, scope·원문 hash·소유권·SQL 오류와 이미 성공한 journal의 재검증 실패는 이 경계 밖에 있어요. 이 오류를 만난 완료 transaction은 원문·상태를 일부만 저장하지 않고 되돌려요.
...
- 본문·로어 등만 수정하고 `behavior` 정의가 같으면 상태·추첨·journal을 유지해요. 동작·parser·schema·initialState·behavior revision이 바뀌면 `BEHAVIOR_MIGRATION_REQUIRED`로 해당 상태의 자동 실행을 중단하고 기존 상태를 보여 줘요. 본문 전체를 차단하는 조건은 아니에요. 아래의 명시적 상태 업데이트를 사용하거나 **초깃값으로 복구**를 선택할 수 있어요. 복구는 최신 정의로 재설정하며 이전 상태와 정의 출처를 journal에 남겨 당시 schema로 검증해요.

## 자료 상태 업데이트

정의 변경 후 **상태 유지·변환 업데이트**에서 새 schema가 기존 값을 허용하면 **현재 상태 값 그대로 유지**, 제작자가 `behavior.migration`을 제공하면 **제작자의 상태 변환 사용**을 선택해요. **변경 내용 확인**으로 이전 값과 적용할 값을 비교하고 **확인한 상태로 업데이트**를 눌러야 저장해요. 변환 실패는 기존 상태를 보존하며 본문을 막거나 자동으로 초기화하지 않아요.

변환 코드는 명시적 preview POST에서만 실행해요. GET·일반 미리보기·가져오기·자료 저장·모델 예약·복원에서는 migration을 실행하지 않아요. 미리보기는 영구 저장을 하지 않고 서버가 후보를 최대 64개, 10분 동안 보관해요. 적용은 미리 확인한 후보만 자료·상태 revision과 원문 hash 등 문맥의 CAS 검사 후 한 transaction으로 저장하며 코드를 다시 실행하지 않아요. 만료·서버 재시작·후보 교체 후에는 다시 확인해야 해요. 이미 성공한 동일 적용 요청은 영구 영수증으로 중복 적용을 막아요.
...
분기는 선택한 원문 직후 상태를 사용해요. 과거 원문이 바뀐 후보는 거부하고 새 분기의 상태는 stale로 유지해요. **DB schema v17 / archive v15**는 현재 상태뿐 아니라 판정 기회·임시 실행·결과·출처의 일치도 검증해요. 알려진 v15 DB는 [migration](DATA-MIGRATIONS.md)으로 보존하며 올리고, **v0.1.0부터 지원 업그레이드의 데이터 보존**은 [베타 결정](DECISIONS-2026-09-12-BETA.md)을 따라 준비해요. 테스트용 개발 DB를 새로 시작하는 방법은 [개발 안내](DEVELOPMENT.md)를 봐요.

## 제공 범위와 검증

작업 내역에 **자료 처리 일부 미적용**을 표시하고 상세 경고는 요청할 때 조회해요. 목록 조회에는 불가 여부만 넣고 보존한 상태·도구 인자를 추가로 싣지 않아요. 이 처리는 본문 프롬프트 자체의 잘못된 계산, DB 손상·저장 실패, 원문/분기 소유권 충돌을 성공으로 바꾸지 않아요. 일반 본문 프롬프트의 `state`는 이야기 상태이고, 패키지 상태는 `packages` 안에서 읽어요. 불능 패키지를 동적 `get`·`let`으로 선택한 뒤 `state`를 숫자로 계산하는 master 프롬프트도 현재 실패하므로 명시적인 조건·대체값이 필요해요. master 전체나 블록을 자동 삭제하지 않으며, 공통 선택적 프롬프트 조각/fallback 제작 계약은 이식 과정에서 필요한 후속 구현으로 남겨요.
...
`model.generate` 또는 `conversation.read`를 선언한 사용자 행동은 기본 버튼·커스텀 패널의 기존 actions API에서 영구 작업으로 접수해요. 같은 Host API와 기존 상태 채택 경로를 사용하며 화면 이동 뒤에도 진행돼요. 자료 패널·작업 활동에서 진행·취소·계산 결과를 확인해요. 원문은 바꾸지 않고 현재 상태와 원문 의존성에 충돌이 없을 때만 기존 `ui-action` journal로 상태를 저장해요. 실패한 계산 결과는 상태에 미반영으로 남길 수 있어요. 대화 읽기와 모델 호출은 각각 정확한 자료 개정의 별도 허용을 사용하고 실제 모델 호출에는 전역 확장 모델도 필요해요. [사용자 작업 계약](EXTENSION-PROGRAMS.md#사용자-버튼의-영구-작업)을 봐요.
