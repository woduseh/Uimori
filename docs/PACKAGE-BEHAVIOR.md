...
복잡한 알고리즘은 사용자 행동의 [상태 계산 코드](EXTENSION-PROGRAMS.md)에 둘 수 있어요. `program`이 있는 행동은 해당 상태/입력으로 JavaScript를 실행하고 반환한 결과를 기존 schema·개정 검사 후 저장해요. 선언형 자동·모델 행동과 출력 파서는 계속 아래 계약을 사용해요.
