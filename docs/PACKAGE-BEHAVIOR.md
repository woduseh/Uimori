...
`model.generate`를 선언한 사용자 행동은 기본 버튼·커스텀 패널의 기존 actions API에서 영구 작업으로 접수해요. 같은 Host API와 기존 상태 채택 경로를 사용하며 화면 이동 뒤에도 진행돼요. 자료 패널·작업 활동에서 진행·취소·계산 결과를 확인해요. 원문은 바꾸지 않고 현재 상태와 원문 의존성에 충돌이 없을 때만 기존 `ui-action` journal로 상태를 저장해요. 실패한 계산 결과는 상태에 미반영으로 남길 수 있어요. 모델이 없어도 코드가 그 결과를 처리해 상태만 반환할 수 있으며 실제 호출에만 정확한 자료 허가와 전역 확장 모델이 필요해요. [사용자 작업 계약](EXTENSION-PROGRAMS.md#사용자-버튼의-모델-작업)을 봐요.
