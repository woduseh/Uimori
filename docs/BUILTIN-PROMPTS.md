...
원문은 `server/builtin-prompts/`의 Risu 네이티브 JSON으로 보관해요. 작문 본문 1개, 번역 본문 1개, 기본 선택값 2개, 협업 정의 3개를 사용해요. 조건과 값 삽입은 원본 CBS, 옵션 입력은 Risu 토글 선언을 사용해요. 별도의 블록 AST·변환기·실행용 표현식 문법은 보관하지 않아요. OOC 사본은 같은 작문 원본에 OOC 선택값과 협업 설정을 적용해요.

Phēmē의 인물/설정 래퍼는 원본 `#when` 조건문으로 현재 모드 하나만 펼쳐요. Global Note 기본 본문은 비어 있으며 카드의 후문 지침 교체는 그대로 지원해요. 옵션 그룹·설명문과 문체 참고·독자 취향·직전 응답 피드백·직접 지정 장르의 여러 줄 입력을 제공해요. Hermēneía는 번역 원칙을 유지하고 네이티브 `chat` 블록으로 전체 원문과 당시 참고 자료를 받아요.

Phēmē의 캐시 지점 네 개를 사용하는 Responses·Anthropic 연결은 **명시적 캐시**를 선택해요. 자동 캐시는 호스트 지점 하나를 추가하므로 작성된 지점 네 개와 함께 쓰면 `PROMPT_CACHE_LIMIT`가 발생해요. 프리셋 선택은 모델의 캐시·출력 토큰 설정을 바꾸지 않아요.

## 검증 범위

`tests/builtin-prompts.test.ts`는 새 환경의 초기 선택, 기존 DB 재시작 시 보존, 목록 조회의 비변경성, 추가와 적용의 분리, 독립 사본, 네이티브 원본과 독립 선택값을 확인해요. 합성 브라우저 검사는 `npm run verify:packages`에 포함돼요. 실제 모델의 창작·번역 품질과 비용·지연은 별도 평가 대상이에요.
