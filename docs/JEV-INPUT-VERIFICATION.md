...
The default probe sends 128- and 16,000-estimated-token candidates through both judgment paths (four live calls). `--within-budget` selects only the two long calls. `--diagnostic-max-main` instead expects the 500,000-token candidate to be rejected locally with `JEV_INPUT_BUDGET`, zero attempts and zero HTTP transports; that expected result exits successfully. Any other error or unexpected transport still fails. The report records each case's expected outcome and `passed` separately from its application status. Diagnostic HTTP errors retain at most 8 KiB of provider error data and never headers or credentials:
