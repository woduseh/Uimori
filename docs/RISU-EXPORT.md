...
Uimori의 채팅·분기·사용자 메모·JEV 설정·이미지 위임 설정·협업 설정·현재 토글 선택은 Risu 자료 형식에 포함하지 않아요. 프리셋 가져오기에서 제외했던 모델·연결·인증정보·생성 설정도 복원하지 않아요. 작업실 전체는 **설정 → 데이터 관리**의 SQLite 백업으로 보존해요. 개별 자료와 채팅의 휴대용 파일은 새 사본으로 가져오며, 구형 전체 JSON 아카이브 복원은 지원하지 않아요.
