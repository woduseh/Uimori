...
For an existing save, preserve the native editing model and supply the latest revision according to `resource.save`'s discovered schema. Mutation identity is host-owned: app-operation schemas do not ask the model to invent an `operationId`; the helper derives a stable receipt key from the helper task and provider tool-call ID while revision checks remain explicit. Editing tools intentionally retain full models where necessary; a simple factual question should use the data tools instead. A user request to review or propose still does not authorize saving.
