...
`data.read` also accepts `refs` (up to four) instead of `ref`. Each entry returns its own result or error; a shared text offset/limit applies to all entries. Mixed directory and text reads work with omitted limits. A non-null `nextIndex` identifies refs not returned because of the batch output budget. Submit those remaining refs in another call. Source-specific `nextOffset` still indicates unread content inside each field.
