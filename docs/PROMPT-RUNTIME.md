...
재컴파일은 고정 CBS 결과를 투영하며 평가를 반복하지 않아요. 해시나 필드 목록이 원본과 다르면 영수증을 거절해요. 번역은 번역 작업 예약 시 고정한 현재 프롬프트를 별도로 평가하되 카드 상태나 저장된 프리셋을 바꾸지 않아요. 호스트는 번역할 원문과 원문 시점의 참고 자료·사용자 메모를 현재 user 메시지에 담고, 네이티브 `chat` 블록이 이를 배치해요. 원문은 CBS로 평가하지 않으며 프리셋이 현재 메시지를 제외한 경우에만 모델 입력의 고정 자료로 별도 전달해요.
