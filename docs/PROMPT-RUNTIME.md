...
2. 네이티브 CBS worker가 활성 블록에서 사용하는 필드를 순서대로 평가하고 `nativeRisuPresetProgram`에 원본·평가 입력 해시, 평가문, 변수, 진단을 기록해요. 꺼진 jailbreak·cot, 호스트가 실행하지 않는 memory 블록, 비어 있는 슬롯의 사용하지 않는 형식은 변수 부작용도 실행하지 않아요.
...
새 컴파일러 표시는 `risu-native-prompt-2`예요. 저장된 `risu-native-prompt-1`은 과거 네이티브 메시지·영수증을 재현하는 용도로만 지원해요. 과거 독자 컴파일러 버전은 실행하지 않아요. 히스토리 식별자와 원문 해시를 유지하고 같은 히스토리를 중복 삽입하는 구성은 거절해요. Risu의 `memory` 블록은 별도 기억을 주입하지 않으며 Uimori가 준비한 기억·문맥을 사용해요. 모델 요청 전 준비가 필요한 네이티브 프롬프트는 CBS 평가가 끝나기 전 전송하지 않아요.

카드의 `post_history_instructions`는 `type2: globalNote` 블록의 원문을 교체하고 `{{original}}`을 그 블록 본문으로 치환한 뒤 CBS를 평가해요. 별도의 전역 지침으로 중복 삽입하지 않아요. `mes_example`은 원문의 화자와 `<START>` 경계를 먼저 읽고 각 메시지 본문을 CBS로 평가하며, `chat` 범위의 앞부분에 역할을 유지해 배치해요. 실제 분기 기록에는 예시를 저장하지 않아요. `chatML` 메시지 역할, `sendName`의 이름 표기, `postEverything` 위치의 `postEndInnerFormat`도 구성에 반영해요. 후문은 Risu처럼 CBS·슬롯 치환 없이 원문으로 들어가며 해당 위치 블록이 없으면 삽입하지 않아요. `innerFormat`과 `role2`는 persona·description·authornote에만 적용해요. 기존 `assistantPrefill`은 Uimori 공급자 prefill 계약을 유지해요. 가져오기 검토는 미지원 블록과 Uimori의 독립 호스트 정책 때문에 적용하지 않는 스위치를 알려요.
...
`tests/risu-native-prompt-composition.test.ts`, `tests/risu-native-preset.test.ts`, `tests/risu-native-semantics.test.ts`는 실제 vendor CBS, 활성 조건·평가 순서, 글로벌노트 교체, 예시 역할, 토글, 슬롯, 대화 범위, 캐시, prefill과 영수증 일치를 확인해요. `tests/chat-options.test.ts`는 합산 토글의 채팅 선택·예약·백업 경계를 확인해요. `tests/risu-preset-import.test.ts`는 프롬프트만 가져오는 경계와 저장을 확인해요. 이 검사는 실제 공급자의 창작·번역 품질을 판정하지 않아요.
