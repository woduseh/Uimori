...
모델 요청의 봇·페르소나 설명은 `uimori_source`로, 일반 로어는 `uimori_lore_sources` 아래 자료별 `source`와 항목별 `entry`로 구분해요. 연결 역할·자료 이름·자료 ID와 로어 ID·제목만 표시해요. 호스트 참고 메시지에 표지의 의미를 한 번 안내하고 출처에 따른 우선순위나 적용 대상을 부여하지 않아요. 특정 프리셋이나 토글에 의존하지 않으며 기존 `innerFormat` 적용 범위는 유지해요.
