...
설정 → 일반 → 원고 읽기 또는 채팅 메뉴 → 읽기 설정에서 바꿔요. 읽기 스타일·간격·부호별 역할은 `uimori:readability` localStorage에 저장하며 같은 origin의 다른 탭에도 반영해요. 도우미 답변·독립 가정 장면·생성 중 공개 텍스트와 설정 예문은 앱의 인용 스타일을 사용해요. 원문·저장된 번역의 Risu 메시지 프레임은 글꼴·크기·줄간격·테마를 따르며, 카드가 만든 HTML에 앱의 인용 강조나 줄바꿈 요소를 덧씌우지 않아요. 입력한 요청, 편집 초안, 설정 지침, 도구 JSON, 상태·진단 카드는 적용 대상이 아니에요.
