...
본문은 불투명한 바탕, 요청은 작은 연출 메모, 복사·수정·장면 메뉴는 본문 위에 배치해요. 읽기 글꼴·크기·행간·폭 설정과 봇 HTML이 우선하며, 장면 본문에 별도 고정 높이나 스크롤 상자를 만들지 않아요. 유리 효과는 작은 입력창에만 제한하고, 자동 색상 추출·배경 일러스트 생성·외부 폰트·스크롤 애니메이션은 사용하지 않아요.
