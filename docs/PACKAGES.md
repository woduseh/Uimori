...
DB schema와 전체 archive의 현재 버전은 [현재 버전](DATA-MIGRATIONS.md#현재-버전)을 봐요. [DB migration](DATA-MIGRATIONS.md)과 작품 교환 형식의 버전을 구분해요. 서재의 분류·폴더는 자료 ID에 붙는 별도 정리 정보이며, 프롬프트 관리는 분리돼요. 분류 이동은 패키지 내용·개정이나 장착 역할을 바꾸지 않아요. 독립 로어·작가 설정·창작 스킬·명칭집 종류는 제거했고 패키지 내부 로어·대상별 지침과 원문시점 참조는 유지해요. 저장·CAS·보관 계약은 [서재와 프롬프트 관리](LIBRARY.md)를 봐요.
...
- 선택적 `compat.risuCbs.fields: string[]`는 공통 AST로 옮길 수 없어 원문 그대로 보존한 Risu CBS 항목을 가리켜요. 각 항목은 `body`이거나 `lore:<로어 ID>`·`instruction:<지침 ID>`·`start:<도입문 ID>` 형식이며 같은 패키지 안의 실제 ID만 가리켜요. 최대 500개예요. 선언은 저장된 텍스트를 바꾸지 않고 "이 필드는 예약 시점에 [Risu 스냅샷 평가기](RISU-IMPORT.md#기본-변수와-읽기-cbs)가 한 번 평가한다"는 표시예요. 평가 결과는 [Run snapshot 영수증](RESERVATION-SNAPSHOTS.md#snapshot-영수증)에 고정되고, 영수증이 없는 조회에서는 저장된 원문을 그대로 사용해요. 가져오기만 이 선언을 만들며 직접 만든 패키지에는 없어요.
...
- 선택적 `loreActivation: {mode, scanDepth?, recursiveScanning?, fullWordMatching?}`은 이 패키지의 로어가 모델에 닿는 방식이에요. `mode`가 `keyword`이면 `activation` 규칙이 있는 로어를 예약 시점에 Risu 스냅샷의 활성화 엔진이 최근 대화·요청으로 판정하고, 판정된 항목만 그 Run의 고정 자료가 돼요. 판정되지 않은 항목은 그 Run에서 조회 목록에도 나오지 않아요. `model`이면 생성마다 문맥 정리 모델이 `discoverable` 로어 가운데 이번 턴에 필요한 항목을 한 번 골라 그 Run의 고정 자료로 넣고, 고르지 않은 항목은 조회 목록과 읽기 도구에 그대로 남아요. `discoverable`이거나 필드가 없으면 규칙을 무시하고 `loading`대로 동작해요. 나머지 세 값은 Risu 로어북 설정이며 생략값은 Risu 기본(5·켬·끔)이에요. 활성 예산은 패키지가 아니라 [채팅 설정의 조회 로어 문자 한도](LORE-CONTEXT.md#키워드-활성화)가 정해요.
- 로어의 선택적 `activation: {keys, secondaryKeys?, selective?, regex?, child?, rules?}`는 가져오기가 보존한 Risu 활성화 규칙이에요. `keys`·`secondaryKeys`는 Risu의 쉼표 구분 키 문자열, `rules`는 Risu가 본문 앞에서 읽는 `@@` 지시문 줄(최대 64줄·4,000자, 모든 줄이 `@@`로 시작)이에요. 엔진의 `insertorder`는 `loreContext.order`, 항상 활성 여부는 `loading === 'pinned'`, 검색·예산 계산 본문은 저장된 `text`를 사용해요. 규칙은 데이터일 뿐이며 패키지가 `keyword` 모드일 때만 읽어요.
...
서재 분류와 패키지의 장착 역할은 별도 값이에요. 봇·페르소나·모듈 어느 종류의 패키지든 장착 계약의 `role`로 실제 역할을 지정하며, 화면별 선택 후보 정책은 별도로 적용해요. 복제는 별도 자료를 만드는 선택이고 역할 사용 자체는 ID나 원본 분류를 바꾸지 않아요. 페르소나도 본문·로어·이미지·지침·옵션·동작을 같은 형식으로 작성해요.
...
Reader의 **이미지 자동 배치 / 이미지 다시 배치**는 현재 보고 있는 원문 또는 번역문을 읽고, 등록된 이미지와 그 글의 문단 위치를 함께 골라요. 자료의 대표 이미지와 프로필 전용 이미지는 배치 후보에서 제외해요. 원문용·번역문용 결과를 별도 슬롯에 저장하고 각 문단 뒤에 표시하며, 저장된 원문·번역문 텍스트는 고치지 않아요. 두 보기는 서로 다른 이미지와 위치를 선택할 수 있어요.

채팅의 **봇·페르소나·모듈** 설정에서 **원문 이미지 자동 배치**(기본 OFF)와 **번역 이미지 자동 배치**(기본 ON)를 따로 조절해요. 자동 배치를 꺼도 각 보기의 버튼으로 수동 요청할 수 있어요. 번역 자동 배치는 모델 번역이 성공한 뒤 배치 후보가 있을 때 예약해요. 직접 번역 저장은 모델을 호출하지 않으며, 배치는 버튼으로 다시 요청해요.

수동 배치는 현재 장착 자료·이미지 후보와 전역 이미지 모델을 예약 시 고정해요. 번역 자동 배치의 후보·모델·실행 여부는 번역 요청 시 고정해요. 원문 hash와 보기별 최신 이미지 작업 개정의 CAS로 중복·낡은 요청을 거부하고, 진행 중인 같은 작업을 재사용해요. 번역용 배치는 번역 job ID·개정·텍스트 hash에도 연결해요. 새 번역이 진행 중이거나 실패하면 이전 성공 번역과 그 배치를 유지하고, 새 번역이 완료되거나 직접 수정하면 이전 번역 배치를 무효화해요. 원문 수정은 양쪽 배치를 무효화해요. 재시도는 고정된 입력을 유지하고 현재 글과의 연결을 재검사해요. 포크·보관 복원에서도 번역 연결·문단 위치·이미지 참조를 검증해요.

패키지 JSON 내보내기는 이미지가 있으면 이미지 bytes를 포함한 `uimori-package-bundle`을 만들어요. 가져오기는 64 MB 이내 파일의 hash와 모든 이미지 참조를 검증한 뒤 초안 적용을 제안해요. 연결한 공통 모듈은 이 단일 패키지 파일에 포함하지 않으므로 대상 서재에 같은 ID·개정이 필요해요. 선택한 자료·연결 모듈·이미지·프롬프트를 새 사본으로 옮길 때는 [자료 파일 이동](NATIVE-TRANSFER.md)을, 채팅 이력까지 옮길 때는 [채팅 전체 백업](CHAT-BACKUP.md)이나 전체 보관 기능을 사용해요. 이미지 목록이 없으면 비어 있으며 합성 기본 이미지를 자동 주입하지 않아요.

이미지 bundle의 초안 준비는 검증한 불변 이미지 blob을 먼저 저장하며 자료 정의는 사용자가 검토 후 저장할 때 적용해요. 초안 취소가 과거 자료나 다른 초안이 참조할 수 있는 이미지의 삭제를 뜻하지는 않아요. [자료 파일 이동](NATIVE-TRANSFER.md)의 prepare는 이미지도 저장하지 않고, 확인한 자료 묶음을 새 ID로 한 번에 가져오는 별도 경로예요.

## 도입문과 채팅 시작

`starts`는 최대 20개의 `{id, title, description?, mode, text, template?, values?, initialAction?}` 선택지예요. `authored`는 100,000자까지의 작성된 도입문을 저장하고 `generate`는 4,000자까지의 요청을 기존 메인 실행기에 전달해요. 두 방식의 선택적인 `template`은 기존 `PromptTemplate` AST를 사용해요. 템플릿은 선택한 옵션과 `bot.name`·`user.name`·`variables`를 읽고 원래 `text`·AST는 패키지에 보존해요. 이름 안의 문법은 다시 평가하지 않아요. 템플릿이 없으면 원래 글을 그대로 사용해요. 새 채팅 화면에서 미리보기·이번 채팅 옵션을 확인하고 확정하며, 단순 미리보기는 Run·상태·모델 호출을 만들지 않아요.
...
새 채팅 화면은 패키지 `controls`와 `visibleWhen`을 시작 선택 안에서 그대로 사용해 앞선 선택에 따른 하위 옵션을 보여 줘요. 시작을 처음 고를 때 해당 `values`를 기본값으로 적용하고, 직접 요청과 각 시작 사이를 오갈 때 사용자가 바꾼 값은 선택별로 유지해요. 최종 선택값은 미리보기와 확정에 같은 값으로 사용해요.

생성형 시작(`mode: "generate"`)도 같은 이름·옵션·변수 문맥의 `template`를 사용할 수 있어요. 미리보기에는 실제 생성 요청을 표시하고 명시적으로 확정할 때만 기존 본문 Run을 예약해요. 새 예약의 `Run.request`·`snapshot.request`·`packageStart.text`는 그 렌더 결과와 같아요. 응답 유실 후 동일 명령을 보내면 현재 옵션으로 다시 만들지 않고 원래 요청을 돌려주며, 시작 참조·설정/프로필 개정이 다른 명령은 충돌해요. 일반 요청과 마찬가지로 생성 요청은 4,000자 한도를 유지하고 초과하면 상태/Run 저장 전에 실패해요. 특정 자료를 수용하기 위해 생성형 시작에만 별도 길이 예외를 두지 않아요.
