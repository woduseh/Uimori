...
창작 언어·분량·문체는 프롬프트 본문과 그 프롬프트의 옵션으로 작성해요. 별도 `CreativeControls`나 전역 고정 분량 지침을 덧붙이지 않아요. 저장된 옵션 조합은 특정 프롬프트 ID/revision을 참조해요. 선택한 페르소나는 새 본문 요청에 사용하며, 사용하지 않으려면 ‘페르소나 없음’을 선택해요. 빠른 선택과 채팅 설정은 같은 장착 상태를 사용해요. 장착 해제는 기존 대화에 적힌 내용을 삭제하지 않아요. 과거 `personaReference: false` 설정은 새 요청에 적용하지 않으며, 과거 Run의 snapshot·실행 범위는 그대로 보존해요.
