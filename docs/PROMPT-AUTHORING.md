...
편집기의 **간단 편집**은 선택한 메시지의 평문 본문을, **구성 편집**은 같은 데이터의 역할·순서·조건·자료 슬롯을 수정해요. 보기만 바꾸면 저장할 내용은 변하지 않아요. 조건이나 값 참조가 있는 템플릿은 평문으로 펼쳐 덮어쓰지 않고 구성 편집으로 안내해요. `.txt`/`.md` 불러오기도 선택한 평문 메시지 본문만 수정해요.

**프롬프트 구성** 제목을 누르면 섹션 전체를 접거나 펼칠 수 있어요. 접어도 편집 초안·옵션·미적용 문법은 유지하고, 저장 버튼은 바깥에 남겨요. 미적용 문법이 있으면 기존 저장 제한도 유지해요.
