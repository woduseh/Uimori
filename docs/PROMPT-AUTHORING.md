...
저장된 `PromptProgram` AST 하나가 작문·번역 프롬프트의 원본이에요. 사람은 블록 편집기로, 도우미는 공통 초안 도구로 같은 AST를 수정해요. 저장된 `PromptPreset`의 본문은 `program` 하나이며 별도의 `text` 초안을 함께 보관하거나 실행하지 않아요. API의 단순 `{title,role,text}` 입력은 작성 편의용으로 저장 전에 `createDefaultPromptProgram(text,role)`을 통해 같은 AST로 만들어요. `program`을 전달하면 그 구성이 기준이고 추가 `text`는 별도 지침으로 저장하지 않아요. 구형 저장 데이터를 자동 이관하거나 과거 Run snapshot을 다시 작성하지 않아요.
...
기본 main 구성은 지침과 출처가 붙은 자료·메모·현재 입력을 포함하는 대화로, translation 구성은 지침과 원문·번역 문맥·출력 스키마·현재 요청으로 작성해요. 기본 자료의 출처·hash와 번역 원문 귀속을 보존하며 공급자 인코더가 연속된 같은 역할을 지원 형식으로 묶을 수 있어요. 사용자가 작성한 program의 논리 역할·순서·조건은 유지해요.

## 선택형 제작 문법

창작 언어·분량·문체는 프롬프트 본문과 그 프롬프트의 옵션으로 작성해요. 별도 `CreativeControls`나 전역 고정 분량 지침을 덧붙이지 않아요. 저장된 옵션 조합은 프리셋 ID 또는 출처 없는 기본 작업본의 역할에 속하고 저장 당시 옵션 정의를 보관해요. 같은 소속·같은 옵션 정의에서 선택값만 재사용하며 프리셋 전체 revision에 고정하지 않아요. 자세한 적용 경계는 [현재 작업본 계약](RUNTIME-SIMPLIFICATION.md)을 봐요. 선택한 페르소나는 새 본문 요청에 사용하며, 사용하지 않으려면 ‘페르소나 없음’을 선택해요. 빠른 선택과 채팅 설정은 같은 장착 상태를 사용해요. 장착 해제는 기존 대화에 적힌 내용을 삭제하지 않아요. 과거 `personaReference: false` 설정은 새 요청에 적용하지 않으며, 과거 Run의 snapshot·실행 범위는 그대로 보존해요.
...
2026-09-10 [결정 4](DECISIONS-2026-09-10.md)에 따라 기존 본문/JSON 편집을 유지하고, 선택형 템플릿 문법은 **실험 기능**으로 표시해요. 문법을 제거하지 않지만 새 AST 기능을 문법에도 반영할 의무는 없어요. 문법 초안은 적용 전까지 별도 편집 버퍼이고, 적용한 AST를 저장해요. TypeScript 제작 API는 **앱 밖에서 작성할 때 한 번 실행해 AST를 생성하는 입력 경로**예요. 생성 뒤의 원본은 저장된 AST이며 TS 소스를 함께 수정·실행하지 않아요.
...
| 재현·보관 | 적용·저장한 AST가 원본 | 생성·저장한 AST가 원본. TS·빌드 환경은 외부 제작 증거로 보관 가능 | 코드 외에 runtime 버전·의존성·외부 상태도 고정해야 함 |
...
제작 API는 복잡한 프롬프트를 만들 때 재사용·타입 검사에 활용해요. 일반 TS의 표현력을 제공하되, 실행 중에 옵션을 읽어 달라지는 동작은 명시적 AST 연산으로 제한해요. 표의 요청별 TypeScript runtime은 비교를 위해 남긴 미채택 방식이며, [결정 5](DECISIONS-2026-09-10.md)에 따라 런타임 코드 실행은 미지원이에요.
