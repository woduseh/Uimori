...
기존 본문/JSON 편집을 유지하고, 선택형 템플릿 문법은 **실험 기능**으로 표시해요. 문법을 제거하지 않지만 새 AST 기능을 문법에도 반영할 의무는 없어요. 문법 초안은 적용 전까지 별도 편집 버퍼이고, 적용한 AST를 저장해요. TypeScript 제작 API는 **앱 밖에서 작성할 때 한 번 실행해 AST를 생성하는 입력 경로**예요. 생성 뒤의 원본은 저장된 AST이며 TS 소스를 함께 수정·실행하지 않아요.
...
제작 API는 복잡한 프롬프트를 만들 때 재사용·타입 검사에 활용해요. 일반 TS의 표현력을 제공하되, 실행 중에 옵션을 읽어 달라지는 동작은 명시적 AST 연산으로 제한해요. 표의 요청별 TypeScript runtime은 비교를 위해 남긴 미채택 방식이며, 런타임 코드 실행은 미지원이에요.
...
Risu 자료를 수동 변환할 때는 [이식 참고](RISU-PORTING.md)를 볼 수 있어요.
