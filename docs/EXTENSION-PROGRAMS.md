...
전송문 편집은 대화 본문을 그대로 넘기므로 `conversation.read` 선언과 그 채팅·자료 개정의 대화 읽기 허용을 함께 요구해요. 허용이 없거나 대화가 위 한도를 넘으면 그 행동만 건너뛰고 편집 없이 진행하며 Reader의 안내로 알려요. 여러 자료가 선언했고 일부만 건너뛴 경우에도 실행한 편집은 그대로 적용하고 건너뛴 사실을 함께 알려요. 시스템 프롬프트·로어·다른 자료의 지침·프롬프트 프리셋은 넘기지 않아요.
...
엔진 실행, `uimori-state-action-v1` 입력 계약, 상태 저장 호스트는 별도 모듈이에요. 자동 준비와 모델 행동은 `server/package-behavior-run.ts`에서 행동 해석·영수증·효과 채택과 공통 Host를 공유해요. 응답 후 코드는 `server/package-after-response.ts`가 별도 응답 귀속 영수증으로 같은 Worker·상태 저장 경계를 사용해요. 과거 Lua wrapper에 저장된 입력·요청·출력·표시 편집 hook은 `server/extension-lua-worker.ts`의 기존 격리·영수증 경계로 실행해요. 새 Risu 카드는 별도 변환 없이 `server/risu-native-runtime.ts`·`server/risu-native-lua-session.ts`의 네이티브 경로를 사용해요. Risu 플러그인 파일의 훅·인자·저장소도 `server/risu-plugin-adapter.ts`가 같은 방식으로 옮겨, 플러그인 소스를 `risuai` shim으로 감싼 JavaScript 프로그램 행동으로 만들어요. 일반 HTTP와 설치·의존성 권한 관리는 아직 미연결이며 후속 작업이에요. 자료별 함수명을 서버 분기로 옮기지 않아요.
