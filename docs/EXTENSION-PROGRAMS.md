...
버튼 실행은 이 자료의 상태/입력 읽기와 반환한 상태의 반영을 요청하는 동작이에요. 선택한 권한으로 자기 자료를 읽을 수 있지만 채팅 전체·다른 자료·키·환경변수·DB 객체·앱 DOM을 받지 않아요. ID나 권한을 결과에 추가해도 권한이 늘어나지 않아요. 파일·통신·모델 호출·동적 모듈 import는 연결하지 않았어요.
