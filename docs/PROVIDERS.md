...
# 실패 구간과 명시적 재시도 승인을 확인한 뒤 실행
node scripts/verify-live-retry.mjs --source 'output/live/완료된-시험-폴더' --job '번역-job-ID' --execute
```

이 검증기는 원본 DB+WAL을 함께 보존·복사하고 이전 attempt와 완료 구간을 유지해요. 선택한 번역 job의 실패 구간에 대해 재시도 명령을 한 번만 보내요. 별도 누적 한도 옵션은 없으며 작업별 호출·시간·출력 한도를 유지해요. preflight의 READY는 유료 실행 승인을 대신하지 않아요.
