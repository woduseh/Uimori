...
모델에는 접속 프로바이더와 별도로 **모델 계열**을 저장해요. 계열은 OpenAI·Anthropic·Google·DeepSeek·xAI 중 하나이며 생성 옵션의 종류를 정해요. 공식/직접 프로바이더는 자연스러운 계열을 기본값으로 사용하고, Vercel AI Gateway는 `provider/model`의 앞부분으로 계열을 추정해요. 새 ID를 Uimori가 몰라도 계열을 선택하면 같은 제조사 옵션을 사용할 수 있고, 사용자가 고른 계열은 모델 이름 추정보다 우선해요.
