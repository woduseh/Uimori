...
설정 → 연결과 모델에서 템플릿을 고르고 연결을 저장해요. 준비 상태는 서버의 허용 주소와 인증 참조 설정 여부만 확인하며, 실제 인증이나 모델 요청을 실행하지 않아요. 모델 목록을 명시적으로 조회하거나 모델 ID를 직접 입력하고, 지원 여부를 확인한 옵션을 저장한 뒤 새 이야기/이야기 설정에서 역할에 배정해요. Vertex는 이 어댑터가 지원하는 고정 로컬 모델만 등록할 수 있어요.
