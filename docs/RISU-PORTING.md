...
> 이 문서는 지원 범위를 넘는 자료별 수동 native 이식 안내예요. 앱의 현재 파일·실행 지원은 [RISU-IMPORT](RISU-IMPORT.md)와 해당 계약이, 승인된 베타 기능 개발은 [BETA-PLAN](../project-plan/BETA-PLAN.md)이 소유해요. 여기의 읽기 절차를 이유로 이미 승인된 제품 가져오기 구현·공통 기능 보완·합성 검사를 보류하지 않아요. [표본 기준](../project-plan/BETA-SAMPLES.md)의 기능 인수와 현재 부분 지원을 구분해요.

이 문서는 개발 에이전트가 `.risup`·`.risum`·`.charx` 자료를 조사하고 Uimori native JSON으로 작성하는 절차예요. 앱의 카드·프리셋·지원 정규식·이름/기본 변수 템플릿 가져오기는 [현재 가져오기 안내](RISU-IMPORT.md)를 따르고, 여기서는 그 범위를 넘는 동작의 이식을 다뤄요. Risu 런타임 전체의 호환을 의미하지 않으며 완성한 native 산출물은 패키지·프롬프트 편집기의 native JSON 가져오기 경로로 검토·등록해요.

## 1. 원본을 구조화해서 조사해요

- 사용자가 지정한 파일과 승인한 읽기 범위를 확인하고 원본은 수정하지 않아요. 특정 도구·필드로 제한한 요청을 개인 컨테이너 직접 해제나 숨겨진 필드 읽기로 넓히지 않아요. 이미 승인된 제품 가져오기 작업에서는 그 형식의 기존 Uimori 해독기·원본 보존 경로를 재사용할 수 있어요. 읽기 허가를 코드 실행·개인 자료 외부 전송 허가로 확대하지 않아요.
- RisuToki 구조화 도구를 사용할 때는 해당 checkout의 `skills/using-mcp-tools/SKILL.md`를 읽고 현재 도구 metadata와 관련된 `next_actions`를 참고해요. RisuToki 위치는 세션에서 지정한 checkout이나 제공된 도구로 확인하며 Uimori 작업트리의 형제 디렉터리라고 가정하지 않아요. `inspect_document`로 외부 파일의 구조를 조사하고 `read_content`로 필요한 필드·항목을 읽어요. 활성 문서를 바꾸거나 파일을 열 필요는 없어요. 도구 인자는 현재 스키마를 확인하며 추측하지 않아요.
...
- MCP가 없으면 승인된 구조화 추출물이나 허용된 기존 파일 해독 경로가 있는지 확인해요. 있으면 그 범위에서 진행하고, 모두 없으면 필요한 입력·읽기 범위를 구체적으로 알려요. 파일명만 JSON으로 바꾸거나 제한된 읽기 범위를 임의 해제로 우회하지 않아요. 이 상태는 원본 의존 이식의 공백이며 독립적인 제품 구현·합성 검사의 중단 사유는 아니에요.
...
원본 의미가 불분명한 부분은 고정한 Risu 소스·공개 명세와 관련 안내를 확인해요. RisuToki를 사용한다면 현재 skill 목록과 `read_skill`에서 필요한 안내만 읽어요. 파일시스템으로 안내를 읽는 경우 기준 위치는 `RisuToki/risu/common/skills/`예요. 스킬이 쓰기·보관 절차도 설명하더라도 이번 이식에 필요한 읽기·의미 분석 범위만 적용해요.
...
| 변수 변경·트리거 | 자료 인스턴스 state와 채팅 공유 변수를 구분해요. `behavior`·격리 코드·Host API의 지원 범위를 확인하고 사용자/생성 전/후 실행 시점을 맞춰요. Risu 기본값을 initialState에 복사하거나 공유 쓰기를 자료별 state로 대체해 동등하다고 표시하지 않아요. |
| 추첨·pick/random | behavior의 기록된 draws. 원본의 재평가/고정 선택 규칙과 Uimori의 Run·판정 기회 재사용 차이를 적어요. 미리보기에서 추첨하지 않아요. |
| 출력에서 상태 읽기 | 선언형 outputParsers 또는 지원하는 after-turn 격리 코드와 완성 응답 읽기를 사용해요. API·권한·채택 순서는 [확장 코드](EXTENSION-PROGRAMS.md)를 확인하고 원본 Lua의 자동 실행과 구분해요. |
...
| 정규식 | 프리셋 editprocess/editdisplay와 카드의 지원 표시 규칙은 [텍스트 변환](PROMPT-TRANSFORMS.md)·[가져오기](RISU-IMPORT.md)를 따라요. 저장 원문·전송 사본·표시 사본을 구분하고 미지원 처리 단계를 지원으로 표시하지 않아요. |
| 로어 | pinned 또는 discoverable. 키 활성화·확률·검색 깊이·순서와 자동 동등하지 않아요. |
| HTML·외부 코드·전용 UI·에셋 | 지원되는 텍스트·stateView·자기 소유 panels와 격리 코드/Host API로 표현 가능한지 확인하고 경험 차이를 보고해요. 원본 HTML/JS를 주 앱 DOM 권한으로 실행하지 않아요. |
...
원래 설정·인물 역할·사용자 행위 결정권·조건별 지침을 임의로 축약하거나 바꾸지 않아요. 표현할 수 없는 동작을 설명문으로만 바꾸고 완료 처리하지 않아요. 자료 이식마다 `core/`·`server/`에 자료 이름 전용 분기나 변환기를 추가하지 않아요. 공통 기능이 부족하면 필요한 계약과 영향을 파악해요. 자료 파일만 이식하는 요청에서는 앱 변경 범위를 확인하고, 승인된 베타 사용 흐름 개발에서는 필요한 공통 API 보완까지 구현해요. 제품 의미·권한·지원 범위의 새로운 선택만 구체적인 대안을 준비해 사용자에게 요청해요.

## 4. 산출물과 등록

아래는 수동 이식의 전달 정보예요. 기존 가져오기 영수증과 진행 문서에 같은 원본·범위·결과가 있으면 그 근거를 참조하고 중복 manifest·새 검증 실행기를 만들지 않아요. 사용자가 별도 산출물을 지정했거나 실제 전달에 필요하면 해당 파일로 제공해요.
