...
이 문서는 개발 에이전트가 `.risup`·`.risum`·`.charx` 자료를 조사하고 Uimori native JSON으로 작성하는 절차예요. 앱의 기본 카드·정적 표시 정규식·이름 템플릿 가져오기는 [현재 가져오기 안내](RISU-IMPORT.md)를 따르고, 여기서는 그 범위를 넘는 동작의 이식을 다뤄요. Risu 런타임 전체의 호환을 의미하지 않으며 완성한 native 산출물은 패키지·프롬프트 편집기의 native JSON 가져오기 경로로 검토·등록해요.
