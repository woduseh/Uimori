...
실행 중 작업을 마치고 앱의 일관된 DB 스냅샷과 기존 이미지 정보를 보관한 뒤 진행해요. 코드는 원하는 릴리스 태그로 선택하고, `UIMORI_IMAGE`를 지정했다면 이전 이미지를 덮어쓰지 않는 새 참조를 사용해요. 실제 설치의 Compose 파일·프로젝트·환경 파일·데이터 볼륨은 계속 동일하게 사용해요.
...
데이터를 유지하려면 `down`에 `-v`를 붙이지 마세요. Compose 프로젝트 이름은 기본 `uimori`로 고정돼요. 데이터 전환과 복구에서는 이미지 태그와 호환되는 `UIMORI_DATA_VOLUME`을 함께 지정해요. 현재 앱은 빈 DB와 지원하는 개인 작업실 DB를 열며, 이전 지원 버전은 한 번 갱신해요. 정확한 지원 범위와 별도 자료 복사가 필요한 옛 형식은 [데이터 형식](DATA-MIGRATIONS.md#현재-버전)을 확인해요. 업데이트는 기존 자료를 보존하고, 의도적으로 처음부터 시작할 때만 별도의 빈 volume을 지정해요. 업데이트 전 백업과 실행 이미지 정보를 함께 보관해요. 운영자용 update controller는 아래 [한 번의 업데이트](#한-번의-업데이트)에 있어요. 앱 안의 Update 버튼과 무인 자동 업데이트는 제공하지 않아요. [수동 업데이트와 복구](UPDATES.md#manual-release-update)에서 앱과 DB를 함께 되돌리는 절차를 확인해요.
...
새 이미지로 옮길 때는 `npm run update -- start --config <설정> --image <참조> --key <요청 키>`를 사용해요. 유지보수 게이트를 닫고, 진행 중인 작업이 끝나기를 기다리고, 앱을 정지해 data volume을 백업한 뒤, 그 백업으로 만든 **새 volume**에서 후보 이미지의 현재 DB 기준선·읽기만 확인하고, 확인이 끝나야 이미지와 volume을 함께 바꾸고 쓰기를 다시 열어요. 이미지·volume 전환 단계가 끝나기 전의 실패·취소는 이전 구성을 복구하려고 시도해요. 전환이 끝난 뒤에는 쓰기 재개가 실패해도 자동으로 되돌리지 않아요. 절차와 설정 예시는 [업데이트](UPDATES.md#operator-cli)를 봐요. 후보 앱은 복사본에서 지원하는 이전 개인 작업실 DB를 현재 형식으로 갱신할 수 있어요. 지원 범위 밖의 형식은 거절하며 임의 변환하지 않아요. 실제 Docker 호스트에서의 검증은 아직 남아 있어요.
...
`UIMORI_MAINTENANCE=1`로 시작하면 **후보 검증 부팅**이에요. DB 호환성 검사·지원하는 마이그레이션과 읽기를 수행하되 작업 복구·worker를 시작하지 않으며, 이 모드는 API로 열 수 없어요(`409 MAINTENANCE_BOOT`). DB를 읽기 전용으로 여는 모드는 아니므로 반드시 복사한 data로 검사해요.

앱의 **내보내기와 복원 → SQLite 백업 다운로드**로 일관된 백업을 받아 별도로 보관할 수 있어요. 복원할 백업의 형식이 사용할 앱에서 지원되는지 [데이터 형식](DATA-MIGRATIONS.md#현재-버전)을 확인해요. 구버전 앱으로 돌아갈 때는 갱신된 DB가 아니라 그 앱에 맞는 업데이트 전 백업을 사용해요. 백업·컨테이너 전환은 [업데이트 CLI](UPDATES.md)를 봐요. 실행 중 DB 파일 하나만 복사하면 WAL의 변경분을 놓칠 수 있어요.
