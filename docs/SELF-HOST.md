...
공개 저장소는 [woduseh/Uimori](https://github.com/woduseh/Uimori)예요. 배포 revision이 그 저장소에 실제 공개되어 있고 다운로드할 수 있는지 확인한 뒤 해당 revision의 소스 링크를 안내해요. 로컬 수정이나 미공개 커밋으로 빌드했다면 그 수정까지 포함하는 대응 소스를 먼저 제공해야 해요. 현재 라이선스 적용 작업의 로컬 변경이 push되기 전에는 기존 공개 저장소 링크만으로 새 빌드의 소스 제공을 완료했다고 볼 수 없어요. 소스 공개와 배포는 별도의 승인 범위이며 이 문서는 push·배포를 실행한 기록이 아니에요.
