...
Empty and personal-v1 databases are supported. Opening a personal schema-1, schema-2 or schema-3 copy upgrades it transactionally to schema 4; preserve its backup before starting the candidate. Other legacy formats still require the separate schema-24-to-personal-v1 transfer tool and a verified new database. The controller does not infer or reset unrelated formats. See [data formats](DATA-MIGRATIONS.md).
