...
The Oracle runner's `--source-ref` currently accepts a branch, not a release tag. For this release, deploy the verified `main` commit matching `v0.1.0`; do not pass the tag as a branch. Tag creation and GitHub release publication never deploy the running service.
