...
UI recovery expansion also covers the reorganized chat settings, card-variable rows and JSON drafts, settings and license views, collaboration option selection, and bot-scoped transcript import. `verify:ui-recovery` includes the focused `CHATREC`, `SCUI04`, `LEAVE`, and `BOTIMPORT01` cases. Save-and-leave checks exercise unapplied input, rejected saves, Escape during an in-flight save, and persisted revisions before navigation. `LEAVE` also checks matching confirmation widths and desktop/mobile button placement; `CSUI05` and `SCUILEAVE` cover saving chat notes and global settings before closing. `verify:providers -- --grep PMLEAVE` covers provider draft replacement with validation and disable-impact confirmation preserved. Import checks confirm the chosen bot only for transcripts and preserve backup ownership, including a committed request whose response was lost. These checks use synthetic data at 2560 × 1440 and 412 × 915; they do not establish live-provider or physical-device behavior.

`npm run verify:ui-recovery -- --grep NATIVELORE` covers native lore folder expansion/search, creation/renaming, moving entries, folder-only deletion, switch layout and persisted saves at both recovery sizes; a card-inline case checks native keyword arrays and folder references. Pair `tests/native-lore-document.test.ts` and `tests/risu-native-lore-folders.test.ts` for orphan/cycle handling, source-preserving folder removal and exclusion of folder records from runtime lore.

`verify:native-transfer` now checks the unified Risu import UI: one entry per library context, bot/persona/module registration, no chat creation for personas/modules, and a downloadable standalone RISUM at both recovery viewport sizes. Proprietary transfer APIs remain internal import/backup plumbing; their graph, receipt and archive coverage lives in `tests/native-transfer.test.ts`. Pair `tests/risu-export.test.ts` with `tests/risu-native-import.test.ts` for linked-module CHARX bundling and native format round trips. Conflicting module scopes/assets are deliberate export errors, not silently discarded data.
