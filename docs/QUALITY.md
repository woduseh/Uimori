...
`quality`는 Biome의 서식·lint를 한 번에 실행해 중복 스캔을 줄여요. 기본 검사에는 서버 실행, 브라우저, 모델 호출, 네트워크 조회, 의존성 설치가 없어요. 매 저장·커밋마다 전체 테스트를 강제하는 Git hook은 설치하지 않아요. 작은 커밋은 검토·보존 단위이며 사용자 흐름의 완료나 베타 인수와 같지 않아요. 사용 흐름의 결과와 완료 지점은 구현 전에 정하고 [베타 계획](../project-plan/BETA-PLAN.md)에 남겨 전체 검사를 무기한 미루지 않아요.

새 테스트는 보존해야 할 동작·실패 조건을 확인할 필요가 있을 때 추가해요. 구현을 그대로 옮겨 적는 assertion이나 되돌릴 수 있는 작은 문구·배치 변경마다 새 테스트/하네스를 만들지 않아요. 기존 집중 검사로 충분한지 먼저 판단하고, 발견한 결함의 회귀는 그 결함을 드러낼 수 있는 입력과 기대 동작으로 확인해요.

병렬 구현에서는 공유 타입·모듈 등 필요한 의존성이 준비되는 순서에 맞춰 검사해요. 아직 없는 의존성을 이미 확인했다면 같은 전체 타입 검사·테스트 수집을 반복하지 않고, 독립적으로 실행 가능한 검사를 하거나 구현을 이어가요. 경계 회귀는 필요한 의존성이 갖춰지는 즉시 실행하고, 주 작업자가 변경을 통합한 뒤 전체 검사를 조율해요. 의존성이 미완성인 상태를 검사 통과나 완료된 코드 커밋으로 취급하지 않아요.

각 에이전트는 담당 변경의 집중 검사와 실행한 소스 범위를 공유하고, 주 작업자는 이를 확인해 통합 후 필요한 검사를 선택해요. 담당자가 바뀌었다는 이유만으로 같은 소스·환경·검사 계약의 검사를 다시 실행하지 않아요. 같은 작업트리의 전체 검사·빌드는 한 담당자가 조율하고, 입력 소스 수정이나 다른 빌드가 실행 중 검사와 겹치지 않게 해요. 독립된 집중 검사는 입력 파일 수정·DB·포트·출력 경로가 충돌하지 않을 때 병렬로 실행해요. 릴리스 영수증 재사용 조건은 아래의 별도 기준을 따라요.
