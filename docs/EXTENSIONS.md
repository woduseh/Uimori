...
작업 순서는 [베타 계획의 6단계](../project-plan/BETA-PLAN.md)를 따라요. 현재 2~3단계에서 표본은 공통 계약의 요구를 확인하는 용도예요. 선언형 구성은 정적인 옵션·제한된 계산·일반적인 레코드 읽기에 사용하고, 복잡한 알고리즘은 확장 코드가 맡도록 설계해요. 표본의 함수마다 본체 분기나 선언 필드를 추가하는 방식으로 임의 코드 실행 기반을 대체하지 않아요. 현재 코드 실행기의 결과는 같은 소유권·상태 schema·개정/원문 검사·원자적 효과 적용 경계를 사용하며, 후속 확장도 이를 유지해야 해요.
