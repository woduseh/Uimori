...
현재 구현 범위(2026-09-14)는 [커스텀 패널](PACKAGE-PANELS.md)과 [상태 계산 코드](EXTENSION-PROGRAMS.md)의 계약을 따라요. 격리된 패널은 현재 상태/옵션을 HTML/CSS로 표시하고 실제 사용자 행동을 기존 상태 API에 연결해요. `user`·`model`·`before-turn`·`after-turn`의 JavaScript는 별도 Worker/고정 크기 QuickJS WASM에서 실행하며 비동기 Host API로 자기 자료 읽기와 `model.generate`를 지원해요. 완성 응답 읽기는 `after-turn`에서만 허용해요. 모델 호출에는 전역 확장 모델과 정확한 자료 개정별 grant가 필요해요. 사용자 모델 행동은 독립 영구 작업이고, Run의 상태 결과는 성공 원문 저장 때 채택해요. Lua도 같은 소유권·격리·영수증 경계에서 실행하며 `editInput`·`editRequest`·`editOutput`·`editDisplay` 네 편집 hook과 선언형 `setvar`·혼합 트리거·`runLLM`·`extractRegex`를 연결했어요. 자세한 변환 범위는 [Lua 콜백 가져오기](RISU-IMPORT.md#lua-콜백-가져오기)가 소유해요. 일반 HTTP·제공자 어댑터·공유 자료 권한·확장 설치 관리·동적 확장 화면은 아직 미구현이에요. 아래의 제안은 이 구현 범위를 넘어서는 설계이며 설치 가능한 SDK 전체를 뜻하지 않아요.

작업 순서는 [베타 계획의 6단계와 채택한 작업 방식](../project-plan/BETA-PLAN.md)을 따라요. 2~3단계의 기반 보완과 4단계의 호환 기능 연결을 실제 사용 흐름 하나로 진행하고, 고정한 Risu 명세·실행 동작을 공통 API에 대응시킨 뒤 표본으로 사용 결과를 확인해요. 선언형 구성은 정적인 옵션·제한된 계산·일반적인 레코드 읽기에 사용하고, 복잡한 알고리즘은 확장 코드가 맡도록 설계해요. 표본의 함수마다 본체 분기나 선언 필드를 추가하는 방식으로 임의 코드 실행 기반을 대체하지 않아요. 현재 코드 실행기의 결과는 같은 소유권·상태 schema·개정/원문 검사·원자적 효과 적용 경계를 사용하며, 후속 확장도 이를 유지해야 해요.
...
이 후보 검토는 특정 엔진을 사용자 확정 선택으로 만드는 결정이 아니에요. 현재 상태 계산과 모델 Host API는 WASM 선형 메모리를 별도로 고정한 QuickJS와 Worker 메시지 RPC를 사용해요. 내부 allocator 한도와 Worker의 전체 RSS 제한을 혼동하지 않아요. 일반 통신·추가 언어의 실행 환경까지 확정한 것은 아니에요.
...
2026-09-13: 이 흐름 중 생성 전 상태 계산·순서·별도 진행 기록·실패/건너뛰기·원문 성공 때 효과 게시는 기존 before-turn/action API 위에 구현했어요. 예약된 요청/기본 상태를 보존하고 실제 상태 투영 뒤 문맥/프롬프트를 계산해요. 자기 자료 읽기·모델 호출의 비동기 Host API도 구현했어요. 아래의 요청 본문 가공·일반 HTTP broker·의존성 권한 전체는 계속 설계 범위예요.
...
실제 지원은 [상태 계산 코드](EXTENSION-PROGRAMS.md)와 [베타 실행 기록](../project-plan/BETA-PLAN.md)의 구현·검증 결과로 판정해요. 현재 네 행동 시점의 코드·모델 호출 지원을 요청 본문 가공·일반 통신·확장 설치 전체나 원본 표본의 인수 완료로 넓혀 해석하지 않아요.

## 가져오기 어댑터와 라이선스

2026-09-13 후속 사용자 승인에 따라 `.risup` 가져오기의 RPack 해독 치환표를 포함하고 Uimori 소스에는 AGPL-3.0-only를 적용해요. [원본 조건과 고지](../THIRD_PARTY_NOTICES.md), [결정 이력](DECISIONS-2026-09-12-BETA.md)을 확인해요. 파일 해독 어댑터의 채택은 Risu 런타임·내부 DB 의존이나 미래 변경의 자동 추종을 의미하지 않아요. 프리셋의 프롬프트·토글 변환과 정규식·memory slot의 남은 실행 연결은 [가져오기 안내](RISU-IMPORT.md)에서 구분해요.
