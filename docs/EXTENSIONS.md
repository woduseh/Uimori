...
현재 구현 범위(2026-09-13)는 [커스텀 패널](PACKAGE-PANELS.md)과 [상태 계산 코드](EXTENSION-PROGRAMS.md)의 계약을 따라요. 격리된 패널은 현재 상태/옵션을 HTML/CSS로 표시하고 실제 사용자 행동을 기존 상태 API에 연결해요. `user`·`model`·`before-turn`·`after-turn`의 JavaScript는 별도 Worker/고정 크기 QuickJS WASM에서 실행하며 비동기 Host API로 자기 자료 읽기와 `model.generate`를 지원해요. 완성 응답 읽기는 `after-turn`에서만 허용해요. 모델 호출에는 전역 확장 모델과 정확한 자료 개정별 grant가 필요해요. 사용자 모델 행동은 독립 영구 작업이고, Run의 상태 결과는 성공 원문 저장 때 채택해요. 일반 HTTP·Lua·제공자 어댑터·공유 자료 권한·확장 설치 관리는 미구현이에요. 아래의 제안은 이 구현 범위를 넘어서는 설계이며 설치 가능한 SDK 전체를 뜻하지 않아요.
