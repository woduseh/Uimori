...
2026-09-13 첫 화면 경계는 [커스텀 패널](PACKAGE-PANELS.md)로 구현했어요. 현재 상태/옵션에서 읽기 전용 HTML을 만들고, 격리된 패널의 실제 사용자 행동만 기존 상태 API로 전달해요. 이어서 사용자 행동의 JavaScript 상태 계산을 별도 Worker/고정 크기 QuickJS WASM과 기존 저장 경계에 연결했어요. Lua·제공자 어댑터와 비동기 host broker는 후속 범위예요. 아래 before-request 제안을 이미 지원하는 SDK로 읽지 않아요.
