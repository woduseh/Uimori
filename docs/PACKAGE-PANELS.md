...
서재의 자료 편집 → **표현 → 커스텀 패널**에서 패널 JSON을 입력하고 **패널 검증 후 적용 → 자료 저장**해요. 미리보기는 자료의 기본값과 상태 초깃값만 읽고 행동은 실행하지 않아요. 미적용 JSON은 편집 초안으로 유지해요.

관련 기능이 있는 자료를 장착한 채팅에는 **채팅 설정 → 자료 기능**이 고급 항목으로 나타나요. 그 안의 **공유 변수**에서 분기별 문자열 값을 명시적으로 편집하고 **기본 상태와 행동**에서 기존 패키지 상태·입력 폼·복구 기능을 사용해요. 공유 변수·상태·행동·패널·진행 중인 자료 작업이 모두 없으면 이 항목 자체를 표시하지 않아요. 패널 표시나 버튼 처리가 실패해도 원문과 채팅 입력은 유지돼요.
...
템플릿은 `state`(자기 패키지 상태), `options`와 `control`(자기 옵션), `bot.name`·`user.name`, `variables`(공유 override와 자료 기본값)를 읽어요. 공유 변수는 [프롬프트 실행](PROMPT-RUNTIME.md#분기-공유-변수)의 같은 resolver를 사용하며 변수층 한도 초과 때 해당 값을 읽는 패널만 오류로 표시해요. `if/each/let`으로 선택 단계나 중첩 목록을 표현할 수 있어요. 다른 자료 본문·전체 대화·모델·인증 정보와 slot은 읽지 못해요. 계산한 문자열을 다시 코드나 HTML로 해석하지 않아요. 동적 값은 텍스트 또는 따옴표로 감싼 속성 값에 넣고 HTML 태그/속성 이름이나 CSS 소스로 사용하지 않아요.
...
패널 버튼이 호출한 제작자 코드도 [공유 변수 Host API](EXTENSION-PROGRAMS.md#host-api로-분기-공유-변수-읽기와-쓰기)를 사용할 수 있어요. 쓰기는 해당 자료 개정의 **공유 변수 변경 허용**이 필요하고, 패키지 상태·공유 변수·후속 요청을 같은 transaction에서 채택해요. 패널은 채택된 값을 같은 변수 resolver로 읽어요. iframe 자체에 DB 쓰기나 임의 Host 호출 권한을 주지 않으며 표시·실패·재접속만으로 코드를 실행하지 않아요. Risu trigger·Lua를 직접 실행하는 어댑터는 [Lua 콜백 가져오기](RISU-IMPORT.md#lua-콜백-가져오기)에 있으며, 패널은 그 실행이 서버 transaction에서 채택한 상태·공유 변수만 읽어요.
