...
- `{{char}}`는 카드 이름으로 바꾸고, `{{user}}`는 원래 표기와 현재 배역을 뜻한다는 지시를 유지해요. 시작문 화면의 동적 사용자 이름 치환, CBS·Lua·트리거·정규식·커스텀 UI와 내장 `module.risum`은 아직 자동 이식하지 않아요. 내장 모듈이 있는 경우 카드 안의 로어만 읽으므로 실제 적용 자료와 다를 수 있어요. 보고된 미지원 기능은 명시적으로 부분 가져오기를 선택해야 진행해요.
