...
요청한 사용자 결과와 확인할 동작을 짧게 정하고, 알고 있는 진입점에서 관련 계약·호출자·검사를 찾아요. 위치가 불명확하면 [코드 지도](../project-plan/CODE-MAP.md)를 사용해요. 작고 명확한 수정에 별도 계획서·전체 저장소 분석·새 검증 도구를 만들지 않아요. 큰 출력은 파일 목록·심볼 검색으로 좁힌 뒤 구간을 읽고, 서로 독립인 조회/작업만 묶거나 위임해요. 실패 조사도 다음 구현 선택을 바꾸는 근거가 확보되면 끝내고 수정으로 넘어가요. 근본 해결에 필요한 기존 공통 API 보완은 합의된 사용 흐름에 포함해요.
