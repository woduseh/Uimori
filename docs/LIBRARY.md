...
프리셋 적용은 앱 전역의 현재 작업본에 내용·옵션을 복사하는 동작이에요. 채팅 고정과 옵션 조합의 소속·정의 검사는 [전역 모델과 현재 프롬프트](GLOBAL-MODELS.md)에 있어요. 현재 프로필의 `optionAdjustments`는 패키지 조회용 안내이며 과거 snapshot의 옵션은 당시 정의로 검증해요.
