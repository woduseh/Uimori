...
**변수·토글**에서는 **토글 정의 원문**을 먼저 표시하고, **GUI 편집기**는 그 아래에 접어 둬요. GUI 편집기를 펼치면 목록에서 항목을 골라 변수 키·표시 이름·종류·선택지를 편집하고 추가·삭제·순서 변경을 할 수 있어요. 입력 종류끼리 또는 그룹·문구 종류끼리 전환할 수 있으며 채팅 옵션 값은 따로 보존해요. 원문을 수정한 동안은 목록 편집과 저장을 잠시 막고 **원문 적용**으로 반영해요. 탭을 바꿔도 미적용 원문을 유지하고, 구조 편집은 수정하지 않은 줄·알 수 없는 구문·빈 줄·개행을 보존해요.
