...
기본 옵션은 원본 정의에 따라 `text`는 한 줄 입력, `textarea`는 여러 줄 입력으로 표시해요. 켜기/끄기는 **OFF / ON** 라디오로 표시하며 선택한 값은 Risu 형식인 `"0"` / `"1"`로 저장해요. 기존 미설정 값은 OFF로 표시하되 화면을 여는 것만으로 저장 값을 바꾸지 않아요. 명시적인 `select` 정의는 작성된 선택 목록을 사용해요.

**변수·토글**에서는 목록에서 항목을 골라 변수 키·표시 이름·종류·선택지를 편집하고 추가·삭제·순서 변경을 할 수 있어요. 입력 종류끼리 또는 그룹·문구 종류끼리 전환할 수 있으며 채팅 옵션 값은 따로 보존해요. **토글 정의 원문**도 함께 제공하며, 원문을 수정한 동안은 목록 편집과 저장을 잠시 막고 **원문 적용**으로 반영해요. 탭을 바꿔도 미적용 원문을 유지하고, 구조 편집은 수정하지 않은 줄·알 수 없는 구문·빈 줄·개행을 보존해요.
