...
프롬프트 옵션과 전역 창작 조합은 같은 프롬프트 ID에서 재사용해요. 같은 control ID의 유효한 값은 유지하고 삭제·타입·범위 변경으로 무효가 된 값은 최신 기본값으로 조정해 안내해요. 패키지 옵션도 같은 ID·역할 기준으로 처리해요. 현재 프로필의 `optionAdjustments`는 조회용 안내이며 저장·실행 snapshot에는 포함하지 않아요. 과거 snapshot의 옵션은 당시 정의로 엄격하게 검증해요.
