...
기본 옵션은 원본 정의에 따라 `text`는 한 줄 입력, `textarea`는 여러 줄 입력으로 표시해요. 켜기/끄기는 스위치로 표시하며 선택한 값은 Risu 형식인 `"0"` / `"1"`로 저장해요. 기존 미설정 값은 꺼짐으로 표시하되 화면을 여는 것만으로 저장 값을 바꾸지 않아요. 명시적인 `select` 정의는 작성된 선택 목록을 사용해요.
