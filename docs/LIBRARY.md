...
프리셋의 **정규식**, 자료의 **고급 설정 → 스크립트**에서는 같은 폼으로 규칙의 이름·적용 단계·찾을 표현식(IN)·바꿀 내용(OUT)을 편집해요. 목록에서 추가·삭제·복제·순서 변경이 가능하며 변경사항은 상단 저장에 포함돼요. **사용자 지정 플래그**는 `ableFlag`를 바꾸며 끄면 기본 `g` 플래그를 사용해요. 규칙을 끄려면 적용 단계를 **비활성**으로 바꿔요. 고급 플래그의 우선순위는 native `<order N>`으로 저장하고 큰 값부터 실행하며, 같은 우선순위에서는 목록 순서를 유지해요.
