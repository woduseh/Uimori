...
현재 장면 원문 전체를 일반 텍스트로 번역해요. 구간별 분할 계획과 자동 재분할은 없으며 입력·출력 한도, timeout, 작업별 `maxCalls`를 유지해요. 번역 모델·프롬프트·정책을 예약 시 고정하며, 활성화된 전체 응답 JEV 거절 판정과 자동 재요청도 같은 전체 호출 예산을 사용해요. 수동 재번역은 현재 설정으로 새 작업을 만들며 새 작업의 실패·취소에도 같은 source/hash의 이전 성공본을 표시해요. 세부 계약은 [RUNTIME-SIMPLIFICATION.md](RUNTIME-SIMPLIFICATION.md)를 봐요.
