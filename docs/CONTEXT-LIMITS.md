...
- [모델 선별](LORE-CONTEXT.md#모델-선별) 호출도 같은 `maxCalls`를 쓰고 본문 호출 한 번을 남겨요. 이미 쓴 호출 때문에 본문 한 번을 남길 수 없으면 공급자를 부르지 않고 선별을 건너뛰며, 이 호출의 사용량은 이어지는 자동 정리의 남은 예산 계산에 그대로 들어가요.
