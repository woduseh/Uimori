...
정리 대상은 **이미 완료된 성공한 읽기 도구 교환**이에요. `story.*`, `notes.*`, `knowledge.*`, `skills.*`, `context.read`의 등록된 조회 결과만 대상으로 삼아요. 거절된 호출, `context.write` 같은 저장 작업, 카드 스크립트 실행 영수증과 작문 보조 응답은 원래 호출·결과를 그대로 남겨요. 요청·고정 지침·원문 ancestry를 이 경로에서 지우거나 다시 쓰지 않아요.
