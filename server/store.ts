...
export class Store {
  ...
  claimJob(id: string, owner: string, input: unknown, plan?: {chunks:{id:string}[]}): Job | null {
    return this.transaction(() => {
      ...
      const claimedInput = input && typeof input === 'object' && !Array.isArray(input) ? {...input,...promptSelection,...translationModelSelection,...translationModelSnapshot,...promptControlSelection,...imageSelection} : input;
