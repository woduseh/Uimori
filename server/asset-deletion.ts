...
export function deleteChatAsset(store: Store, chatId: string, assetId: string, value: unknown) {
  ...
  return store.transaction(() => {
    ...
    if (
      store.db
        .prepare(
          "SELECT 1 FROM runs WHERE chat_id=? AND status IN ('queued','running','waiting_for_state') LIMIT 1"
        )
        .get(chatId) ||
      store.db
        .prepare("SELECT 1 FROM jobs WHERE chat_id=? AND status IN ('queued','running') LIMIT 1")
        .get(chatId)
    ) {
      throw new HttpError(
        409,
        '진행 중인 생성이나 보조 작업이 있어요. 완료하거나 취소한 뒤 이미지를 삭제해 주세요.'
      );
    }
    ...
    const referenced = [
      ...
      store.db
        .prepare(
          `SELECT 1 FROM ${table}, json_tree(${table}.${column}) ref WHERE ref.type='text' AND ref.atom IN (?,?) LIMIT 1`
        )
        .get(assetId, `/api/assets/${assetId}`)
    );
    if (referenced)
      throw new HttpError(
        409,
        '이 이미지가 이야기 설정이나 과거 실행·이미지 작업에 저장되어 있어요. 해당 이야기를 삭제하면 이미지도 함께 삭제돼요.'
      );
