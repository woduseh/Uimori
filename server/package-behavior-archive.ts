...
} from './package-behavior-run-archive.js';

export const packageBehaviorTables = [
...
function text(value: unknown, max = 200): asserts value is string {
  if (typeof value !== 'string' || !value.length || value.length > max) reject('string');
}
function revision(value: unknown, min = 0): asserts value is number {
...
function definition(store: Store, scope: BehaviorScope): PackageBehavior {
  text(scope.chatId);
  text(scope.branchId);
  text(scope.attachmentInstanceId);
  text(scope.packageId);
  revision(scope.packageRevision, 1);
  revision(scope.behaviorRevision, 1);
  revision(scope.schemaVersion, 1);
  store.chat(scope.chatId);
  ...
  if (b.revision !== scope.behaviorRevision || b.schemaVersion !== scope.schemaVersion)
    reject('definition revision');
  if (
    !['bot', 'persona', 'module'].some(
      (role) => scope.attachmentInstanceId === `${scope.packageId}:${role}`
    )
  )
    reject('attachment identity');
  return b;
}
function scopeOf(value: unknown): BehaviorScope {
  return object(value, [
    'chatId',
    'branchId',
...
function sourceHashInChat(store: Store, chatId: string, hash: string | null) {
  ...
  if (
    !store.db
      .prepare(
        'SELECT 1 FROM sources s LEFT JOIN source_edits e ON e.source_id=s.id WHERE s.chat_id=? AND (s.hash=? OR e.hash=?) LIMIT 1'
      )
      .get(chatId, hash, hash)
  )
    reject('source hash scope');
}
function recordedDrawShape(b: PackageBehavior, value: unknown) {
  const draws = behaviorRecord(value);
  inspectRuntimeValue(draws);
  for (const [id, result] of Object.entries(draws)) {
    const declarations = b.actions.flatMap((a) => a.draws ?? []).filter((d) => d.id === id);
    const valid = declarations.some((d) =>
      d.type === 'integer'
        ? Number.isSafeInteger(result) && result >= d.min && result <= d.max
        : d.type === 'choice'
          ? d.values.some((v) => isDeepStrictEqual(v, result))
          : Array.isArray(result) &&
            isDeepStrictEqual(
              result.map(behaviorPayloadHash).sort(),
              d.values.map(behaviorPayloadHash).sort()
            )
    );
...
function executionState(
  ...
  if (!['bot', 'persona', 'module'].includes(s.role) || s.instanceId !== `${s.packageId}:${s.role}`)
    reject('execution identity');
  const b = definition(store, {
    chatId,
    branchId,
    attachmentInstanceId: s.instanceId,
    packageId: s.packageId,
    packageRevision: s.packageRevision,
    behaviorRevision: s.behaviorRevision,
    schemaVersion: s.schemaVersion,
  });
  revision(s.stateRevision);
  validateBehaviorValue(b.stateSchema, s.state);
  recordedDrawShape(b, s.draws);
  if (packages) {
    if (
      !packages.packageAttachments?.some(
        (r) => r.id === s.packageId && r.revision === s.packageRevision && r.role === s.role
      )
    )
      reject('snapshot attachment');
    const pkg = packages.packages?.find(
      (p) => p.id === s.packageId && p.revision === s.packageRevision
    );
    if (!pkg?.behavior || !isDeepStrictEqual(b, pkg.behavior)) reject('frozen behavior definition');
  }
...
export function validatePackageBehaviorRunSnapshot(store: Store, snapshot: RunSnapshot): void {
  ...
  if (snapshot.executionClock !== undefined) {
    ...
    text(c.iso, 100);
    revision(c.unix);
    const parsed = Date.parse(c.iso);
    if (
      !Number.isFinite(parsed) ||
      new Date(parsed).toISOString() !== c.iso ||
      Math.floor(parsed / 1000) !== c.unix
    )
  ...
  if (snapshot.packageStates !== undefined) {
    if (!Array.isArray(snapshot.packageStates) || snapshot.packageStates.length > 100)
      reject('snapshot states');
    const seen = new Set<string>();
    for (const state of snapshot.packageStates) {
  ...
  if (snapshot.behaviorExecution !== undefined) {
    ...
    if (
      ...
      e.baseStates.length > 100 ||
      !Array.isArray(e.automaticResults) ||
    ...
    for (const state of e.baseStates) {
      ...
      if (ids.has(s.instanceId)) reject('duplicate base state');
      ids.add(s.instanceId);
    }
    same([...ids].sort(), expected.sort(), 'base state scope');
    for (const raw of e.automaticResults) {
      const result = object(raw, ['instanceId', 'actionId', 'result']);
      text(result.instanceId);
      text(result.actionId);
      inspectRuntimeValue(result.result);
    }
  }
}
export function validatePackageBehaviorArchive(store: Store): void {
  const db = store.db,
    rows = (table: string) => db.prepare(`SELECT * FROM ${table}`).all() as Row[];
  const states = new Map<string, { row: Row; scope: BehaviorScope; behavior: PackageBehavior }>();
  const key = (chat: string, branch: string, instance: string) =>
    JSON.stringify([chat, branch, instance]);
  for (const row of rows('package_behavior_states')) {
    const scope = scopeOf(JSON.parse(row.scope)),
      b = definition(store, scope);
    same(
      [scope.chatId, scope.branchId, scope.attachmentInstanceId],
      [row.chat_id, row.branch_id, row.instance_id],
      'state row scope'
    );
    revision(row.state_revision);
    digest(row.definition_hash);
    if (behaviorPayloadHash(b) !== row.definition_hash) reject('definition hash');
    validateBehaviorValue(b.stateSchema, JSON.parse(row.state));
  ...
  for (const row of rows('package_behavior_journal')) {
    ...
    same(payload.scope, scope, 'journal payload scope');
    same(
      Object.fromEntries(Object.keys(scope).map((k) => [k, r[k]])),
      scope,
      'journal result scope'
    );
    ...
    if (behaviorPayloadHash(payload) !== row.payload_hash) reject('journal payload hash');
    text(row.created_at, 100);
    if (!Number.isFinite(Date.parse(row.created_at))) reject('journal timestamp');
    revision(r.beforeStateRevision);
    ...
    if (['ui-action', 'before-turn', 'model-tool'].includes(r.provenance)) {
      if (
        ...
        reject('action expectation');
      const action = b.actions.find((a) => a.id === payload.actionId);
      if (!action || !behaviorActionAllowed(action, r.beforeState, payload.input, hostRuntime))
        reject('action contract');
      if (
        !behaviorActionTriggers(action).includes(
          r.provenance === 'ui-action'
            ? 'user'
      ...
      if (r.provenance !== 'ui-action') {
        ...
        if (
          ...
          (run.snapshot.branchId ?? `main:${run.chatId}`) !== scope.branchId ||
        ...
        same(
          [
            entry.trigger,
            entry.input,
            entry.before.stateRevision,
            entry.before.state,
            entry.after.stateRevision,
            entry.after.state,
            entry.draws,
            entry.drawSeed,
            entry.hostRuntime,
      ...
      if (
        ...
        reject('reset contract');
      same(r.state, b.initialState, 'reset state');
    } else if (r.provenance === 'local-output-parser') {
      if (Object.hasOwn(r, 'actionResult')) reject('unexpected action result');
      if (
        payload.baseStateRevision !== r.beforeStateRevision ||
        payload.sourceHash !== r.sourceHash ||
        r.drawSeed !== null ||
        Object.keys(r.draws).length
      )
        reject('output expectation');
      text(payload.text, 500_000);
      if (createHash('sha256').update(payload.text).digest('hex') !== r.sourceHash)
        reject('output source text');
      if (
        !Array.isArray(payload.parserIds) ||
        !payload.parserIds.length ||
        new Set(payload.parserIds).size !== payload.parserIds.length
      )
        reject('output parser IDs');
      let next = r.beforeState;
      const paths: string[] = [];
      for (const id of payload.parserIds) {
        const parser = b.outputParsers.find((p) => p.id === id);
        if (!parser) reject('output parser');
        for (const field of parser.fields) {
          const p = field.path.join('/');
          if (paths.some((q) => q === p || q.startsWith(p + '/') || p.startsWith(q + '/')))
            reject('output overlapping fields');
          paths.push(p);
        }
        if (parser.when !== undefined) {
          const allowed = evaluatePromptExpression(
            parser.when,
            {},
            { runtime: { ...hostRuntime, state: r.beforeState, input: {}, draws: {} } }
          );
          if (typeof allowed !== 'boolean') reject('output condition');
          if (!allowed) continue;
        }
        next = parseBehaviorOutput(b, parser, next, payload.text);
      }
      same(next, r.state, 'output result');
    } else reject('journal provenance');
    const k = key(row.chat_id, row.branch_id, row.instance_id),
      previous = latest.get(k);
    if (previous && previous.stateRevision === r.stateRevision)
      reject('duplicate journal revision');
    if (!previous || previous.stateRevision < r.stateRevision) latest.set(k, r);
  }
  ...
  for (const row of rows('package_behavior_heads')) {
    const owner = states.get(key(row.chat_id, row.branch_id, row.instance_id));
    if (!owner) reject('orphan head');
    recordedDrawShape(owner.behavior, JSON.parse(row.draws));
    if (!['ready', 'stale', 'failed'].includes(row.status)) reject('head status');
    if (row.error !== null) text(row.error, 4000);
    const deps = JSON.parse(row.dependencies);
    if (!Array.isArray(deps) || deps.length > 10_000) reject('head dependencies');
    const seen = new Set<string>();
    for (const dep of deps) {
      const d = object(dep, ['id', 'hash']);
      text(d.id);
      digest(d.hash);
      if (seen.has(d.id)) reject('duplicate dependency');
      seen.add(d.id);
      const source = store.sourceAtHash(d.id, d.hash);
      if (source.chatId !== row.chat_id) reject('dependency source scope');
    }
  }
  for (const row of rows('package_behavior_outputs')) {
    const source = store.source(row.source_id),
      run = store.run(source.runId),
      branch = run.snapshot.branchId ?? `main:${source.chatId}`;
    const out = object(JSON.parse(row.body), ['before', 'after', 'status', 'error', 'sourceHash']);
    if (!['ready', 'failed'].includes(out.status)) reject('output status');
    if (out.error !== null) text(out.error, 4000);
    digest(out.sourceHash);
    store.sourceAtHash(source.id, out.sourceHash);
    const before = executionState(store, out.before, source.chatId, branch, run.snapshot.profile),
      after = executionState(store, out.after, source.chatId, branch, run.snapshot.profile);
    same(
      before,
      (run.snapshot.behaviorExecution?.baseStates ?? run.snapshot.packageStates)?.find(
        (s) => s.instanceId === row.instance_id
      ),
      'output frozen before state'
    );
    if (row.instance_id !== before.instanceId || row.instance_id !== after.instanceId)
      reject('output instance');
    same(
      [
        before.packageId,
        before.packageRevision,
        before.role,
        before.behaviorRevision,
    ...
    if (out.status === 'failed') {
      const behavior = run.snapshot.profile?.packages?.find(
        (pkg) => pkg.id === before.packageId && pkg.revision === before.packageRevision
      )?.behavior;
      if (behavior?.mode === 'annotation' && !isDeepStrictEqual(before, after))
        same(
          after,
          progress?.states.find(
            (state: PackageExecutionState) => state.instanceId === row.instance_id
          ),
          'failed annotation staged state'
        );
      else same(before, after, 'failed output mutation');
    } else if (
      after.stateRevision < before.stateRevision ||
      after.stateRevision > before.stateRevision + actionCount + 1
    )
      reject('output revision');
  }
  validateRunBehaviorArchive(store, (value, chatId, branchId, profile) =>
    executionState(store, value, chatId, branchId, profile)
  );
}
