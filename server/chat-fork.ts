  ...
  return store.transaction(()=>{
    ...
    for(const original of ancestors){
      ...
      for(const job of completed){
        ...
        const input=job.kind==='image' ? forkImageInput(oldInput,assetIds) : job.kind==='translation' && oldInput && Object.hasOwn(oldInput,'promptSelection') ? {promptSelection:structuredClone(oldInput.promptSelection),...(Object.hasOwn(oldInput,'translationModelSelection')?{translationModelSelection:structuredClone(oldInput.translationModelSelection)}:{}),...(Object.hasOwn(oldInput,'promptControlSelection')?{promptControlSelection:structuredClone(oldInput.promptControlSelection)}:{})} : null;
    ...
    for(const [oldId,copiedId] of runIds){
      ...
      snapshot=compileSnapshotPrompt(snapshot);
