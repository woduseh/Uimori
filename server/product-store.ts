...
export class ProductStore {
  ...
  migrate(version: number) {
    if (version >= 2) { this.migrateSourceEdits(version); return; }
    ...
    this.migrateSourceEdits(2);
  }
  private migrateSourceEdits(version:number) {
    if(version>=3)return;
    if(this.db.prepare('SELECT 1 FROM chats LIMIT 1').get())this.db.prepare('VACUUM INTO ?').run(`${this.store.path}.pre-v3-${Date.now()}-${randomUUID().slice(0,8)}.sqlite`);
    this.store.transaction(()=>{this.db.exec('CREATE TABLE source_edits(source_id TEXT NOT NULL REFERENCES sources(id),revision INTEGER NOT NULL,text TEXT NOT NULL,hash TEXT NOT NULL,created_at TEXT NOT NULL,PRIMARY KEY(source_id,revision)); PRAGMA user_version=3;');this.db.prepare("UPDATE jobs SET status='interrupted',generation=generation+1,owner=NULL,error='On-demand translation enabled; open Translation view to resume' WHERE kind='translation' AND status IN ('queued','running')").run();});
  }
  ...
  snapshot(chatId: string): ProfileSnapshot | undefined {
    ...
    return structuredClone({...p,contents,models,...(p.prompts !== undefined ? {promptPresets:resolvedPrompts(this,p.prompts)} : {})});
  }
