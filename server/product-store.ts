...
import { defaultProfile, type Content, type ContentRef, type CreativeControls, type ChatProfile, type ProfileSnapshot, type Connection, type ModelPreset, type Library, type Branch, type Asset, type Attempt } from '../core/product.js';
...
export class ProductStore {
  ...
  migrate(version: number) {
    if (version >= 2) return;
    // Schema-1 data gets a consistent, standalone pre-upgrade copy before any DDL.
    if (version === 1 && this.db.prepare('SELECT 1 FROM chats LIMIT 1').get()) {
      const destination = `${this.store.path}.pre-m1-${Date.now()}-${randomUUID().slice(0,8)}.sqlite`;
    ...
    try { this.store.transaction(() => {
      this.db.exec(`
        ...
        ALTER TABLE runs ADD COLUMN branch_id TEXT REFERENCES branches(id);
        ALTER TABLE runs ADD COLUMN partial_text TEXT;
        ALTER TABLE runs ADD COLUMN issue TEXT;
        INSERT INTO branches SELECT 'main:'||id,id,'기본 분기',head_revision,1,1 FROM chats;
        UPDATE runs SET branch_id='main:'||chat_id;
        DROP INDEX one_active_run_per_chat;
        CREATE UNIQUE INDEX one_active_run_per_branch ON runs(branch_id) WHERE status IN ('queued','running');
        CREATE TABLE jobs_new (id TEXT PRIMARY KEY,chat_id TEXT NOT NULL REFERENCES chats(id),source_revision TEXT NOT NULL REFERENCES sources(id),source_hash TEXT NOT NULL,kind TEXT NOT NULL CHECK(kind IN ('translation','status','image')),status TEXT NOT NULL,generation INTEGER NOT NULL DEFAULT 0,owner TEXT,input TEXT,error TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,revision INTEGER NOT NULL DEFAULT 1,plan TEXT,retry_chunk TEXT,UNIQUE(source_revision,kind,revision));
        INSERT INTO jobs_new(id,chat_id,source_revision,source_hash,kind,status,generation,owner,input,error,created_at,updated_at) SELECT id,chat_id,source_revision,source_hash,kind,status,generation,owner,input,error,created_at,updated_at FROM jobs;
        DROP TABLE jobs;
        ...
        CREATE TABLE attempts (id TEXT PRIMARY KEY,chat_id TEXT NOT NULL REFERENCES chats(id),run_id TEXT REFERENCES runs(id),job_id TEXT REFERENCES jobs(id),role TEXT NOT NULL,connection_id TEXT NOT NULL,model_id TEXT NOT NULL,status TEXT NOT NULL,request TEXT NOT NULL,response TEXT,input_tokens INTEGER,output_tokens INTEGER,cost_usd REAL,raw_usage TEXT,price_revision TEXT,error TEXT);
      ...
      `);
      if (this.db.prepare('PRAGMA foreign_key_check').all().length) throw new Error('Migration reference check failed');
  ...
  save(kind: string, value: Row, id?: string, expected?: number) {
    return this.store.transaction(() => {
      ...
      const result = { ...value, id: id ?? randomUUID(), revision: (prior?.revision ?? 0) + 1 };
  ...
  content(value: unknown, id?: string) {
    const b = record(value); fields(b,['kind','title','description','text','loading','relatedIds','expectedRevision']);
    if (!Array.isArray(b.relatedIds) || b.relatedIds.length > 100) throw new HttpError(400,'Invalid related IDs');
    return this.save('content',{ kind: choice(b.kind,['bot','persona','lore','canon','skill','glossary'],'content kind'), title: text(b.title,'title',200), description: text(b.description,'description',2000,true), text: text(b.text,'text',100000), loading: choice(b.loading,['pinned','discoverable'],'loading'), relatedIds: [...new Set(b.relatedIds.map((x: unknown) => text(x,'related ID',100)))] },id,id ? number(b.expectedRevision,'revision') : undefined);
  }
  ...
  connection(value: unknown, id?: string) {
    ...
    const endpoint = text(b.endpoint,'endpoint',2000); let url: URL;
    try { url = new URL(endpoint); } catch { throw new HttpError(400,'Invalid endpoint'); }
    if (url.username || url.password || url.search || url.hash || !['http:','https:'].includes(url.protocol)) throw new HttpError(400,'Invalid endpoint');
    const credentialEnv = b.credentialEnv ? text(b.credentialEnv,'credential reference',200) : undefined;
    if (credentialEnv && !/^NARRATIVE_PROVIDER_[A-Z0-9_]+$/.test(credentialEnv)) throw new HttpError(400,'Invalid credential reference');
    return this.save('connection',{ title: text(b.title,'title',200), protocol: choice(b.protocol,['fixture-sse-v1'],'protocol'), endpoint, ...(credentialEnv ? {credentialEnv} : {}), enabled: boolean(b.enabled), catalog: id ? this.get<Connection>('connection',id).catalog : [], catalogError: null },id,id ? number(b.expectedRevision,'revision') : undefined);
  }
  model(value: unknown, id?: string) {
    const b = record(value); fields(b,['title','connectionId','connectionRevision','modelId','maxOutputTokens','temperature','expectedRevision']);
    const connectionId = text(b.connectionId,'connection ID',100); const connectionRevision = number(b.connectionRevision,'connection revision'); this.get('connection',connectionId,connectionRevision);
    if (b.temperature !== null && (typeof b.temperature !== 'number' || !Number.isFinite(b.temperature) || b.temperature < 0 || b.temperature > 2)) throw new HttpError(400,'Invalid temperature');
    return this.save('model',{ title: text(b.title,'title',200), connectionId, connectionRevision, modelId: text(b.modelId,'model ID',300), maxOutputTokens: number(b.maxOutputTokens,'output limit',1,200000), temperature: b.temperature },id,id ? number(b.expectedRevision,'revision') : undefined);
  }
  ...
  updateProfile(chatId: string, value: unknown): ChatProfile {
    const b = record(value); fields(b,['expectedRevision','attachments','creative','routes','image']); const routes = record(b.routes); fields(routes,['main','translation','status','image']);
    ...
    return this.store.transaction(() => {
      ...
      const result: ChatProfile = { chatId, revision: prior.revision+1, attachments, creative: controls, routes: selected, image };
  ...
  snapshot(chatId: string): ProfileSnapshot | undefined {
    ...
    return structuredClone({...p,contents,models});
  }
  ...
  library(): Library { return {contents:this.all('content'),presets:this.all('preset'),connections:this.all('connection'),models:this.all('model'),assets:this.assets()}; }
  export() { const tables = Object.fromEntries(archiveTables.map(t => [t,(this.db.prepare(`SELECT * FROM ${t}`).all() as Row[]).map(r => t === 'assets' ? {...r,bytes:Buffer.from(r.bytes).toString('base64')} : r)])); return {format:'narrative-archive',version:2,createdAt:new Date().toISOString(),tables}; }
  ...
  import(value: unknown) {
    ...
    const a = record(copy); fields(a,['format','version','createdAt','tables']); if (a.format !== 'narrative-archive' || a.version !== 2) throw new HttpError(400,'Unsupported archive'); const tables = record(a.tables); fields(tables,archiveTables);
...
const archiveTables = ['chats','resources','versions','profiles','branches','runs','sources','jobs','job_results','model_inputs','tool_events','events','job_chunks','attempts','assets'];
...
function validateArchiveVersion(row: Row) {
  ...
  if (row.kind === 'content') {
    ...
    fields(body,['id','revision','title','protocol','endpoint','credentialEnv','enabled','catalog','catalogError']); choice(body.protocol,['fixture-sse-v1'],'protocol');
    const endpoint = new URL(text(body.endpoint,'endpoint',2000)); if (endpoint.username || endpoint.password || endpoint.search || endpoint.hash || !['http:','https:'].includes(endpoint.protocol)) throw new HttpError(400,'Invalid archived endpoint');
    ...
    fields(body,['id','revision','title','connectionId','connectionRevision','modelId','maxOutputTokens','temperature']); archiveId(body.connectionId); number(body.connectionRevision,'connection revision'); text(body.modelId,'model ID',300); number(body.maxOutputTokens,'output limit',1,200000);
    if (body.temperature !== null && (typeof body.temperature !== 'number' || !Number.isFinite(body.temperature) || body.temperature < 0 || body.temperature > 2)) throw new HttpError(400,'Invalid temperature');
...
function validateArchiveProfile(product: ProductStore, value: unknown, chatId: string, frozen = false): ProfileSnapshot | ChatProfile {
  const p = record(value); fields(p,['chatId','revision','attachments','creative','routes','image',...(frozen ? ['contents','models'] : [])]);
...
function validateStoredChunk(plan: TranslationPlan, raw: unknown): TranslationResult {
...
function validateArchiveGraph(product: ProductStore) {
  ...
  for (const row of rows('versions')) if (row.kind === 'model') { const model = record(parse(row.body)); product.get('connection',model.connectionId,model.connectionRevision); }
  ...
  for (const run of runs.values()) {
    ...
    if (!isDeepStrictEqual(snapshot.history,product.store.history(run.parent_revision))) throw new HttpError(400,'Snapshot history differs from source ancestry');
  ...
  for (const job of jobs.values()) {
    sameChat(job.source_revision,job.chat_id,sources); const source = product.store.source(job.source_revision); const snapshot = product.store.run(source.runId).snapshot;
  ...
  for (const attempt of rows('attempts')) {
    sameChat(attempt.run_id,attempt.chat_id,runs); sameChat(attempt.job_id,attempt.chat_id,jobs); if ((attempt.run_id === null) === (attempt.job_id === null)) throw new HttpError(400,'Attempt target mismatch');
    choice(attempt.role,['main','translation','status','image'],'attempt role'); if (attempt.run_id !== null ? attempt.role !== 'main' : jobs.get(attempt.job_id)?.kind !== attempt.role) throw new HttpError(400,'Attempt role mismatch');
