...
export async function runMain(snapshot: RunSnapshot, hooks: MainHooks): Promise<MainResult> {
  ...
  while (true) {
    ...
    const built = buildMainProviderRequest(fixed,{results,...(evaluation?{evaluation:{definitions:evaluation.definitions,bootstrap:evaluation.bootstrap,...(evaluation.toolChoice(results.length)?{toolChoice:evaluation.toolChoice(results.length)}:{})}}:{}),...(opaqueState!==undefined?{opaqueState}:{})});
    const {input,request}=built;
    if(evaluation&&request.generation){const configured=request.generation;request.generation=evaluation.generation(configured,results.length);const binding=evaluation.generationBinding(configured,results.length);if(binding)request.generationBinding=binding;}
    ...
    const evaluationTerminals=result.toolCalls.filter(call=>call.name==='eval_submit_artifact');
    if(evaluationTerminals.length){
      if(!evaluation||evaluationTerminals.length!==1||result.toolCalls.some(call=>!evaluation.allNames.includes(call.name as typeof evaluation.allNames[number]))||result.refusal||result.error)return fail('INVALID_EVALUATION_ARTIFACT');
      for(const call of result.toolCalls.filter(call=>call.name!=='eval_submit_artifact')){const event=evaluation.execute(call);results.push(event);await hooks.onToolEvent(structuredClone(event));}
      const submitted=evaluation.submit(evaluationTerminals[0],evaluationTerminals[0].recoveredFromTruncation===true);
      if(!submitted.ok){results.push(submitted.event);await hooks.onToolEvent(structuredClone(submitted.event));opaqueState=result.opaqueState;continue;}
      if(hooks.signal.aborted)return fail('CANCELLED');
      await hooks.onToolEvent({callId:evaluationTerminals[0].id,name:'eval_submit_artifact',args:{},denied:false,result:{accepted:true,sha256:submitted.artifact.sha256,characters:submitted.artifact.text.length,utf8Bytes:submitted.artifact.utf8Bytes,noticeProvided:submitted.artifact.noticeProvided,noticeCharacters:submitted.artifact.noticeCharacters,correctionCount:submitted.artifact.correctionCount}});
    ...
    for (const call of result.toolCalls) {
      ...
      } else event = evaluation?.allNames.includes(call.name as typeof evaluation.allNames[number]) ? evaluation.execute(call) : executeTool(fixed, action, hooks.signal);
      results.push(event);
