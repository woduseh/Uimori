...
export type RegistrationAgentResult = {
...
const evaluationToolsSchema = objectSchema({
  contextMode: { enum: ['model-selected', 'preloaded'] },
  approvalReasoningMode: { enum: ['configured', 'economized'] },
  maximumToolRounds: { type: 'integer', minimum: 0, maximum: 32 },
  terminalLateCorrections: { type: 'boolean' },
  outputRecovery: { type: 'boolean' },
});
const proposalSchema = objectSchema({
  connection: {
    oneOf: [
      objectSchema({ kind: { const: 'existing' }, id: shortString, revision }),
      objectSchema({
        kind: { const: 'new' },
        draft: objectSchema(
          {
            title: shortString,
            protocol: { enum: [...PROVIDER_PROTOCOLS] },
            endpoint: { type: 'string', minLength: 1, maxLength: 2048 },
            credentialEnv: { type: 'string', pattern: CREDENTIAL_ENV_PATTERN, maxLength: 200 },
            enabled: { const: false },
          },
  ...
  model: objectSchema(
    {
      ...
      outputEffort: { enum: ['low', 'medium', 'high', 'xhigh', 'max'] },
      verbosity: { enum: ['low', 'medium', 'high'] },
      reasoningMode: { enum: ['standard', 'pro'] },
      reasoningContext: { enum: ['auto', 'all_turns', 'current_turn'] },
      ...
      serviceTier: { type: 'string', minLength: 1, maxLength: 40 },
      cacheMode: { enum: ['disabled', 'explicit', 'automatic'] },
      cacheTtl: { enum: ['5m', '30m', '1h'] },
      evaluationTools: evaluationToolsSchema,
    },
    ['title', 'modelId', 'maxOutputTokens', 'temperature']
  ),
});
const contract =
  'Prepare a provider/model registration proposal from the user request and supplied adapter metadata. This is a proposal-only task, separate from story generation. ' +
...
function sanitizedContext(value: Json): Json {
  ...
  const rows = (key: string, metadata: boolean): Json[] => {
    ...
    const keys = metadata
      ? ['protocol', 'id', 'title', 'modelId', 'revision']
      : [
          'id',
          'revision',
          'label',
      ...
      for (const key of keys) {
        ...
        if (key === 'revision') {
          if (!Number.isSafeInteger(entry) || Number(entry) < 1)
            throw new Error('REGISTRATION_CONTEXT_INVALID');
          result[key] = Number(entry);
        } else if (['optionKeys', 'limitations'].includes(key)) {
          if (
            !Array.isArray(entry) ||
            entry.length > 100 ||
            entry.some((item) => typeof item !== 'string' || item.length > 4000)
          )
            throw new Error('REGISTRATION_CONTEXT_INVALID');
          ...
          result[key] = Object.fromEntries(
            ['kind', 'reference', 'checkedAt'].flatMap((field) =>
              typeof entry[field] === 'string' && entry[field].length <= 4000
          ...
          if (typeof entry !== 'string' || entry.length > 4000)
            throw new Error('REGISTRATION_CONTEXT_INVALID');
          result[key] = entry;
        }
      }
  ...
  const result = {
    definitions: rows('definitions', false),
    connections: rows('connections', true),
    models: rows('models', true),
  };
...
export async function runRegistrationAgent(
  target: ModelSnapshot,
  request: string,
  hooks: RegistrationAgentHooks
): Promise<RegistrationAgentResult> {
  const failed = (error: string): RegistrationAgentResult => ({
    status: hooks.signal.aborted ? 'cancelled' : 'failed',
    error: hooks.signal.aborted ? 'CANCELLED' : error,
  });
  if (hooks.signal.aborted) return failed('CANCELLED');
  if (typeof request !== 'string' || !request.trim() || request.length > 6000)
    return failed('REGISTRATION_REQUEST_INVALID');
  const duration = hooks.timeoutMs ?? Math.min(target.timeoutMs ?? 60000, 60000);
  if (!Number.isSafeInteger(duration) || duration < 1 || duration > 60000)
    return failed('REGISTRATION_TIMEOUT_INVALID');
  const fixed = structuredClone(target);
  if (fixed.enabled === false) return failed('REGISTRATION_MODEL_DISABLED');
  if (!fixed.connection.enabled || fixed.connection.id !== fixed.connectionId)
    return failed('CONNECTION_NOT_AUTHORIZED');
  const timeout = AbortSignal.timeout(duration);
  const signal = AbortSignal.any([hooks.signal, timeout]);
  ...
  const check = async () => {
    ...
    try {
      connection = await hooks.authorize(structuredClone(fixed.connection));
    } catch {
    ...
    if (
      !connection.enabled ||
      connection.id !== fixed.connection.id ||
      connection.endpoint !== fixed.connection.endpoint ||
      connection.protocol !== fixed.connection.protocol ||
      connection.credentialEnv !== fixed.connection.credentialEnv
    )
      throw new Error('CONNECTION_NOT_AUTHORIZED');
  };
  try {
    ...
    const input: ProviderRequest = {
      ...
      generation: generationFromModel(fixed, fixed.connection.protocol),
      contextBudget: contextBudgetForModel(fixed),
      stable: {
        contract: contract + '\nProposal schema: ' + JSON.stringify(proposalSchema),
        tools: [
          {
            name: 'registration.propose',
            description:
              'Return a reviewable registration proposal without saving, enabling, assigning or testing it.',
      ...
      input: {
        task: request,
        controls: { purpose: 'provider-registration', maxCalls: 1 },
        catalog: context,
        results: [],
      },
    };
    const result = await executeProvider(
      {
        ...
        protocol: fixed.connection.protocol,
        endpoint: fixed.connection.endpoint,
        ...(fixed.connection.credentialEnv
          ? { credentialEnv: fixed.connection.credentialEnv }
          : {}),
      },
      input,
      {
        ...
        onWire: async (wire) => {
          try {
            ...
            boundaryError = (error as Error).message;
            throw error;
          }
          try {
            attempt = await hooks.onAttemptStart(wire);
          } catch {
            boundaryError = 'REGISTRATION_ATTEMPT_START_FAILED';
    ...
    if (attempt !== undefined) {
      ...
      if (timeout.aborted && !hooks.signal.aborted && recorded.status === 'cancelled') {
        recorded.status = recorded.text || recorded.toolCalls.length ? 'partial' : 'error';
        recorded.error = { code: 'TIMEOUT' };
      }
      try {
        await hooks.onAttemptFinish(attempt, recorded);
      } catch {
        return failed('REGISTRATION_ATTEMPT_FINISH_FAILED');
      }
    }
    if (hooks.signal.aborted) return failed('CANCELLED');
    ...
    if (result.error || !['completed', 'tool_calls'].includes(result.status))
      return failed(result.error?.code ?? 'REGISTRATION_PROVIDER_INCOMPLETE');
    let raw: unknown;
    if (result.status === 'tool_calls') {
      if (
        result.toolCalls.length !== 1 ||
        result.toolCalls[0].name !== 'registration.propose' ||
        result.text.trim()
      )
        return failed('REGISTRATION_TOOL_NOT_ALLOWED');
      raw = result.toolCalls[0].arguments;
      if (JSON.stringify(raw).length > 32000) return failed('REGISTRATION_PROPOSAL_TOO_LARGE');
    } else {
      if (result.toolCalls.length || result.text.length > 32000)
        return failed('REGISTRATION_PROPOSAL_TOO_LARGE');
      try {
        raw = JSON.parse(result.text);
      } catch {
        return failed('REGISTRATION_OUTPUT_JSON_INVALID');
      }
    }
    if (!plain(raw) || !plain(raw.connection) || !plain(raw.model))
      return failed('REGISTRATION_PROPOSAL_INVALID');
    if (
      raw.connection.kind === 'new' &&
      (!plain(raw.connection.draft) || raw.connection.draft.enabled !== false)
    )
      return failed('REGISTRATION_CONNECTION_MUST_BE_DISABLED');
    let proposal: unknown;
    try {
      proposal = hooks.onProposal(raw);
    } catch {
      return failed('REGISTRATION_PROPOSAL_INVALID');
    }
    await check();
    ...
    const code =
      ...
      /^(REGISTRATION_[A-Z_]+|CONNECTION_NOT_AUTHORIZED|TIMEOUT|CANCELLED)$/u.test(error.message)
        ? error.message
