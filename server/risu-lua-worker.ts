...
import type {
  NativeLuaHostReply,
  NativeLuaWorkerInput,
  NativeLuaWorkerLimits,
  NativeLuaWorkerPhase,
  NativeLuaWorkerResultCode,
} from './risu-lua-protocol.js';
...
function validLimits(value: unknown): value is NativeLuaWorkerLimits {
...
function harnessSource(limits: NativeLuaWorkerLimits) {
  return String.raw`
...
async function run() {
  const input = workerData as NativeLuaWorkerInput;
  const port = parentPort;
  if (!port) return;
  const fail = (code: NativeLuaWorkerResultCode) =>
  ...
  const hook = lua.module.addFunction((state: number) => {
    if (timedOut || performance.now() > deadline) {
      timedOut = true;
      lua.lua_pushstring(state, 'RISU_LUA_PROGRAM_TIMEOUT');
      lua.lua_error(state);
    }
  }, 'vii');
  ...
  const phase = (value: NativeLuaWorkerPhase, hostIds: number[]) =>
  ...
  while (true) {
    ...
    const response = new Promise<NativeLuaHostReply>((resolve, reject) => {
      port.once('message', (value: NativeLuaHostReply) => {
        if (
          !value ||
    ...
    if (result.ok) {
      ...
      thread.pushValue(false);
      thread.pushValue(
        input.hostErrorCodes.includes(result.code) ? result.code : 'RISU_LUA_HOST_CALL_FAILED'
      );
    }
...
void run().catch(() => {
  parentPort?.postMessage({ type: 'result', ok: false, code: 'RISU_LUA_PROGRAM_RUNTIME_FAILED' });
});
