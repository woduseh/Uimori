...
export type CodexProcessErrorCode = 'CODEX_START_FAILED' | 'CODEX_CLOSED' | 'CODEX_PROTOCOL_ERROR' | 'CODEX_REQUEST_FAILED' | 'CODEX_TIMEOUT' | 'CODEX_CANCELLED';
/** Never include provider errors, stderr, request parameters, or auth data. */
export class CodexProcessError extends Error {
  constructor(readonly code: CodexProcessErrorCode) { super(code); this.name = 'CodexProcessError'; }
}
export interface CodexProcessOptions {
  command: string; args?: string[]; cwd: string; env: NodeJS.ProcessEnv; timeoutMs?: number; experimentalApi?: boolean;
}
...
export class CodexProcess {
  ...
  private async initialize(): Promise<void> {
    try {
      this.child = spawn(this.options.command, this.options.args ?? ['app-server'], {
        cwd: this.options.cwd, env: this.options.env, shell: false, windowsHide: true, stdio: ['pipe', 'pipe', 'pipe'],
      });
      ...
      await this.request('initialize', {
        ...
        capabilities: { experimentalApi: this.options.experimentalApi ?? false, requestAttestation: false },
      });
      this.write({ method: 'initialized' });
    } catch (error) {
      this.fail(error instanceof CodexProcessError ? error.code : 'CODEX_START_FAILED');
      throw error instanceof CodexProcessError ? error : new CodexProcessError('CODEX_START_FAILED');
    }
  }
  ...
  request<T>(method: string, params: unknown, options: { signal?: AbortSignal; timeoutMs?: number } = {}): Promise<T> {
    ...
    if (this.pending.size >= MAX_PENDING) return Promise.reject(new CodexProcessError('CODEX_REQUEST_FAILED'));
    const id = this.nextId++;
    return new Promise<T>((resolve, reject) => {
      const finish = (code: CodexProcessErrorCode) => {
        const item = this.pending.get(id);
        if (!item) return;
        this.pending.delete(id); item.cleanup(); item.reject(new CodexProcessError(code));
      };
      const abort = () => finish('CODEX_CANCELLED');
      const timeout = setTimeout(() => finish('CODEX_TIMEOUT'), options.timeoutMs ?? this.options.timeoutMs ?? 30_000);
      ...
      try { this.write({ id, method, params }); } catch { finish('CODEX_CLOSED'); }
    });
  }
  ...
  private write(value: unknown): void {
    ...
    let line: string;
    try { line = JSON.stringify(value) + '\n'; } catch { throw new CodexProcessError('CODEX_REQUEST_FAILED'); }
    if (Buffer.byteLength(line) > MAX_LINE_BYTES || this.child.stdin.writableLength > MAX_LINE_BYTES) {
      this.fail('CODEX_PROTOCOL_ERROR'); throw new CodexProcessError('CODEX_PROTOCOL_ERROR');
    }
  ...
  private receive(data: Buffer): void {
    ...
    while (start < data.length && !this.closed) {
      ...
      if (this.buffer.length + end - start > MAX_LINE_BYTES) { this.fail('CODEX_PROTOCOL_ERROR'); return; }
      this.buffer = Buffer.concat([this.buffer, data.subarray(start, end)]);
      if (newline < 0) return;
      try {
        const line = this.decoder.decode(this.buffer); this.buffer = Buffer.alloc(0);
        this.dispatch(JSON.parse(line));
      } catch { this.fail('CODEX_PROTOCOL_ERROR'); return; }
      start = newline + 1;
    }
  }
  private dispatch(message: unknown): void {
    ...
    if ('method' in record) {
      ...
      if ('id' in record) {
        ...
        this.write({ id: record.id, error: { code: -32601, message: 'Client request not supported' } });
        for (const listener of this.notifications) {
          try { listener('uimori/unsupportedRequest', { method: record.method }); } catch {}
        }
      } else for (const listener of this.notifications) {
        try { listener(record.method, record.params); } catch { /* Consumer failure is not provider data. */ }
      }
      return;
    }
    if (typeof record.id !== 'number' || (('result' in record) === ('error' in record))) throw new Error();
    const item = this.pending.get(record.id);
    if (!item) return; // Timed-out/cancelled requests can still finish; never replay them.
    this.pending.delete(record.id); item.cleanup();
  ...
  private fail(code: CodexProcessErrorCode): void {
    ...
    for (const item of this.pending.values()) { item.cleanup(); item.reject(new CodexProcessError(code)); }
    this.pending.clear();
    this.child?.stdin.destroy();
    this.child?.kill();
    for (const listener of this.exits) { try { listener(); } catch {} }
    this.exits.clear(); this.notifications.clear();
  }
  close(): Promise<void> {
    return this.closing ??= new Promise<void>(resolve => {
      const child = this.child;
      if (!child || child.exitCode !== null || child.signalCode !== null) { this.fail('CODEX_CLOSED'); resolve(); return; }
      let done = false;
      const finish = () => { if (done) return; done = true; clearTimeout(timer); child.removeListener('close', finish); resolve(); };
      const timer = setTimeout(() => { child.kill('SIGKILL'); finish(); }, 1000);
