import { GENERATION_KEYS, generationFromModel, modelCapability, validateGenerationShape } from '../core/model-capabilities.js';
import { validCredentialEnv } from '../core/credential-reference.js';
import { createHash, randomUUID } from 'node:crypto';
import { isDeepStrictEqual } from 'node:util';
import { PROVIDER_PROTOCOLS, validateProviderEndpoint, type Connection, type ContentRef, type ModelPreset, type ModelSnapshot } from '../core/product.js';
...
const modelFields = ['title','modelId',...GENERATION_KEYS,'timeoutMs','evaluationTools','enabled'];
...
export function normalizeRegistrationPlan(product:ProductStore,value:unknown,options:{connectionSnapshot?:Connection}={}):RegistrationPlan {
  ...
  if(selection.kind==='existing') {
    ...
    if(connection.id!==requested.id||connection.revision!==requested.revision)throw new HttpError(409,'Connection changed; review a new proposal');
  ...
  const prepared=product.prepareModel({...model,connectionId:connection.id},undefined,connection).value;
...
export class RegistrationStore {
  ...
  create(value:unknown):{run:RegistrationRun;created:boolean;target:ModelSnapshot} {
    ...
    if(current.revision!==targetRef.revision)throw new HttpError(409,'Registration assistant model changed');
    const model=current;
    const connection=this.product.authorize(this.product.get<Connection>('connection',model.connectionId));
    ...
    const run:RegistrationRun={id,revision:0,intentHash,createdAt:new Date().toISOString(),finishedAt:null,status:'running',request,target:targetRef,connection:{id:connection.id,revision:connection.revision},targetSnapshot,attempts:[],plan:null,planHash:null,planConnectionSnapshot:null,error:null,applied:null,appliedSnapshot:null};
  ...
  finish(id:string,status:'ready'|'failed'|'cancelled',proposal?:unknown,error:string|null=null) {
    ...
    const planConnectionSnapshot=plan?.connection.kind==='existing'?this.product.get<Connection>('connection',plan.connection.id,plan.connection.revision):null;
    return this.append({...run,status,plan,planHash:plan?hash(plan):null,planConnectionSnapshot,error,finishedAt:new Date().toISOString()},run.revision);
  }
  recover() {
  ...
  view(id:string):RegistrationView {
    const {intentHash:_intent,attempts,targetSnapshot:_target,planConnectionSnapshot:_planConnection,appliedSnapshot:_applied,...run}=this.get(id);return {...run,modelCalls:attempts.length,usage:attempts.at(-1)?.usage??null};
  }
  apply(id:string,value:unknown):RegistrationRun {
    ...
    return this.product.store.transaction(()=>{
      ...
      const prepared=this.product.prepareModel({...plan.model,connectionId:connection.id}).value;
      const model=this.product.saveInTransaction('model',prepared) as ModelPreset;
      return this.append({...run,status:'applied',applied:{connection:{id:connection.id,revision:connection.revision},model:{id:model.id,revision:model.revision}},appliedSnapshot:structuredClone({connection,model})},run.revision);
    });
  }
}
...
function validateWire(value:unknown) {
  const wire=record(value);fields(wire,['connectionId','protocol','role','modelId','method','url','headers','body','bodySha256','stablePrefixSha256']);
...
export function validateRegistrationArchive(value:unknown) {
  const b=record(value);fields(b,['id','revision','intentHash','createdAt','finishedAt','status','request','target','connection','targetSnapshot','attempts','plan','planHash','planConnectionSnapshot','error','applied','appliedSnapshot']);
  ...
  if(!isDeepStrictEqual({id:target.id,revision:target.revision},b.target)||!isDeepStrictEqual({id:target.connection.id,revision:target.connection.revision},b.connection))throw new HttpError(400,'Registration target snapshot mismatch');
  ...
  if(b.plan===null){if(b.planHash!==null||b.planConnectionSnapshot!==null||b.applied!==null||b.appliedSnapshot!==null||['ready','applied'].includes(b.status))throw new HttpError(400,'Invalid registration plan');}
  else {
    ...
    if(c.kind==='existing'){fields(c,['kind','id','revision']);ref({id:c.id,revision:c.revision});const captured=record(b.planConnectionSnapshot),draft=record(plan.model);if(captured.id!==c.id||captured.revision!==c.revision)throw new HttpError(400,'Registration review connection mismatch');validateModelSnapshot({...draft,id:'registration-plan-preview',revision:1,connectionId:captured.id,capabilityRevision:modelCapability(captured.protocol,draft.modelId)?.revision,connection:captured});}
    else if(c.kind==='new'){fields(c,['kind','draft']);if(b.planConnectionSnapshot!==null)throw new HttpError(400,'Unexpected registration review connection');const draft=record(c.draft);fields(draft,connectionFields);if(draft.enabled!==false||!PROVIDER_PROTOCOLS.includes(draft.protocol))throw new HttpError(400,'Invalid proposed authority');text(draft.title,'connection title',200);validateProviderEndpoint(draft.protocol,text(draft.endpoint,'endpoint',2000));if(draft.credentialEnv!==undefined&&!validCredentialEnv(draft.credentialEnv))throw new HttpError(400,'Invalid credential reference');}
    ...
    try { validateGenerationShape(generationFromModel(model as ModelPreset)); } catch { throw new HttpError(400,'Invalid model option'); }
  ...
  if(b.applied!==null){
    const applied=record(b.applied);fields(applied,['connection','model']);ref(applied.connection);ref(applied.model);if(b.status!=='applied')throw new HttpError(400,'Invalid applied registration');
    const snapshot=record(b.appliedSnapshot);fields(snapshot,['connection','model']);const model=validateModelSnapshot({...record(snapshot.model),connection:snapshot.connection});
    if(!isDeepStrictEqual(applied.model,{id:model.id,revision:model.revision})||!isDeepStrictEqual(applied.connection,{id:model.connection.id,revision:model.connection.revision}))throw new HttpError(400,'Registration applied snapshot mismatch');
  }
  else if(b.status==='applied'||b.appliedSnapshot!==null)throw new HttpError(400,'Missing applied registration');
}
...
export function normalizeRegistrationArchiveRow(row:Record<string,any>):void {
  ...
  for(const connection of [run.targetSnapshot?.connection,run.planConnectionSnapshot,run.appliedSnapshot?.connection])if(connection){delete connection.credentialEnv;connection.enabled=false;}
...
export function validateRegistrationGraph(product:ProductStore) {
  ...
  for(const row of rows){const run=JSON.parse(row.body) as RegistrationRun;validateRegistrationArchive(run);
    const target=run.targetSnapshot,connection=target.connection;
    ...
    if(run.plan){const normalized=normalizeRegistrationPlan(product,run.plan,{connectionSnapshot:run.planConnectionSnapshot??undefined});if(!isDeepStrictEqual(normalized,run.plan))throw new HttpError(400,'Registration plan normalization mismatch');}
    if(run.applied){
      const c=run.appliedSnapshot!.connection,m=run.appliedSnapshot!.model;
      if(!run.plan||m.connectionId!==c.id||m.revision!==1)throw new HttpError(400,'Registration applied reference mismatch');
      if(run.plan.connection.kind==='existing'){if(c.id!==run.plan.connection.id||c.revision!==run.plan.connection.revision||!isDeepStrictEqual(c,run.planConnectionSnapshot))throw new HttpError(400,'Registration applied connection mismatch');}
      ...
      const expected=product.prepareModel({...run.plan.model,connectionId:c.id},undefined,c).value;const {id:_modelId,revision:_modelRevision,...actualModel}=m;if(!isDeepStrictEqual(expected,actualModel))throw new HttpError(400,'Registration applied model mismatch');
    ...
    if(!old){if(run.revision!==1||run.status!=='running'||run.attempts.length!==0)throw new HttpError(400,'Missing initial registration revision');}
      ...
      if(run.revision!==old.revision+1||!isDeepStrictEqual([run.intentHash,run.request,run.target,run.connection,run.targetSnapshot,run.createdAt],[old.intentHash,old.request,old.target,old.connection,old.targetSnapshot,old.createdAt])||run.attempts.length<old.attempts.length)throw new HttpError(400,'Registration history mismatch');
      ...
      if(old.status!=='running'&&!(old.status==='ready'&&run.status==='applied')&&(!isDeepStrictEqual([old.status,old.plan,old.planHash,old.planConnectionSnapshot,old.applied,old.appliedSnapshot,old.error,old.finishedAt],[run.status,run.plan,run.planHash,run.planConnectionSnapshot,run.applied,run.appliedSnapshot,run.error,run.finishedAt])))throw new HttpError(400,'Registration terminal history mismatch');
      if(old.status==='ready'&&run.status==='applied'&&!isDeepStrictEqual([old.plan,old.planHash,old.planConnectionSnapshot,old.finishedAt],[run.plan,run.planHash,run.planConnectionSnapshot,run.finishedAt]))throw new HttpError(400,'Registration review changed');
    }
