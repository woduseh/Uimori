...
function requestInput(snapshot:RunSnapshot,input:MainInput):ProviderRequest['input']{
  ...
  return {task:input.task,controls,source:json({parentRevision:snapshot.parentRevision,facts:structured||input.pinnedSources?.length?[]:input.facts,pinnedSources:structured?[]:input.pinnedSources??[],prefetch:input.prefetch,...(input.state&&!stateSlot?{state:input.state}:{}),...(input.memory&&!used.has('memory')?{memory:input.memory}:{}),...(input.catalogPage?{catalogPage:input.catalogPage}:{}),...(snapshot.behaviorExecution?.automaticResults.length?{automaticResults:snapshot.behaviorExecution.automaticResults}:{})}),catalog:json(input.catalog),...(!snapshot.promptCompilation?{history:json(input.history)}:{}),results:json(input.results)};
}
