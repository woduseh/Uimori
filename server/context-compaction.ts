...
Merge the entire previousSummary with every supplied fragment. Fragments may be parts of one logical user/assistant exchange; preserve their order, speaker roles, explicit user wishes and constraints, relationships and motivations. A user's out-of-story direction must remain attributed to the user; an assistant's fiction, a character's belief, and a derived summary must never become author canon or a new user instruction. The userNotes field contains explicit, source-anchored user corrections and notes.
Produce bounded working memory around the current situation, active relationships and commitments, unresolved threads, and attributed user constraints or corrections. Rewrite the previous structure instead of appending scene recaps. The original sources remain available for detailed lookup.
...
Use controls.targetSummaryTokens as the approximate token budget for the entire rewritten summary. Finish every statement within the target where possible; do not truncate text to fit. This summary changes no stored source, notes or state. Return a concise, complete summary within the target where possible; the provider's output limit is safety headroom, not the desired summary length.`;
...
const fragment = (
  ...
  role: message.role,
  offsetUtf16: start,
  totalUtf16: message.text.length,
  text: message.text.slice(start, end),
});
const wholeFragments = (units: SourceUnit[]) =>
  units.flatMap((unit) => unit.messages.map((message) => fragment(unit, message)));
...
export async function prepareInputContext(
  ...
  const plan: ContextPlan = {
    ...structuredClone(snapshot.contextPlan),
    status: 'pending',
    ...
    usage,
    ...(previous
      ? { compacted: structuredClone(previous.compacted), summary: previous.summary }
      : {}),
  };
  plan.recentSourceRevisions = fixed.history
    .slice(plan.compacted.length)
    .map((source) => source.revision);
  let estimate = Infinity;
  const progress = async () => {
    plan.summaryCalls = usage.modelCalls;
    plan.estimatedInputTokens = Number.isFinite(estimate) ? estimate : null;
    try {
      await hooks.onProgress(structuredClone(plan));
    } catch {
      fail('CONTEXT_PROGRESS_SAVE_FAILED');
    }
    if (hooks.signal.aborted) fail('CANCELLED');
  };
  const projected = () => ({
    ...withContextProjection(fixed, plan.compacted, plan.summary),
    contextPlan: structuredClone({ ...plan, status: 'ready' as const }),
  });
  const measure = () => {
    const measured = measureInput(projected());
    estimate = measured.estimatedInputTokens;
    return measured.snapshot;
  };
  try {
    const budget = validateContextBudget(plan.budget),
      limit = budget.inputTokenLimit;
    if (hooks.signal.aborted) fail('CANCELLED');
    for (const source of fixed.history) {
      const hash = sha(source.text);
      if (source.contentHash !== undefined && source.contentHash !== hash)
    ...
    if (
      ...
      (plan.compacted.length > 0 && !plan.summary?.trim())
    )
      fail('CONTEXT_CHECKPOINT_INVALID');
    if (!hooks.measureInput && fixed.loreContext)
      fixed.loreContext = fitFixedLoreContext(
    ...
    const summaryPolicy = contextSummaryPolicy({
      ...
      consumerInputTokenLimit: limit,
      generation: generationFromModel(target),
      fixedInputTokens: fixedWithoutSummary,
    });
    ...
    const summarize = async (summary: string | null, fragments: Fragment[]): Promise<string> => {
      ...
      try {
          ...
          ),
          {
            ...
            onWire: async (wire) => {
              try {
                ...
                boundaryError =
                  error instanceof Error ? error.message : 'CONNECTION_NOT_AUTHORIZED';
                throw error;
              }
              try {
                attempt = await hooks.onAttemptStart(wire);
              } catch {
                boundaryError = 'CONTEXT_ATTEMPT_START_FAILED';
                fail(boundaryError);
              }
              usage.modelCalls++;
              plan.summaryCalls = usage.modelCalls;
        ...
        result = {
          ...
          toolCalls: [],
          refusal: null,
          error: { code: hooks.signal.aborted ? 'CANCELLED' : 'CONTEXT_PROVIDER_EXECUTION_FAILED' },
          usage: {
            inputTokens: null,
            outputTokens: null,
            costUsd: null,
      ...
      if (attempt !== undefined) {
        ...
        try {
          ...
          fail('CONTEXT_ATTEMPT_FINISH_FAILED');
        }
      }
      ...
      if (result.error?.code === 'UNEXPECTED_EOF') fail('CONTEXT_COMPACTION_EOF');
      if (result.status === 'partial') fail('CONTEXT_COMPACTION_PARTIAL');
      if (result.status === 'tool_calls' || result.toolCalls.length)
        fail('CONTEXT_COMPACTION_UNEXPECTED_TOOLS');
      if (result.error?.code === 'EMPTY_COMPLETION' || result.error?.code === 'EMPTY_RESPONSE')
        fail('CONTEXT_COMPACTION_EMPTY');
    ...
    const validateUnit = (unit: SourceUnit) => {
      const pair =
        unit.messages.length === 2 &&
        unit.messages[0].role === 'user' &&
        unit.messages[1].role === 'assistant' &&
        unit.messages.every((message) => message.sourceKind === undefined);
      const authoredStart =
        unit.messages.length === 1 &&
        unit.messages[0].role === 'assistant' &&
        unit.messages[0].sourceKind === 'authored-start';
      if (
        (!pair && !authoredStart) ||
        unit.messages.some(
          (message) => message.sourceHash !== undefined && message.sourceHash !== unit.ref.hash
        )
      )
    ...
    while (estimate > limit * TARGET_RATIO || plan.compacted.length < manualTarget) {
      ...
      if (batch.length) nextSummary = await summarize(plan.summary, wholeFragments(batch));
        ...
        validateUnit(unit);
        batch.push(unit);
        let stagedSummary = plan.summary,
          messageIndex = 0,
          offset = 0;
        while (messageIndex < unit.messages.length) {
          ...
          while (messageIndex < unit.messages.length) {
            const message = unit.messages[messageIndex],
              whole = fragment(unit, message, offset);
            if (fits(stagedSummary, [...fragments, whole])) {
            ...
            while (low <= high) {
              const size = Math.floor((low + high) / 2),
                trialEnd = utf16End(message.text, offset, size);
              if (
                trialEnd > offset &&
                fits(stagedSummary, [...fragments, fragment(unit, message, offset, trialEnd)])
              ) {
                end = trialEnd;
                low = size + 1;
              } else high = size - 1;
            ...
            if (end > offset) {
              fragments.push(fragment(unit, message, offset, end));
              offset = end;
            }
      ...
      plan.compacted.push(...batch.map((unit) => unit.ref));
      plan.recentSourceRevisions = fixed.history
        .slice(plan.compacted.length)
        .map((source) => source.revision);
      prepared = measure();
    ...
    plan.status = 'failed';
    plan.error = hooks.signal.aborted
      ? 'CANCELLED'
    ...
    try {
      ...
      plan.error = 'CONTEXT_PROGRESS_SAVE_FAILED';
    }
