...
export function packagePresentationRoutes(app: FastifyInstance, store: Store) {
  app.get<{ Params: { id: string; sourceId: string } }>('/api/chats/:id/sources/:sourceId/presentation', async request => {
    ...
    if (job?.status === 'completed' && job.sourceHash === source.hash) {
      ...
      translation = { text: job.result!.text ?? '', sourceRevision: source.id, sourceHash: source.hash };
    }
    const issues: string[] = [];
    if (snapshot.hiddenStory && snapshot.profile?.packages?.some(p => p.transforms.length)) {
      ...
      issues.push('히든 구간 경계를 보존하기 위해 이 장면의 패키지 정규식 표시는 적용하지 않았어요.');
    }
    const detail = store.story.sourceDetail(source.id);
    const state = detail.status === 'ready' && detail.state?.sourceHash === source.hash
      ? { sourceRevision: source.id, sourceHash: source.hash, values: detail.state.values } : undefined;
    return { ...await buildPackagePresentation(snapshot, { ...source, ...(translation ? { translation } : {}) }, state),
      translationId: translation ? job!.id : null, translationRevision: translation ? job!.revision ?? 0 : null, issues };
  });
}
