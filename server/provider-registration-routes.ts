...
export function registrationRoutes(
  ...
  app.post('/api/provider-management/registrations', async (request) => {
    ...
    if (admission.created) {
      ...
      const context = registrationJson({
        definitions: PROVIDER_DEFINITIONS,
        connections: (store.product.all('connection') as Connection[])
          .slice(0, 100)
          .map(({ id, revision, title, protocol }) => ({ id, revision, title, protocol })),
        models: (store.product.all('model') as ModelPreset[])
          .slice(0, 100)
          .map(({ id, revision, title, modelId }) => ({ id, revision, title, modelId })),
      });
      options.track(
        (async () => {
          try {
            const result = await runRegistrationAgent(target, run.request, {
              ...
              signal,
              timeoutMs: REGISTRATION_LIMITS.timeoutMs,
              vertexRequestTier: options.vertexRequestTier,
              context,
              resolveCredential: options.resolveCredential,
              executeCodex: options.executeCodex,
              authorize: (connection) => {
                if (!options.authenticated(request.headers.cookie) || signal.aborted)
                  throw new HttpError(403, 'Registration session no longer authorized');
                const current = store.product.get<ModelPreset>('model', target.id);
                if (current.enabled === false)
                  throw new HttpError(403, 'Registration assistant model disabled');
                return store.product.authorize(connection);
              },
              onAttemptStart: (wire) => journal.startAttempt(run.id, wire),
              onAttemptFinish: (attempt, response) =>
                journal.finishAttempt(run.id, attempt, response),
  ...
  app.get<{ Params: { key: string } }>(
    '/api/provider-management/registrations/by-key/:key',
    async (request) => journal.byKey(request.params.key)
  );
  app.get<{ Params: { id: string } }>(
    '/api/provider-management/registrations/:id',
    async (request) => journal.view(request.params.id)
  );
  app.post<{ Params: { id: string } }>(
    '/api/provider-management/registrations/:id/cancel',
    async (request) => {
      fields(record(request.body ?? {}), []);
      journal.get(request.params.id);
      controllers.get(request.params.id)?.abort();
      return journal.view(request.params.id);
    }
  );
  app.post<{ Params: { id: string } }>(
    '/api/provider-management/registrations/:id/apply',
    async (request) => {
      journal.apply(request.params.id, request.body);
      return journal.view(request.params.id);
    }
  );
}
