...
2. **설정 → 프로바이더와 모델**에서 프로바이더·모델을 등록하고 채팅의 역할별 모델로 선택해요. 모델 없이 채팅을 만들거나 작성된 시작문을 사용할 수 있지만, 본문 생성·번역 등 모델을 실행하는 역할에는 모델 선택이 필요해요. 프로바이더 추가에서 **TypeSafe AI**를 선택하면 **JEV**가 모델 목록에 등록돼요. 본문·번역 거절 판정, 로어 관련성, 기존 이미지 선택은 JEV가 담당해요. JEV를 사용하는 기능만 키 등록이 필요하며 본문 거절 감지는 기본으로 꺼져 있어요. API 키는 앱에서 바로 입력해요.
