...
- 기본 DB는 `.local/uimori.sqlite`예요. 빈 DB와 현재 지원하는 DB만 열어요. 구체적인 기준 버전은 [현재 데이터 형식](docs/DATA-MIGRATIONS.md#현재-버전)을 확인해요. 개인 작업실 schema 1·2·3는 백업 후 현재 schema 4로 갱신하며 다른 형식은 자동 변환하지 않아요. schema 24 사용자 자료는 별도 일회성 도구로 새 DB에 옮길 수 있어요. [DB와 자료 교환 형식](docs/DATA-MIGRATIONS.md) 테스트용 개발 DB를 초기화하려면 서버 종료 후 `npm run reset:dev`를 실행해요.
