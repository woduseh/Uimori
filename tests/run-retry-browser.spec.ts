...
test('failed request edit, draft protection and uncertain retry reuse one admission', async ({page, request}, info) => {
  const response = await request.post('/api/chats', {data:{title:'Synthetic retry'}});
  ...
  await page.route(`**/api/chats/${chat.id}/reader?*`, async route => {
    ...
    body.runs.push({id:'synthetic-failure',chatId:chat.id,parentRevision:null,settingsRevision:1,
      snapshot:{loreContextReset:true},request:'다시 쓸 합성 요청',status:'failed',sourceRevision:null,
      error:'Synthetic failure',usage:{modelCalls:0,inputTokens:null,outputTokens:null,costUsd:null}});
  ...
  await edit.click(); await expect(input).toHaveValue('다시 쓸 합성 요청');
  await expect(input).toBeFocused();
  await expect(page.getByRole('button',{name:'조회 로어 제외 해제'})).toBeVisible();
  await input.fill('보존할 초안');
  page.once('dialog',dialog=>dialog.dismiss()); await edit.click();
  ...
  await expect(page.getByTestId('pending-run').filter({hasText:'Synthetic failure'})).toBeVisible();
});
