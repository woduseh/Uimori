...
test(
  'PWS01 current options apply globally without changing chat profiles',
  { tag: '@smoke' },
  async ({ page, request }, info) => {
    ...
    await expect(
      page
        .getByRole('tabpanel', { name: '모든 채팅 옵션' })
        .getByLabel('합성 문체', { exact: true })
    ).toHaveValue('간결하게');
  }
);

test(
  'PWS02 translation policy and prompt options save in the independent workspace',
  { tag: '@smoke' },
  async ({ page, request }, info) => {
    await page.goto('/');
