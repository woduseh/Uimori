...
const modelBody=(c:Connection,extra:Record<string,unknown>={})=>({title:'Synthetic',connectionId:c.id,modelId:'synthetic-model',maxOutputTokens:8192,temperature:null,...extra});
...
test('only selected model presets persist evaluation tools and captured runs retain earlier settings',()=>{
  ...
  for(const protocol of ['openai-responses-v1','anthropic-messages-v1'] as const){
    ...
    const chat=store.createChat(`Synthetic ${protocol}`),prior=store.product.profile(chat.id);
    store.product.updateProfile(chat.id,{expectedRevision:prior.revision,attachments:prior.attachments,creative:prior.creative,routes:{...prior.routes,main:{id:selected.id}},image:false});
    const captured=store.product.snapshot(chat.id)!;
    const run=store.createRun(chat.id,{request:'Synthetic evaluation snapshot',expectedRevision:chat.headRevision,expectedSettingsRevision:chat.settingsRevision,idempotencyKey:'evaluation-snapshot'},current=>({chatId:chat.id,parentRevision:current.headRevision,settingsRevision:current.settingsRevision,settings:current.settings,request:'Synthetic evaluation snapshot',history:[],resources:store.product.resources(chat.id,captured),profile:captured})).run;
    ...
    expect(updated).not.toHaveProperty('evaluationTools');expect(store.product.get('model',selected.id)).toEqual(updated);
    expect(()=>store.product.get('model',selected.id,selected.revision)).toThrow('Setting not found');
    ...
    expect(store.run(run.id).snapshot.profile?.models.main?.evaluationTools).toEqual(defaultEvaluationToolOptions());
    expect(store.db.prepare("SELECT COUNT(*) AS n FROM provider_settings WHERE kind='model' AND id=?").get(selected.id)).toEqual({n:1});
...
test('archive preserves current tool settings and rejects malformed tool settings atomically',()=>{
  ...
  const archive=source.product.export(),target=database();expect(target.product.import(archive)).toMatchObject({restored:true});expect(target.product.get('model',m.id)).toEqual(m);
  for(const attack of [{maximumToolRounds:-1},{hiddenRetry:true}]){const bad=structuredClone(archive);const row=bad.tables.provider_settings.find(row=>row.kind==='model')!;const body=JSON.parse(row.body);Object.assign(body.evaluationTools,attack);row.body=JSON.stringify(body);const empty=database();expect(()=>empty.product.import(bad)).toThrow();expect(empty.product.all('model')).toEqual([]);}
});
