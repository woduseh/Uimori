  ...
  return {
    ...
    stateSchema: {
      ...
      properties: {
        ...
        lastRoll: { type: 'number', min: 0, max: 6 },
      },
    },
    ...
    actions: [
      {
        id: 'check',
        inputSchema: { type: 'record', properties: { cost: { type: 'number', min: 0, max: 10 } } },
        when: {
          op: 'gte',
          args: [{ context: ['state', 'energy'] }, { context: ['input', 'cost'] }],
        },
        ...
        effects: [
          ...
          { path: ['lastRoll'], value: { context: ['draws', 'die'] } },
        ],
        ...action,
      },
    ],
...
function evaluate(
  b: PackageBehavior,
  input: RuntimeValue = { cost: 3 },
  draws: Record<string, RuntimeValue> = { die: 4 },
...
describe('shared package behavior action calculation', () => {
  it('keeps omitted triggers as user actions and preserves explicitly disabled actions', () => {
    ...
    const triggers = behaviorActionTriggers(b.actions[0]);
    triggers.push('model');
    expect(behaviorActionTriggers(b.actions[0])).toEqual(['user']);
    expect(
      behaviorActionTriggers(validatePackageBehavior(definition({ triggers: [] })).actions[0])
    ).toEqual([]);
    expect(
      behaviorActionTriggers(
        validatePackageBehavior(
          definition({ triggers: ['user', 'before-turn', 'model'], automaticInput: { cost: 1 } })
        ).actions[0]
      )
    ).toEqual(['user', 'before-turn', 'model']);
  });

  it.each([['model', 'model'], ['turn'], [false], null])(
    'rejects invalid trigger declarations %j',
    (triggers) => {
      expect(() =>
        validatePackageBehavior(definition({ triggers: triggers as BehaviorAction['triggers'] }))
      ).toThrow('BEHAVIOR_ACTION_TRIGGERS');
    }
  );

  it('validates automatic input at registration and allows the empty-record default only for its matching schema', () => {
    expect(() => validatePackageBehavior(definition({ triggers: ['before-turn'] }))).toThrow(
      'BEHAVIOR_RECORD_FIELDS'
    );
    expect(() =>
      validatePackageBehavior(
        definition({ triggers: ['before-turn'], automaticInput: { cost: 11 } })
      )
    ).toThrow('BEHAVIOR_NUMBER_VALUE');
    expect(() => validatePackageBehavior(definition({ automaticInput: { cost: 1 } }))).toThrow(
      'BEHAVIOR_AUTOMATIC_INPUT_TRIGGER'
    );
    expect(() =>
      validatePackageBehavior(
        definition({ triggers: ['before-turn'], inputSchema: { type: 'record', properties: {} } })
      )
    ).not.toThrow();
    expect(() =>
      validatePackageBehavior(
  ...
  it('requires record inputs for model tools while retaining typed direct-user inputs', () => {
    expect(() =>
      validatePackageBehavior(definition({ triggers: ['model'], inputSchema: { type: 'boolean' } }))
    ).toThrow('BEHAVIOR_MODEL_INPUT_ROOT');
    expect(() =>
      validatePackageBehavior(definition({ inputSchema: { type: 'boolean' } }))
    ).not.toThrow();
  });
  ...
  it('produces the same state and compact result from the same supplied draw without ambient randomness', () => {
    ...
    try {
      ...
      expect(first).toEqual({
        state: { energy: 7, lastRoll: 4 },
        result: { applied: true, draws: { die: 4 } },
      });
  ...
  it('projects an explicit result from before state, input, draws, host context and validated next state', () => {
      ...
      definition({
        result: {
          ...
          args: [
            ...
            {
              op: 'subtract',
              args: [{ context: ['state', 'energy'] }, { context: ['nextState', 'energy'] }],
            },
            'cost',
            { context: ['input', 'cost'] },
            'roll',
    ...
    expect(result).toEqual({
      state: { energy: 7, lastRoll: 4 },
      result: { spent: 3, cost: 3, roll: 4, place: 'library' },
    });
  ...
  it('evaluates every effect against the before state and hides nextState until result projection', () => {
      ...
      definition({
        effects: [
          { path: ['energy'], value: 0 },
          {
            ...
            value: {
              op: 'typedIf',
              args: [{ op: 'typedEqual', args: [{ context: ['state', 'energy'] }, 10] }, 6, 1],
            },
          },
        ],
    ...
    expect(evaluate(b)).toEqual({
      state: { energy: 0, lastRoll: 6 },
      result: { energy: 0, lastRoll: 6 },
    });
    const unavailable = definition({
      when: { op: 'typedEqual', args: [{ context: ['nextState'] }, null] },
    });
    expect(
      evaluate(unavailable, { cost: 3 }, { die: 4 }, { nextState: { energy: 999 } }).state
    ).toEqual({ energy: 7, lastRoll: 4 });
  });
  ...
  it('enforces the exact serialized 8000-character result bound, including JSON escaping', () => {
    ...
    expect(() => evaluate(definition({ result: '\n'.repeat(4000) }))).toThrow(
      'BEHAVIOR_RESULT_SIZE'
    );
    expect(() => evaluate(definition(), { cost: 3 }, { die: 4, large: 'x'.repeat(8000) })).toThrow(
      'BEHAVIOR_RESULT_SIZE'
    );
  });
  ...
  it('shares one execution budget across condition, effects and result', () => {
    ...
    const b = definition({
      when: { op: 'gte', args: [expensive, 0] },
      effects: [{ path: ['energy'], value: { op: 'mod', args: [expensive, 100] } }],
      result: expensive,
    });
  ...
  it('rejects getter-bearing host data before reading or copying it', () => {
    let reads = 0;
    const host = Object.defineProperty({}, 'location', {
      enumerable: true,
      get() {
        reads++;
        return 'secret';
    ...
    expect(() => evaluate(definition(), { cost: 3 }, { die: 4 }, host)).toThrow(
      'BEHAVIOR_ACCESSOR'
    );
    expect(reads).toBe(0);
  });
  ...
  it('rejects unsupported ambient operations in result expressions at registration', () => {
    expect(() =>
      validatePackageBehavior(
        definition({ result: { op: 'random', args: [] } as unknown as PromptExpression })
      )
    ).toThrow('PROMPT_INVALID_EXPRESSION');
  });
});
