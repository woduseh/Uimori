...
const request = (variant: Variant): ProviderRequest => ({
  ...
  stable: {
    ...
    tools: [
      {
        name: 'knowledge.read',
        description: 'Read an allowed source',
        inputSchema: {
          type: 'object',
  ...
  input: {
    ...
    source: { text: `Synthetic masking fixture: ${secret}` },
    history: [],
    catalog: [],
    results: [],
  },
...
const message: Json = {
  ...
  role: 'assistant',
  status: 'completed',
  content: [{ type: 'output_text', text: '등대 🌊', annotations: [] }],
};
function events(variant: Variant): (Json | '[DONE]')[] {
    ...
    return [
      {
        ...
        response: { id: 'response-test', usage: { input_tokens: 5, output_tokens: 2 } },
      },
  ...
  if (variant.protocol === 'anthropic-messages-v1')
    return [
      {
        type: 'message_start',
        message: {
  ...
  return [
    {
      id: 'chat-test',
      choices: [
        { index: 0, delta: { role: 'assistant', content: '등대 🌊' }, finish_reason: 'stop' },
      ],
      usage: null,
    },
    { id: 'chat-test', choices: [], usage: { prompt_tokens: 5, completion_tokens: 2 } },
    '[DONE]',
  ];
}
const payload = (items: (Json | '[DONE]')[]) =>
  ': keepalive\r\n' +
  items.map((item) => `data: ${item === '[DONE]' ? item : JSON.stringify(item)}\r\n\r\n`).join('');
function stream(items: (Json | '[DONE]')[], stalled = false) {
  const bytes = new TextEncoder().encode(payload(items));
  let cancelled = 0;
  let delivered = 0;
  ...
  vi.spyOn(reader, 'read').mockImplementation(async () => {
    ...
    try {
      ...
      if (!value.done) delivered += value.value.byteLength;
      return value;
    } finally {
      readPending = false;
    }
  ...
  return {
    ...
    get fullyReading() {
      return delivered === bytes.length && readPending;
    },
  };
}
afterEach(() => {
...
describe('native provider HTTP boundary with synthetic fetch', () => {
  ...
  test.each(variants.slice(0, 2))(
    ...
    async (variant) => {
      ...
      expect(
        (
          await executeProvider(connection(variant), input, {
            ...options(variant),
            onWire: (wire) => {
              wires.push(wire);
            },
  ...
  test.each([200, 429])(
    'official GPT Chat sends Flex once and preserves HTTP %s without changing tier',
    async (status) => {
  ...
  test.each(variants)(
    '$protocol validates credentials and journals before its single exact wire request',
    async (variant) => {
      const wires: WireRecord[] = [];
      let release!: () => void;
      ...
      const response = stream(events(variant));
      let sentBody = '';
      const fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        expect(wires).toHaveLength(1);
        expect(String(input)).toBe(variant.endpoint + variant.path);
        ...
        expect(JSON.parse(sentBody).model).toBe(request(variant).modelId);
        return response.response;
      });
      vi.stubGlobal('fetch', fetch);
      const pending = executeProvider(connection(variant), request(variant), {
        ...options(variant),
        onWire: async (wire) => {
          wires.push(wire);
          await admitted;
      ...
      expect(result).toMatchObject({
        status: 'completed',
        text: '등대 🌊',
        usage: { inputTokens: 5, outputTokens: 2, costUsd: null },
      });
      expect(fetch).toHaveBeenCalledTimes(1);
  ...
  test.each(variants.flatMap((variant) => [401, 429].map((status) => ({ variant, status }))))(
    '$variant.protocol HTTP $status never retries or exposes provider error bodies',
  ...
  test.each(
    variants.flatMap((variant) => ['timeout', 'cancel'].map((mode) => ({ variant, mode })))
  )(
    '$variant.protocol $mode interrupts a pending read and preserves observed usage',
    async ({ variant, mode }) => {
      const response = stream(events(variant).slice(0, -1), true);
      ...
      await vi.waitFor(() => expect(response.fullyReading).toBe(true));
      if (mode === 'cancel') controller.abort();
      const result = await pending;
      expect(result).toMatchObject({
        status: mode === 'cancel' ? 'cancelled' : 'partial',
        text: '등대 🌊',
      ...
      expect(fetch).toHaveBeenCalledTimes(1);
      expect(response.cancelled).toBeGreaterThan(0);
    }
  );

  test.each(variants)(
    '$protocol reports EOF without a provider terminal and retains partial evidence',
    async (variant) => {
      const fetch = vi.fn(async () => stream(events(variant).slice(0, -1)).response);
      vi.stubGlobal('fetch', fetch);
  ...
  test.each(variants)(
    '$protocol rejects a successful HTTP response without a body',
    async (variant) => {
      const fetch = vi.fn(
      ...
      vi.stubGlobal('fetch', fetch);
      expect(
        await executeProvider(connection(variant), request(variant), options(variant))
      ).toMatchObject({ status: 'error', error: { code: 'INVALID_CONTENT_TYPE' } });
      expect(fetch).toHaveBeenCalledTimes(1);
    }
  );
  ...
  test.each(variants)(
    ...
    async (variant) => {
      ...
      expect(fetch).not.toHaveBeenCalled();
      expect(
        await executeProvider(connection(variant), request(variant), {
          ...options(variant),
  ...
  test.each(variants)(
    '$protocol honors denied admission and caller cancellation before fetch',
    async (variant) => {
      const fetch = vi.fn();
      vi.stubGlobal('fetch', fetch);
      const denied = await executeProvider(connection(variant), request(variant), {
        ...options(variant),
        onWire: () => {
          throw new ProviderContractError('ATTEMPT_PERSISTENCE_FAILED');
        },
      });
  ...
  test('a local compatible endpoint may omit authentication without reading environment credentials', async () => {
    ...
    const result = await executeProvider(
      { id: 'local-test', protocol: 'openai-chat-v1', endpoint: 'http://127.0.0.1:9876/v1' },
      request(variant),
      {
        signal: new AbortController().signal,
