...
function behavior(): PackageBehavior {
  return { revision: 1, schemaVersion: 1, stateSchema: { type: 'record', properties: { hiddenCount: { type: 'number', min: 0, max: 100 } } }, initialState: { hiddenCount: 37 }, outputParsers: [], actions: [
    { id: 'user_only', inputSchema: { type: 'record', properties: {} }, effects: [] },
    { id: 'resolve_check', label: 'Resolve persuasion', description: 'Resolve one persuasion attempt in the current scene.', triggers: ['model'], inputSchema: { type: 'record', properties: { intent: { type: 'string', maxLength: 120 }, difficulty: { type: 'enum', values: ['easy', 'hard'] } } }, draws: [{ id: 'die', type: 'integer', min: 1, max: 20 }], effects: [] },
  ] };
}
...
function hooks(origin: string, extra: Partial<MainHooks> = {}) {
  const inputs: ModelInput[] = [], events: ToolEvent[] = [], attempts: WireRecord[] = [];
  const value: MainHooks = { signal: new AbortController().signal, approvedOrigins: [origin], authorize: value => value, onInput: input => { inputs.push(input); }, onToolEvent: event => { events.push(event); }, onAttemptStart: wire => { attempts.push(wire); return String(attempts.length); }, onAttemptFinish: () => {}, ...extra };
...
describe('Author-selected behavior tool surface', () => {
  test('BT01 converts constraints recursively without exposing internal rules or state', () => {
    const schema = behaviorInputJsonSchema({ type: 'record', properties: {
      amount: { type: 'number', integer: true, min: 1, max: 6, label: 'Amount', description: 'Requested amount' },
      flag: { type: 'boolean' }, tags: { type: 'list', maxItems: 2, items: { type: 'string', maxLength: 10 } },
      choice: { type: 'enum', values: [true, 'unknown', 3] }, nested: { type: 'record', properties: { value: { type: 'number', min: -1, max: 1 } } },
    } });
    expect(schema).toEqual({ type: 'object', additionalProperties: false, required: ['amount', 'flag', 'tags', 'choice', 'nested'], properties: {
      amount: { type: 'integer', minimum: 1, maximum: 6, title: 'Amount', description: 'Requested amount' }, flag: { type: 'boolean' }, tags: { type: 'array', maxItems: 2, items: { type: 'string', maxLength: 10 } },
      choice: { enum: [true, 'unknown', 3] }, nested: { type: 'object', additionalProperties: false, required: ['value'], properties: { value: { type: 'number', minimum: -1, maximum: 1 } } },
    } });
    const work = snapshot(), before = structuredClone(work), bindings = listBehaviorTools(work), built = buildMainProviderRequest(work);
    expect(bindings).toHaveLength(1); expect(bindings[0]).toMatchObject({ instanceId: 'rules:bot', actionId: 'resolve_check' });
    ...
    expect(JSON.stringify(built.request.stable)).not.toMatch(/hiddenCount|initialState|outputParsers|effects|"draws"|user_only/);
    expect(work).toEqual(before);
  });

  test('BT02 defaults to user only and exact attachment/role selection, excluding disabled persona', () => {
    const work = snapshot(), pkg = work.profile!.packages![0];
    delete pkg.behavior!.actions[1].triggers;
    expect(listBehaviorTools(work)).toEqual([]);
    expect(buildMainInput(work).tools).not.toEqual(expect.arrayContaining([expect.stringMatching(/^behavior_/)]));
    ...
    work.profile!.creative.personaReference = false;
    expect(listBehaviorTools(work)).toEqual([bot]);
    work.profile!.packages!.push({ ...structuredClone(pkg), id: 'unattached', title: 'UNATTACHED_RULE_CANARY' });
    expect(JSON.stringify(listBehaviorTools(work))).not.toContain('UNATTACHED_RULE_CANARY');
    const action = { callId: 'permission-test', name: bot.tool.name, args: { intent: 'test', difficulty: 'easy' } };
    for (const role of ['main', 'translation', 'status', 'image'] as const) expect(executeTool(work, action, undefined, role)).toMatchObject({ denied: true, args: {}, result: { code: 'TOOL_NOT_ALLOWED' } });
  });

  test('BT03 enforces the shared tool limit and rejects known unsupported models before attempts', async () => {
    const work = snapshot(), template = work.profile!.packages![0].behavior!.actions[1];
    work.profile!.packages![0].behavior!.actions = Array.from({ length: 20 }, (_, index) => ({ ...template, id: `action_${index}` }));
    ...
    expect(await runMain(work, observed.value)).toMatchObject({ status: 'error', error: 'BEHAVIOR_MODEL_ACTION_LIMIT', usage: { modelCalls: 0 } });
    for (const kind of ['catalog', 'override'] as const) {
      const unsupported = snapshot(), target = unsupported.profile!.models.main!;
      if (kind === 'catalog') target.connection.catalog = [{ id: target.modelId, name: target.modelId, capabilities: { tools: false }, priceRevision: null }];
      else target.userOverrides = { tools: false, structuredOutput: null, note: 'Synthetic unsupported route' };
      expect(() => buildMainProviderRequest(unsupported)).toThrow('BEHAVIOR_MODEL_TOOLS_UNSUPPORTED');
      expect(await runMain(unsupported, observed.value)).toMatchObject({ status: 'error', error: 'BEHAVIOR_MODEL_TOOLS_UNSUPPORTED', usage: { modelCalls: 0 } });
    }
...
describe('Main behavior tool execution through local provider transports', () => {
  test('BT05 executes exact frozen bindings, resumes with compact results and keeps prompt inputs stable', async () => {
    let actionName = '', executions = 0;
    const server = await loopbackProvider(async (captured, response) => {
      if (server.requests.length === 1) await writeSse(response, [{ type: 'tool_delta', index: 0, id: 'resolve-1', name: actionName, argumentsDelta: '{"intent":"persuade","difficulty":"easy"}' }, { type: 'opaque_state', state: { continuation: 'synthetic-continuation' } }, { type: 'done', reason: 'tool_calls' }]);
      else {
        const body = JSON.parse(captured.body);
        expect(body.input.results).toEqual([{ callId: 'resolve-1', name: actionName, args: { intent: 'persuade', difficulty: 'easy' }, denied: false, result: { accepted: true, result: { success: true, total: 17 } } }]);
        expect(body.opaqueState).toEqual({ continuation: 'synthetic-continuation' });
        await writeSse(response, [{ type: 'text_delta', delta: 'The keeper accepted the appeal.' }, { type: 'done', reason: 'stop' }]);
      }
    }); cleanups.push(server.close);
    const work = snapshot(server.endpoint), before = structuredClone(work); actionName = listBehaviorTools(work)[0].tool.name;
    const observed = hooks(server.origin, { onBehaviorTool: (binding, action) => {
      executions++; expect(binding).toEqual({ instanceId: 'rules:bot', actionId: 'resolve_check' });
      return { ...action, denied: false, result: { accepted: true, result: { success: true, total: 17 } } };
    } });
    expect(await runMain(work, observed.value)).toMatchObject({ status: 'completed', text: 'The keeper accepted the appeal.', usage: { modelCalls: 2 } });
    expect(executions).toBe(1); expect(work).toEqual(before); expect(observed.events).toHaveLength(1);
  ...
  test.each(['unregistered', 'missing-executor', 'host-denial'] as const)('BT06 %s cannot fall through to read permissions or another provider call', async scenario => {
    let actionName = '', executions = 0;
    const server = await loopbackProvider(async (_captured, response) => writeSse(response, [{ type: 'tool_delta', index: 0, id: 'denied-1', name: actionName, argumentsDelta: '{"secret":"PRIVATE_ARGUMENT"}' }, { type: 'done', reason: 'tool_calls' }])); cleanups.push(server.close);
    const work = snapshot(server.endpoint); actionName = scenario === 'unregistered' ? 'behavior_not_registered' : listBehaviorTools(work)[0].tool.name;
    const observed = hooks(server.origin, scenario === 'missing-executor' ? {} : { onBehaviorTool: (_binding, action) => { executions++; return { callId: action.callId, name: action.name, args: {}, denied: true, result: { code: 'BEHAVIOR_INVALID_ARGUMENTS' } }; } });
    expect(await runMain(work, observed.value)).toMatchObject({ status: 'error', error: 'ACTION_TOOL_DENIED', usage: { modelCalls: 1 } });
    expect(executions).toBe(scenario === 'host-denial' ? 1 : 0); expect(server.requests).toHaveLength(1);
