...
const request = (role: ProviderRequest['role'] = 'main'): ProviderRequest => ({
  ...
  input: {
    task: 'Write a scene at the lighthouse.',
    controls: { tone: 'calm', perspective: 'third' },
    source: { revision: 'source-7', hash: 'synthetic-hash' },
    catalog: [{ id: 'lore-1', title: '등대' }],
    results: [],
  },
});
  ...
  const contents: Content[] = [
    {
      ...
      title: 'Ada',
      description: 'Keeper',
      text: 'Ada tends a coastal lighthouse.',
      loading: 'pinned',
      relatedIds: [],
    },
    {
...
function runnerHooks(origin: string, extra: Partial<MainHooks> = {}) {
  ...
  const hooks: MainHooks = {
    ...
    onAttemptStart: (wire) => {
      ...
      attempts.push({ id, request: wire });
      return id;
    },
    onAttemptFinish: (id, result) => {
      attempts.find((item) => item.id === id)!.result = result;
    },
    ...extra,
  };
...
describe('server main runner through the actual loopback adapter', () => {
  ...
  test('P01 P02 P05 pins attached canon, compiles active controls and journals real tool turns before fetch', async () => {
    ...
    const server = await fixture(async (captured, response) => {
      ...
      if (count === 1)
        ...
        expect(body.input.results.map((item: ToolEvent) => item.callId)).toEqual([
          'read-lore',
          'read-skill',
        ]);
        ...
        await writeSse(response, [
          { type: 'text_delta', delta: 'Ada walked toward the copper observatory.' },
    ...
    expect(outcome).toMatchObject({
      ...
      usage: { modelCalls: 2, inputTokens: 18, outputTokens: 5, costUsd: null },
    });
    ...
    expect(first.input).not.toHaveProperty('history');
    expect(
      first.prompt.messages
        .filter((m: any) => m.provenance.origin === 'history')
        .map((m: any) => m.content[0].text)
    ).toEqual(snapshot.history.map((h) => h.text));
    expect(JSON.stringify(first)).not.toContain('AUXILIARY_ONLY');
    expect(first.input.catalog.find((item: Content) => item.id === 'lore-1')).not.toHaveProperty(
      'text'
    );
  ...
  test('P05 rejects reused tool IDs across main rounds before executing any tools from the invalid round', async () => {
    ...
    const server = await fixture(async (_captured, response) => {
      ...
      if (calls === 1)
        ...
        await writeSse(response, [
          { type: 'text_delta', delta: 'Unexpected third request.' },
          { type: 'done', reason: 'stop' },
        ]);
    });
    const observed = runnerHooks(server.origin);
    ...
    expect(outcome).toMatchObject({
      ...
      text: '',
      usage: { modelCalls: 2, inputTokens: 18, outputTokens: 6, costUsd: null },
    });
    ...
    expect(
      JSON.parse(server.requests[1].body).input.results.map((event: ToolEvent) => event.callId)
    ).toEqual([originalId]);
  });
  ...
  test.each([
    ...
    'P05 honors main timeout from $selection and forwards selected thinking level',
    async ({ hookTimeout, presetTimeout }) => {
      ...
      expect(outcome).toMatchObject({
        ...
        error: 'TIMEOUT',
        usage: { modelCalls: 1, inputTokens: 5, outputTokens: 2, costUsd: null },
      });
      expect(server.requests).toHaveLength(1);
      expect(observed.tools).toEqual([]);
      expect(observed.attempts).toHaveLength(1);
      expect(observed.attempts[0].result).toMatchObject({
        status: 'partial',
        error: { code: 'TIMEOUT' },
      });
      expect(JSON.parse(server.requests[0].body).generation).toEqual({
        maxOutputTokens: 4096,
        temperature: null,
        thinkingLevel: 'LOW',
      });
  ...
  test('P04 P05 checks current connection policy before each call and never retries a revoked route', async () => {
    ...
    expect(result).toMatchObject({
      ...
      usage: { modelCalls: 1, inputTokens: null, costUsd: null },
    });
    expect(server.requests).toHaveLength(1);
    expect(observed.attempts).toHaveLength(1);
      ...
      await writeSse(response, [
        {
          ...
          id: 'escape',
          name: 'shell.execute',
          argumentsDelta: '{"command":"PRIVATE_ARGUMENT"}',
        },
        { type: 'done', reason: 'tool_calls' },
      ]);
    });
...
describe('fixture HTTP transport (no live provider compatibility claim)', () => {
  test('P05 handles fragmented UTF8, tool argument JSON, call IDs and opaque state across real fetch turns', async () => {
      ...
      await writeSse(
        ...
        calls === 1
          ? [
              ...
              {
                type: 'opaque_state',
                state: { response: 'fixture-r7', signature: 'opaque-roundtrip-value' },
              },
              {
                type: 'usage',
                inputTokens: 12,
                outputTokens: 4,
                costUsd: null,
                raw: { input_tokens: 12, output_tokens: 4, cached_tokens: 0 },
                priceRevision: null,
              },
              { type: 'done', reason: 'tool_calls' },
            ]
          : [
              { type: 'text_delta', delta: '등대의 불빛 🌊' },
              { type: 'done', reason: 'stop' },
            ],
        true
      );
    });
    const first = await executeProvider(
    ...
    expect(first).toMatchObject({
      status: 'tool_calls',
      toolCalls: [{ id: 'call-lore-7', name: 'knowledge.search', arguments: { query: '등대 🌊' } }],
      usage: { inputTokens: 12, outputTokens: 4, costUsd: null },
    });
    const next = request();
    next.opaqueState = first.opaqueState;
    next.input.results = first.toolCalls.map((call) => ({
      callId: call.id,
      name: call.name,
      result: { items: [{ id: 'lore-1', title: '등대' }] },
    }));
  ...
  test('P02 P05 preserves exact role requests and stable prefix while dynamic controls and source change', async () => {
    const server = await fixture(async (_req, response) => {
      await writeSse(response, [
        { type: 'text_delta', delta: 'fixture narrative' },
        { type: 'done', reason: 'stop' },
      ]);
    });
    ...
    second.input.controls.tone = 'vivid';
    second.input.source = { revision: 'source-8', hash: 'different' };
    const translated = request('translation');
    translated.stable.tools = [];
    const evaluated = request();
    evaluated.bootstrap = [
      {
        callId: 'bootstrap-context',
        name: 'eval_get_context',
        args: {},
        result: { session: 'synthetic' },
        denied: false,
      },
    ];
    evaluated.toolChoice = 'knowledge.search';
    ...
    expect(wire[2].role).toBe('translation');
    expect(wire[2].modelId).toBe('fixture-translation');
    for (let index = 0; index < wire.length; index++) {
      expect(wire[index].body).toEqual(JSON.parse(server.requests[index].body));
      expect(wire[index].bodySha256).toBe(
        createHash('sha256').update(server.requests[index].body).digest('hex')
      );
    }
    expect(server.requests[2].body).not.toContain('main role contract');
    expect(JSON.parse(server.requests[3].body)).toMatchObject({
      bootstrap: [{ callId: 'bootstrap-context', name: 'eval_get_context' }],
      toolChoice: 'knowledge.search',
    });
    expect(server.requests[0].body.indexOf('stable')).toBeLessThan(
      server.requests[0].body.indexOf('controls')
    );
  ...
  test('P04 validates catalog data separately, keeps unknown metadata and preserves manual models on refresh failure', () => {
    ...
    expect(() =>
      parseCatalog({ models: [{ id: 'a', label: 'a', pricing: { inputUsdPerMillion: -1 } }] })
  ...
  test('P12 P04 refuses unapproved origins, redirects and connection options; credentials stay server-side and redacted', async () => {
    ...
    const server = await fixture(async (req, response) => {
      ...
      await writeSse(response, [
        { type: 'text_delta', delta: 'synthetic' },
        { type: 'done', reason: 'stop' },
      ]);
    });
    ...
    expect(redirect.error?.code).toBe('TRANSPORT_ERROR');
    expect(target.requests).toHaveLength(0);
    for (const endpoint of [
      `http://user:password@127.0.0.1:${new URL(server.origin).port}/turn`,
  ...
  test('P05 P06 keeps refusal, trailing usage, partial output and remote errors distinct without retry', async () => {
    const cases: { events: Json[]; status: string; code?: string }[] = [
      {
        events: [
          { type: 'text_delta', delta: 'do not commit this text' },
          { type: 'refusal', message: 'Fixture explicit refusal' },
          {
            type: 'usage',
            inputTokens: 0,
            outputTokens: 2,
      ...
      {
        events: [{ type: 'text_delta', delta: 'unfinished' }],
        status: 'partial',
        code: 'UNEXPECTED_EOF',
      },
      {
        events: [
          ...
          { type: 'error', message: 'raw provider secret must not leak' },
        ],
        status: 'partial',
        code: 'PROVIDER_ERROR',
      },
      {
        events: [{ type: 'error', message: 'raw provider secret must not leak' }],
        status: 'error',
        code: 'PROVIDER_ERROR',
      },
    ...
    let index = 0;
    const server = await fixture(async (_req, response) => {
      await writeSse(response, cases[index++].events);
    });
    for (const entry of cases) {
      ...
      if (entry.code) expect(output.error?.code).toBe(entry.code);
        ...
        expect(output).toMatchObject({
          refusal: 'Fixture explicit refusal',
          usage: { inputTokens: 0, outputTokens: 2, costUsd: 0, priceRevision: 'fixture-zero-v1' },
        });
  ...
  test('P05 rejects malformed UTF8/JSON, changed or duplicate tool IDs and incomplete terminal results', async () => {
    const cases: { bytes?: Buffer; events?: Json[]; code: string }[] = [
      ...
      { bytes: Buffer.from('data: {bad}\n\n'), code: 'INVALID_EVENT' },
      {
        events: [
          ...
          { type: 'tool_delta', index: 0, id: 'changed' },
        ],
        code: 'TOOL_ID_CHANGED',
      },
      {
        events: [
          {
    ...
    for (const entry of cases) {
      ...
      expect(output.status).not.toBe('tool_calls');
      expect(output.error?.code).toBe(entry.code);
    }
    expect(server.requests).toHaveLength(cases.length);
  });
  ...
  test('P05 abort and timeout stop a real stream, retain observed text and never start another request', async () => {
    ...
    const server = await fixture((_req, response) => {
      response.writeHead(200, { 'content-type': 'text/event-stream' });
      response.write(sse({ type: 'text_delta', delta: 'observed prefix' }));
      release?.();
    });
    ...
    const pending = executeProvider(
      connection(server.endpoint),
      request(),
      options(server.origin, { signal: abort.signal })
    );
    await received;
    ...
    const timed = await executeProvider(
      connection(server.endpoint),
      request(),
      options(server.origin, { timeoutMs: 80 })
    );
    ...
    expect(
      (
        await executeProvider(
          connection(server.endpoint),
          request(),
          options(server.origin, { signal: before.signal })
        )
      ).status
    ).toBe('cancelled');
    expect(server.requests).toHaveLength(2);
...
test('custom main prompt remains literal across tools after the caller changes its profile', async () => {
  ...
  const server = await fixture(async (captured, response) => {
    ...
    if (!body.input.results.length)
      ...
      await writeSse(response, [
        { type: 'text_delta', delta: 'Custom scene received.' },
        { type: 'done', reason: 'stop' },
      ]);
  });
