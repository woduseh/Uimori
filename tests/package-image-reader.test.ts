...
test('Reader resolves only the exact approved inline revision and bytes in this chat', () => {
  ...
  expect(
    resolveInlineImage([asset], 'chat', { ...annotation, assetHash: 'old-bytes' })
  ).toBeUndefined();
  expect(
...
vi.mock('../web/PackagePresentation.js', async (importOriginal) => ({
  ...(await importOriginal<typeof import('../web/PackagePresentation.js')>()),
  usePackagePresentation: () => (projection.data ? { data: projection.data } : undefined),
}));
beforeEach(() => {
...
test('transformed prose suppresses unplaceable images and identifies an authored opening as source content', () => {
  const source: Source = {
    ...
    parentRevision: null,
    text: 'First.\n\nSecond.',
    hash: createHash('sha256').update('First.\n\nSecond.').digest('hex'),
  };
  ...
  const props = {
    ...
    onError: () => {},
    onFork: async () => {},
    request: 'INTERNAL_START_COMMAND',
    packageStart: { mode: 'authored' as const, title: '작성된 첫 장면' },
  };
  ...
  projection.data = {
    sourceRevision: source.id,
    sourceHash: source.hash,
    original: { text: 'A merged display.', changed: true, applied: ['merge'] },
    stateViews: [],
    issues: [],
  };
  const transformed = renderToStaticMarkup(createElement(SourceReader, props));
  expect(transformed).toContain('A merged display.');
  expect(transformed).toContain('이미지를 생략');
  expect(transformed).not.toContain('/api/assets/asset');
  expect(source.text).toBe('First.\n\nSecond.');
  projection.data = undefined;
  const newer = { ...job, id: 'new-image', revision: 2, status: 'queued', result: null } as Job;
  expect(
    renderToStaticMarkup(createElement(SourceReader, { ...props, jobs: [job, newer] }))
  ).not.toContain('/api/assets/asset');
  const translation = {
    ...job,
    id: 'translation',
    kind: 'translation',
  ...
  projection.data = {
    sourceRevision: source.id,
    sourceHash: source.hash,
    original: { text: source.text, changed: false, applied: [] },
    translation: { text: '표시용 번역.', changed: true, applied: ['merge'] },
    stateViews: [],
    issues: [],
  };
