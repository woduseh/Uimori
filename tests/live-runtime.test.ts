...
afterEach(async () => {
  for (const item of owned.splice(0).reverse()) {
    ...
    if (
      isAbsolute(within) ||
      within.startsWith('..') ||
      !basename(target).startsWith('uimori live runtime fixture ')
    )
...
for (const observed of [
  ...
  test(`P05 P06 cancellation after observed HTTP usage keeps terminal state and settles usage once (${observed.costUsd === null ? 'unknown cost' : 'known cost'})`, async () => {
    const item = {
      directory: await mkdtemp(join(tmpdir(), 'uimori live runtime fixture ')),
    ...
    const provider = await loopbackProvider(async (_request, response) =>
      writeSse(response, [
        {
          type: 'text_delta',
          delta: 'Synthetic text received before cancellation; never commit it as source.',
        },
        {
          type: 'usage',
          ...observed,
          raw: { synthetic: true },
          priceRevision: observed.costUsd === null ? null : 'fixture-price-v1',
        },
    ...
    const model = app.store.product.model({
      ...
      modelId: 'fixture-cancelled-usage',
      maxOutputTokens: 100,
      temperature: null,
    }) as ModelPreset;
    ...
    let recorded!: (value: { id: string; result: ProviderResult }) => void;
    const persisted = new Promise<{ id: string; result: ProviderResult }>((resolve) => {
      recorded = resolve;
    });
    ...
    vi.spyOn(app.store.product, 'finishAttempt').mockImplementation(async (id, result) => {
      finishAttempt(id, result);
      recorded({ id, result });
      await gate;
    });
    const run = await api<Run>(url, `/api/chats/${chat.id}/runs`, {
      request: 'Synthetic short scene',
      expectedRevision: chat.headRevision,
      expectedSettingsRevision: chat.settingsRevision,
      expectedProfileRevision: profile.revision,
      idempotencyKey: randomUUID(),
    });
    const attempt = await persisted;
    expect(provider.requests).toHaveLength(1);
    expect(app.store.product.attempts(chat.id)[0]).toMatchObject({
      status: 'completed',
      ...observed,
    });
    const cancelled = await api<Run>(url, `/api/runs/${run.id}/cancel`, {});
    expect(cancelled).toMatchObject({ status: 'cancelled', sourceRevision: null });
    release();
    const expected: Usage = { modelCalls: 1, ...observed };
    await expect
      .poll(async () => (await api<Run>(url, `/api/runs/${run.id}`)).usage)
      .toEqual(expected);
    const preserved = await api<ChatDetail>(url, `/api/chats/${chat.id}`);
    expect(preserved.sources).toHaveLength(0);
    expect(preserved.jobs).toHaveLength(0);
    expect(preserved.runs[0]).toMatchObject({
      status: 'cancelled',
      sourceRevision: null,
      usage: expected,
    });
    expect(preserved.chat.headRevision).toBeNull();
    await api(url, `/api/runs/${run.id}/cancel`, {});
    app.store.finishRun(run.id, 'cancelled', 'duplicate terminal write', '', {
      modelCalls: 999,
      inputTokens: 999,
      outputTokens: 999,
      costUsd: 999,
    });
    app.store.settleCancelledUsage(run.id, {
      modelCalls: 999,
      inputTokens: 999,
      outputTokens: 999,
      costUsd: 999,
    ...
    finishAttempt(attempt.id, {
      ...attempt.result,
      usage: { ...attempt.result.usage, inputTokens: 999, outputTokens: 999, costUsd: 999 },
    });
    expect((await api<Run>(url, `/api/runs/${run.id}`)).usage).toEqual(expected);
    expect(app.store.product.attempts(chat.id)[0]).toMatchObject(observed);
    expect(provider.requests).toHaveLength(1);
  });
