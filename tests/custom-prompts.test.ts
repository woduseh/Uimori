...
const preset = (role: PromptPreset['role'], text = literal): PromptPreset => ({
  id: 'prompt-' + role,
  revision: 3,
  title: 'Custom ' + role,
  role,
...
describe('full editable prompt boundaries', () => {
  test('defaults stay compatible, exact empty and whitespace prompts replace defaults, and snapshots remain independent', () => {
    ...
    expect(
      compileSnapshotPrompt(legacySnapshot).promptCompilation!.messages[0].content[0].text
    ).toBe(DEFAULT_MAIN_PROMPT);
  });
...
function request(
  ...
  contract: string,
  protocol: ProviderProtocol = 'vertex-gemini-v1'
): ProviderRequest {
  const modelId =
  ...
  return {
    ...
    input: {
      task: 'Return the requested output.',
      controls:
        role === 'translation'
          ? { customPrompt: true, instructionRevision: 'prompt:translation@3' }
          : {},
      source: {
    ...
    return [
      {
        ...
        response: { id: 'response-custom', status: 'completed', output: [message] },
      },
    ];
  }
...
function systemText(protocol: ProviderProtocol, body: Record<string, any>): string[] {
  if (protocol === 'vertex-gemini-v1')
    return body.systemInstruction.parts.map((part: { text: string }) => part.text);
  if (protocol === 'anthropic-messages-v1')
    return body.system.map((part: { text: string }) => part.text);
  ...
  test.each(variants)(
    ...
    async (variant) => {
      ...
      const local = await loopbackProvider(async (_request, response) => {
        ...
        response.end(
          events(variant.protocol, text)
            .map(
              (event) => 'data: ' + (event === '[DONE]' ? event : JSON.stringify(event)) + '\n\n'
            )
            .join('')
        );
      });
      ...
      vi.stubGlobal(
        ...
        vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
          if (!String(input).startsWith(variant.endpoint + '/'))
      ...
      for (const [role, contract] of [
        ...
        const captured = local.requests.at(-1)!;
        const body = JSON.parse(captured.body);
        const instructions = systemText(variant.protocol, body);
        if (contract) expect(instructions[0].startsWith(contract)).toBe(true);
        const combined = instructions.join('\n');
        expect(combined).not.toContain(DEFAULT_MAIN_PROMPT);
