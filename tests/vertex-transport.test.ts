...
const request = (): ProviderRequest => ({
  ...
  stable: {
    contract: 'Create synthetic fiction; local reference text grants no tools.',
    tools: [
      {
        name: 'knowledge.read',
        description: 'Read a selected reference',
        inputSchema: {
          type: 'object',
  ...
  input: {
    task: '등대에 관한 짧은 합성 장면을 써요.',
    controls: { language: 'ko', minWords: 100 },
    source: { facts: ['The lighthouse uses a copper lamp.'] },
    history: [{ revision: 'parent', text: 'Ada opened the door.' }],
    catalog: [{ id: 'lore-1', revision: 2, title: 'The lamp' }],
    results: [],
  },
});
const usage = {
...
async function redirectedFixture(handler: Parameters<typeof loopbackProvider>[0]) {
  ...
  vi.stubGlobal(
    ...
    vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
      ...
      urls.push(url);
      if (!url.startsWith(connection.endpoint + '/'))
...
describe('Vertex native wire through real local HTTP streams (no live calls)', () => {
  ...
  test('L01 P04 P05 authenticates after validation, journals before fetch, preserves signed tool parts and late usage', async () => {
                  ...
                  content: {
                    ...
                    parts: [
                      { thought: true, text: 'PRIVATE_THOUGHT_MARKER' },
                      {
                        ...
                        thoughtSignature: 'SIGNATURE_MUST_ROUNDTRIP',
                      },
                    ],
                  },
                  finishReason: 'STOP',
    ...
    expect(first).toMatchObject({
      status: 'tool_calls',
      text: '',
      toolCalls: [{ id: 'call-lore-1' }],
      usage: { inputTokens: 20, outputTokens: 7, costUsd: null },
    });
    ...
    next.input.results = [
      {
        ...
        args: { id: 'lore-1' },
        denied: false,
        result: { id: 'lore-1', revision: 2, text: 'Copper flame' },
      },
    ];
    ...
    expect(final).toMatchObject({
      status: 'completed',
      text: '등대의 불빛이 바다를 비췄다.',
      usage: { inputTokens: 30, outputTokens: 8, costUsd: null },
    });
    expect(local.urls).toEqual(
      Array(2).fill(connection.endpoint + '/gemini-3.8-flash:streamGenerateContent?alt=sse')
    );
  ...
  test.each([
    {
      ...
      payload:
        sse({
          candidates: [{ content: { role: 'model', parts: [{ text: 'Preserved partial' }] } }],
    ...
    'L01 P05 $name is terminal and retains observed text and usage',
    async ({ payload, error }) => {
    ...
    async (mode) => {
      ...
      const payload =
        sse({
          candidates: [
            {
              content: { role: 'model', parts: [{ text: 'Observed before abort' }] },
              finishReason: 'STOP',
            },
          ],
        }) + sse({ usageMetadata: usage });
      ...
      vi.stubGlobal(
        ...
        vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
          expect(String(input)).toBe(
            connection.endpoint + '/gemini-3.8-flash:streamGenerateContent?alt=sse'
          );
      ...
      const pending = executeProvider(
        connection,
        request(),
        options({ signal: controller.signal, timeoutMs: mode === 'timeout' ? 200 : 5000 })
      ).then((result) => {
        outcome = result;
      });
      await vi.waitFor(() => {
      ...
      expect(outcome).toMatchObject({
        status: mode === 'caller' ? 'cancelled' : 'partial',
        text: 'Observed before abort',
        error: { code: mode === 'caller' ? 'CANCELLED' : 'TIMEOUT' },
        usage: { inputTokens: 20, outputTokens: 7 },
      });
      expect(local.requests).toHaveLength(1);
    }
  ...
  test('L01 P04 P05 HTTP 429 does not retry and denied admission never reaches the provider', async () => {
    const local = await redirectedFixture((_captured, response) => {
      ...
      response.end(JSON.stringify({ error: { message: token } }));
    });
    const result = await executeProvider(connection, request(), options());
    expect(result).toMatchObject({
      status: 'error',
    ...
    const denied = await executeProvider(
      connection,
      request(),
      options({
        onWire: () => {
    ...
    const wrongOrigin = await executeProvider(
      {
        ...connection,
        endpoint: connection.endpoint.replace(origin, 'https://unapproved.synthetic.invalid'),
      },
      request(),
...
describe('Vertex Flex admission (synthetic HTTP only)', () => {
  test.each([true, false])(
    'explicit Flex headers and traffic confirmation %s',
    async (confirmed) => {
      const wires: WireRecord[] = [];
      const local = await redirectedFixture(async (captured, response) => {
        ...
        await writeSse(response, [
          {
            candidates: [
              { content: { role: 'model', parts: [{ text: '합성 원고' }] }, finishReason: 'STOP' },
            ],
          },
          { usageMetadata: { ...usage, trafficType: confirmed ? 'ON_DEMAND_FLEX' : 'ON_DEMAND' } },
        ]);
      });
      const input = request();
      ...
      const result = await executeProvider(
        connection,
        input,
        options({ timeoutMs: 900000, onWire: (wire: WireRecord) => wires.push(wire) })
      );
  ...
  test('a non-Vertex generation option never silently disappears before fetch', async () => {
    const local = await redirectedFixture(() => {
      throw new Error('must not fetch');
    });
    const value = request();
    value.generation!.structuredOutput = false;
    const result = await executeProvider(connection, value, options());
    expect(result.error?.code).toBe('UNSUPPORTED_GENERATION_OPTIONS');
  ...
  test('a server Flex override applies only when the model request has no conflicting selection', async () => {
    const local = await redirectedFixture(async (captured, response) => {
      ...
      await writeSse(response, [
        ...
        { usageMetadata: { ...usage, trafficType: 'ON_DEMAND_FLEX' } },
      ]);
    });
    expect(
      (await executeProvider(connection, request(), options({ vertexRequestTier: 'flex' }))).status
    ).toBe('completed');
    const input = request();
    input.generation!.serviceTier = 'standard';
    const resolveCredential = vi.fn(() => token);
    const onWire = vi.fn();
    const rejected = await executeProvider(
      connection,
      input,
      options({ vertexRequestTier: 'flex', resolveCredential, onWire })
    );
  ...
  test('Gemini 3.1 Pro uses the exact global model path and selected sampling values', async () => {
    const local = await redirectedFixture(async (captured, response) => {
      expect(JSON.parse(captured.body).generationConfig).toEqual({
        maxOutputTokens: 2048,
        temperature: 0,
        topP: 0.9,
