...
const request: ProviderRequest = { role: 'main', modelId: 'gemini-3.8-flash', stable: { contract: 'Synthetic only', tools: [] }, input: { task: 'A synthetic lamp.', controls: {} } };
const connection = { id: 'adc-fixture', protocol: 'vertex-gemini-v1' as const, endpoint: 'https://aiplatform.googleapis.com/v1/projects/synthetic-project/locations/global/publishers/google/models' };
...
test('L01 P04 missing ADC and SDK auth errors cannot disclose credentials or start a model request', async () => {
  ...
  await expect(vertexAccessToken(new AbortController().signal)).rejects.toMatchObject({ code: 'CREDENTIAL_UNAVAILABLE' });
  ...
  const generation = vi.fn(() => { throw new Error('Unexpected model request'); }); vi.stubGlobal('fetch', generation);
  const onWire = vi.fn();
  const result = await executeProvider(connection, request, { approvedOrigins: ['https://aiplatform.googleapis.com'], signal: new AbortController().signal, onWire });
  ...
  expect(generation).not.toHaveBeenCalled(); expect(onWire).not.toHaveBeenCalled();
});
...
test('L01 P05 abort while obtaining ADC prevents late token from starting generation', async () => {
  ...
  const generation = vi.fn(() => { throw new Error('Unexpected model request'); }); vi.stubGlobal('fetch', generation);
  const controller = new AbortController(); const onWire = vi.fn();
  const pending = executeProvider(connection, request, { approvedOrigins: ['https://aiplatform.googleapis.com'], signal: controller.signal, onWire });
  ...
  finish('SYNTHETIC_LATE_TOKEN'); await token; await Promise.resolve();
  expect(generation).not.toHaveBeenCalled(); expect(onWire).not.toHaveBeenCalled();
  expect(JSON.stringify(result)).not.toContain('SYNTHETIC_');
});
