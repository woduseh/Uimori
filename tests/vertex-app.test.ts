...
const usage = { promptTokenCount: 10, candidatesTokenCount: 5, thoughtsTokenCount: 2, totalTokenCount: 17, cachedContentTokenCount: 0, toolUsePromptTokenCount: 0 };
...
afterEach(async () => {
  for (const item of owned.splice(0).reverse()) {
    ...
    if (isAbsolute(within) || within.startsWith('..') || !basename(target).startsWith('uimori vertex app fixture ')) throw new Error('Refusing cleanup outside owned fixture directory');
...
async function fixture(handler: Parameters<typeof loopbackProvider>[0]) {
  ...
  const nativeFetch = globalThis.fetch; const urls: string[] = [];
...
const read = (id: string, contentId: string): Json[] => [
  { candidates: [{ index: 0, content: { role: 'model', parts: [{ thought: true, text: 'PRIVATE_VERTEX_APP_THOUGHT' }, { functionCall: { id, name: 'knowledge.read', args: { id: contentId } }, thoughtSignature: 'PRIVATE_VERTEX_APP_SIGNATURE' }] }, finishReason: 'STOP' }] },
  { usageMetadata: usage },
];
async function setup(app: App, translation = true) {
  let chat = await api<Chat>(app, '/api/chats', { title: 'Synthetic Vertex app story' });
  chat = await api<Chat>(app, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: chat.settingsRevision, ...chat.settings, translation, status: false, maxCalls: 12 }, 'PATCH');
  const contents: Content[] = [];
  for (const [kind, text, loading] of [
    ['bot', 'ORIGINAL_BOT: Ada keeps the lighthouse.', 'pinned'],
...
function command(chat: Chat, profile: ChatProfile) {
  return { request: 'Continue the synthetic lighthouse scene.', expectedRevision: chat.headRevision, expectedSettingsRevision: chat.settingsRevision, expectedProfileRevision: profile.revision, idempotencyKey: randomUUID() };
}
const runDone = async (app: App, runId: string) => {
  await expect.poll(async () => (await api<Run>(app, `/api/runs/${runId}`)).status, { timeout: 4000 }).toBe('completed');
...
test('L01 P05 P07 P08 P09 preserves source-time Main/Aux snapshots, long chunks and sibling ownership across duplicate commands and restart', async () => {
  let firstReceived!: () => void; const received = new Promise<void>(resolve => { firstReceived = resolve; });
  let release!: () => void; const gate = new Promise<void>(resolve => { release = resolve; });
  let mainRequests = 0; let completedMain = 0;
  const paragraph = 'The keeper watched the tide turn beneath the copper lamp and listened to the wind. '.repeat(31);
  const sourceTexts = ['Alpha', 'Beta'].map(name => [name + ' beginning. ' + paragraph, name + ' middle. ' + paragraph, name + ' ending. ' + paragraph].join('\n\n'));
  const state = await fixture(async (captured, response) => {
    ...
    if (!packet.source.chunkId) {
      mainRequests++;
      expect(body.generationConfig).toMatchObject({ maxOutputTokens: 8192, thinkingConfig: { thinkingLevel: 'MEDIUM' } });
      if (body.contents.length === 1) {
        if (mainRequests === 1) { firstReceived(); await gate; }
        await writeSse(response, read('main-read-lore', selected.contents.find(item => item.kind === 'lore')!.id));
      } else {
        expect(last.parts[0].functionResponse).toMatchObject({ id: 'main-read-lore', name: 'knowledge.read', response: { text: 'ORIGINAL_LORE: The observatory lies north.' } });
      ...
      expect(body.generationConfig).toMatchObject({ maxOutputTokens: 4096, thinkingConfig: { thinkingLevel: 'LOW' } });
      expect(packet.source.context!.bot?.text).toContain('ORIGINAL_BOT');
      expect(packet.source.context!.glossary[0].text).toContain('ORIGINAL_GLOSSARY');
      if (body.contents.length === 1) await writeSse(response, read(`aux-${packet.source.chunkId}`, selected.contents.find(item => item.kind === 'glossary')!.id));
      else {
        expect(last.parts[0].functionResponse?.response.text).toContain('ORIGINAL_GLOSSARY');
        expect(body.contents[1].parts[1].thoughtSignature).toBe('PRIVATE_VERTEX_APP_SIGNATURE');
        await writeSse(response, complete(JSON.stringify({ sourceRevision: packet.source.sourceRevision, sourceHash: packet.source.sourceHash, chunkId: packet.source.chunkId, segments: packet.source.blocks!.map(block => ({ anchors: [block.anchor], text: `합성 번역 ${block.text}` })) })));
      }
    }
  });
  state.item.release = release;
  const { app } = state; const selected = await setup(app);
  ...
  release(); const first = await runDone(app, originalRun.id);
  expect(first.snapshot).toEqual(originalSnapshot); expect(first.usage).toEqual({ modelCalls: 2, inputTokens: 20, outputTokens: 14, costUsd: null });
  ...
  expect((await api<Run>(app, `/api/runs/${first.id}/candidate`, candidateInput)).id).toBe(second.id);
  ...
  expect(secondFrozen).toEqual(firstFrozen); expect(candidateOf).toBe(first.id); expect(second.parentRevision).toBe(first.parentRevision);
  ...
  await expect.poll(async () => (await api<ChatDetail>(app, `/api/chats/${selected.chat.id}`)).jobs.map(job => job.status), { timeout: 5000 }).toEqual(['completed', 'completed']);
  ...
  expect(detail.branches!.find(branch => branch.id === secondBranch)?.headRevision).toBe(second.sourceRevision);
  expect(detail.sources).toHaveLength(2); expect(detail.jobs).toHaveLength(2);
  for (const [index, run] of [first, second].entries()) {
    const source = detail.sources.find(item => item.id === run.sourceRevision)!;
    expect(source).toMatchObject({ text: sourceTexts[index], hash: createHash('sha256').update(sourceTexts[index]).digest('hex'), runId: run.id, parentRevision: run.parentRevision });
    const job = detail.jobs.find(item => item.sourceRevision === source.id)!;
    expect(job).toMatchObject({ chatId: selected.chat.id, sourceHash: source.hash, status: 'completed', result: { mock: false, sourceRevision: source.id, sourceHash: source.hash } });
    expect(job.chunks).toHaveLength(3); expect(job.chunks!.every(chunk => chunk.status === 'completed' && chunk.attempt === 1)).toBe(true);
    expect(job.result!.segments!.flatMap(segment => segment.anchors)).toEqual(source.blocks!.map(block => block.anchor));
    const packets = state.provider.requests.map(request => decoded(request.body).packet).filter(packet => packet.source.sourceRevision === source.id);
    expect(packets).toHaveLength(6); expect(packets.every(packet => packet.source.sourceHash === source.hash)).toBe(true);
    expect(new Set(packets.map(packet => packet.source.chunkId)).size).toBe(3);
  }
  expect(state.provider.requests).toHaveLength(16); expect(state.urls).toEqual(Array(16).fill(providerUrl));
  expect(JSON.stringify(state.provider.requests)).not.toContain('FUTURE_MUTATION');
  expect(detail.attempts).toHaveLength(16);
  expect(JSON.stringify(detail.attempts)).not.toMatch(/PRIVATE_VERTEX_APP_THOUGHT|PRIVATE_VERTEX_APP_SIGNATURE|synthetic-vertex-app-bearer/u);
  expect(app.store.db.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
  await app.close(); state.item.app = undefined;
  const reopened = await launch(state.item); const restored = await api<ChatDetail>(reopened, `/api/chats/${selected.chat.id}`);
  expect(restored.sources).toEqual(detail.sources); expect(restored.jobs).toEqual(detail.jobs); expect(restored.runs).toEqual(detail.runs); expect(restored.attempts).toEqual(detail.attempts);
  expect(reopened.store.queuedJobs()).toEqual([]); expect(state.provider.requests).toHaveLength(16);
  expect((await api<Run>(reopened, `/api/chats/${selected.chat.id}/runs`, input)).id).toBe(first.id);
  expect((await api<Run>(reopened, `/api/runs/${first.id}/candidate`, candidateInput)).id).toBe(second.id);
  expect(state.provider.requests).toHaveLength(16);
});

test.each(['main', 'translation'] as const)('L01 P06 P11 restarting an in-flight Vertex %s stream preserves old records and never replays it', async stalledRole => {
  let stalled!: () => void; const received = new Promise<void>(resolve => { stalled = resolve; });
  const state = await fixture(async (captured, response) => {
    const { packet } = decoded(captured.body); const role = packet.source.chunkId ? 'translation' : 'main';
    if (role !== stalledRole) { await writeSse(response, complete('A synthetic keeper waits beside the lamp.')); return; }
    response.writeHead(200, { 'content-type': 'text/event-stream' });
    response.write(sse({ candidates: [{ content: { role: 'model', parts: [{ text: `Observed ${role} prefix.` }] } }] }));
    response.write(sse({ usageMetadata: usage })); stalled();
  });
  ...
  if (stalledRole === 'translation') expect(before.jobs[0]).toMatchObject({ status: 'running', sourceRevision: before.sources[0].id, sourceHash: before.sources[0].hash });
  ...
  if (stalledRole === 'main') {
    ...
    expect(after.jobs[0]).toMatchObject({ id: before.jobs[0].id, status: 'interrupted', sourceRevision: before.sources[0].id, sourceHash: before.sources[0].hash, result: null });
    expect(after.jobs[0].chunks!.every(chunk => chunk.status === 'interrupted')).toBe(true);
  }
  expect(reopened.store.queuedJobs()).toEqual([]);
  expect((await api<Run>(reopened, `/api/chats/${selected.chat.id}/runs`, input)).id).toBe(run.id);
  await new Promise<void>(resolve => setImmediate(resolve));
  expect(state.provider.requests).toHaveLength(expectedRequests); expect(state.urls).toEqual(Array(expectedRequests).fill(providerUrl));
