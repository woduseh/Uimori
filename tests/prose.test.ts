...
describe('safe prose rendering', () => {
  ...
  test('keeps fenced code, protected placeholders, template syntax and machine identifiers literal', () => {
    ...
    expect(html).toContain('<pre><code>{&quot;assetRef&quot;:&quot;harbor_01&quot;,&quot;mode&quot;:&quot;READY&quot;}\n\n&lt;script&gt;literal&lt;/script&gt;</code></pre>');
  ...
  test('rejects unsafe and ambiguous URL schemes without accepting control-character obfuscation', () => {
    for (const value of ['javascript:alert(1)', 'JaVaScRiPt:alert(1)', 'java\nscript:alert(1)', 'data:text/html,hi', '//example.com', '/\\example.com', 'https:\\example.com']) expect(safeProseLink(value)).toBeUndefined();
    for (const value of ['https://example.com', 'http://example.com/story', 'mailto:writer@example.com', '/help', '#scene']) expect(safeProseLink(value)).toBe(value);
  });

  test('renders a long source without interpreting unsupported syntax as HTML', () => {
    const html = render(Array.from({ length: 300 }, (_, index) => `문단 ${index}: **기억**과 바람을 따라 걷는다. <x-state mode="READY">asset:harbor_01</x-state>`).join('\n\n'));
...
test('reader defaults to Korean, preserves source identity and many-to-one translation anchors without commands', () => {
  const text = '# 실제 제목\n\n첫 번째 문단.\n\n두 번째 문단.';
  const source: Source = { editRevision: 0, id: 'source-test', runId: 'run-test', chatId: 'chat-test', parentRevision: null, text, hash: createHash('sha256').update(text).digest('hex') };
  source.blocks = splitSource(source);
  const original = structuredClone(source);
  const job: Job = { id: 'translation-test', chatId: source.chatId, sourceRevision: source.id, sourceHash: source.hash, kind: 'translation', status: 'completed', attempt: 1, error: null, result: { mock: true, sourceRevision: source.id, sourceHash: source.hash, segments: [{ anchors: source.blocks.map(block => block.anchor), text: '## 한국어 번역\n\n합쳐진 문단.' }] } };
  let commands = 0;
  const html = renderToStaticMarkup(createElement(SourceReader, { source, index: 0, jobs: [job], assets: [], request: '요청 원문', refresh: async () => { commands++; }, onError: () => {}, onFork: async () => { commands++; } }));
  expect(html).toContain('data-testid="translation-text"');
  expect(html).toContain(`data-block-anchor="${source.blocks.map(block => block.anchor).join(' ')}"`);
...
test('reader shows only the latest matching translation and never a version selector', () => {
  const source: Source = { id: 'latest-source', runId: 'run', chatId: 'chat', parentRevision: null, text: 'Original text', hash: 'current', editRevision: 0 };
  const job = (id: string, revision: number, text: string, hash = source.hash): Job => ({ id, revision, chatId: source.chatId, sourceRevision: source.id, sourceHash: hash, kind: 'translation', status: 'completed', attempt: 1, error: null, result: { mock: false, manual: true, sourceRevision: source.id, sourceHash: hash, text } });
  const html = renderToStaticMarkup(createElement(SourceReader, { source, index: 0, jobs: [job('old', 1, 'OLD_TRANSLATION'), job('new', 2, 'LATEST_TRANSLATION'), job('stale', 3, 'STALE_TRANSLATION', 'old-hash')], assets: [], refresh: async () => {}, onError: () => {}, onFork: async () => {} }));
...
test('reader without a current translation renders the original even when translation is preferred', () => {
  const source: Source = { id: 'missing-source', runId: 'run', chatId: 'chat', parentRevision: null, text: 'UNTRANSLATED_ORIGINAL', hash: 'current', editRevision: 0 };
  const html = renderToStaticMarkup(createElement(SourceReader, { source, index: 0, jobs: [], assets: [], refresh: async () => { throw new Error('must not request on render'); }, onError: () => {}, onFork: async () => {} }));
