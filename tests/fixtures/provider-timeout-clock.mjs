...
const init = { method: 'POST', body: 'synthetic request', headers: { authorization: `Bearer ${secret}` }, signal: controller.signal, redirect: 'error' };
...
const server = createServer(async (request, response) => {
  for await (const _chunk of request) { /* Fully consume this synthetic request. */ }
...
const io = async predicate => {
  for (let turn = 0; turn < 2000 && !predicate(); turn++) { mock.timers.tick(1); await nextTurn(); }
  assert.ok(predicate(), 'Local network did not reach the requested test state');
};
const advance = async milliseconds => { for (let elapsed = 0; elapsed < milliseconds; elapsed += 500) { mock.timers.tick(500); await nextTurn(); } };
let requests = [];
try {
  ...
  globalThis.setTimeout = (...args) => { const timer = mockedSetTimeout(...args); timer.refresh = undefined; return timer; };
  const start = (name, options) => fetch(`${origin}/${name}`, options)
    .then(response => { headersSeen.add(name); return response.text(); })
    .then(text => outcomes.set(name, { status: 'completed', text }), error => outcomes.set(name, { status: 'error', code: transportFailureCode(error) }));
  ...
  assert.equal(outcomes.size, 0, 'Neither request should reach its header/body deadline before 300 seconds');
  ...
  assert.equal(outcomes.has('fixed'), false, 'The app deadline remains in charge after the idle-timeout boundary');
  ...
  process.stdout.write(JSON.stringify({ phase, beforeSeconds: 295, afterSeconds: 315, baselineCode: code, fixedStatus: 'completed', requests: 2, requestRetries: 0, globalDispatcherUnchanged: true, secretLeaked: false }));
} finally {
  controller.abort();
