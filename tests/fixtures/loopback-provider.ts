...
export async function loopbackProvider(handler: (request: CapturedRequest, response: ServerResponse) => void | Promise<void>) {
  const requests: CapturedRequest[] = [];
  const server = createServer(async (request, response) => {
    try {
      const chunks: Buffer[] = [];
      for await (const chunk of request) chunks.push(Buffer.from(chunk));
      const captured = { url: request.url ?? '', headers: request.headers, body: Buffer.concat(chunks).toString('utf8') };
  ...
  return { origin, endpoint: `${origin}/turn`, requests,
...
export async function writeSse(response: ServerResponse, events: Json[], fragments = false) {
  ...
  if (fragments) for (let offset = 0; offset < bytes.length; offset += 3) {
