...
function client(mode = 'normal') {
  const value = new CodexProcess({ command: process.execPath, args: [fixture], cwd: process.cwd(), env: { ...process.env, UIMORI_CODEX_FIXTURE_MODE: mode }, timeoutMs: 1000 });
  active.push(value); return value;
}
afterEach(async () => { await Promise.all(active.splice(0).map(value => value.close())); });
describe('Codex app-server stdio process', () => {
  it('handshakes and correlates concurrent responses in different order', async () => {
    const value = client(); await value.start();
  ...
  it('rejects server approvals without granting privileges', async () => {
    const value = client(); await value.start();
    const unsupported = new Promise<unknown>(resolve => value.onNotification((method, params) => { if (method === 'uimori/unsupportedRequest') resolve(params); }));
    const response = new Promise<unknown>(resolve => value.onNotification((method, params) => { if (method === 'fixture/clientResponse') resolve(params); }));
  ...
  it('cancels and times out locally, ignores late results, and keeps the session usable', async () => {
    ...
    await expect(value.request('fixture/delay', {}, { timeoutMs: 5 })).rejects.toMatchObject({ code: 'CODEX_TIMEOUT' });
    await new Promise(resolve => setTimeout(resolve, 100));
    expect(await value.request('fixture/echo', { alive: true })).toEqual({ alive: true });
  });
  it.each(['fixture/malformed', 'fixture/oversized', 'fixture/invalidUtf8'])('terminates and rejects all pending work on %s', async method => {
    const value = client(); await value.start();
    const pending = expect(value.request('fixture/hang', {})).rejects.toMatchObject({ code: 'CODEX_PROTOCOL_ERROR' });
    await expect(value.request(method, {})).rejects.toMatchObject({ code: 'CODEX_PROTOCOL_ERROR' });
    await pending;
  ...
  it('sanitizes provider errors and process exit diagnostics', async () => {
    ...
    let exits = 0; value.onExit(() => exits++);
  ...
  it('cleans pending requests on close without a restart', async () => {
    ...
    await value.close(); await pending; await value.close();
  ...
  it('reports a missing executable without leaking process diagnostics', async () => {
    const value = new CodexProcess({ command: 'uimori-nonexistent-codex-test', cwd: process.cwd(), env: process.env });
    active.push(value);
