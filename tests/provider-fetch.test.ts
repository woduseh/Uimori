...
const variants: ProviderConnection[] = [
  {
    ...
    credentialEnv: 'NARRATIVE_PROVIDER_TIMEOUT_TEST',
  },
  {
    id: 'chat-timeout',
...
const request = (connection: ProviderConnection): ProviderRequest => ({
  role: 'main',
  modelId: connection.protocol === 'vertex-gemini-v1' ? 'gemini-3.8-flash' : 'synthetic-model',
  stable: { contract: 'Write synthetic fiction.', tools: [] },
...
const partial = (connection: ProviderConnection) =>
  connection.protocol === 'vertex-gemini-v1'
    ? sse({
        candidates: [
          { content: { role: 'model', parts: [{ text: 'Observed before the deadline.' }] } },
        ],
      }) +
      sse({ usageMetadata: { promptTokenCount: 5, candidatesTokenCount: 2, totalTokenCount: 7 } })
    : sse({
        id: 'chat-timeout',
        choices: [
          {
            index: 0,
            delta: { role: 'assistant', content: 'Observed before the deadline.' },
            finish_reason: null,
          },
        ],
        usage: null,
      }) +
      sse({ id: 'chat-timeout', choices: [], usage: { prompt_tokens: 5, completion_tokens: 2 } });

describe('provider-specific HTTP dispatcher and safe failure codes', () => {
  test.each(['headers', 'body'])(
    'native Node fetch crosses the actual Undici 300s %s boundary using virtual time',
    async (phase) => {
      const result = await exec(
        process.execPath,
        [fileURLToPath(new URL('./fixtures/provider-timeout-clock.mjs', import.meta.url)), phase],
        { timeout: 8000, maxBuffer: 32_768 }
      );
      expect(JSON.parse(result.stdout)).toEqual({
        phase,
        beforeSeconds: 295,
        afterSeconds: 315,
        baselineCode: phase === 'headers' ? 'UND_ERR_HEADERS_TIMEOUT' : 'UND_ERR_BODY_TIMEOUT',
  ...
  test.each(
    variants.flatMap((connection) =>
      ['headers', 'body'].flatMap((phase) =>
        ['timeout', 'cancel'].map((mode) => ({ connection, phase, mode }))
      )
    )
  )(
    '$connection.protocol $phase $mode closes the native local request without retry',
    async ({ connection, phase, mode }) => {
      ...
      const local = await loopbackProvider((_captured, response) => {
        ...
        if (phase === 'body') {
          ...
          response.write(partial(connection));
        }
      });
      ...
      const send = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (response.body) {
          ...
          vi.spyOn(reader, 'read').mockImplementation(async () => {
            ...
            if (!value.done) readingBody = true;
            return value;
          });
        }
        return response;
      ...
      await vi.waitFor(() => {
        ...
        if (phase === 'body') expect(readingBody).toBe(true);
      });
      if (mode === 'cancel') controller.abort(new Error(secret));
      const result = await pending;
      expect(result.status).toBe(
        mode === 'cancel' ? 'cancelled' : phase === 'body' ? 'partial' : 'error'
      );
  ...
  test.each(
    variants.flatMap((connection) =>
      ...
      ].map((code) => ({ connection, code }))
    )
  )('$connection.protocol exposes only allowlisted cause $code', async ({ connection, code }) => {
  ...
  test('unknown, cyclic and throwing diagnostic properties never disclose arbitrary error data', () => {
    const cyclic: { code: string; cause?: unknown } = { code: secret };
    cyclic.cause = cyclic;
    for (const error of [
      new Error(secret),
      { code: secret, cause: { message: secret } },
      cyclic,
      {
        get code() {
      ...
      expect(transportFailureCode(error)).toBe('TRANSPORT_ERROR');
    expect(
      transportFailureCode({ cause: { cause: { code: 'UND_ERR_SOCKET', message: secret } } })
    ).toBe('UND_ERR_SOCKET');
  });
});
