...
function assign(s:Store,chatId:string,routes:Record<string,ModelRef|null>){const p=s.product.profile(chatId);return s.product.updateProfile(chatId,{expectedRevision:p.revision,attachments:p.attachments,creative:p.creative,image:p.image,routes:{...p.routes,...routes}});}
...
test('disabled models retain assigned IDs but block new assignments and new snapshots while completed runs stay immutable',()=>{
  ...
  expect(s.source(source.id)).toEqual(original);expect(s.run(run.id).snapshot).toEqual(snapshot);
  expect(s.product.get('model',m.id)).toEqual(disabled);expect(()=>s.product.get('model',m.id,m.revision)).toThrow('Setting not found');
...
test('ID-based selections follow current connections while protocol changes require a model review before execution',()=>{
  for(const change of [{title:'Renamed'},{endpoint:'http://127.0.0.1:9998/v1'},{credentialEnv:'NARRATIVE_PROVIDER_DIFFERENT'},{protocol:'fixture-sse-v1',endpoint:'http://127.0.0.1:9999/v1'}]){
    const {s,chat,c,m}=setup();assign(s,chat.id,{main:ref(m)});
    const latest=s.product.connection(connectionBody({expectedRevision:c.revision,...change}),c.id) as Connection;
    expect(assign(s,chat.id,{translation:ref(m)}).routes).toMatchObject({main:ref(m),translation:ref(m)});
    ...
    if('protocol' in change){
      ...
      const reviewed=s.product.model(modelBody(latest,{expectedRevision:m.revision}),m.id) as ModelPreset;
      expect(reviewed.capabilityProtocol).toBe(latest.protocol);expect(s.product.modelSnapshot(m.id).connection).toEqual(latest);
    }else expect(s.product.modelSnapshot(m.id).connection).toEqual(latest);
  }
  const {s,chat,c,m}=setup();assign(s,chat.id,{main:ref(m)});
  const disabled=s.product.connection(connectionBody({enabled:false,expectedRevision:c.revision}),c.id) as Connection;
  expect(isModelSelectable(m,[m],[disabled])).toBe(false);expect(()=>assign(s,chat.id,{translation:ref(m)})).toThrow('연결');
  expect(assign(s,chat.id,{main:ref(m)}).routes.main).toEqual(ref(m));expect(()=>s.product.snapshot(chat.id)).toThrow('Connection disabled');
  const vertex=s.product.connection(connectionBody({protocol:'vertex-gemini-v1',endpoint:'https://aiplatform.googleapis.com/v1/projects/synthetic-project/locations/global/publishers/google/models'})) as Connection;
  const vm=s.product.model(modelBody(vertex,{modelId:'gemini-3.8-flash'})) as ModelPreset;
  const flex=s.product.model(modelBody(vertex,{modelId:'gemini-3.8-flash',serviceTier:'flex',expectedRevision:vm.revision}),vm.id) as ModelPreset;
  expect(()=>assertModelSelection(s.product,ref(vm))).not.toThrow();expect(isModelSelectable(vm,[flex],[vertex])).toBe(true);
});
...
test('model draft validation uses explicit matching unsaved connection without any DB writes',()=>{
  ...
  expect(s.product.prepareModel(modelBody(draft),undefined,draft).value.source).toEqual({kind:'manual',catalogUpdatedAt:null});
  expect(()=>s.product.prepareModel(modelBody(draft,{connectionRevision:2}),undefined,draft)).toThrow();
  expect(()=>s.product.prepareModel(modelBody(draft,{connectionId:'another'}),undefined,draft)).toThrow('Validation connection mismatch');
  expect(()=>s.product.prepareModel(modelBody(draft,{thinkingMode:'enabled'}),undefined,draft)).toThrow('UNSUPPORTED_GENERATION_OPTIONS');
...
test('model snapshots are detached from current settings and selection checks use the latest connection ID',()=>{
  const {s,c,m}=setup();const snapshot=s.product.modelSnapshot(m.id);
  const replacement=s.product.connection(connectionBody({title:'Replacement connection'})) as Connection;
  const moved=s.product.model(modelBody(replacement,{expectedRevision:m.revision,maxOutputTokens:2048}),m.id) as ModelPreset;
  const disabled=s.product.connection(connectionBody({expectedRevision:c.revision,enabled:false}),c.id) as Connection;
  ...
  snapshot.connection.catalog.push({id:'mutated-copy',name:'Not stored',capabilities:{},priceRevision:null});snapshot.title='Changed copy';
  expect(s.product.get('connection',c.id)).toEqual(disabled);expect(s.product.get('model',m.id)).toEqual(moved);
  for(const [kind,id] of [['model',m.id],['connection',c.id]]) expect(s.db.prepare('SELECT COUNT(*) AS count FROM provider_settings WHERE kind=? AND id=?').get(kind,id)?.count).toBe(1);
  expect(s.db.prepare("SELECT COUNT(*) AS count FROM versions WHERE kind IN ('model','connection')").get()?.count).toBe(0);
});
