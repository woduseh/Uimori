...
describe('M1 durable auxiliary orchestration with actual fixture HTTP', () => {
  ...
  test('P08 failed chunk retries alone over actual HTTP and preserves already completed siblings', async () => {
    ...
    const state = bridge(seed);
    const observed = hooks(server.origin);
    observed.options.maxChunkChars = 100;
    const partial = await runAuxiliaryJob(state.store, seed.job.id, 'server-a', observed.options);
    expect(partial).toMatchObject({
      status: 'partial',
      result: { completedChunks: 2, totalChunks: 3 },
      error: 'CHUNK_COVERAGE_INVALID',
    });
    const before = [...state.chunks.values()]
      .filter((chunk) => chunk.status === 'completed')
      .map((chunk) => structuredClone(chunk));
    expect(before).toHaveLength(2);
    expect(server.requests).toHaveLength(5);
    const afterRetry = await runAuxiliaryJob(
      state.store,
      seed.job.id,
      'server-b',
      observed.options
    ...
    expect(afterRetry).toMatchObject({
      status: 'completed',
      result: { completedChunks: 3, totalChunks: 3 },
      error: null,
    });
    expect(server.requests).toHaveLength(6);
    const requestedChunks = server.requests.map(
      (request) => JSON.parse(request.body).input.source.chunkId as string
    );
    ...
    expect(afterRetry?.result?.segments?.flatMap((segment) => segment.anchors)).toEqual(
      (state.data.plan as TranslationPlan).blocks.map((block) => block.anchor)
    );
  });
    ...
    for (const terminal of ['refused', 'partial'] as const) {
      const server = await fixture(async (_request, response) => {
        const events: Json[] =
          terminal === 'refused'
            ? [
                { type: 'refusal', message: 'Fixture refusal' },
                {
                  type: 'usage',
                  inputTokens: 8,
                  outputTokens: 1,
      ...
      expect(outcome).toMatchObject({
        status: 'failed',
        result: null,
        error: terminal === 'refused' ? 'AUXILIARY_PROVIDER_REFUSED' : 'AUXILIARY_PROVIDER_PARTIAL',
      });
      expect(server.requests).toHaveLength(terminal === 'refused' ? 3 : 1);
      expect(observed.finishes[0].result.status).toBe(terminal);
  ...
  test('P08 explicit chunk retry leaves other failed chunks untouched and still reports partial', async () => {
    ...
    observed.options.maxChunkChars = 100;
    expect(
      (await runAuxiliaryJob(state.store, seed.job.id, 'server-a', observed.options))?.status
    ).toBe('partial');
    const plan = state.data.plan!;
    firstPass = false;
    state.data.retryChunkIds = [plan.chunks[0].id];
    ...
    expect(JSON.parse(server.requests[7].body).input.source.chunkId).toBe(plan.chunks[0].id);
  ...
  test('P07 P13 disabled transport uses labeled local fixture; annotations stay separate and cancellation preserves source', async () => {
    ...
    const outcome = await runAuxiliaryJob(state.store, seed.job.id, 'server-a', observed.options);
    expect(outcome?.status).toBe('completed');
    expect(outcome?.result?.text).toContain('모의 번역');
...
test('custom translation prompt survives tool continuation and failed chunk retry without inheriting later edits', async () => {
  ...
  const server = await fixture(async (captured, response) => {
    ...
    if (requests === 1)
      ...
      await writeSse(response, [
        { type: 'text_delta', delta: JSON.stringify(result) },
        { type: 'done', reason: 'stop' },
      ]);
    }
  });
