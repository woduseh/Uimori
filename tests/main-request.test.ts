      ...
      models: {
        main: {
          ...
          connection: {
            ...
            revision: 1,
            title: 'Synthetic',
            protocol: 'openai-chat-v1' as ProviderProtocol,
            endpoint,
            enabled: true,
            catalog: [],
            catalogError: null,
...
function hooks(origin: string) {
  ...
  return { value, events, attempts, finished };
}
...
const toolOutput = (
  ...
  index: id === 'Call.Read' ? 1 : 0,
  type: 'function',
  id,
  function: {
    name:
      body.tools.find((tool: { function: { name: string } }) =>
        tool.function.name.endsWith(name.replaceAll('.', '_'))
      )?.function.name ?? name,
    arguments: JSON.stringify(typeof content === 'string' ? { content } : content),
  },
});
const toolTurn = (output: Json[]): Json => ({
  id: 'tool-synthetic',
  choices: [
    { index: 0, delta: { role: 'assistant', tool_calls: output }, finish_reason: 'tool_calls' },
  ],
  usage: { prompt_tokens: 10, completion_tokens: 5 },
});
...
describe('Exact native main preview and terminal submission (synthetic loopback only)', () => {
  test('mock style settings never enter a custom prompt request or its host context', () => {
    ...
    expect(JSON.stringify(after)).not.toMatch(
      /"preset"|minWords|maxWords|customWords|personaReference/
    );
  });
  ...
  test('submission uses the compiled explicit decision; source provenance and familiar option names grant no tool', () => {
    ...
    expect(storySubmissionEnabled(enabled)).toBe(true);
    expect(
      buildMainProviderRequest(enabled).request.stable.tools.some(
        (tool) => tool.name === 'story.submit'
      )
    ).toBe(true);
    expect(
      storySubmissionEnabled(
  ...
  test('NMR01 preview route and runner use byte-equivalent encoder bodies, current once, no call during preview', async () => {
    ...
    const response = await injectWithFixtureBot(app, {
      method: 'POST',
      url: `/api/chats/${work.chatId}/prompt-preview`,
      payload: {
        program: work.profile!.promptPresets!.main!.program,
        request: work.request,
        role: 'main',
        values: { writing_mode: 'fiction' },
      },
    });
    ...
    const log = hooks(server.origin),
      result = await runMain(frozen, log.value);
    expect(result.status).toBe('completed');
    expect(log.attempts).toHaveLength(1);
    expect(log.attempts[0].body).toEqual(expected.body);
    expect(JSON.parse(server.requests[0].body)).toEqual(expected.body);
    expect(server.requests[0].body.split(work.request).length - 1).toBe(1);
    expect(
      response
        .json()
        .compilation.messages.some((m: { id: string }) => m.id === 'native.host-context')
    ).toBe(true);
  });
  ...
  test('NMR03 explicitly enabled terminal body completes once with host provenance and no tool-result round', async () => {
    const prose = 'Synthetic submitted fiction.';
    ...
    expect(result).toMatchObject({ status: 'completed', text: prose, error: null });
    ...
    expect(log.events[0]).toMatchObject({
      ...
      result: {
        ...
        host: {
          ...
          parentRevision: null,
          settingsRevision: 1,
          promptPreset: { id: 'prompt', revision: 1 },
        },
      },
    });
  ...
  test('NMR04 mixed terminal/read, foreign source identity and empty/oversized terminal contents fail without executing tools', async () => {
    for (const invalid of ['mixed', 'foreign', 'empty', 'oversized'] as const) {
      const server = await loopbackProvider(async (request, response) => {
        ...
        const content =
          invalid === 'foreign'
            ? { content: 'Synthetic prose', sourceId: 'another-source' }
            : invalid === 'empty'
              ? '   '
              : invalid === 'oversized'
                ? 'x'.repeat(STORY_SUBMIT_MAX_CHARS + 1)
                : 'Synthetic prose';
  ...
  test('NMR05 absent or false conditions omit submission; text fallback remains supported; evaluation owns its terminal', async () => {
    for (const [terminal, mode] of [
      [false, 'fiction'],
      [true, 'planning'],
    ] as const) {
    ...
    expect(
      buildMainProviderRequest(evaluated).request.stable.tools.some(
        (t) => t.name === 'story.submit'
      )
    ).toBe(false);
  });
  ...
  test('NMR07 tool continuation keeps the frozen host message and original cache bindings stable', async () => {
    ...
    const server = await loopbackProvider(async (request, response) => {
      ...
      await writeSse(
        ...
        count === 1
          ? [
              toolTurn([toolOutput(body, 'knowledge.search', { query: '' }, 'Call.Lookup')]),
              '[DONE]',
            ]
          : [
              toolTurn([toolOutput(body, 'story.submit', 'Synthetic second-round fiction.')]),
              '[DONE]',
            ]
      );
    });
    ...
    const work = compileSnapshotPrompt(snapshot(`${server.origin}/v1`, true)),
      ...
      log = hooks(server.origin),
      result = await runMain(work, log.value);
    expect(result).toMatchObject({ status: 'completed', text: 'Synthetic second-round fiction.' });
    expect(server.requests).toHaveLength(2);
    expect(log.events.map((e) => e.name)).toEqual(['knowledge.search', 'story.submit']);
    const first = JSON.parse(server.requests[0].body),
      second = JSON.parse(server.requests[1].body);
    expect(second.messages.slice(0, first.messages.length)).toEqual(first.messages);
    expect(
      second.messages.filter((m: { content: unknown }) =>
        JSON.stringify(m.content).includes('Host context (JSON reference data')
      ).length
    ).toBe(1);
  ...
  test.each(['END_SCENE', 'UNREQUESTED_SEQUENCE'])(
    'NMR10 Claude stop %s keeps the requested completion contract through the main runner',
    async (stopSequence) => {
      ...
      const server = await loopbackProvider(async (_request, response) =>
        writeSse(response, [
          ...
          {
            type: 'message_delta',
            delta: { stop_reason: 'stop_sequence', stop_sequence: stopSequence },
      ...
      vi.stubGlobal(
        ...
        vi.fn((input: RequestInfo | URL, init?: RequestInit) => {
          ...
          return nativeFetch(`${server.origin}/v1/messages`, init);
        })
      );
      const before = structuredClone(work),
        log = hooks('https://api.anthropic.com');
      log.value.resolveCredential = () => 'synthetic-native-test-token';
      const result = await runMain(work, log.value),
        matched = stopSequence === 'END_SCENE';
      expect(result).toMatchObject({
        status: matched ? 'completed' : 'partial',
        text: prose,
        error: matched ? null : 'ANTHROPIC_STOP_SEQUENCE',
        usage: { modelCalls: 1, inputTokens: 10, outputTokens: 4, costUsd: null },
      });
      expect(server.requests).toHaveLength(1);
      expect(JSON.parse(server.requests[0].body).stop_sequences).toEqual(['END_SCENE']);
      expect(log.finished).toHaveLength(1);
      expect(log.finished[0].status).toBe(result.status);
