...
test('PKUI04 hundreds of lore entries support folders, search, bulk move and persistent compact editing', async ({ page, request }, info) => {
  test.setTimeout(60000);
  const pkg = { version: 1, id: 'many-lore', revision: 1, title: '폴더 관리 합성 자료', description: '', body: 'Synthetic world', loreFolders: [{ id: 'places', name: '장소' }], lore: Array.from({ length: 240 }, (_, index) => ({ id: `lore-${index}`, title: `로어 항목 ${String(index).padStart(3, '0')}`, description: `설명 ${index}`, text: `합성 본문 ${index}`, loading: index % 2 ? 'pinned' : 'discoverable', ...(index < 120 ? { folderId: 'places' } : {}) })), instructions: [], controls: [], transforms: [] };
  ...
  await library.getByLabel('패키지 JSON 가져오기', { exact: true }).setInputFiles({ name: 'many.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(pkg)) });
  ...
  expect(await manager.locator('.lore-search').evaluate(element => getComputedStyle(element).flexDirection)).toBe('row');
  expect(await manager.locator('.lore-list-summary label').evaluate(element => getComputedStyle(element).flexDirection)).toBe('row');
  ...
  await manager.locator('.lore-row').filter({ hasText: '로어 항목 239' }).getByRole('button').click();
  await manager.getByLabel('로어 240 이름', { exact: true }).fill('검색에서 사라지는 이름');
  await expect(manager.getByLabel('로어 240 이름', { exact: true })).toHaveValue('검색에서 사라지는 이름');
  await expect(manager.getByLabel('로어 240 본문', { exact: true })).toHaveValue('수정된 합성 본문 239');
  ...
  await expect(manager.getByRole('region', { name: '로어 목록', exact: true })).toContainText('70개');
  ...
  const listing: Library = await (await request.get('/api/library')).json(); const saved = listing.contents.find(row => row.title === pkg.title)!;
  const content: Content = await (await request.get(`/api/revisions/content/${saved.id}/${saved.revision}`)).json();
  expect(content.package!.lore).toHaveLength(240); expect(content.package!.lore[239].text).toBe('수정된 합성 본문 239');
  expect(content.package!.lore[239].title).toBe('검색에서 사라지는 이름'); expect(content.package!.lore[230].title).toBe('로어 항목 230');
  ...
  const download = await downloadWait; const stream = await download.createReadStream(); const chunks: Buffer[] = []; for await (const chunk of stream!) chunks.push(Buffer.from(chunk));
  const exported = JSON.parse(Buffer.concat(chunks).toString()); expect(exported.loreFolders).toEqual(content.package!.loreFolders); expect(exported.lore).toEqual(content.package!.lore);
  ...
  await expect(manager.getByRole('region', { name: '로어 목록', exact: true })).toContainText('170개');
...
test('PKUI03 native JSON import remains a reviewed persona draft and preserves long content on save', async ({ page, request }, info) => {
  ...
  const desktopCards=await cards.evaluateAll(items=>items.map(item=>({top:item.getBoundingClientRect().top,bottom:item.getBoundingClientRect().bottom})));
  expect(desktopCards[0].top).toBe(desktopCards[1].top);expect(desktopCards[0].bottom).toBe(desktopCards[1].bottom);
  await transfers.screenshot({path:info.outputPath('library-package-tools-desktop.png')});
  await page.setViewportSize({width:390,height:844});
  const mobileCards=await cards.evaluateAll(items=>items.map(item=>({top:item.getBoundingClientRect().top,bottom:item.getBoundingClientRect().bottom,right:item.getBoundingClientRect().right})));
  expect(mobileCards[1].top).toBeGreaterThan(mobileCards[0].bottom);expect(mobileCards.every(item=>item.right<=390)).toBe(true);
  ...
  await expect(library.getByLabel('자료 이름', { exact: true })).toHaveValue('Unsaved native draft');
  await expect(library.getByRole('button', { name: '가져온 패키지로 초안 바꾸기', exact: true })).toHaveCount(0);
  const pkg = { version: 1, id: 'synthetic-native-persona', revision: 1, title: 'Synthetic imported persona', description: 'Native JSON import', body: 'Synthetic text. '.repeat(10000), lore: [], instructions: [], controls: [], transforms: [] };
  await input.setInputFiles({ name: 'native.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(pkg)) });
  ...
  const saved = after.contents.find(item => item.title === pkg.title)!; expect(saved.kind).toBe('persona');
  const content: Content = await (await request.get(`/api/revisions/content/${saved.id}/${saved.revision}`)).json();
  expect(content.text).toBe(pkg.body); expect(content.package?.body).toBe(pkg.body);
...
test('PKUI01 package editing preserves internal lore, instructions, unsaved work and cross-role copies', async ({ page, request }, info) => {
  ...
  await page.setViewportSize({width:390,height:844});
  expect(await guard.evaluate(element=>element instanceof HTMLDialogElement&&element.matches(':modal'))).toBe(true);
  const bounds=await guard.boundingBox();expect(bounds!.x).toBeGreaterThanOrEqual(16);expect(bounds!.width).toBeLessThanOrEqual(358);expect(bounds!.height).toBeLessThan(400);
  ...
  await page.keyboard.press('Escape');await expect(guard).toBeHidden();
  await expect(library.getByLabel('자료 이름',{exact:true})).toHaveValue('Synthetic package editor bot');
  await library.getByRole('button',{name:'← 서재 목록',exact:true}).click();await expect(guard).toBeVisible();
