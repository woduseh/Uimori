...
describe('native provider wire (synthetic, no live calls)',()=>{
  test('preserves roles/order and sends current/history once across four codecs',()=>{
    ...
    const anthropic=wire(encodeAnthropic(request('claude-opus-5')).body);expect(anthropic.messages.map((m:any)=>m.role)).toEqual(['user','assistant','user']);expect(anthropic.system.at(-1).text).toBe('NATIVE_SYSTEM');
  ...
  test('native cache is explicit for eligible Responses and Anthropic while compatible chat remains unsupported',()=>{
    ...
    const a=wire(encodeAnthropic({...r,modelId:'claude-opus-5'}).body);expect(a.system.at(-1).cache_control).toEqual({type:'ephemeral'});
  ...
  test.each(['explicit','automatic'] as const)('Responses %s cache mode retains authored points and selected 30m TTL',cacheMode=>{
    const r=request();r.generation={maxOutputTokens:8192,temperature:null,cacheMode,cacheTtl:'30m'};
    r.prompt!.cachePlan=[{blockId:'cache',afterMessageId:'system',policy:'require'}];
    const before=structuredClone(r);const body=wire(encodeResponses(r).body);
    expect(body.prompt_cache_options).toEqual({mode:cacheMode==='automatic'?'implicit':'explicit',ttl:'30m'});
    expect(body.input[0].content[0].prompt_cache_breakpoint).toEqual({mode:'explicit'});expect(r).toEqual(before);
  });
  test.each(['5m','1h',undefined] as const)('Claude applies TTL %s to automatic caching and every authored point',cacheTtl=>{
    const r=request('claude-fable-5-1');r.generation={maxOutputTokens:8192,temperature:null,cacheMode:'automatic',...(cacheTtl?{cacheTtl}:{})};
    r.prompt!.cachePlan=[{blockId:'cache-system',afterMessageId:'system',policy:'require'},{blockId:'cache-user',afterMessageId:'old-user',policy:'require'}];
    const before=structuredClone(r);const body=wire(encodeAnthropic(r).body);const control={type:'ephemeral',...(cacheTtl?{ttl:cacheTtl}:{})};
    expect(body.cache_control).toEqual(control);expect(body.system.at(-1).cache_control).toEqual(control);expect(body.messages[0].content[0].cache_control).toEqual(control);
    r.generation.cacheMode='explicit';const explicit=wire(encodeAnthropic(r).body);
    expect(explicit).not.toHaveProperty('cache_control');expect(explicit.system.at(-1).cache_control).toEqual(control);expect(explicit.messages).toEqual(body.messages);
    r.generation.cacheMode='automatic';expect(r).toEqual(before);
  });
  test.each(['openai-responses-v1','anthropic-messages-v1'] as const)('%s reserves one automatic slot and never silently displaces a preferred point',protocol=>{
    const r=request(protocol==='openai-responses-v1'?'gpt-5.6-sol':'claude-opus-5');const encode=protocol==='openai-responses-v1'?encodeResponses:encodeAnthropic;
    r.generation={maxOutputTokens:8192,temperature:null,cacheMode:'automatic'};
    r.prompt!.cachePlan=r.prompt!.messages.slice(0,3).map(m=>({blockId:`cache-${m.id}`,afterMessageId:m.id,policy:'prefer'}));
    expect(()=>encode(r)).not.toThrow();
    r.prompt!.cachePlan.push({...r.prompt!.cachePlan[0],blockId:'duplicate-point'});expect(()=>encode(r)).not.toThrow();
    r.prompt!.cachePlan.push({blockId:'cache-current',afterMessageId:'current',policy:'prefer'});expect(()=>encode(r)).toThrow('PROMPT_CACHE_LIMIT');
    r.generation.cacheMode='explicit';expect(()=>encode(r)).not.toThrow();
  });
  test.each(['openai-responses-v1','anthropic-messages-v1'] as const)('%s cache OFF overrides authored required points with explicit diagnostics',protocol=>{
    const r=request(protocol==='openai-responses-v1'?'gpt-6-astra':'claude-fable-5-1');const encode=protocol==='openai-responses-v1'?encodeResponses:encodeAnthropic;
    r.generation={maxOutputTokens:8192,temperature:null,cacheMode:'disabled'};
    r.prompt!.cachePlan=r.prompt!.messages.map(m=>({blockId:`cache-${m.id}`,afterMessageId:m.id,policy:'require'}));
    const original=structuredClone(r);const body=wire(encode(r).body);const plan=planNativeMessages(r,protocol)!;
    expect(plan.diagnostics.filter(d=>d.code==='PROMPT_CACHE_DISABLED_BY_MODEL')).toHaveLength(4);
    expect(plan.diagnostics.filter(d=>d.code==='PROMPT_CACHE_DISABLED_BY_MODEL').every(d=>d.status==='not-applied')).toBe(true);
    expect(JSON.stringify(body)).not.toMatch(/"cache_control"|"prompt_cache_breakpoint"/);
    if(protocol==='openai-responses-v1')expect(body.prompt_cache_options).toEqual({mode:'explicit'});else expect(body).not.toHaveProperty('prompt_cache_options');
    expect(r).toEqual(original);
  });
  test('unreviewed model IDs do not inherit explicit cache support',()=>{
    for(const [protocol,modelId]of [['openai-responses-v1','gpt-future'],['anthropic-messages-v1','claude-future']] as const){
      const r=request(modelId);r.prompt!.cachePlan=[{blockId:'cache',afterMessageId:'system',policy:'require'}];
  ...
  test('Anthropic signed content and Vertex thought signatures survive native continuation',()=>{
    const r=request('claude-opus-5');const encoded=encodeAnthropic(r);const d=new AnthropicDecoder(encoded.context);
  ...
  test('Anthropic accepts M2 state and memory roles without enabling a model call',()=>{for(const role of ['state','memory'] as const)expect(wire(encodeAnthropic({...request('claude-opus-5'),role}).body).messages).toHaveLength(3);});
});
