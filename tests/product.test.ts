...
import { buildMainInput, syntheticResources } from '../core/provider.js';
import { compileCreative, defaultCreative, type ChatProfile, type Connection, type Content, type CreativePreset, type ModelPreset } from '../core/product.js';
...
afterEach(async () => {
  for (const item of owned.splice(0).reverse()) {
    await item.close?.(); item.store?.close();
    const target = resolve(item.directory); const within = relative(resolve(tmpdir()), target);
    if (isAbsolute(within) || within.startsWith('..') || !basename(target).startsWith('서사 M1 product ')) throw new Error('Refusing cleanup outside owned test directory');
    await rm(target, { recursive: true, force: true });
  }
});
async function directory() { const path = await mkdtemp(join(tmpdir(), '서사 M1 product ')); const item = { directory: path } as (typeof owned)[number]; owned.push(item); return item; }
...
const contentBody = (kind: Content['kind'], text: string, loading: Content['loading'] = 'pinned') => ({ kind, title: `${kind} fixture`, description: `synthetic ${kind}`, text, loading, relatedIds: [] });
const noUsage: Usage = { modelCalls: 1, inputTokens: null, outputTokens: null, costUsd: null };
function profile(product: ProductStore, chat: Chat, attachments: { id: string; revision: number }[] = [], changes: Partial<ChatProfile> = {}) {
  const prior = product.profile(chat.id);
  return product.updateProfile(chat.id, { expectedRevision: prior.revision, attachments, creative: prior.creative, routes: prior.routes, image: prior.image, ...changes });
}
function queuedRun(store: Store, product: ProductStore, chatId: string, request = 'Synthetic scene', options: { branchId?: string; expectedRevision?: string | null; expectedProfileRevision?: number; idempotencyKey?: string } = {}) {
  const chat = store.chat(chatId); const captured = product.snapshot(chatId);
  const head = product.branch(chatId, options.branchId).headRevision;
  return store.createRun(chatId, { request, expectedRevision: head, expectedSettingsRevision: chat.settingsRevision, idempotencyKey: randomUUID(), ...options }, current => ({
    chatId, parentRevision: current.headRevision, settingsRevision: current.settingsRevision, settings: current.settings, request,
    history: store.history(current.headRevision), resources: product.resources(chatId, captured), ...(captured ? { profile: captured } : {}),
  } satisfies RunSnapshot)).run;
}
function completedSource(store: Store, product: ProductStore, chatId: string, text = 'A synthetic keeper watched the evening tide.') {
  const run = queuedRun(store, product, chatId); store.startRun(run.id);
  return store.completeRun(run.id, text, noUsage, run.snapshot.settings);
}
const pixel = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a3ioAAAAASUVORK5CYII=';
const assetBody = { title: 'Synthetic pixel', mime: 'image/png', base64: pixel, description: 'A one pixel synthetic fixture', actor: '', outfit: '', location: '', allowedUse: 'both' };

describe('M1 product data with actual file SQLite', () => {
  test('P01 P02 P03 freezes exact content/profile revisions and swaps the complete creative control group', async () => {
    const { store, product } = await database();
    const chat = store.createChat('attached-story', 'calm', syntheticResources);
    const separate = store.createChat('separate-story', 'calm', syntheticResources);
    const bot = product.content(contentBody('bot', 'Ada is the keeper.')) as Content;
    const canon = product.content(contentBody('canon', 'The harbor has never had electricity.')) as Content;
    const lore = product.content(contentBody('lore', 'UNREAD_LORE_V1', 'discoverable')) as Content;
    const first = profile(product, chat, [bot, canon, lore].map(reference));
    const preset = product.preset({ title: 'Whole group', controls: { ...defaultCreative(), mode: 'rp', language: 'ko', coNarration: true, declarationFinal: true, pov: 'first', style: 'vivid', personaReference: false, worldFocus: false } }) as CreativePreset;
    const applied = product.applyPreset(chat.id, { expectedRevision: first.revision, presetId: preset.id, presetRevision: preset.revision });
    expect(applied.creative).toEqual(preset.controls);
    expect(applied.attachments).toEqual(first.attachments); expect(applied.routes).toEqual(first.routes);
    const run = queuedRun(store, product, chat.id);
    const oldSnapshot = structuredClone(run.snapshot);
    const edited = product.content({ ...contentBody('lore', 'EDITED_LORE_V2', 'discoverable'), expectedRevision: lore.revision }, lore.id) as Content;
    expect(edited.revision).toBe(2);
    profile(product, chat, [bot, canon, edited].map(reference), { creative: { ...preset.controls, lengthMode: 'custom', customWords: 12345 } });
    expect(store.run(run.id).snapshot).toEqual(oldSnapshot);
    expect(store.run(run.id).snapshot.resources.find(item => item.id === lore.id)?.text).toBe('UNREAD_LORE_V1');
    expect(product.get<Content>('content', lore.id, 1).text).toBe('UNREAD_LORE_V1');
    expect(product.snapshot(chat.id)?.contents.find(item => item.id === lore.id)?.text).toBe('EDITED_LORE_V2');
    expect(product.snapshot(separate.id)).toBeUndefined();
    const input = buildMainInput(oldSnapshot);
    expect(input.facts).toContain(canon.text);
    expect(input.pinnedSources?.find(item => item.kind === 'canon')).toMatchObject({ id: canon.id, revision: 1, hash: createHash('sha256').update(canon.text).digest('hex') });
    ...
    expect(JSON.stringify(input.controls)).not.toContain('15000');
    expect(compileCreative(oldSnapshot.profile!.creative).length).toEqual({ minWords: 4500, maxWords: 7500 });
    expect(() => product.content({ ...contentBody('lore', 'STALE_WRITE'), expectedRevision: 1 }, lore.id)).toThrow('Revision conflict');
    expect(() => product.applyPreset(chat.id, { expectedRevision: first.revision, presetId: preset.id, presetRevision: 1 })).toThrow('Profile revision conflict');
  });
  ...
  test('P09 preserves sibling candidates from the exact original snapshot and each descendant ancestry with branch CAS', async () => {
    const { store, product } = await database(); const chat = store.createChat('candidate-story');
    ...
    const original = queuedRun(store, product, chat.id, 'Same candidate request'); store.startRun(original.id);
    const a = store.completeRun(original.id, 'CANDIDATE_A', noUsage, original.snapshot.settings);
    const changedCanon = product.content({ ...contentBody('canon', 'Later canon revision.'), expectedRevision: 1 }, canon.id) as Content;
    profile(product, chat, [reference(changedCanon)], { creative: { ...defaultCreative(), style: 'vivid' } });
    store.settings(chat.id, store.chat(chat.id).settingsRevision, { ...store.chat(chat.id).settings, preset: 'vivid' });
    const key = randomUUID(); const candidate = store.candidate(original.id, key, 'Sibling B');
    const { branchId: _oldBranch, candidateOf: _oldCandidate, ...originalFrozen } = original.snapshot;
    ...
    expect(candidate.run.parentRevision).toBe(base.id); expect(candidateOf).toBe(original.id);
    ...
    expect(store.candidate(original.id, key, 'Sibling B')).toMatchObject({ created: false, run: { id: candidate.run.id } });
    expect(() => store.candidate(original.id, key, 'Different title')).toThrow('Idempotency key reused');
    store.startRun(candidate.run.id);
    const b = store.completeRun(candidate.run.id, 'CANDIDATE_B', noUsage, candidate.run.snapshot.settings);
    ...
    store.startRun(bRun.id); const bDescendant = store.completeRun(bRun.id, 'B_DESCENDANT', noUsage, bRun.snapshot.settings);
    expect(store.history(aDescendant.id).map(item => item.text)).toEqual(['BASE_SCENE', 'CANDIDATE_A', 'A_DESCENDANT']);
    expect(store.history(bDescendant.id).map(item => item.text)).toEqual(['BASE_SCENE', 'CANDIDATE_B', 'B_DESCENDANT']);
    expect(() => queuedRun(store, product, chat.id, 'Stale B', { branchId: candidateBranch, expectedRevision: b.id })).toThrow('Source revision conflict');
    expect(() => queuedRun(store, product, chat.id, 'Stale profile', { expectedProfileRevision: 1 })).toThrow('Profile revision conflict');
    const other = store.createChat('other-candidate-story');
    expect(() => product.createBranch(other.id, { title: 'Cannot cross chat', fromRevision: a.id })).toThrow('Source outside chat');
  ...
  test('P05 P06 P12 records fixture refusals and partials without source/jobs; duplicate attempt finishes cannot rewrite usage', async () => {
    const { item, store, product } = await database(); let calls = 0;
    const server = await loopbackProvider(async (_captured, response) => {
      calls++;
      await writeSse(response, calls === 1
        ? [{ type: 'refusal', message: 'Explicit fixture refusal' }, { type: 'usage', inputTokens: 13, outputTokens: 2, costUsd: null, raw: { total_tokens: 15 } }, { type: 'done', reason: 'refusal' }]
        : [{ type: 'text_delta', delta: 'Unfinished fixture fragment' }, { type: 'usage', inputTokens: 7, outputTokens: 1, costUsd: null }]);
    }); item.close = server.close;
    const chat = store.createChat('refusal-story');
    ...
    for (const expected of ['refused', 'partial'] as const) {
      ...
      const result = await runMain(run.snapshot, { signal: new AbortController().signal, approvedOrigins: [server.origin], authorize: value => product.authorize(value),
        onInput: input => store.input(run.id, input), onToolEvent: tool => store.tool(run.id, tool),
        onAttemptStart: wire => product.startAttempt(chat.id, run.id, null, wire), onAttemptFinish: (id, value) => product.finishAttempt(id, value) });
      expect(result.status).toBe(expected);
      store.finishRun(run.id, expected, result.error ?? '', result.text, result.usage);
      expect(store.run(run.id)).toMatchObject({ status: expected, sourceRevision: null, usage: { costUsd: null } });
    }
    expect(store.detail(chat.id).sources).toHaveLength(1); expect(store.source(preserved.id).text).toBe('PRESERVED_ORIGINAL');
    expect(store.chat(chat.id).headRevision).toBe(preserved.id); expect(store.detail(chat.id).jobs).toHaveLength(originalJobCount);
    ...
    product.finishAttempt(attempts[0].id, { ...firstResult, status: 'completed', usage: { ...firstResult.usage, inputTokens: 999, costUsd: 0 } });
  ...
  test('P11 exports/restores source bytes, lineage and assets while disabling connections and unfinished work', async () => {
    const { store, product } = await database();
    const chat = store.createChat('archive-story');
    const canon = product.content(contentBody('canon', 'A portable author declaration.')) as Content;
    const bound = product.connection({ title: 'Fixture', protocol: 'fixture-sse-v1', endpoint: 'http://127.0.0.1:49999/turn', credentialEnv: 'NARRATIVE_PROVIDER_ARCHIVE_FIXTURE', enabled: true }) as Connection;
    profile(product, chat, [reference(canon)]);
    ...
    expect(target.product.get<Connection>('connection', bound.id)).toMatchObject({ enabled: false });
    expect(target.product.get<Connection>('connection', bound.id)).not.toHaveProperty('credentialEnv');
    expect(target.store.queuedJobs()).toEqual([]);
    expect(target.store.detail(chat.id).jobs.every(job => job.status === 'interrupted')).toBe(true);
  ...
  test('P11 rejects changed source/asset bytes and invalid ancestry atomically in an empty restore target', async () => {
    ...
    const mutations: { name: string; mutate: (value: typeof archive) => void }[] = [
      { name: 'source hash', mutate: value => { value.tables.sources[0].text = 'Changed bytes'; } },
      { name: 'asset hash', mutate: value => { value.tables.assets[0].bytes = Buffer.from('corrupt bytes').toString('base64'); } },
      { name: 'source cycle', mutate: value => { value.tables.sources[0].parent_revision = source.id; } },
      { name: 'missing attachment revision', mutate: value => { value.tables.profiles.push({ chat_id: chat.id, body: JSON.stringify({ ...product.profile(chat.id), attachments: [{ id: 'missing', revision: 1 }] }) }); } },
    ];
    for (const mutation of mutations) {
      const target = await database(); const invalid = structuredClone(archive); mutation.mutate(invalid);
      expect(() => target.product.import(invalid), mutation.name).toThrow();
      expect(target.store.chats(), mutation.name).toEqual([]);
      expect(target.store.db.prepare('SELECT count(*) AS count FROM sources').get()).toEqual({ count: 0 });
  ...
  test('P11 reopens actual backup bytes as standalone SQLite while the source database remains owned', async () => {
    const { item, store, product } = await database(); const chat = store.createChat('live-backup-fixture');
    const source = completedSource(store, product, chat.id, 'WAL-backed source committed before the backup.');
    const asset = product.createAsset(chat.id, assetBody);
...
async function application(options: { approvedOrigins?: string[]; accessToken?: string } = {}) {
  ...
  const app = await createApp({ dbPath: join(item.directory, 'http.sqlite'), buildId: 'm1-product-tests', instanceId: randomUUID(), testMode: true, ...options });
  item.close = () => app.close();
  const url = await app.listen({ port: 0, host: '127.0.0.1' });
  return { item, app, url };
}
async function api<T = unknown>(url: string, path: string, body?: unknown, options: { method?: string; status?: number; cookie?: string; origin?: string } = {}): Promise<T> {
  const response = await fetch(`${url}${path}`, { method: options.method ?? (body === undefined ? 'GET' : 'POST'),
    headers: { ...(body === undefined ? {} : { 'content-type': 'application/json' }), ...(options.cookie ? { cookie: options.cookie } : {}), ...(options.origin ? { origin: options.origin } : {}) },
    body: body === undefined ? undefined : JSON.stringify(body) });
  const value = await response.json(); expect(response.status, JSON.stringify(value)).toBe(options.status ?? 200); return value as T;
}
...
describe('M1 real HTTP application boundaries', () => {
  test('P07 P08 P12 runs source-time translation through HTTP and file SQLite, retaining partial siblings and retrying only the failed chunk', async () => {
    const fixtureItem = await directory(); const chunkRequests = new Map<string, number>(); let glossaryId = '';
    const originalText = ['First', 'Middle', 'Last'].map(label => `${label} block carries \`KEEP_LITERAL\`. ${'The wind moves along the quiet pier. '.repeat(55)}`).join('\n\n');
    const provider = await loopbackProvider(async (captured, response) => {
      const body = JSON.parse(captured.body);
      if (body.role === 'main') { await writeSse(response, [{ type: 'text_delta', delta: originalText }, { type: 'usage', inputTokens: 5, outputTokens: 9 }, { type: 'done', reason: 'stop' }]); return; }
      expect(body.role).toBe('translation'); expect(body.modelId).toBe('fixture-translator');
      const source = body.input.source;
      expect(source.context.glossary).toMatchObject([{ id: glossaryId, revision: 1, text: 'SOURCE_TIME_GLOSSARY_OLD' }]);
      expect(JSON.stringify(body)).not.toContain('FUTURE_GLOSSARY_NEW');
      const count = (chunkRequests.get(source.chunkId) ?? 0) + 1; chunkRequests.set(source.chunkId, count);
      if (source.chunkId.endsWith('-0') && body.input.results.length === 0) {
        await writeSse(response, [{ type: 'tool_delta', index: 0, id: 'translation-read-glossary', name: 'knowledge.read', argumentsDelta: JSON.stringify({ id: glossaryId }) }, { type: 'usage', inputTokens: 3, outputTokens: 1 }, { type: 'done', reason: 'tool_calls' }]); return;
      }
      if (source.chunkId.endsWith('-0')) expect(body.input.results[0]).toMatchObject({ callId: 'translation-read-glossary', denied: false, result: { text: 'SOURCE_TIME_GLOSSARY_OLD', source: { revision: 1 } } });
      if (source.chunkId.endsWith('-1') && count === 1) { await writeSse(response, [{ type: 'error', message: 'One intentional technical fixture failure' }]); return; }
      const output = { sourceRevision: source.sourceRevision, sourceHash: source.sourceHash, chunkId: source.chunkId, segments: source.blocks.map((block: { anchor: string; text: string }) => ({ anchors: [block.anchor], text: `[모의 번역 결과] ${block.text}` })) };
      await writeSse(response, [{ type: 'text_delta', delta: JSON.stringify(output) }, { type: 'usage', inputTokens: 4, outputTokens: 2 }, { type: 'done', reason: 'stop' }]);
    ...
    expect(app.store.source(source.id).hash).toBe(source.hash); expect(app.store.source(source.id).text).toBe(originalText);
  ...
  test('P05 P06 P09 routes selected main through fetch, preserves source on refusal/partial and creates a sibling over HTTP', async () => {
    ...
    const provider = await loopbackProvider(async (captured, response) => {
      const body = JSON.parse(captured.body);
      if (body.input.task === 'refuse main') await writeSse(response, [{ type: 'refusal', message: 'Fixture refuses explicitly' }, { type: 'usage', inputTokens: 8, outputTokens: 1 }, { type: 'done', reason: 'refusal' }]);
      else if (body.input.task === 'partial main') await writeSse(response, [{ type: 'text_delta', delta: 'PARTIAL_NOT_SOURCE' }]);
      else await writeSse(response, [{ type: 'text_delta', delta: 'HTTP_PROVIDER_ORIGINAL' }, { type: 'usage', inputTokens: 4, outputTokens: 2 }, { type: 'done', reason: 'stop' }]);
    }); fixtureItem.close = provider.close;
    const { app, url } = await application({ approvedOrigins: [provider.origin] });
    ...
    const created = await create('success main'); const completed = await terminal(url, created.id);
    ...
    const duplicate = await api<Run>(url, `/api/runs/${created.id}/candidate`, { idempotencyKey: key, title: 'HTTP sibling' });
  ...
  test('P04 catalog refresh updates data only and retains prior data on HTTP failure without changing model routes or credentials', async () => {
    const fixtureItem = await directory(); let fail = false;
    const provider = await loopbackProvider((captured, response) => {
      expect(captured.url).toBe('/models');
      if (fail) { response.writeHead(503, { 'content-type': 'application/json' }); response.end('{"error":"fixture unavailable"}'); return; }
      response.writeHead(200, { 'content-type': 'application/json' });
      response.end(JSON.stringify({ models: [{ id: 'catalog-new-entry', label: 'New fixture model', capabilities: { tools: true } }] }));
    }); fixtureItem.close = provider.close;
    const envName = 'NARRATIVE_PROVIDER_CATALOG_FIXTURE'; const priorEnv = process.env[envName]; process.env[envName] = 'SYNTHETIC_CATALOG_CREDENTIAL';
    try {
      const { app, url } = await application({ approvedOrigins: [provider.origin] });
      ...
      expect(refreshed.credentialEnv).toBe(envName); expect(refreshed.enabled).toBe(true);
      expect(app.store.product.get<ModelPreset>('model', model.id)).toEqual(model);
      fail = true; await api(url, `/api/connections/${bound.id}/catalog`, {});
      const failed = app.store.product.get<Connection>('connection', bound.id);
      expect(failed.catalog).toEqual(refreshed.catalog); expect(failed.catalogError).toBeTruthy();
      expect(failed.credentialEnv).toBe(envName); expect(failed.enabled).toBe(true);
      expect(app.store.product.get<ModelPreset>('model', model.id).modelId).toBe('manual-unlisted-id');
      expect(JSON.stringify(app.store.product.library())).not.toContain('SYNTHETIC_CATALOG_CREDENTIAL');
      expect(provider.requests).toHaveLength(2);
    } finally { if (priorEnv === undefined) delete process.env[envName]; else process.env[envName] = priorEnv; }
  });
  ...
  test('P12 protects data, events and asset bytes with an HttpOnly session, rejects foreign origins and revokes logout cookies', async () => {
    ...
    const chat = app.store.createChat('private synthetic chat'); const asset = app.store.product.createAsset(chat.id, assetBody);
    ...
    for (const path of ['/api/chats', `/api/chats/${chat.id}/events`, `/api/assets/${asset.id}`]) {
      const response = await fetch(`${url}${path}`); expect(response.status, path).toBe(401); await response.body?.cancel();
    }
    await api(url, '/api/session', { token: 'wrong synthetic token' }, { status: 401 });
    const login = await fetch(`${url}/api/session`, { method: 'POST', headers: { 'content-type': 'application/json', origin: url }, body: JSON.stringify({ token: accessToken }) });
    expect(login.status).toBe(200); const setCookie = login.headers.get('set-cookie') ?? ''; await login.body?.cancel();
    expect(setCookie).toMatch(/HttpOnly/i); expect(setCookie).toMatch(/SameSite=Strict/i); expect(setCookie).not.toContain(accessToken);
    const cookie = setCookie.split(';')[0];
    expect(await api(url, '/api/session', undefined, { cookie })).toMatchObject({ authenticated: true });
    expect(await api<Chat[]>(url, '/api/chats', undefined, { cookie })).toHaveLength(1);
    await api(url, '/api/chats', { title: 'foreign write' }, { cookie, origin: 'https://foreign.invalid', status: 403 });
    const assetResponse = await fetch(`${url}/api/assets/${asset.id}`, { headers: { cookie } });
    expect(assetResponse.status).toBe(200); expect(Buffer.from(await assetResponse.arrayBuffer())).toEqual(Buffer.from(pixel, 'base64'));
    const events = await fetch(`${url}/api/chats/${chat.id}/events`, { headers: { cookie } });
    expect(events.status).toBe(200); expect(events.headers.get('content-type')).toContain('text/event-stream'); await events.body?.cancel();
...
function schema1Fixture(path: string) {
  ...
  const settings = { preset: 'calm', mode: 'direct', translation: true, status: false, maxCalls: 8 };
  const time = '2026-09-06T00:00:00Z'; const text = 'Legacy original preserved.';
  try {
    ...
    db.prepare('INSERT INTO chats VALUES(?,?,?,?,?,?)').run('old-chat', 'Legacy fixture', 'old-source', 1, JSON.stringify(settings), time);
    const snapshot = { chatId: 'old-chat', parentRevision: null, settingsRevision: 1, settings, request: 'Legacy request', history: [], resources: [] };
    db.prepare('INSERT INTO runs VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)').run('old-run', 'old-chat', null, 'completed', 'Legacy request', JSON.stringify(snapshot), 'legacy-key', '{}', 'old-source', null, JSON.stringify(noUsage), time, time);
    const hash = createHash('sha256').update(text).digest('hex');
    db.prepare('INSERT INTO sources VALUES(?,?,?,?,?,?,?)').run('old-source', 'old-chat', 'old-run', null, text, hash, time);
    db.prepare('INSERT INTO jobs VALUES(?,?,?,?,?,?,?,?,?,?,?,?)').run('old-job', 'old-chat', 'old-source', hash, 'translation', 'completed', 1, 'old-owner', '{}', null, time, time);
    db.prepare('INSERT INTO job_results VALUES(?,?,?,?)').run('old-job', 1, JSON.stringify({ mock: true, text: '기존 모의 번역' }), time);
    db.prepare('INSERT INTO events(chat_id,kind,entity_id,at) VALUES(?,?,?,?)').run('old-chat', 'source.ready', 'old-source', time);
  } finally { db.close(); }
}
