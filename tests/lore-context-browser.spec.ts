...
async function seed(request:APIRequestContext,title:string,part:Partial<ContentPackage>={}){
  const pkg:ContentPackage={version:1,id:'synthetic_lore_ui',revision:1,title,description:'Synthetic lore context UI',body:'Synthetic harbor keeper.',lore:[],instructions:[],controls:[],transforms:[],...part};
  const response=await request.post('/api/content',{data:{kind:'bot',title,description:pkg.description,text:pkg.body,loading:'pinned',relatedIds:[],package:pkg}});expect(response.ok(),await response.text()).toBe(true);return response.json() as Promise<Content>;
}
async function createChat(request:APIRequestContext,bot:Content){const response=await request.post('/api/chats',{data:{title:bot.title,botId:bot.id}});expect(response.ok(),await response.text()).toBe(true);return response.json() as Promise<Chat>;}
...
async function evidence(page:Page,target:Locator,info:TestInfo,name:string){
  for(const [suffix,width,height] of [['desktop',1440,1000],['mobile',390,844]] as const){await page.setViewportSize({width,height});await target.scrollIntoViewIfNeeded();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1)).toBe(true);const dialog=page.getByRole('dialog').filter({visible:true});if(await dialog.count())expect(await dialog.evaluate(element=>element.scrollWidth<=element.clientWidth+1)).toBe(true);await page.screenshot({path:info.outputPath(`${name}-${suffix}.png`)});}
}

test('LCUI01 lore placement and invalid order drafts stay independent from folders and loading',async({page,request},info)=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));
  const original=await seed(request,`합성 로어 제작 ${Date.now()}`,{loreFolders:[{id:'folder_one',name:'편집용 폴더'}],lore:[{id:'first',title:'첫 로어',description:'',text:'A😀B',loading:'discoverable',folderId:'folder_one'},{id:'second',title:'둘째 로어',description:'',text:'Untouched lore.',loading:'pinned'}]});
  await page.setViewportSize({width:1440,height:1000});await page.goto('/');await page.getByRole('navigation',{name:'자료 탐색',exact:true}).getByRole('button',{name:'봇',exact:true}).click();
  const library=page.getByTestId('library-panel');await library.getByRole('button',{name:`${original.title} 자료 편집`,exact:true}).click();const fields=library.getByRole('region',{name:'패키지 구성',exact:true}),save=library.getByRole('button',{name:'새 revision 저장',exact:true});
  await fields.getByLabel('사용 방법',{exact:true}).selectOption('pinned');await fields.getByLabel('로어 1 배치',{exact:true}).selectOption('scene');await fields.getByLabel('로어 1 배치 묶음',{exact:true}).fill('장면 인물');await fields.getByLabel('로어 1 배치 순서',{exact:true}).fill('1.5');await expect(save).toBeDisabled();
  await fields.getByRole('button',{name:'옵션',exact:true}).click();await fields.getByRole('button',{name:'로어',exact:true}).click();await expect(fields.getByLabel('로어 1 배치 순서',{exact:true})).toHaveValue('1.5');await expect(fields.getByLabel('로어 소속 폴더',{exact:true})).toHaveValue('folder_one');
  await fields.getByLabel('로어 1 배치 순서',{exact:true}).fill('-2');await fields.getByLabel('사용 방법',{exact:true}).selectOption('discoverable');await expect(fields.getByLabel('로어 1 배치',{exact:true})).toHaveValue('scene');await expect(fields.getByRole('region',{name:'선택한 로어 편집',exact:true})).toContainText('UTF-16 4자');await expect(save).toBeEnabled();
  const saved=page.waitForResponse(response=>response.url().endsWith(`/api/content/${original.id}`)&&response.request().method()==='PUT');await save.click();const response=await saved;expect(response.ok(),await response.text()).toBe(true);const changed=await response.json() as Content;
  expect(changed.package!.lore[0]).toMatchObject({loading:'discoverable',folderId:'folder_one',loreContext:{placement:'scene',group:'장면 인물',order:-2}});expect(changed.package!.lore[1]).toEqual(original.package!.lore[1]);expect(changed.package!.loreFolders).toEqual(original.package!.loreFolders);expect((await(await request.get(`/api/revisions/content/${original.id}/1`)).json()).package).toEqual(original.package);
  await evidence(page,fields.getByLabel('로어 1 배치',{exact:true}),info,'lore-placement');expect(errors).toEqual([]);
});
...
test('LCUI02 policy drafts survive tabs and preview reflects the unsaved policy without writing or calling models',async({page,request},info)=>{
  ...
  const before=await detail(request,chat.id);await policy.getByText('다음 생성의 로어 미리보기',{exact:true}).click();const previewResponse=page.waitForResponse(response=>response.url().endsWith(`/api/chats/${chat.id}/prompt-preview`)&&response.request().method()==='POST');await policy.getByRole('button',{name:'로어 미리보기 갱신',exact:true}).click();const preview=await previewResponse;expect(preview.ok(),await preview.text()).toBe(true);
  const expected={enabled:true,maxRetainedChars:1234,maxRetainedEntries:8,maxPinnedChars:5000};expect(preview.request().postDataJSON()).toMatchObject({request:'합성 다음 장면',role:'main',loreContext:expected,loreContextReset:true});expect(preview.request().postDataJSON().program).toBeUndefined();const compiled=await preview.json();expect(compiled.loreContext.policy).toEqual(expected);expect(compiled.loreContext.stats.reasons).toContain('new-scene');expect(JSON.stringify(compiled.compilation.messages)).toContain('Synthetic fixed lighthouse.');
  const result=policy.getByRole('region',{name:'미리보기의 조회 로어',exact:true});await expect(result).toContainText('새 장면 요청으로 조회 로어 정리');await expect(result).toContainText('5,000자');const after=await detail(request,chat.id);expect(after.profile).toEqual(before.profile);expect(after.runs).toEqual(before.runs);expect(after.attempts).toEqual(before.attempts);expect(after.sources).toEqual(before.sources);
  await evidence(page,result,info,'lore-policy-preview');const saved=page.waitForResponse(response=>response.url().endsWith(`/api/chats/${chat.id}/profile`)&&response.request().method()==='PUT');await save.click();expect((await saved).ok()).toBe(true);expect((await detail(request,chat.id)).profile!.loreContext).toEqual(expected);expect(errors).toEqual([]);
});
