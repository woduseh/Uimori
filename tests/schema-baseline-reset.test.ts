...
test('BASE01 a fresh database creates the complete schema 14 and a current database reopens directly', () => {
  const f = fixture(),
...
test('BASE03 fresh initialization rolls back as one transaction and releases its owner after a failure', () => {
  ...
  expect(f.owner.store.db.prepare('PRAGMA user_version').get()).toEqual({ user_version: 14 });
});

test('BASE04 only schema 14 archives restore, and a rejected version leaves both databases untouched', () => {
  ...
  for (const version of [2, 3, 4, 5, 6, 7, 9, 10, 11, 12, 13, 15]) {
