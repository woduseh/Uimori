...
import { AnthropicDecoder, AnthropicProtocolError, diagnosticAnthropicBody, encodeAnthropic } from '../core/anthropic-protocol.js';
...
const request = (): ProviderRequest => ({
  ...
  input: { task: 'Continue at the harbor.', controls: { words: 900, tone: 'quiet', enabled: false, empty: null },
    source: { revision: 'source-1', text: '원문 🌊\nOriginal prose.' }, catalog: [{ id: 'lore-1', description: 'A harbor.' }],
    history: [{ revision: 'past-1', text: 'Earlier prose.' }], results: [] },
});
const native = (value: Json) => value as Record<string, any>;
const begin = (decoder: AnthropicDecoder, usage?: Json) => decoder.accept({ type: 'message_start', message: {
  id: 'msg_fixture', type: 'message', role: 'assistant', model: 'canonical-provider-model-id', content: [], stop_reason: null, ...(usage !== undefined ? { usage } : {}),
} });
...
function start(input = request()) {
  const encoded = encodeAnthropic(input); return { ...encoded, decoder: new AnthropicDecoder(encoded.context) };
}
function toolPart(id = 'call-1', name = 'tool_0_knowledge_read', input: Json = { id: 'lore-1' }): Json { return { type: 'tool_use', id, name, input }; }
function turn(input = request(), parts: Json[] = [toolPart()]) {
  const active = start(input); begin(active.decoder);
  parts.forEach((part, index) => block(active.decoder, index, part));
  return end(active.decoder, 'tool_use');
}
function results(output: ProviderResult): Json[] {
  return output.toolCalls.map(call => ({ callId: call.id, name: call.name, args: call.arguments, result: { text: 'source ' + call.arguments.id }, denied: false }));
}
function continued(input: ProviderRequest, output: ProviderResult): ProviderRequest {
  return { ...structuredClone(input), opaqueState: output.opaqueState, input: { ...structuredClone(input.input), results: [...input.input.results as Json[], ...results(output)] } };
}
function translationRequest() {
  const text = 'He had served forty years. The gauge read 40.';
  const plan = createTranslationPlan({ id: 'source-translation', chatId: 'chat-1', text, hash: createHash('sha256').update(text).digest('hex') },
    { revision: 'context-1', bot: null, persona: null, glossary: [], canon: [], scene: 'A room.', previousSources: [], instructionRevision: 'hermeneia-native-1', modelPresetRevision: 'translation-preset-1' });
  const chunk = plan.chunks[0]; const input = request(); input.role = 'translation';
  input.input.source = { sourceRevision: plan.sourceRevision, sourceHash: plan.sourceHash, chunkId: chunk.id, blocks: chunk.blocks,
    outputSchema: { sourceRevision: 'exact input value', segments: [{ anchors: ['ordered source anchors'], text: 'HUMAN_SCHEMA_SENTINEL' }] } };
...
describe('Anthropic Messages request and opaque continuation', () => {
  test('encodes the complete request and collision-free aliases without changing optional tool parameters', () => {
    ...
    expect(wire).not.toHaveProperty('thinking'); expect(wire).not.toHaveProperty('temperature'); expect(wire).not.toHaveProperty('output_config');
  ...
  test('applies translation JSON format with tools while preserving original input and continuation binding', () => {
    ...
    expect(input).toEqual(before);
    const output = turn(input); const next = continued(input, output); const continuedWire = native(encodeAnthropic(next).body);
  ...
  test('explicit legacy-model fallback keeps prompt schema and host translation validation', () => {
    ...
    const value = { sourceRevision: plan.sourceRevision, sourceHash: plan.sourceHash, chunkId: chunk.id,
      segments: [{ anchors: chunk.anchors, text: '그는 사십 년을 근무했다. 계기는 ' + chunk.protectedSpans[0].token + '을 가리켰다.' }] };
    const { decoder } = start(input); begin(decoder); block(decoder, 0, { type: 'text', text: JSON.stringify(value) });
    const response = end(decoder);
    expect(validateTranslationChunk(plan, chunk.id, response.text).segments[0].text).toContain('사십 년');
    value.segments[0].text = value.segments[0].text.replace('사십', '40');
    expect(() => validateTranslationChunk(plan, chunk.id, JSON.stringify(value))).toThrow('UNPROTECTED_SYNTAX_RETURNED');
  });
  test.each(['main', 'status', 'image'] as const)('does not send translation formatting for %s', role => {
    const input = request(); input.role = role; input.generation!.structuredOutput = true;
    const wire = native(encodeAnthropic(input).body);
    expect(wire).not.toHaveProperty('output_config'); expect(wire.system).toHaveLength(2);
  });
  test.each(['sourceRevision', 'sourceHash', 'chunkId'])('rejects native translation without %s', field => {
    const { input } = translationRequest(); delete (input.input.source as Record<string, Json>)[field];
  ...
  test('preserves signed, redacted and opaque content in order across parallel and sequential tool rounds', () => {
    const input = request(); const { decoder } = start(input); begin(decoder);
    block(decoder, 0, { type: 'thinking', thinking: '', signature: '', future_metadata: { binding: 'opaque-original' } }, [
      { type: 'thinking_delta', thinking: 'SYNTHETIC_PRIVATE_THOUGHT' }, { type: 'signature_delta', signature: 'SYNTHETIC_SIGNATURE' },
    ]);
    ...
    expect(output.toolCalls.map(call => [call.id, call.name])).toEqual([['call-A', 'knowledge.read'], ['call-B', 'knowledge.read']]);
    const next = continued(input, output); (next.input.results as Json[]).reverse();
    const { body, decoder: second } = start(next); const wire = native(body);
    const preserved = native(output.opaqueState).messages.at(-1);
    expect(wire.messages[1]).toEqual(preserved);
    expect(wire.messages[1].content[0]).toEqual({ type: 'thinking', thinking: 'SYNTHETIC_PRIVATE_THOUGHT', signature: 'SYNTHETIC_SIGNATURE', future_metadata: { binding: 'opaque-original' } });
    expect(wire.messages[2].content.map((part: any) => part.tool_use_id)).toEqual(['call-A', 'call-B']);
    ...
    begin(second); block(second, 0, toolPart('call-C', 'tool_1_skills_list', {}));
    const secondOutput = end(second, 'tool_use'); const thirdInput = continued(next, secondOutput);
    ...
    (thirdInput.input.results as Json[])[0] = { callId: 'call-B', name: 'knowledge.read', result: { text: 'changed old result' } };
    expect(() => encodeAnthropic(thirdInput)).toThrow('TOOL_RESULT_MISMATCH');
  });
  test.each(['missing', 'extra', 'duplicate', 'id', 'name', 'args', 'denied'])('rejects %s tool result correspondence', mutation => {
    const input = request(); const output = turn(input, [toolPart('call-A'), toolPart('call-B')]);
    const next = continued(input, output); const fresh = next.input.results as Record<string, Json>[];
  ...
  test('encodes a denied result as an error while preserving its payload', () => {
    const input = request(); const next = continued(input, turn(input));
    const first = (next.input.results as Record<string, Json>[])[0]; first.denied = true; first.result = { error: 'SCOPE_DENIED', body: ['unchanged'] };
    const content = native(encodeAnthropic(next).body).messages.at(-1).content[0];
    expect(content.is_error).toBe(true); expect(JSON.parse(content.content)).toEqual(first.result);
  });
  test.each(['input', 'contract', 'generation', 'model', 'tools', 'state'])('rejects changed %s during continuation', mutation => {
    const input = request(); const next = continued(input, turn(input));
    ...
    expect(() => encodeAnthropic(next)).toThrow(mutation === 'state' ? 'INVALID_ANTHROPIC_CONTINUATION' : 'ANTHROPIC_CONTINUATION_MISMATCH');
  });
});
...
describe('Anthropic Messages stream lifecycle and usage', () => {
  test('assembles split tool JSON including escaped strings and nested input before exposing the call', () => {
    ...
    decoder.accept({ type: 'content_block_start', index: 0, content_block: toolPart('call-1', 'tool_0_knowledge_read', {}) });
    const args = { id: '한글 🌊', nested: { quote: 'a"b\\c', flags: [true, null, 4] } };
    const serialized = JSON.stringify(args);
    for (let index = 0; index < serialized.length; index += 3) {
      decoder.accept({ type: 'content_block_delta', index: 0, delta: { type: 'input_json_delta', partial_json: serialized.slice(index, index + 3) } });
    }
  ...
  test('accepts zero-argument tool calls and signature-only thinking without displaying hidden content', () => {
    const { decoder } = start(); begin(decoder);
    block(decoder, 0, { type: 'thinking', thinking: '' }, [{ type: 'signature_delta', signature: 'synthetic-signature-only' }]);
    block(decoder, 1, toolPart('zero-args', 'tool_1_skills_list', {}));
    const output = end(decoder, 'tool_use');
    expect(output.toolCalls[0].arguments).toEqual({}); expect(output.text).toBe('');
  });
  test('rejects reused IDs across rounds and unadvertised wire names', () => {
    const input = request(); const output = turn(input); const { decoder } = start(continued(input, output)); begin(decoder);
    expect(() => block(decoder, 0, toolPart())).toThrow('DUPLICATE_TOOL_ID');
    const fresh = start().decoder; begin(fresh);
    expect(() => block(fresh, 0, toolPart('call-unknown', 'knowledge.read'))).toThrow('ANTHROPIC_UNKNOWN_TOOL');
  });
  test.each(['[1]', 'null', '42', '"string"', '{"unfinished":'])('rejects non-object or incomplete JSON at a tool terminal: %s', partial => {
    const { decoder } = start(); begin(decoder);
    block(decoder, 0, toolPart('call-1', 'tool_0_knowledge_read', {}), [{ type: 'input_json_delta', partial_json: partial }]);
  ...
  test('retains partial status for truncated tool JSON without executing or continuing it', () => {
    const input = request(); const { decoder } = start(input); begin(decoder, { input_tokens: 9, output_tokens: 1 });
    block(decoder, 0, toolPart('truncated', 'tool_0_knowledge_read', {}), [{ type: 'input_json_delta', partial_json: '{"id":"' }]);
    const output = end(decoder, 'max_tokens', { output_tokens: 10 });
    expect(output.status).toBe('partial'); expect(output.error).toEqual({ code: 'MAX_TOKENS' }); expect(output.toolCalls).toEqual([]);
    expect(output.usage.outputTokens).toBe(10);
    expect(() => encodeAnthropic(continued(input, output))).toThrow('TOOL_RESULT_MISMATCH');
  });
  test.each([
    ['end_turn', 'completed', null], ['max_tokens', 'partial', 'MAX_TOKENS'],
    ['model_context_window_exceeded', 'partial', 'CONTEXT_WINDOW_EXCEEDED'], ['pause_turn', 'partial', 'ANTHROPIC_PAUSE_TURN'],
    ['stop_sequence', 'partial', 'ANTHROPIC_STOP_SEQUENCE'], ['refusal', 'refused', null],
  ])('distinguishes %s from successful prose', (reason, status, code) => {
    const { decoder } = start(); begin(decoder); block(decoder, 0, { type: 'text', text: 'Visible provider prose.' });
  ...
  test('recognizes explicit refusal details without requiring visible refusal text', () => {
    const { decoder } = start(); begin(decoder);
    decoder.accept({ type: 'message_delta', delta: { stop_reason: 'end_turn', stop_details: { type: 'refusal', explanation: 'synthetic refusal detail' } } });
    decoder.accept({ type: 'message_stop' });
    expect(decoder.finish()).toMatchObject({ status: 'refused', refusal: 'ANTHROPIC_REFUSAL', text: '' });
  });
  test('requires message_stop and preserves partial text and observed usage on EOF', () => {
    const { decoder } = start(); begin(decoder, { input_tokens: 5, output_tokens: 1 });
    block(decoder, 0, { type: 'text', text: 'Partial prose' });
    decoder.accept({ type: 'message_delta', delta: { stop_reason: 'end_turn' }, usage: { output_tokens: 9 } });
    expect(decoder.snapshot().status).toBe('partial');
    expect(() => decoder.finish()).toThrow('UNEXPECTED_EOF');
    expect(decoder.snapshot()).toMatchObject({ status: 'partial', text: 'Partial prose', error: { code: 'UNEXPECTED_EOF' }, usage: { inputTokens: 5, outputTokens: 9 } });
  });
  test('merges cumulative usage and adds disjoint cache input buckets once, keeping detailed raw counts and null cost', () => {
    const { decoder } = start(); begin(decoder, { input_tokens: 10, cache_creation_input_tokens: 20, cache_read_input_tokens: 30, output_tokens: 1,
      cache_creation: { ephemeral_5m_input_tokens: 7, ephemeral_1h_input_tokens: 13 } });
    block(decoder, 0, { type: 'text', text: 'Prose.' });
    decoder.accept({ type: 'message_delta', delta: { stop_reason: null }, usage: { output_tokens: 4 } });
    const output = end(decoder, 'end_turn', { output_tokens: 9, output_tokens_details: { thinking_tokens: 6 }, service_tier: 'standard' });
    expect(output.usage).toEqual({ inputTokens: 60, outputTokens: 9, costUsd: null, priceRevision: null, raw: {
      input_tokens: 10, cache_creation_input_tokens: 20, cache_read_input_tokens: 30, output_tokens: 9,
      cache_creation: { ephemeral_5m_input_tokens: 7, ephemeral_1h_input_tokens: 13 }, output_tokens_details: { thinking_tokens: 6 }, service_tier: 'standard',
    } });
  });
  test.each([undefined, {}, { input_tokens: null, output_tokens: null }, { input_tokens: 10, cache_read_input_tokens: null }])('keeps absent or explicitly unknown usage unknown: %j', usage => {
    const { decoder } = start(); begin(decoder, usage as Json | undefined); block(decoder, 0, { type: 'text', text: 'Prose.' });
    const output = end(decoder); expect(output.usage.inputTokens).toBeNull(); expect(output.usage.outputTokens).toBeNull(); expect(output.usage.costUsd).toBeNull();
  });
  test.each([{ input_tokens: -1 }, { output_tokens: 0.5 }, { cache_read_input_tokens: '12' }, { output_tokens: Number.MAX_SAFE_INTEGER + 1 }, { cache_creation: { ephemeral_5m_input_tokens: -1 } }])('rejects invalid usage %j', usage => {
    const { decoder } = start(); expect(() => begin(decoder, usage as unknown as Json)).toThrow('INVALID_USAGE');
  });
  test('rejects regressing cumulative usage and arithmetic overflow', () => {
    const first = start().decoder; begin(first, { input_tokens: 3, output_tokens: 7 });
    expect(() => first.accept({ type: 'message_delta', delta: {}, usage: { output_tokens: 6 } })).toThrow('INVALID_USAGE');
    const second = start().decoder;
    expect(() => begin(second, { input_tokens: Number.MAX_SAFE_INTEGER, cache_read_input_tokens: 1 })).toThrow('INVALID_USAGE');
  });
  test('preserves citations and complete opaque provider blocks while exposing only text', () => {
    const input = request(); const { decoder } = start(input); begin(decoder);
    const citation = { type: 'page_location', document_index: 0, start_page_number: 1, end_page_number: 2, cited_text: 'source' };
    block(decoder, 0, { type: 'text', text: '', extra: 'metadata' }, [{ type: 'text_delta', text: 'Visible.' }, { type: 'citations_delta', citation }]);
    block(decoder, 1, toolPart());
    const output = end(decoder, 'tool_use');
    expect(output.text).toBe('Visible.');
    const wire = native(encodeAnthropic(continued(input, output)).body);
    expect(wire.messages[1].content[0]).toEqual({ type: 'text', text: 'Visible.', extra: 'metadata', citations: [citation] });
  });
  test('ignores informational events without letting them mark successful completion', () => {
    const { decoder } = start(); decoder.accept({ type: 'ping' }); decoder.accept({ type: 'future_info', payload: { value: 1 } });
    expect(() => decoder.finish()).toThrow('UNEXPECTED_EOF');
  });
  test.each([
    ['before start', [{ type: 'content_block_stop', index: 0 }], 'ANTHROPIC_INVALID_EVENT_ORDER'],
    ['unknown stop', [{ type: 'message_delta', delta: { stop_reason: 'new_reason' } }], 'ANTHROPIC_UNKNOWN_STOP_REASON'],
    ['missing stop reason', [{ type: 'message_stop' }], 'ANTHROPIC_INVALID_EVENT_ORDER'],
    ['wrong index', [{ type: 'content_block_start', index: 1, content_block: { type: 'text', text: '' } }], 'ANTHROPIC_INVALID_EVENT_ORDER'],
    ['unknown delta', [{ type: 'content_block_start', index: 0, content_block: { type: 'text', text: '' } }, { type: 'content_block_delta', index: 0, delta: { type: 'new_content_delta', data: 'x' } }], 'ANTHROPIC_UNSUPPORTED_CONTENT_DELTA'],
    ['delta after stop', [{ type: 'content_block_start', index: 0, content_block: { type: 'text', text: '' } }, { type: 'content_block_stop', index: 0 }, { type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: 'x' } }], 'ANTHROPIC_INVALID_EVENT_ORDER'],
    ['duplicate terminal', [{ type: 'message_delta', delta: { stop_reason: 'end_turn' } }, { type: 'message_delta', delta: { stop_reason: 'end_turn' } }], 'ANTHROPIC_DUPLICATE_TERMINAL'],
  ])('rejects malformed sequence: %s', (name, events, code) => {
    const { decoder } = start(); if (name !== 'before start') begin(decoder);
    expect(() => (events as Json[]).forEach(event => decoder.accept(event))).toThrow(code);
  });
  test('does not accept tool calls under end_turn or an empty tool_use terminal', () => {
    const first = start().decoder; begin(first); block(first, 0, toolPart());
    expect(() => end(first, 'end_turn')).toThrow('ANTHROPIC_TOOL_TERMINAL_MISMATCH');
    const second = start().decoder; begin(second); block(second, 0, { type: 'text', text: 'No call.' });
  ...
  test('does not replay unsigned thinking or provider errors, and errors never contain raw provider text', () => {
    const first = start().decoder; begin(first); block(first, 0, { type: 'thinking', thinking: 'synthetic unfinished' }); block(first, 1, toolPart());
    expect(() => end(first, 'tool_use')).toThrow('ANTHROPIC_INCOMPLETE_THINKING');
    const second = start().decoder; begin(second, { input_tokens: 17, output_tokens: 2 });
    expect(() => second.accept({ type: 'error', error: { type: 'overloaded_error', message: 'synthetic-secret-canary' } })).toThrow(AnthropicProtocolError);
    const snapshot = second.snapshot();
    expect(snapshot.error).toEqual({ code: 'ANTHROPIC_PROVIDER_ERROR' }); expect(snapshot.usage.inputTokens).toBe(17);
  ...
  test('snapshots are independent copies and late events cannot change a completed message', () => {
    const { decoder } = start(); begin(decoder); block(decoder, 0, { type: 'text', text: 'Original.' });
    const output = end(decoder); native(output.opaqueState).messages[1].content[0].text = 'altered outside';
    expect(decoder.snapshot().text).toBe('Original.');
    expect(() => decoder.accept({ type: 'message_delta', delta: {}, usage: { output_tokens: 9 } })).toThrow('ANTHROPIC_EVENT_AFTER_STOP');
  });
});
