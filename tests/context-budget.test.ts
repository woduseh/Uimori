...
const request = (variant = variants[0] as Variant): ProviderRequest => ({
  ...
  stable: {
    contract: 'Keep the source and current request intact.',
    tools: [
      {
        name: 'knowledge.read',
...
function continued(
  ...
  variant: Variant,
  privateText = 'opaque synthetic provider state'
): ProviderRequest {
  ...
  if (variant.protocol === 'openai-responses-v1') {
    ...
    const parts = [
      { type: 'thinking', thinking: 'synthetic reasoning', signature: privateText },
      { type: 'tool_use', id: 'call-budget', name: 'tool_0_knowledge_read', input: {} },
    ];
    for (const [index, content_block] of parts.entries()) {
      decoder.accept({ type: 'content_block_start', index, content_block });
      ...
      candidates: [
        {
          ...
          content: {
            ...
            parts: [
              {
                functionCall: { id: 'call-budget', name: 'knowledge.read', args: {} },
      ...
      choices: [
        {
          ...
          delta: {
            role: 'assistant',
            reasoning_content: privateText,
            tool_calls: [
              {
                index: 0,
                id: 'call-budget',
                type: 'function',
                function: { name: 'tool_0_knowledge_read', arguments: '{}' },
              },
            ],
          },
  ...
  return {
    ...
    input: {
      ...
      results: result.toolCalls.map((call) => ({
        callId: call.id,
        name: call.name,
        args: call.arguments,
        result: { found: true },
...
describe('offline host context budget', () => {
  ...
  test('validates host policy strictly, preserves explicit limits and applies the model default', () => {
    expect(contextBudgetForModel({})).toEqual({
    ...
    expect(() =>
      contextBudgetForModel({ inputTokenLimit: null } as unknown as { inputTokenLimit: number })
    ).toThrow('INVALID_CONTEXT_BUDGET');
    for (const invalid of [
      ...
      { ...budget(), extra: true },
      { ...budget(), estimator: 'provider-exact' },
      ...[0, 8191, 1000001, 8192.5, NaN, Infinity, '8192'].map((inputTokenLimit) => ({
        ...budget(),
        inputTokenLimit,
      })),
    ]) {
      expect(() => validateContextBudget(invalid)).toThrow('INVALID_CONTEXT_BUDGET');
      expect(() => validateRequest({ ...request(), contextBudget: invalid })).toThrow(
        'INVALID_CONTEXT_BUDGET'
      );
    }
    const checked = validateRequest(request());
    expect(checked.contextBudget).toEqual(budget());
    expect(() =>
      validateRequest({
        ...request(),
        generation: { ...request().generation, inputTokenLimit: 8192 },
      })
  ...
  test('accepts the inclusive boundary and never mutates oversized payloads or silently imposes an absent policy', () => {
    ...
    expect(() => assertContextBudget(body, { ...budget(), inputTokenLimit: count - 1 })).toThrow(
      'INPUT_CONTEXT_LIMIT_EXCEEDED'
    );
    expect(() => assertContextBudget(body)).not.toThrow();
  ...
  test.each([
    ...
    '$protocol reserves the selected output inside the reviewed native window without rewriting settings',
    ({ modelId, protocol, output, effective }) => {
      const model = {
      ...
      expect(contextBudgetForModel({ ...model, maxOutputTokens: undefined }).inputTokenLimit).toBe(
        effective
      );
      expect(contextBudgetForModel({ ...model, inputTokenLimit: 8192 }).inputTokenLimit).toBe(8192);
      expect(contextBudgetForModel({ ...model, inputTokenLimit: undefined }).inputTokenLimit).toBe(
        272000
      );
  ...
  test('does not invent a native window for Codex, a routed model or an unknown model ID', () => {
    for (const connection of [
      ...
      expect(contextBudgetForModel({ modelId: 'gpt-6-astra', connection }).inputTokenLimit).toBe(
        272000
      );
      expect(
        contextBudgetForModel({
...
describe('final wire body budget before credentials and transmission', () => {
  ...
  test.each(variants.slice(0, -1))(
    '$protocol measures the native body, not duplicated raw snapshot fields',
    async (variant) => {
      const input = request(variant);
        ...
        messages: [
          {
            id: 'current',
            role: 'user',
            content: [{ type: 'text', text: input.input.task }],
            completion: 'complete',
            provenance: { blockId: 'current', origin: 'current' },
          },
        ],
      };
      expect(estimateContextTokens(input)).toBeGreaterThan(8192);
      expect(estimateContextTokens(encoded(input, variant).body)).toBeLessThan(8192);
      const fetch = vi.fn(async (_url: unknown, init?: RequestInit) => {
        const body = JSON.parse(String(init?.body));
        expect(body).not.toHaveProperty('contextBudget');
        expect(JSON.stringify(body)).not.toContain(large);
        return new Response(null, { status: 503 });
      });
      const resolveCredential = vi.fn(() => 'synthetic-secret'),
        onWire = vi.fn();
      vi.stubGlobal('fetch', fetch);
      const result = await executeProvider(connection(variant), input, {
        signal: new AbortController().signal,
  ...
  test.each(variants.slice(0, -1))(
    '$protocol includes unredacted opaque continuation content in the budget before any wire work',
    async (variant) => {
      const input = continued(request(variant), variant, large),
        before = structuredClone(input);
      expect(estimateContextTokens(input.input)).toBeLessThan(8192);
      expect(estimateContextTokens(encoded(input, variant).body)).toBeGreaterThan(8192);
      const fetch = vi.fn(),
        resolveCredential = vi.fn(() => 'synthetic-secret'),
        onWire = vi.fn();
      vi.stubGlobal('fetch', fetch);
  ...
  test.each(variants.slice(0, -1))(
    ...
    (variant) => {
      ...
      const initial = request(variant);
      delete initial.contextBudget;
      const unbudgeted = continued(initial, variant);
      expect(() => encoded({ ...unbudgeted, contextBudget: budget() }, variant)).toThrow(
        /CONTINUATION_MISMATCH/
      );
  ...
  test('fixture continuation content is counted before credentials and journaling', async () => {
    ...
    input.opaqueState = { providerContinuation: large };
    const fetch = vi.fn(),
      resolveCredential = vi.fn(() => 'synthetic-secret'),
      onWire = vi.fn();
    vi.stubGlobal('fetch', fetch);
  ...
  test('Codex rejects its assembled instructions, input and schema before creating session state or a process', async () => {
    ...
    const runtime = new CodexRuntime(dbPath, {
      enabled: true,
      launch: { command: process.execPath, args: [resolve('tests/fixtures/codex-app-server.mjs')] },
    });
    const input = request();
    input.stable.contract = large;
    const before = structuredClone(input),
    ...
    try {
      ...
      expect(existsSync(`${dbPath}.codex`)).toBe(false);
      expect(input).toEqual(before);
    } finally {
      await runtime.close();
    }
