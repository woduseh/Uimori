...
import { parseHiddenStory, validateHiddenTranslation } from '../core/hidden-story.js';
...
const text = 'The traveler waited.\n\n@hsTitle: A quiet memory\n⟦harbor @ dusk @ keeper⟧\n[hsPortrait: asset:keeper-profile]\nThe keeper remembered an unopened letter.\n@hs\n\nThe traveler walked away.';
const hash = (value: string) => createHash('sha256').update(value).digest('hex');
const program: PromptProgram = { version: 1, controls: [{ id: 'style', label: 'Style', type: 'select', options: [{ label: 'Soft', value: 'soft' }, { label: 'Precise', value: 'precise' }], default: 'soft' }], blocks: [
  { id: 'instructions', title: 'Translation', kind: 'message', role: 'system', template: [{ kind: 'text', text: 'Translate faithfully. Style=' }, { kind: 'value', expression: { control: 'style' } }] },
  { id: 'source', title: 'Requested blocks', kind: 'slot', slot: 'source', role: 'user' },
  { id: 'source-cache', title: 'Cache source', kind: 'cache', depth: 1, role: 'user', policy: 'prefer' },
  { id: 'receipt', title: 'Receipt', kind: 'message', role: 'assistant', template: [{ kind: 'text', text: 'I will preserve the protected tokens.' }] },
  { id: 'current', title: 'Translation task', kind: 'current' },
] };
function bundle(sourceText = text): AuxiliaryBundle {
  const source = { id: 'native-source', chatId: 'native-chat', text: sourceText, hash: hash(sourceText) };
  return { job: { id: 'native-job', kind: 'translation', status: 'queued', sourceRevision: source.id, sourceHash: source.hash }, source, snapshot: {
...
function echo(plan: TranslationPlan, chunkIndex = 0): TranslationResult {
  ...
  return validateTranslationChunk(plan, chunk.id, { sourceRevision: plan.sourceRevision, sourceHash: plan.sourceHash, chunkId: chunk.id, segments: chunk.blocks.map(block => ({ anchors: [block.anchor], text: block.text })) });
}
function bridge(seed: AuxiliaryBundle) {
  const state = structuredClone(seed); const chunks = new Map<string, AuxiliaryChunkRecord>(); const outcomes: AuxiliaryOutcome[] = [];
  const store: AuxiliaryStoreBridge = {
    load: () => structuredClone({ ...state, chunks: [...chunks.values()] }),
    claim: (_job, _owner, prepared) => { state.plan ??= prepared.plan; for (const chunk of prepared.plan?.chunks ?? []) if (!chunks.has(chunk.id)) chunks.set(chunk.id, { id: chunk.id, status: 'queued', attempt: 0, result: null }); return 1; },
    beginChunk: (_job, id) => { const chunk = chunks.get(id)!; chunk.status = 'running'; chunk.attempt++; },
    completeChunk: (_job, id, _generation, _owner, result) => { Object.assign(chunks.get(id)!, { status: 'completed', result: structuredClone(result) }); },
    failChunk: (_job, id, _generation, _owner, error, status) => { Object.assign(chunks.get(id)!, { error, status }); },
    finish: (_job, _generation, _owner, result) => { outcomes.push(structuredClone(result)); },
  };
  return { store, state, chunks, outcomes };
}
const cleanups: (() => Promise<void>)[] = [];
afterEach(async () => { for (const close of cleanups.splice(0)) await close(); });
const hooks = (origin: string): AuxiliaryJobHooks => ({ signal: new AbortController().signal, approvedOrigins: [origin], authorize: connection => connection, onAttemptStart: () => 'native-translation-attempt', onAttemptFinish: () => {} });
...
describe('native translation prompt and hidden source boundaries', () => {
  test('local scripted translation keeps hidden line boundaries across chunks without altering the source', async () => {
    const sourceText = 'Main arrival.\n\n@hsTitle: Quiet Tower\n⟦Tower @ Dawn @ Mira⟧\nMira recalls a blue bell.\n@hs\n\nMain crossing.\n\n@hsTitle: Old Letter\nAnother traveler remembers winter.\n@hs\n\nMain departure.';
    const seed = bundle(sourceText); const original = structuredClone(seed.source); const state = bridge(seed); let attempts = 0;
    const options = hooks('http://127.0.0.1:1'); options.maxChunkChars = 100; options.onAttemptStart = () => { attempts++; return 'unexpected-provider-attempt'; };
    const outcome = await runAuxiliaryJob(state.store, seed.job.id, 'owner', options);
    expect(state.chunks.size).toBeGreaterThan(1);
    expect(outcome).toMatchObject({ status: 'completed', error: null, result: { mock: true, sourceRevision: seed.source.id, sourceHash: seed.source.hash, text: sourceText } });
    expect(validateHiddenTranslation({ sourceRevision: seed.source.id, sourceHash: seed.source.hash, text: sourceText }, outcome!.result!.text!).ok).toBe(true);
    expect(parseHiddenStory({ sourceRevision: seed.source.id, sourceHash: seed.source.hash, text: outcome!.result!.text! }).segments.map(segment => segment.kind)).toEqual(['main','hidden','main','hidden','main']);
    expect(seed.source).toEqual(original); expect(attempts).toBe(0);
    // Plain prose retains the explicit mock label; only structured hidden sources echo.
    const plain = bundle('Plain synthetic source.'); const plainState = bridge(plain); const plainOutcome = await runAuxiliaryJob(plainState.store, plain.job.id, 'owner', options);
    expect(plainOutcome?.result?.text).toBe('[모의 번역 · 의미 품질 미검증] Plain synthetic source.');
  });
  test('protects hidden delimiters, portrait identity and scene separators without omitting prose', () => {
    const seed = bundle(); const context = sourceTimeContext(seed.snapshot, 'translation'); const before = JSON.stringify(context);
    const plan = createTranslationPlan(seed.source, context, 100);
    expect(plan.context.protectedLiterals).toEqual(expect.arrayContaining(['@hsTitle:', '@hs', '⟦', '@', '⟧', '[hsPortrait: asset:keeper-profile]']));
    const protectedText = plan.chunks.flatMap(chunk => chunk.blocks).map(block => block.text).join('\n\n');
    expect(protectedText).toContain('The keeper remembered an unopened letter.');
    expect(protectedText).not.toContain('@hsTitle:'); expect(protectedText).not.toContain('asset:keeper-profile');
    const translated = aggregateTranslation(plan, plan.chunks.map((_, index) => echo(plan, index))).segments.map(segment => segment.text).join('\n\n');
    expect(translated).toBe(seed.source.text);
    expect(validateHiddenTranslation({ sourceRevision: seed.source.id, sourceHash: seed.source.hash, text: seed.source.text }, translated).ok).toBe(true);
  ...
  test('source-time provenance separates reader text, actor knowledge and world truth as unknown', () => {
    const seed = bundle(); const plan = createTranslationPlan(seed.source, sourceTimeContext(seed.snapshot, 'translation'));
    const hidden = plan.context.hiddenKnowledge!.segments.find(segment => segment.kind === 'hidden')!;
    expect(plan.context.hiddenKnowledge).toMatchObject({ sourceRevision: seed.source.id, sourceHash: seed.source.hash, provenance: 'source-markers' });
    expect(hidden).toMatchObject({ readerExposure: 'present-in-source', worldTruth: 'unknown', actorKnowledge: { status: 'unknown', mode: 'unspecified', perspectiveActorIds: null, knownByActorIds: null, evidence: [] } });
    expect(hidden.range).toEqual(parseHiddenStory({ sourceRevision: seed.source.id, sourceHash: seed.source.hash, text: seed.source.text }).segments.find(segment => segment.kind === 'hidden')!.range);
    const forged = structuredClone(plan); forged.context.hiddenKnowledge!.segments[0].worldTruth = 'asserted' as 'unknown';
    expect(() => validateTranslationPlan(seed.source, sourceTimeContext(seed.snapshot, 'translation'), forged)).toThrow('SOURCE_TRANSLATION_PLAN_INVALID');
  });
  test('compiles exactly the frozen translation revision, ordered roles/cache and one current task', () => {
    const seed = bundle(); const plan = createTranslationPlan(seed.source, sourceTimeContext(seed.snapshot, 'translation'));
    const input = translationInput(plan, plan.chunks[0].id, seed.snapshot); const compiled = compileTranslationPrompt(input, seed.snapshot, 'Translate this chunk.')!;
    expect(compiled.values).toEqual({ style: 'precise' });
    expect(compiled.messages.map(message => message.role)).toEqual(['system', 'user', 'assistant', 'user']);
    ...
    expect(compiled.messages.filter(message => message.provenance.origin === 'current')).toHaveLength(1);
    expect(compiled.cachePlan).toEqual([{ blockId: 'source-cache', afterMessageId: 'source', policy: 'prefer' }]);
    expect(JSON.stringify(compiled)).not.toMatch(/MAIN_TASK_MUST_NOT_REPLAY|MAIN_HISTORY_MUST_NOT_REPLAY|LEGACY_FALLBACK_MUST_NOT_OVERRIDE/);
  ...
  test('actual loopback requests preserve composed messages across source-time tools and freeze later mutations', async () => {
    ...
    const outcome = await runAuxiliaryJob(state.store, seed.job.id, 'owner', options);
    expect(outcome).toMatchObject({ status: 'completed', error: null, result: { text, mock: true } });
    expect(local.requests).toHaveLength(2);
    const bodies = local.requests.map(request => JSON.parse(request.body));
    expect(bodies[0].stable.contract).toBe(''); expect(bodies[0].prompt.values).toEqual({ style: 'precise' });
    expect(bodies[1].prompt).toEqual(bodies[0].prompt); expect(bodies[1].opaqueState).toEqual({ cursor: 'synthetic-source-time' });
    expect(bodies[1].input.results[0].callId).toBe('lookup');
    expect(observed[0].context.hiddenKnowledge!.segments.some(segment => segment.kind === 'hidden')).toBe(true);
  });
  test('a fully mapped result with malformed hidden line placement fails instead of being published completed', async () => {
    const seed = bundle(); const local = await loopbackProvider(async (request, response) => {
