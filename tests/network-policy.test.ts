...
afterEach(async () => {
  for (const item of owned.splice(0).reverse()) {
    ...
    if (
      isAbsolute(within) ||
      within.startsWith('..') ||
      !basename(target).startsWith('uimori-network-')
    )
...
async function fixture(remote = true) {
  const item = {
    directory: await mkdtemp(join(tmpdir(), 'uimori-network-')),
    app: undefined as App | undefined,
  };
  owned.push(item);
  item.app = await createApp({
    dbPath: join(item.directory, 'test.sqlite'),
    buildId: 'network-synthetic',
    ...(remote ? { publicOrigin, accessToken } : {}),
  });
  return item.app;
}
...
describe('self-host network policy', () => {
  ...
  it.each([
    '',
    'http://story.example.test',
    'https://user:password@story.example.test',
    'https://story.example.test/chat',
    'https://story.example.test/path/..',
    'https://story.example.test?',
    'https://story.example.test#',
    'https://story.example.test:0',
    'https://story.example.test,https://other.test',
    ' https://story.example.test',
    'https://story.example.test\\',
  ...
  it('canonicalizes the configured host/default port but grants no wildcard or subpath', () => {
    expect(networkPolicy({ publicOrigin: 'https://STORY.example.test:443/', accessToken })).toEqual(
      policy
    );
    expect(networkPolicy({ publicOrigin: 'https://[::1]:443/', accessToken }).publicHost).toBe(
      '[::1]'
    );
    expect(
      deniedBrowserRequest(policy, { method: 'GET', headers: { host: 'STORY.example.test:443' } })
    ).toBeUndefined();
    for (const host of [
      '127.0.0.1',
      'other.test',
      'story.example.test.evil.test',
      'story.example.test:444',
      'story.example.test@evil.test',
      'story.example.test/path',
      'story.example.test#x',
      'story.example.test\\evil',
      'story.example.test,evil.test',
      'story.example.test%2f',
    ]) {
  ...
  it('requires same-origin mutations and denies foreign/null origins and cross-site reads', () => {
    for (const method of ['POST', 'PUT', 'PATCH', 'DELETE']) {
      for (const origin of [
        ...
        expect(
          deniedBrowserRequest(policy, { method, headers: { host: 'story.example.test', origin } })
        ).toBeTruthy();
      }
      expect(
        deniedBrowserRequest(policy, {
          method,
          headers: { host: 'story.example.test', origin: publicOrigin },
        })
      ).toBeUndefined();
    }
    expect(
      deniedBrowserRequest(policy, { method: 'GET', headers: { host: 'story.example.test' } })
    ).toBeUndefined();
    expect(
      deniedBrowserRequest(policy, {
        method: 'GET',
        headers: { host: 'story.example.test', 'sec-fetch-site': 'cross-site' },
      })
    ).toBeTruthy();
  });
  it('enforces policy on actual session/data routes and never trusts forwarded host or protocol', async () => {
    ...
    for (const badHeaders of [
      {
        host: 'localhost',
        origin: publicOrigin,
        'x-forwarded-host': 'story.example.test',
        'x-forwarded-proto': 'https',
      },
      {
        host: 'story.example.test',
    ...
    const create = await app.inject({
      ...
      url: '/api/chats',
      headers: authed,
      payload: { title: 'Self-host synthetic story', botId: owner.json().id },
    });
    expect(create.statusCode).toBe(200);
    expect((await app.inject({ url: '/api/health', headers: authed })).json().mode).toBe(
      'self-host'
    );
    ...
    expect(
      (await app.inject({ method: 'DELETE', url: '/api/session', headers: authed })).statusCode
    ).toBe(200);
    ...
    expect(
      (
        await app.inject({
          ...
          url: '/api/chats',
          headers: { host: '127.0.0.1' },
          payload: { title: 'Local fixture', botId: owner.json().id },
        })
      ).statusCode
    ).toBe(200);
    expect(
      (await app.inject({ url: '/api/chats', headers: { host: 'external.test' } })).statusCode
    ).toBe(403);
  });
});
