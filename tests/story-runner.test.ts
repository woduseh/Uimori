...
function bundle(
      ...
      module: {
        ...
        fields: {
          coins: {
            ...
            min: 0,
            max: 100,
            description: 'spend-three subtracts 3',
          },
        },
  ...
  const snapshot: RunSnapshot = {
    ...
    parentRevision: null,
    settingsRevision: 1,
    settings: { preset: 'calm', mode: 'direct', translation: false, status: true, maxCalls: 3 },
    request: 'Continue',
    history: [],
    story,
    resources: [
      {
        ...
        revision: 3,
        title: 'Coin rules',
        description: 'Read further guidance',
        text: 'A coin is a local copper unit.',
      },
      {
        id: 'hidden',
        chatId: 'chat-2',
...
const finish = (output: unknown): Json[] => [
...
describe('M2 story runner actual localhost request evidence (synthetic, no live provider quality claim)', () => {
  test('S01 actual provider receives original source, rules and previous state; tool results and opaque continuation stay correctly scoped', async () => {
    ...
    const server = await fixture(async (request, response) => {
      ...
      expect(wire.role).toBe('state');
      expect(wire.modelId).toBe('fixture-state-selected');
      expect(wire.input.source.text).toBe(work.source.text);
      expect(wire.input.source.previousState).toEqual({ coins: 10 });
      expect(wire.input.source.module.rules['spend-three'].delta).toBe(-3);
      expect(wire.stable.contract).toContain('additionalProperties');
      expect(request.body).not.toContain('HIDDEN_CANARY');
      if (count === 1)
        await writeSse(response, [
          {
            ...
            id: 'read-1',
            name: 'knowledge.read',
            argumentsDelta: '{"id":"rule-lore"}',
          },
          { type: 'opaque_state', state: { continuation: 'OPAQUE_SECRET_CANARY' } },
    ...
    expect(log.attempts.every((attempt) => attempt.result)).toBe(true);
    expect(JSON.stringify(log)).not.toContain('OPAQUE_SECRET_CANARY');
    expect(work.snapshot.story!.state!.values.coins).toBe(10);
  });
  ...
  test('S01 provider no-facts empty proposal is preserved without inventing numeric changes', async () => {
    const server = await fixture(async (_request, response) => {
      await writeSse(
        response,
        finish({
          sourceRevision: work.source.id,
          sourceHash: work.source.hash,
          moduleRevision: 1,
          operations: [],
        })
      );
    });
    const work = bundle('state', server.endpoint, 'Mira waited.');
    const log = observed(server.origin);
    expect((await runStoryJob(work, log.hooks)).result).toMatchObject({ operations: [] });
  ...
  test('S03 malformed terminal JSON and invalid evidence preserve usage with no automatic retry', async () => {
    ...
    expect(await runStoryJob(work, log.hooks)).toMatchObject({
      status: 'failed',
      error: 'STORY_OUTPUT_JSON_INVALID',
    });
    expect(log.attempts[0].result!.usage.inputTokens).toBe(11);
    expect(server.requests).toHaveLength(1);
    const invalid = stateResult(work.source);
    invalid.operations[0].evidence.quote = 'invented';
    output = JSON.stringify(invalid);
    expect(await runStoryJob(work, log.hooks)).toMatchObject({
      status: 'failed',
      error: 'STATE_EVIDENCE_QUOTE_INVALID',
    });
  ...
  test('S03 duplicate calls are rejected before reads; connection revocation and budget block the next fetch', async () => {
    ...
    duplicate = false;
    let auth = 0;
    const revoked = observed(server.origin, {
      authorize: (value) => ({ ...value, enabled: ++auth === 1 }),
    });
    expect((await runStoryJob(work, revoked.hooks)).error).toBe('CONNECTION_NOT_AUTHORIZED');
    expect(revoked.attempts).toHaveLength(1);
    work.snapshot.settings.maxCalls = 1;
    const budget = observed(server.origin);
    expect((await runStoryJob(work, budget.hooks)).error).toBe('MODEL_CALL_BUDGET_EXHAUSTED');
          ...
          ? [
              {
                ...
                id: 'denied',
                name: 'knowledge.read',
                argumentsDelta: '{"id":"hidden"}',
              },
              { type: 'done', reason: 'tool_calls' },
            ]
          : [
              { type: 'refusal', message: 'Synthetic refusal' },
  ...
  test('S03 repeated tool IDs across turns are rejected and uncertain partial output is never replayed', async () => {
    ...
    const server = await fixture(async (_request, response) => {
      if (partial) {
        ...
        response.end('data: {"type":"text_delta","delta":"{partial"}\n\n');
        return;
      }
      await writeSse(response, [
        {
    ...
    const work = bundle('state', server.endpoint);
    const log = observed(server.origin);
    expect((await runStoryJob(work, log.hooks)).error).toBe('DUPLICATE_TOOL_ID');
    expect(log.events).toHaveLength(1);
    expect(server.requests).toHaveLength(2);
    partial = true;
    const uncertain = observed(server.origin);
    expect(await runStoryJob(work, uncertain.hooks)).toMatchObject({
      status: 'interrupted',
      result: null,
    });
    expect(uncertain.attempts).toHaveLength(1);
    expect(uncertain.attempts[0].result).toMatchObject({ status: 'partial' });
    expect(server.requests).toHaveLength(3);
  });
  ...
  test('S01 R03 mock is explicit: numeric events need literal markers and memory is extractive with safe UTF16 evidence', async () => {
    ...
    expect(await runStoryJob(noEvent, log.hooks)).toMatchObject({
      mock: true,
      result: { operations: [] },
    });
    const marked = bundle('state', undefined, '[[event:spend-three]] [[event:unknown]]');
    expect((await runStoryJob(marked, log.hooks)).result).toMatchObject({
      operations: [
        {
          kind: 'event',
          event: 'spend-three',
  ...
  test('S03 stale source and pre-aborted jobs cannot reach provider or leak abort reason', async () => {
    const work = bundle();
    work.job.sourceHash = 'wrong';
    const log = observed();
    expect((await runStoryJob(work, log.hooks)).error).toBe('STORY_SOURCE_DEPENDENCY_MISMATCH');
    expect(log.inputs).toHaveLength(0);
    const controller = new AbortController();
    controller.abort('PRIVATE_ABORT_CANARY');
    expect(await runStoryJob(bundle(), { ...log.hooks, signal: controller.signal })).toEqual({
      status: 'interrupted',
      result: null,
      error: 'CANCELLED',
      mock: true,
