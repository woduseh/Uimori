...
const dirs: string[] = [],
  apps: App[] = [];
const directory = () => {
  const path = mkdtempSync(join(tmpdir(), 'uimori-vertex-file-'));
  dirs.push(path);
  return path;
};
const account = {
...
const appAt = async (path: string, accessToken?: string) => {
...
afterEach(async () => {
  ...
  for (const path of dirs.splice(0)) {
    ...
    if (
      isAbsolute(within) ||
      within.startsWith('..') ||
      !basename(path).startsWith('uimori-vertex-file-')
    )
...
test('upload persists normalized file outside SQLite and survives app restart without network', async () => {
  ...
  const oauth = vi
    ...
    .mockRejectedValue(new Error('OAuth forbidden'));
  const db = join(directory(), 'test.sqlite');
  let app = await appAt(db);
  const uploaded = await injectWithFixtureBot(app, {
    method: 'POST',
    url: '/api/provider-management/vertex-credentials',
    payload: { serviceAccount: { ...account, extra: 'discarded' } },
  });
  ...
  const saved = await injectWithFixtureBot(app, {
    method: 'POST',
    url: '/api/connections',
    payload: {
      title: 'JSON Vertex',
      ...
      endpoint,
      credentialEnv: reference.credentialEnv,
      enabled: true,
    },
  });
  expect(saved.statusCode).toBe(200);
  const connection = saved.json<Connection>();
  const files = readdirSync(db + '.vertex-credentials');
  expect(files).toHaveLength(1);
  const filename = join(db + '.vertex-credentials', files[0]!);
  expect(JSON.parse(readFileSync(filename, 'utf8'))).not.toHaveProperty('extra');
  if (process.platform !== 'win32') expect(statSync(filename).mode & 0o777).toBe(0o600);
  expect(
    (
      await injectWithFixtureBot(
        app,
        `/api/provider-management/connections/${connection.id}/readiness`
      )
    ).json()
  ).toMatchObject({ credentialStatus: 'configured' });
  for (const url of ['/api/library', '/api/export']) {
    const result = await injectWithFixtureBot(app, url);
    expect(result.statusCode).toBe(200);
    expect(result.body).not.toContain(account.private_key);
...
test('resolver mints only for matching Vertex project and app store; token failures are sanitized', async () => {
  const db = join(directory(), 'first.sqlite'),
    store = new VertexCredentialStore(db),
    ref = store.upload(account).credentialEnv;
  const connection = {
    id: 'synthetic',
  ...
  expect(await store.resolve(ref, connection, signal)).toBe('synthetic-oauth-token');
  expect(token).toHaveBeenCalledTimes(1);
  await expect(
    store.resolve(ref, { ...connection, protocol: 'openai-chat-v1' }, signal)
  ).rejects.toThrow('CREDENTIAL_UNAVAILABLE');
  await expect(
    store.resolve(
      ref,
      { ...connection, endpoint: endpoint.replace('synthetic-project', 'another-project') },
      signal
    )
  ).rejects.toThrow('CREDENTIAL_UNAVAILABLE');
  await expect(
    new VertexCredentialStore(join(directory(), 'other.sqlite')).resolve(ref, connection, signal)
  ).rejects.toThrow('CREDENTIAL_UNAVAILABLE');
  ...
  expect(() =>
    validateConnection(
      { ...connection, protocol: 'openai-chat-v1', endpoint: 'http://127.0.0.1:1234/v1' },
      ['http://127.0.0.1:1234']
    )
...
test('upload is guarded and hostile service accounts never reach GoogleAuth or storage', async () => {
  ...
  const db = join(directory(), 'guard.sqlite'),
    app = await appAt(db, 'synthetic-access-token');
  const url = '/api/provider-management/vertex-credentials',
    payload = { serviceAccount: account };
  expect((await injectWithFixtureBot(app, { method: 'POST', url, payload })).statusCode).toBe(401);
  const login = await injectWithFixtureBot(app, {
    method: 'POST',
    url: '/api/session',
    payload: { token: 'synthetic-access-token' },
  });
  ...
  for (const change of [
    ...
    const result = await injectWithFixtureBot(app, {
      ...
      url,
      headers: { cookie },
      payload: { serviceAccount: { ...account, ...change } },
    });
  ...
  expect(
    (
      await injectWithFixtureBot(app, {
        ...
        url,
        headers: { cookie },
        payload: { serviceAccount: { ...account, extra: 'x'.repeat(70 * 1024) } },
      })
    ).statusCode
  ).toBe(413);
  expect(oauth).not.toHaveBeenCalled();
  const uploaded = (
    await injectWithFixtureBot(app, { method: 'POST', url, headers: { cookie }, payload })
  ).json();
  const bad = await injectWithFixtureBot(app, {
    method: 'POST',
    url: '/api/connections',
    headers: { cookie },
    payload: {
      title: 'Wrong project',
      protocol: 'vertex-gemini-v1',
      endpoint: endpoint.replace('synthetic-project', 'another-project'),
      credentialEnv: uploaded.credentialEnv,
      enabled: true,
    },
  });
