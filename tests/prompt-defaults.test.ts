...
describe('single prompt program defaults at real native encoder boundaries',()=>{
  test.each(protocols)('%s keeps default main references and compiles explicit instructions through the same path',protocol=>{
    const seed=snapshot(protocol);
    for(const custom of [false,true]){
      if(custom)seed.profile!.promptPresets={main:{id:'prompt',revision:1,title:'Simple',role:'main',program:createDefaultPromptProgram('Literal {{char}}')}};
      const built=buildMainProviderRequest(seed); const before=structuredClone(built.request.prompt);
      const preview=encodeMainPreview(built.request,seed.profile!.models.main!);
      expect(built.request.stable.contract).toBe('');
      expect(built.request.prompt!.messages[0].content[0].text).toBe(custom?'Literal {{char}}':DEFAULT_MAIN_PROMPT);
      expect(JSON.stringify(preview.body)).toContain('Pinned module evidence.');
      expect(JSON.stringify(preview.body)).toContain(createHash('sha256').update('Pinned module evidence.').digest('hex'));
      expect(built.request.prompt).toEqual(before);
    }
  });
  test.each(protocols)('%s compiles default translation with exact source context/schema and no main history',protocol=>{
    const seed=snapshot(protocol),source={id:'source',chatId:seed.chatId,text:'Quiet harbor.',hash:createHash('sha256').update('Quiet harbor.').digest('hex')};
    seed.history=[{revision:'main-history',text:'MAIN HISTORY MUST NOT LEAK'}];
    const plan=createTranslationPlan(source,sourceTimeContext(seed,'translation')),input=translationInput(plan,plan.chunks[0].id,seed);
    const compilation=compileTranslationPrompt(input,seed,'Translate this chunk.')!;
    const request:ProviderRequest={role:'translation',modelId:seed.profile!.models.translation!.modelId,generation:{maxOutputTokens:1024,temperature:null},stable:{contract:'',tools:[]},prompt:{compilerVersion:compilation.compilerVersion,messages:compilation.messages,cachePlan:compilation.cachePlan,values:compilation.values},input:{task:'Translate this chunk.',controls:{},source:{sourceRevision:source.id,sourceHash:source.hash,chunkId:input.chunkId!,blocks:input.blocks,outputSchema:JSON.parse(JSON.stringify(input.outputSchema))},results:[]}};
    const body=protocol==='vertex-gemini-v1'?encodeVertex(request).body:protocol==='anthropic-messages-v1'?encodeAnthropic(request).body:protocol==='openai-responses-v1'?encodeResponses(request).body:encodeChat(request).body;
    ...
    expect(JSON.stringify(body)).toContain(source.hash);expect(JSON.stringify(body)).toContain('outputSchema');
    if(protocol==='vertex-gemini-v1'){
      const mapped=planNativeMessages(request,protocol)!;
      expect(mapped.messages).toHaveLength(1);
      expect(mapped.diagnostics.some(d=>d.code==='CONSECUTIVE_ROLE_PARTS_COMBINED')).toBe(true);
      expect((mapped.messages[0] as {parts:unknown[]}).parts).toHaveLength(compilation.messages.filter(m=>m.role==='user').length);
    }
  });
});
