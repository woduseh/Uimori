...
test('diagnostic lists omit payloads and single-attempt detail preserves stored payload and unknown cost', async () => {
  ...
  if (
    isAbsolute(within) ||
    within.startsWith('..') ||
    !basename(target).startsWith('uimori-diagnostics-test-')
  )
  ...
  try {
    const chat = createFixtureChat(store, 'Synthetic attempts');
    const other = createFixtureChat(store, 'Other synthetic chat');
    ...
    const summary = await injectWithFixtureBot(app, `/api/chats/${chat.id}/attempts`);
    ...
    const detail = await injectWithFixtureBot(app, `/api/attempts/${id}`);
    ...
    expect((await injectWithFixtureBot(app, `/api/chats/${chat.id}/jobs`)).json()).toEqual([]);
  } finally {
    await app.close();
    store.close();
