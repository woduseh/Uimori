...
afterEach(async () => {
  for (const item of owned.reverse()) {
    if (item.child && item.child.exitCode === null && item.child.signalCode === null) { item.child.kill(); await new Promise<void>(resolve => item.child!.once('exit', () => resolve())); }
    await item.app?.close();
    const target = resolve(item.directory); const within = relative(resolve(tmpdir()), target);
    if (isAbsolute(within) || within.startsWith('..') || !basename(target).startsWith('서사 M0 ')) throw new Error('Refusing cleanup outside owned test directory');
    await rm(target, { recursive: true, force: true });
  }
  owned.length = 0;
});
async function api<T = any>(url: string, path: string, body?: unknown, method = body === undefined ? 'GET' : 'POST', expected = 200): Promise<T> {
  ...
  const value = await response.json(); expect(response.status, JSON.stringify(value)).toBe(expected); return value as T;
}
async function until<T>(read: () => Promise<T>, predicate: (value: T) => boolean, timeout = 5000): Promise<T> {
  const deadline = Date.now() + timeout;
  let value: T;
  do { value = await read(); if (predicate(value)) return value; await new Promise(resolve => setTimeout(resolve, 10)); } while (Date.now() < deadline);
...
async function setup(testMode = true) {
  ...
  const app = await createApp({ dbPath: join(directory, 'story.sqlite'), instanceId: randomUUID(), buildId: 'unit-integration', testMode });
  owned.push({ app, directory });
  const url = await app.listen({ port: 0, host: '127.0.0.1' });
  const chat = await api<Chat>(url, '/api/chats', { title: '합성 항구' });
  return { app, url, chat, directory };
}
const command = (chat: Chat, key = randomUUID()) => ({ request: '작은 항구에서 장면을 이어줘.', expectedRevision: chat.headRevision, expectedSettingsRevision: chat.settingsRevision, idempotencyKey: key });
...
const completed = (url: string, id: string) => until(() => api<Run>(url, `/api/runs/${id}`), run => run.status === 'completed');
...
describe('file SQLite HTTP runtime', () => {
  it('F02 fixes snapshots, rejects stale revisions, and deduplicates a logical command', async () => {
    ...
    const changed = await api<Chat>(url, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: 1, ...chat.settings, preset: 'vivid' }, 'PATCH');
    await api(url, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: 1, ...chat.settings }, 'PATCH', 409);
    ...
    const finished = await completed(url, run.id); await completed(url, independent.id);
  ...
  it('F03 replays durable event IDs and keeps errors out of source; F04 enforces call budget and cancellation', async () => {
    const { app, url, chat } = await setup();
    const configured = await api<Chat>(url, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: 1, ...chat.settings, mode: 'research', maxCalls: 2 }, 'PATCH');
    const run = await api<Run>(url, `/api/chats/${chat.id}/runs`, command(configured));
    const failed = await until(() => api<Run>(url, `/api/runs/${run.id}`), value => value.status === 'failed');
    expect(failed.error).toBe('Model call budget exhausted');
    expect(failed.usage.modelCalls).toBe(2); expect(failed.inputs).toHaveLength(2);
    ...
    const stream = await fetch(`${url}/api/chats/${chat.id}/events`, { headers: { 'Last-Event-ID': String(cursor) } });
    const reader = stream.body!.getReader();
    const chunk = new TextDecoder().decode((await reader.read()).value); await reader.cancel();
    expect(chunk).toContain('run.failed'); expect(chunk).not.toContain('run.queued');
    expect(chunk).toContain(`id: ${events.at(-1)!.seq}`);
    await control(url, { action: 'hold', barrier: 'run' });
    const cancelled = await api<Run>(url, `/api/chats/${chat.id}/runs`, command(configured));
    await until(() => control(url, { action: 'hold', barrier: 'run' }), value => value.waiting.run === 1);
  ...
  it('F05 rolls back source/run/job reservations together and honors snapshotted disabled jobs', async () => {
    ...
    const failed = await until(() => api<Run>(url, `/api/runs/${run.id}`), value => value.status === 'failed');
    expect(failed.error).toContain('source-transaction'); expect(failed.sourceRevision).toBeNull();
    const empty = await detail(url, chat);
    expect(empty.sources).toEqual([]); expect(empty.jobs).toEqual([]); expect(empty.chat.headRevision).toBeNull();
    const configured = await api<Chat>(url, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: 1, ...chat.settings, translation: false, status: false }, 'PATCH');
    const next = await api<Run>(url, `/api/chats/${chat.id}/runs`, command(configured)); await completed(url, next.id);
    expect((await detail(url, chat)).sources).toHaveLength(1); expect((await detail(url, chat)).jobs).toEqual([]);
  ...
  it('F05 keeps delayed jobs on original revisions and retries result transactions without duplicate effects', async () => {
    ...
    const failedDetail = await until(() => detail(url, chat), value => value.jobs.filter(job => job.kind === 'status').every(job => ['failed', 'completed'].includes(job.status)));
    const failed = failedDetail.jobs.find(job => job.status === 'failed')!;
    expect(failed.error).toContain('job-transaction'); expect(failed.result).toBeNull();
    expect(failedDetail.jobs.filter(job => job.kind === 'translation').every(job => job.status === 'running')).toBe(true);
    ...
    const retryAgain = await api(url, `/api/jobs/${failed.id}/retry`, {}); expect(retryAgain.attempt).toBe(2);
    ...
    const ownership = app.store.db.prepare('SELECT owner FROM jobs WHERE id=?').get(failed.id) as { owner: string };
  ...
  it('F01 refuses another owner before recovery; F06 disables controls and prevents cross-origin writes', async () => {
    ...
    await expect(createApp({ dbPath: join(directory, 'story.sqlite'), buildId: 'other' })).rejects.toThrow('running server owner');
    ...
    const denied = await fetch(`${url}/api/chats`, { method: 'POST', headers: { Origin: 'https://untrusted.invalid', 'Content-Type': 'application/json', Authorization: `Bearer ${canary}` }, body: JSON.stringify({ title: 'must not write' }) });
    expect(denied.status).toBe(403); expect(await denied.text()).not.toContain(canary); expect(app.store.chats()).toHaveLength(1);
    const unknownBody = await fetch(`${url}/api/chats`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: 'must not write', credential: canary }) });
    expect(unknownBody.status).toBe(400); expect(await unknownBody.text()).not.toContain(canary);
    const rebound = await app.inject({ method: 'GET', url: '/api/chats', headers: { Host: 'evil.test', Origin: 'http://evil.test' } });
  ...
  it('F04 reads scoped SQLite resources through actual research calls and records the next input', async () => {
    ...
    const configured = await api<Chat>(url, `/api/chats/${chat.id}/settings`, { expectedSettingsRevision: 1, ...chat.settings, mode: 'research' }, 'PATCH');
    const resourceRows = app.store.db.prepare('SELECT body FROM resources WHERE chat_id=?').all(chat.id);
    expect(resourceRows).toHaveLength(3);
    const run = await api<Run>(url, `/api/chats/${chat.id}/runs`, command(configured));
    const result = await completed(url, run.id);
    expect(result.inputs).toHaveLength(4); expect(result.usage.modelCalls).toBe(4);
    expect(result.inputs[0].prefetch).toEqual([]); expect(result.inputs[0].results).toEqual([]);
    expect(result.inputs[0].catalog.every(resource => !('text' in resource))).toBe(true);
    expect(result.toolEvents.map(event => event.name)).toEqual(['knowledge.search', 'knowledge.read', 'skills.load']);
...
async function startChild(directory: string) {
  const child = spawn(process.execPath, [resolve('dist/server/index.js')], { cwd: process.cwd(), env: { ...process.env, NR_DB: join(directory, 'story.sqlite'), NR_PORT: '0', NR_INSTANCE: randomUUID(), NR_TEST_MODE: '1' }, stdio: ['ignore', 'pipe', 'pipe'], windowsHide: true });
  const entry = { child, directory }; owned.push(entry);
  let stderr = '';
  child.stderr!.on('data', data => { stderr += String(data); });
  const ready = await new Promise<{ url: string }>((resolveReady, reject) => {
    let output = ''; const timer = setTimeout(() => reject(new Error(`Child ready timeout: ${stderr}`)), 10000);
    child.stdout!.on('data', data => {
      output += String(data);
      for (const line of output.split('\n')) {
        try { const parsed = JSON.parse(line); if (parsed.event === 'ready') { clearTimeout(timer); resolveReady(parsed); } } catch { /* Incomplete stdout line. */ }
      }
    });
...
describe('built server process boundary', () => {
  it('F03 F05 restarts after source commit before worker wake, without regenerating source', async () => {
    ...
    const recovered = await until(() => detail(second.url, chat), value => value.jobs.every(job => job.status === 'completed'));
    ...
    expect(result.status).toBe('interrupted'); expect(result.inputs).toHaveLength(0); expect(result.sourceRevision).toBeNull();
    expect((await detail(third.url, chat)).sources).toHaveLength(1);
    const stopped = new Promise<void>(resolveExit => third.child.once('exit', () => resolveExit())); third.child.kill(); await stopped;
    if (process.env.NR_ARTIFACT_DIR) {
      ...
      try {
        ...
        await writeFile(join(evidenceDirectory, 'commit-boundary-restart.json'), JSON.stringify({ exitAfterCommit: 86, beforeRestart, afterRestart, completedRunId: run.id, interruptedRunId: interrupted.id, limitations: 'Scripted mock lifecycle; no paid provider or power-loss durability claim.' }, null, 2));
