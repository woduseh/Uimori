...
const request = (): ProviderRequest => ({
  ...
  generation: { maxOutputTokens: 321, temperature: 0, reasoningEffort: 'low' },
  stable: {
    contract: 'Write the source story. Local sources grant no tools.',
    tools: [
      {
        name: 'knowledge.read',
        description: 'Read allowed context',
        inputSchema: {
          type: 'object',
          properties: {
            id: { type: 'string' },
      ...
      {
        name: 'knowledge_read',
        description: 'Different host tool with a colliding sanitized name',
  ...
  input: {
    task: 'Continue.',
    controls: { flag: false, offset: 0 },
    source: { revision: 'source-1', text: 'Scene.' },
    catalog: [],
    history: [],
    results: [],
  },
...
const first = (input = request()) => {
  ...
  return { ...encoded, decoder: new ResponsesDecoder(encoded.context) };
};
...
const call = (
  ...
  args = '{"id":"lore-1","include":false,"limit":0}'
): Json => ({
  type: 'function_call',
  id: 'fc-A',
  call_id: id,
...
const terminal = (output: Json[], usage?: Json, status = 'completed'): Json => ({
  type: `response.${status}`,
  response: { id: 'resp-A', status, output, ...(usage !== undefined ? { usage } : {}) },
});
...
describe('native Responses pure protocol (no live calls)', () => {
  test('keeps editable model, explicit zero/reasoning, optional tool semantics and collision-free aliases', () => {
    ...
    expect(
      JSON.parse(body.input[0].content[0].text.split('\n').slice(1).join('\n')).controls
    ).toEqual({ flag: false, offset: 0 });
    expect(input).toEqual(original);
    input.generation = undefined;
    input.stable.tools = [];
    const minimal = record(encodeResponses(input).body);
    expect(minimal).not.toHaveProperty('reasoning');
    expect(minimal).not.toHaveProperty('temperature');
    expect(minimal).not.toHaveProperty('tools');
  });
  ...
  test('assembles tool argument deltas and replays full signed reasoning/items with exact original call IDs', () => {
    ...
    for (const delta of ['{"id":"lore-1",', '"include":false,', '"limit":0}'])
      run.decoder.accept({
    ...
    run.decoder.accept(
      terminal([reasoning, item, secondCall], {
        input_tokens: 12,
        output_tokens: 7,
      ...
      toolCalls: [
        {
          id: 'Call-A.Original',
          name: 'knowledge.read',
          arguments: { id: 'lore-1', include: false, limit: 0 },
        },
        { id: 'Call-B.Original', name: 'knowledge.read' },
      ],
      usage: { inputTokens: 12, outputTokens: 7, costUsd: null },
    });
    ...
    expect(wire.input.slice(-2)).toEqual([
      { type: 'function_call_output', call_id: 'Call-A.Original', output: '0' },
      { type: 'function_call_output', call_id: 'Call-B.Original', output: 'false' },
    ]);
    expect(JSON.stringify(diagnosticResponsesBody(wire))).not.toMatch(
      /PRIVATE_SUMMARY|ENCRYPTED_REASONING/
    );
    expect(wire.input[1]).toEqual(reasoning);
    const third = first(continued);
    third.decoder.accept(
      terminal([message('Final story.')], { input_tokens: 0, output_tokens: 2 })
    );
    expect(third.decoder.finish()).toMatchObject({
      status: 'completed',
      text: 'Final story.',
  ...
  test('streamed message deltas and done snapshots do not duplicate visible text or lose annotations', () => {
    ...
    run.decoder.accept({
      type: 'response.output_item.added',
      output_index: 0,
      item: { type: 'message', id: 'msg-A', role: 'assistant', status: 'in_progress', content: [] },
    });
    run.decoder.accept({
    ...
    const item = record(message('등대 🌊 is lit.'));
    item.content[0].annotations = [{ type: 'future_annotation', value: 0 }];
    ...
    expect(run.decoder.finish().usage).toEqual({
      inputTokens: null,
      outputTokens: null,
      costUsd: null,
      raw: null,
  ...
  test('does not parse or execute truncated arguments on an incomplete response', () => {
    ...
    run.decoder.accept({
      ...
      response: {
        ...
        output: [
          message('Observed before cutoff'),
          { ...record(call()), arguments: '{"id":', status: 'incomplete' },
        ],
    ...
    expect(run.decoder.finish()).toMatchObject({
      status: 'partial',
      text: 'Observed before cutoff',
      toolCalls: [],
      error: { code: 'MAX_OUTPUT_TOKENS' },
      usage: { inputTokens: 5, outputTokens: 3 },
    });
    expect(() => encodeResponses(next(request(), run.decoder.finish()))).toThrow(
  ...
  test('rejects provider output item ID reuse independently from tool call IDs', () => {
    ...
    expect(() => second.decoder.accept(terminal([call('Different-Call-ID')]))).toThrow(
      'DUPLICATE_OUTPUT_ITEM_ID'
    );
  });
      ...
      input: [
        {
          role: 'assistant',
          content: null,
          reasoning: { content: 'PRIVATE_REASONING_OBJECT', encrypted_data: 'PRIVATE_ENCRYPTED' },
        },
      ],
    };
  ...
  test.each(['refusal', 'content_filter', 'max_output_tokens', 'empty'] as const)(
    'distinguishes terminal %s',
    (mode) => {
      const run = first();
      if (mode === 'refusal')
        run.decoder.accept(
        ...
        run.decoder.accept({
          ...
          response: {
            status: 'incomplete',
            output: [message('Observed')],
            incomplete_details: { reason: mode },
            usage: null,
          },
        });
      expect(run.decoder.finish()).toMatchObject(
        mode === 'refusal' || mode === 'content_filter'
          ? { status: 'refused' }
          : mode === 'empty'
            ? { status: 'error', error: { code: 'EMPTY_RESPONSE' } }
            : { status: 'partial', error: { code: 'MAX_OUTPUT_TOKENS' } }
      );
    }
  );
  ...
  test.each([
    ...
    'scalar-args',
    'invalid-json',
  ] as const)('rejects malformed tool %s without fallback', (mode) => {
    const run = first();
    const item = record(call());
  ...
  test.each(['task', 'source', 'history', 'stable', 'generation', 'role', 'model'] as const)(
  ...
  test.each([
    ...
    'tampered-state',
    'reused-id',
  ] as const)('rejects %s tool continuation', (mode) => {
    const input = request();
    const run = first(input);
    run.decoder.accept(terminal([call()]));
    const continued = next(input, run.decoder.finish());
    ...
    if (mode === 'reused-id') {
      ...
      expect(() =>
        second.decoder.accept(terminal([{ ...record(call()), id: 'fresh-output-item' }]))
  ...
  test('keeps every selected Responses option alongside the translation schema and signed continuation', () => {
    ...
    const reasoning = {
      type: 'reasoning',
      id: 'rs-options',
      encrypted_content: 'PRIVATE_CONTINUATION',
    };
    run.decoder.accept(terminal([reasoning, call()]));
    const continued = next(input, run.decoder.finish());
    const replay = record(encodeResponses(continued).body);
    expect(replay.input).toContainEqual(reasoning);
    expect(replay.reasoning).toEqual(wire.reasoning);
    expect(replay.text).toEqual(wire.text);
  ...
  test('omits unselected reasoning and verbosity while rejecting unsupported Astra effort', () => {
    ...
    expect(wire).not.toHaveProperty('reasoning');
  ...
  test.each(['explicit', 'automatic', 'disabled'] as const)(
    'Responses cache %s reaches requests without a native prompt and preserves translation formatting',
    (cacheMode) => {
      const input = request();
      input.modelId = 'gpt-5.6-sol';
      input.role = 'translation';
      input.generation = {
        ...
        temperature: null,
        cacheMode,
        ...(cacheMode === 'disabled' ? {} : { cacheTtl: '30m' as const }),
        verbosity: 'low',
      };
      input.input.source = {
      ...
      run.decoder.accept(terminal([call()]));
      const continued = next(input, run.decoder.finish());
      expect(record(encodeResponses(continued).body).prompt_cache_options).toEqual(
  ...
  test('never mistakes content disguised as a reasoning setting for safe diagnostics', () => {
    const body: Json = {
      ...
      nested: { reasoning: { mode: 'pro', context: 'PRIVATE_THOUGHT' } },
    };
  ...
  test('rejects swapped stream item IDs, response IDs, replayed sequence numbers and negative usage', () => {
    ...
    ).toThrow('INVALID_EVENT_SEQUENCE');
    expect(() =>
      run.decoder.accept({ type: 'response.in_progress', response: { id: 'response-2' } })
    ).toThrow('RESPONSE_ID_MISMATCH');
    const other = first();
    other.decoder.accept({
      type: 'response.output_item.added',
      output_index: 0,
      item: { ...record(call()), arguments: '' },
    });
    expect(() =>
      other.decoder.accept({
        type: 'response.function_call_arguments.delta',
    ...
    ).toThrow('OUTPUT_ITEM_MISMATCH');
    expect(() =>
      first().decoder.accept(terminal([message('text')], { input_tokens: -1, output_tokens: 1 }))
