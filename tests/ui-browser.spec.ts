...
test('UI03 UI12 failed starting profile read survives reload and recovers frozen prompt choices with current routes', async ({ page, request }) => {
  ...
  const modelResponse = await request.post('/api/model-presets', { data: { title: `${title} saved route`, connectionId: connection.id, modelId: 'synthetic-not-called', maxOutputTokens: 500, temperature: null } }); expect(modelResponse.ok()).toBeTruthy(); const model = await modelResponse.json() as { id: string; revision: number };
  ...
  const original = (await data(request, chat.id)).profile!; const currentRoutes = { ...original.routes, main: { id: model.id} };
...
async function startingModels(request: APIRequestContext, title: string) {
  ...
  for (const role of ['main', 'translation']) {
    const response = await request.post('/api/model-presets', { data: { title: `${title} ${role}`, connectionId: connection.id, modelId: `synthetic-${role}`, maxOutputTokens: 500, temperature: null } }); expect(response.ok()).toBeTruthy(); const model = await response.json() as { id: string; revision: number }; models.push({ id: model.id});
  }
...
test('UI03 UI12 starting model choices are saved without execution, reused exactly and dropped when their connection is disabled', async ({ page, request }, info) => {
  ...
  await page.getByLabel('시작 본문 모델').selectOption(`${models[0].id}`); await page.getByLabel('시작 번역 모델').selectOption(`${models[1].id}`);
  ...
  await nav(page, '새 이야기'); await expect(page.getByLabel('시작 본문 모델')).toHaveValue(`${models[0].id}`); await expect(page.getByLabel('시작 번역 모델')).toHaveValue(`${models[1].id}`);
  ...
  await expect(page.getByLabel('시작 본문 모델').locator(`option[value="${models[0].id}"]`)).toBeEnabled();
  await expect(page.getByLabel('시작 번역 모델').locator(`option[value="${models[1].id}"]`)).toBeEnabled();
  await expect(page.getByLabel('시작 본문 모델')).toHaveValue(`${models[0].id}`); await expect(page.getByLabel('시작 번역 모델')).toHaveValue(`${models[1].id}`); await close(page);
...
test('UI03 UI12 pending starting models recover after a failed profile read without overwriting newer model choices', async ({ page, request }) => {
  ...
  await page.goto('/'); await nav(page, '새 이야기'); await page.getByLabel('시작 본문 모델').selectOption(`${models[0].id}`); await page.getByLabel('시작 번역 모델').selectOption(`${models[1].id}`);
...
test('UI17 full writing and empty translation prompts import, save and apply without model execution', async ({ page, request }, info) => {
  ...
  await editor.getByLabel('메시지 본문 파일 불러오기', { exact: true }).setInputFiles({ name: 'UI17-full-main.md', mimeType: 'text/markdown', buffer: Buffer.from(literal) });
  await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue(literal); await editor.getByLabel('프롬프트 이름').fill('UI17-full-main');
  ...
  const main = await (await request.get(`/api/revisions/prompt-preset/${mainRef.id}/${mainRef.revision}`)).json(); expect(main.program).toEqual(createDefaultPromptProgram(literal, 'main')); expect(main.role).toBe('main');
  ...
  await editor.getByLabel('프롬프트 이름').fill('UI17 empty translation'); await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue('');
  await editor.getByRole('button', { name: '저장하고 이야기에 적용', exact: true }).click();
  ...
  const translation = await (await request.get(`/api/revisions/prompt-preset/${selected.id}/${selected.revision}`)).json(); expect(translation.program).toEqual(createDefaultPromptProgram('', 'translation')); expect(translation.role).toBe('translation');
  await editor.getByLabel('불러올 프롬프트').selectOption('builtin'); await expect(editor.getByLabel('메시지 본문', { exact: true })).not.toHaveValue('');
  ...
  await editor.getByLabel('프롬프트 역할').selectOption('main'); await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue(literal);
...
test('UI17 full prompt revisions stay pinned and conflict keeps the edited text', async ({ page, request }) => {
  ...
  const saved = await request.post('/api/prompt-presets', { data: { title: `UI17 archived ${Date.now()}`, role: 'main', program: createDefaultPromptProgram(originalText, 'main') } }); expect(saved.ok()).toBeTruthy(); const first = await saved.json();
  ...
  const latest = await request.put(`/api/prompt-presets/${first.id}`, { data: { expectedRevision: 1, title: first.title, role: 'main', program: createDefaultPromptProgram('Latest library writing prompt.', 'main') } }); expect(latest.ok()).toBeTruthy();
  ...
  const editor = dialog.getByTestId('prompt-editor'); await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue(originalText);
  const edited = 'Unsaved custom full prompt.\n' + 'Keep my edited text intact.\n'.repeat(30);
  await editor.getByLabel('메시지 본문', { exact: true }).fill(edited); await editor.getByLabel('프롬프트 역할').selectOption('translation'); await editor.getByLabel('프롬프트 역할').selectOption('main'); await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue(edited);
  await editor.getByRole('button', { name: '기존 프롬프트 수정 저장', exact: true }).click(); await expect(editor.getByRole('alert')).toContainText('다른 요청이 먼저 반영됐어요.'); await expect(editor.getByLabel('메시지 본문', { exact: true })).toHaveValue(edited);
  expect((await data(request, chat.id)).profile!.prompts!.main).toEqual({ id: first.id, revision: 1 });
  await editor.getByLabel('프롬프트 이름').fill('UI17 recovered copy'); await editor.getByRole('button', { name: '새 프롬프트로 저장', exact: true }).click(); await expect(editor.getByRole('status').filter({ hasText: '프롬프트를 저장했어요' })).toBeVisible();
  expect((await data(request, chat.id)).profile!.prompts!.main).toEqual({ id: first.id, revision: 1 });
  ...
  const library = await (await request.get('/api/library')).json(); const copy = library.promptPresets.find((item: { title: string }) => item.title === 'UI17 recovered copy'); expect(copy.program).toEqual(createDefaultPromptProgram(edited, 'main'));
  await libraryEditor.getByLabel('불러올 프롬프트').selectOption(`${copy.id}@${copy.revision}`); await libraryEditor.getByLabel('메시지 본문', { exact: true }).fill(edited + '\nRevised in library.');
