...
const modelInput=(connection:Connection,title:string)=>({title,connectionId:connection.id,connectionRevision:connection.revision,modelId:'synthetic-model',maxOutputTokens:8192,temperature:null,evaluationTools:defaultEvaluationToolOptions(),enabled:true});
...
test('PMUI01 mobile template registration selects the connection, reports catalog failure honestly and saves manual options',async({page,request},info)=>{
  ...
  await form.getByText('연결 템플릿 정보',{exact:true}).click();await expect(form.getByText(/openai-responses-v1 · v1/)).toBeVisible();await expect(form.getByText('2026-09-07',{exact:true})).toBeVisible();
...
test('PMUI10 Codex subscription login preserves drafts and saves a connection and model without generation',async({page,request},info)=>{
  ...
  const savedConnection=(await library(request)).connections.find(item=>item.title==='보존할 Codex 초안')!;expect(savedConnection).toMatchObject({protocol:'codex-app-server-v1',endpoint:'codex://local',enabled:true});expect(savedConnection.credentialEnv).toBeUndefined();expect(savedConnection.requestTier).toBeUndefined();
  ...
  const savedModel=(await library(request)).models.find(item=>item.title==='PMUI10 Codex 모델')!;expect(savedModel).toMatchObject({connectionId:savedConnection.id,connectionRevision:savedConnection.revision,modelId:'synthetic-codex-model',maxOutputTokens:2048,temperature:null});
  ...
  expect(mutations).toEqual(['POST /api/agent-runtimes/codex/login','POST /api/agent-runtimes/codex/login/cancel','POST /api/agent-runtimes/codex/login','DELETE /api/agent-runtimes/codex/session']);expect(observed.errors).toEqual([]);expect(observed.generations).toEqual([]);
});
