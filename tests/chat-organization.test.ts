...
test('schema v8 reopen keeps explicit organization revisions without automatic preservation files',async()=>{
  ...
  expect(reopened.db.prepare('PRAGMA user_version').get()).toMatchObject({user_version:8});expect(reopened.chat(chat.id)).toMatchObject({botId:bot.id,folderId:folder.id,organizationRevision:2});
