...
describe('bounded translation automatic retry', () => {
  test.each(['schema', 'coverage', 'dependency', 'refusal', 'protected', 'syntax'])('recovers %s with fresh state and records every attempt', async failure => {
    let count = 0;
    const server = await fixture(async (request, response) => {
      count++;
      const output = translationBody(request.body);
      const events: Json[] = [];
      if (count === 1) events.push({ type: 'tool_delta', index: 0, id: 'lookup', name: 'knowledge.read', argumentsDelta: '{"id":"old-glossary"}' }, { type: 'opaque_state', state: { private: 'previous-attempt' } }, { type: 'done', reason: 'tool_calls' });
      else if (count === 2 && failure === 'refusal') events.push({ type: 'refusal', message: 'terminal refusal' }, { type: 'done', reason: 'refusal' });
      else {
        ...
        events.push({ type: 'text_delta', delta: count === 2 && failure === 'schema' ? 'not JSON' : JSON.stringify(output) }, { type: 'usage', inputTokens: 7, outputTokens: 4, costUsd: null, raw: {} }, { type: 'done', reason: 'stop' });
      }
    ...
    expect(server.requests).toHaveLength(3); expect(observed.finishes).toHaveLength(3);
    expect(JSON.parse(server.requests[1].body).opaqueState).toEqual({ private: 'previous-attempt' });
    ...
    expect([...state.chunks.values()][0]).toMatchObject({ attempt: 2, status: 'completed', error: null });
  });

  test('stops at three invalid completed attempts', async () => {
    const server = await fixture(async (_request, response) => writeSse(response, [{ type: 'text_delta', delta: '{}' }, { type: 'done', reason: 'stop' }]));
    const seed = bundle(); selectProvider(seed, server.endpoint); const observed = hooks(server.origin); const state = bridge(seed);
    expect(await runAuxiliaryJob(state.store, seed.job.id, 'owner', observed.options)).toMatchObject({ status: 'failed' });
    expect(server.requests).toHaveLength(3); expect(observed.finishes).toHaveLength(3);
  ...
  test('all retries share the job call budget', async () => {
    const server = await fixture(async (_request, response) => writeSse(response, [{ type: 'text_delta', delta: '{}' }, { type: 'done', reason: 'stop' }]));
    const seed = bundle(); seed.snapshot.settings.maxCalls = 2; selectProvider(seed, server.endpoint); const observed = hooks(server.origin);
    expect(await runAuxiliaryJob(bridge(seed).store, seed.job.id, 'owner', observed.options)).toMatchObject({ status: 'failed', error: 'AUXILIARY_CALL_BUDGET_EXHAUSTED' });
    expect(server.requests).toHaveLength(2); expect(observed.finishes).toHaveLength(2);
  });

  test.each(['eof', 'refusal-eof', 'http', 'timeout'])('does not replay uncertain or transport failure %s', async failure => {
    const server = await fixture(async (_request, response) => {
      if (failure === 'timeout') { await new Promise(resolve => setTimeout(resolve, 100)); response.end(); return; }
      if (failure === 'http') { response.writeHead(503); response.end('unavailable'); return; }
      await writeSse(response, failure === 'refusal-eof' ? [{ type: 'refusal', message: 'unfinished refusal' }] : [{ type: 'text_delta', delta: '{"unfinished"' }]);
    });
    const seed = bundle(); selectProvider(seed, server.endpoint); const observed = hooks(server.origin);
    if (failure === 'timeout') observed.options.timeoutMs = 20;
    expect((await runAuxiliaryJob(bridge(seed).store, seed.job.id, 'owner', observed.options))?.status).toBe('failed');
    expect(server.requests).toHaveLength(1); expect(observed.finishes).toHaveLength(1);
  });

  test('cancellation after recorded response prevents replay', async () => {
    const server = await fixture(async (_request, response) => writeSse(response, [{ type: 'text_delta', delta: '{}' }, { type: 'done', reason: 'stop' }]));
    const seed = bundle(); selectProvider(seed, server.endpoint); const observed = hooks(server.origin); const abort = new AbortController(); observed.options.signal = abort.signal;
    const finish = observed.options.onAttemptFinish;
    observed.options.onAttemptFinish = async (id, result) => { await finish(id, result); abort.abort(); };
    expect(await runAuxiliaryJob(bridge(seed).store, seed.job.id, 'owner', observed.options)).toMatchObject({ status: 'cancelled', error: 'AUXILIARY_CANCELLED' });
    expect(server.requests).toHaveLength(1); expect(observed.finishes).toHaveLength(1);
  });

  test('store source ownership errors are never classified as invalid provider output', async () => {
    const server = await fixture(async (request, response) => writeSse(response, [{ type: 'text_delta', delta: JSON.stringify(translationBody(request.body)) }, { type: 'done', reason: 'stop' }]));
    const seed = bundle(); selectProvider(seed, server.endpoint); const state = bridge(seed);
    state.store.completeChunk = () => { throw new Error('SOURCE_DEPENDENCY_MISMATCH'); };
    expect(await runAuxiliaryJob(state.store, seed.job.id, 'owner', hooks(server.origin).options)).toMatchObject({ status: 'failed', error: 'SOURCE_DEPENDENCY_MISMATCH' });
    expect(server.requests).toHaveLength(1);
  });

  test.each(['status', 'image'] as const)('%s refusal still uses a single attempt', async kind => {
    const server = await fixture(async (_request, response) => writeSse(response, [{ type: 'refusal', message: 'terminal refusal' }, { type: 'done', reason: 'refusal' }]));
    const seed = bundle(); seed.job.kind = kind; selectProvider(seed, server.endpoint);
    expect(await runAuxiliaryJob(bridge(seed).store, seed.job.id, 'owner', hooks(server.origin).options)).toMatchObject({ status: 'failed', error: 'AUXILIARY_PROVIDER_REFUSED' });
    expect(server.requests).toHaveLength(1);
  });
  test('authorization denial never sends or retries a provider request', async () => {
    const seed = bundle(); selectProvider(seed, 'http://127.0.0.1:1/turn'); const observed = hooks(); let authorizations = 0;
    observed.options.authorize = () => { authorizations++; throw new Error('denied'); };
    expect(await runAuxiliaryJob(bridge(seed).store, seed.job.id, 'owner', observed.options)).toMatchObject({ status: 'failed', error: 'CONNECTION_NOT_AUTHORIZED' });
    expect(authorizations).toBe(1); expect(observed.wire).toEqual([]);
  });

  test.each(['EMPTY_COMPLETION', 'EMPTY_RESPONSE'] as const)('retries confirmed terminal %s over HTTP', async code => {
    let count = 0;
    const server = await fixture(async (request, response) => {
      count++;
      if (code === 'EMPTY_COMPLETION') {
        await writeSse(response, [...(count === 1 ? [] : [{ type: 'text_delta', delta: JSON.stringify(translationBody(request.body)) }]), { type: 'done', reason: 'stop' }]);
        return;
      }
    ...
    const seed = bundle(); selectProvider(seed, server.endpoint);
    if (code === 'EMPTY_RESPONSE') seed.snapshot.profile!.models.translation!.connection.protocol = 'openai-chat-v1';
    const observed = hooks(server.origin); const state = bridge(seed);
    expect(await runAuxiliaryJob(state.store, seed.job.id, 'owner', observed.options)).toMatchObject({ status: 'completed', error: null });
    expect(server.requests).toHaveLength(2); expect(observed.finishes).toHaveLength(2);
    expect(observed.finishes[0].result).toMatchObject({ status: 'error', error: { code } });
    expect([...state.chunks.values()][0]).toMatchObject({ status: 'completed', attempt: 2, error: null });
  });
