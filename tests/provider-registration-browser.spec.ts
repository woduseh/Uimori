...
test('PMUI05 natural language proposal is reviewed, survives reload, and applies once before role assignment',async({page,request},info)=>{
  ...
  await page.reload();await settings(page);await expect(assistant).toContainText('synthetic-created-model');await expect(apply).toBeVisible();
  await apply.click();await expect(assistant.getByRole('status')).toContainText('설정 목록에 저장했어요');
  const after=await getLibrary(request);expect(after.connections).toHaveLength(before.connections.length+1);expect(after.models).toHaveLength(before.models.length+1);
  const created=after.models.find(item=>!before.models.some(old=>old.id===item.id))!,createdConnection=after.connections.find(item=>item.id===created.connectionId)!;
  expect(created).toMatchObject({modelId:'synthetic-created-model',maxOutputTokens:512,source:{kind:'manual'}});expect(createdConnection.enabled).toBe(false);
  const repeated=await post<RegistrationView>(request,`/provider-management/registrations/${snapshot.id}/apply`,{expectedRevision:snapshot.revision,planHash:snapshot.planHash});expect(repeated.applied?.model.id).toBe(created.id);
  expect((await getLibrary(request)).models).toHaveLength(after.models.length);
  const stats=await(await request.get(new URL('stats',endpoint).href)).json();expect(stats.registrationCalls).toBe(1);
  await page.setViewportSize({width:1440,height:1000});await assistant.getByRole('region',{name:'모델 등록 제안'}).scrollIntoViewIfNeeded();await page.screenshot({path:info.outputPath('provider-registration-applied.png')});
...
test('PMUI06 rejected admission releases the request key while uncertain delivery reuses it without a second call',async({page,request})=>{
  ...
  let lost=true;await page.route('**/api/provider-management/registrations',async route=>{if(lost){lost=false;await route.fetch();await route.abort('failed');}else await route.continue();});
  await assistant.getByRole('button',{name:'설정안 제안 요청',exact:true}).click();await expect(assistant.getByRole('alert')).toBeVisible();const key=await page.evaluate(()=>sessionStorage.getItem('uimori.provider-registration.request-key'));expect(key).toBeTruthy();
  await assistant.getByRole('button',{name:'동일 요청 상태 확인',exact:true}).click();await expect(assistant.getByRole('button',{name:'검토한 연결·모델 등록 적용',exact:true})).toBeVisible();
  expect(await stats()).toBe(before+1);expect(await page.evaluate(()=>sessionStorage.getItem('uimori.provider-registration.request-key'))).toBe(key);
});
