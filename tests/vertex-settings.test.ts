...
import { VERTEX_GEMINI_MODEL_ID, VERTEX_GEMINI_MAX_OUTPUT_TOKENS, type Connection, type ModelPreset } from '../core/product.js';
...
afterEach(async () => {
  ...
  for (const item of owned.splice(0).reverse()) {
    ...
    if (isAbsolute(within) || within.startsWith('..') || !basename(target).startsWith('Uimori vertex settings ')) throw new Error('Refusing cleanup outside owned test directory');
    await rm(target, { recursive: true, force: true });
  }
});
async function application() {
  const directory = await mkdtemp(join(tmpdir(), 'Uimori vertex settings ')); const item: (typeof owned)[number] = { directory }; owned.push(item);
  item.app = await createApp({ dbPath: join(directory, 'story.sqlite'), buildId: 'vertex-settings-local', instanceId: randomUUID(), testMode: true });
  await item.app.ready(); return item.app;
}
async function request<T>(app: App, path: string, payload: unknown, status = 200, method: 'POST' | 'PUT' = 'POST'): Promise<T> {
...
describe('Vertex connection and model settings with file SQLite', () => {
  test('accepts ADC and named Bearer references, normalizes the global base path and rejects unsupported authority', async () => {
    ...
    const adc = await request<Connection>(app, '/connections', vertexConnection({ endpoint: endpoint + '/' }));
    expect(adc).toMatchObject({ protocol: 'vertex-gemini-v1', endpoint, enabled: true, catalog: [] }); expect(adc).not.toHaveProperty('credentialEnv');
    const bearer = await request<Connection>(app, '/connections', vertexConnection({ credentialEnv: 'NARRATIVE_PROVIDER_VERTEX_TEST' }));
    expect(bearer.credentialEnv).toBe('NARRATIVE_PROVIDER_VERTEX_TEST');
    for (const invalid of [
      endpoint.replace('https:', 'http:'), endpoint.replace('aiplatform.googleapis.com', 'example.invalid'),
      endpoint.replace('aiplatform.googleapis.com', 'aiplatform.googleapis.com:444'), endpoint.replace('/global/', '/us-central1/'),
  ...
  test('stores the selected model defaults and bounds while keeping fixture rows unchanged', async () => {
    ...
    const fixture = await request<Connection>(app, '/connections', { title: 'Synthetic fixture', protocol: 'fixture-sse-v1', endpoint: 'http://127.0.0.1:9/turn', enabled: false });
    const legacy = await request<ModelPreset>(app, '/model-presets', modelBody(fixture, { modelId: 'manual-fixture-id', maxOutputTokens: 200000, temperature: 0.6 }));
    expect(legacy).not.toHaveProperty('thinkingLevel'); expect(legacy).not.toHaveProperty('timeoutMs');
    expect(legacy).toMatchObject({ modelId: 'manual-fixture-id', maxOutputTokens: 200000, temperature: 0.6 });
  ...
  test('returns only the local Vertex support manifest without fetching or requiring credentials', async () => {
    const app = await application();
    const fetch = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => { throw new Error('No network expected for local support manifest'); });
  ...
  test('exports and restores optional settings and immutable snapshots, with every connection disabled and credential reference removed', async () => {
    const source = await application(); const product = source.store.product;
    const connection = await request<Connection>(source, '/connections', vertexConnection({ credentialEnv: 'NARRATIVE_PROVIDER_VERTEX_TEST' }));
    const model = await request<ModelPreset>(source, '/model-presets', modelBody(connection, { thinkingLevel: 'HIGH', timeoutMs: 123456 }));
    const fixture = await request<Connection>(source, '/connections', { title: 'Legacy fixture', protocol: 'fixture-sse-v1', endpoint: 'http://127.0.0.1:9/turn', credentialEnv: 'NARRATIVE_PROVIDER_FIXTURE_TEST', enabled: true });
    const oldModel = await request<ModelPreset>(source, '/model-presets', modelBody(fixture, { modelId: 'fixture-only' }));
    const chat = source.store.createChat('Synthetic Vertex snapshot'); const initial = product.profile(chat.id);
    const profile = product.updateProfile(chat.id, { expectedRevision: initial.revision, attachments: [], creative: initial.creative, routes: { main: ref(model), translation: ref(model), status: ref(oldModel), image: null }, image: false });
    const captured = product.snapshot(chat.id)!; const prompt = 'Synthetic snapshot, never sent to a provider';
    const run = source.store.createRun(chat.id, { request: prompt, expectedRevision: null, expectedSettingsRevision: chat.settingsRevision, expectedProfileRevision: profile.revision, idempotencyKey: randomUUID() }, current => ({
      chatId: chat.id, parentRevision: null, settingsRevision: current.settingsRevision, settings: current.settings, request: prompt, history: [], resources: [], profile: captured,
    } satisfies RunSnapshot)).run;
    await request<ModelPreset>(source, `/model-presets/${model.id}`, { ...modelBody(connection, { thinkingLevel: 'LOW', timeoutMs: 1 }), expectedRevision: model.revision }, 200, 'PUT');
    expect(source.store.run(run.id).snapshot.profile!.models.main).toMatchObject({ revision: 1, thinkingLevel: 'HIGH', timeoutMs: 123456 });
    ...
    expect(frozen.models.main!.connection).not.toHaveProperty('credentialEnv'); expect(frozen.models.status).not.toHaveProperty('thinkingLevel');
    expect(target.store.run(run.id).status).toBe('interrupted'); expect(target.store.db.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
  });
