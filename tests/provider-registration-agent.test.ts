...
import {
  runRegistrationAgent,
  type RegistrationAgentHooks,
} from '../server/provider-registration-agent.js';
...
const proposed = () => ({
  connection: {
    kind: 'new',
    draft: {
      title: 'Proposed local connection',
...
const context = (): Json => ({
  definitions: JSON.parse(JSON.stringify(PROVIDER_DEFINITIONS)),
  connections: [
    {
      id: 'existing',
      revision: 2,
      ...
      endpoint: 'PRIVATE_EXISTING_ENDPOINT',
      credentialEnv: 'PRIVATE_EXISTING_CREDENTIAL_REF',
      catalog: [{ text: 'PRIVATE_CATALOG_BODY' }],
    },
  ],
  models: [
    {
      ...
      modelId: 'saved/model',
      protocol: 'openai-chat-v1',
      endpoint: 'PRIVATE_MODEL_ENDPOINT',
      body: 'PRIVATE_MODEL_BODY',
    },
  ],
  story: 'PRIVATE_STORY_BODY',
});
type Target = ModelPreset & { connection: Connection };
function target(origin: string, protocol: Connection['protocol'] = 'fixture-sse-v1'): Target {
  return {
    id: 'selected',
    revision: 1,
    ...
    connection: {
      ...
      protocol,
      endpoint: `${origin}${protocol === 'fixture-sse-v1' ? '/turn' : '/v1'}`,
      credentialEnv: env,
      enabled: true,
      catalog: [],
      catalogError: null,
...
async function fixture(handler: (body: any, response: ServerResponse) => void | Promise<void>) {
  ...
  const server = await loopbackProvider(async (request, response) => {
    try {
      expect(request.headers.authorization).toBe('Bearer synthetic-registration-key');
...
function observe(origin: string, extra: Partial<RegistrationAgentHooks> = {}) {
  ...
  const hooks: RegistrationAgentHooks = {
...
const tool = (
  ...
  id = 'proposal',
  args: unknown = proposed()
): Json => ({ type: 'tool_delta', index: 0, id, name, argumentsDelta: JSON.stringify(args) });
const complete = (args: unknown = proposed()): Json[] => [
  { type: 'text_delta', delta: JSON.stringify(args) },
  { type: 'usage', inputTokens: 11, outputTokens: 7 },
  { type: 'done', reason: 'stop' },
];
const toolComplete = (events: Json[] = [tool()]): Json[] => [
  ...events,
  { type: 'usage', inputTokens: 11, outputTokens: 7 },
...
test('one proposal call records attempt before HTTP and sends only registration metadata', async () => {
  const server = await fixture(async (body, response) => {
    ...
    const proposalSchema = JSON.stringify(body.stable.tools[0]);
    expect(proposalSchema).toContain('approvalReasoningMode');
    expect(proposalSchema).toContain('economized');
    expect(body.input.controls).toEqual({ purpose: 'provider-registration', maxCalls: 1 });
    expect(body.input).not.toHaveProperty('source');
    expect(body.input).not.toHaveProperty('history');
    expect(JSON.stringify(body)).not.toMatch(/PRIVATE_|credentialEnvDefault/);
    expect(body.input.catalog.connections[0]).toEqual({
      id: 'existing',
      revision: 2,
      protocol: 'openai-chat-v1',
      title: 'Saved connection',
  ...
  const chosen = target(server.origin);
  const original = structuredClone(chosen);
  const log = observe(server.origin);
  expect(
    await runRegistrationAgent(chosen, '로컬 모델 등록안을 만들어 주세요.', log.hooks)
  ).toEqual({ status: 'ready', proposal: { normalized: proposed() }, error: null });
  expect(chosen).toEqual(original);
  expect(log.proposals).toEqual([proposed()]);
...
test('native Chat completed JSON preserves selected generation options and unknown usage', async () => {
  const server = await fixture(async (body, response) => {
    ...
    expect(body.tools[0].function.name).toContain('registration_propose');
    ...
    response.end(
      sse({
        ...
        choices: [
          {
            index: 0,
            delta: { role: 'assistant', content: JSON.stringify(proposed()) },
            finish_reason: 'stop',
          },
        ],
      }) + 'data: [DONE]\n\n'
  ...
  const chosen = target(server.origin, 'openai-chat-v1');
  chosen.reasoningEffort = 'high';
  const log = observe(server.origin);
  expect((await runRegistrationAgent(chosen, 'Prepare a proposal', log.hooks)).status).toBe(
    'ready'
  );
  expect(server.failures).toEqual([]);
  expect(server.requests).toHaveLength(1);
  expect(log.attempts[0].result!.usage).toMatchObject({
    inputTokens: null,
    outputTokens: null,
    costUsd: null,
  });
...
test('registration exposes only its proposal tool even on evaluation-enabled presets', async () => {
  for (const mode of ['proposal', 'artifact', 'local']) {
    const server = await fixture(async (body, response) => {
      expect(body.stable.tools).toHaveLength(1);
      expect(body.stable.tools[0].name).toBe('registration.propose');
      expect(body.input).not.toHaveProperty('source');
      await writeSse(
        response,
        toolComplete([
    ...
    const chosen = target(server.origin);
    chosen.evaluationTools = defaultEvaluationToolOptions();
    const original = structuredClone(chosen);
    const log = observe(server.origin);
    const result = await runRegistrationAgent(chosen, 'Prepare a proposal', log.hooks);
    expect(server.failures).toEqual([]);
    expect(result.status, JSON.stringify(result)).toBe(mode === 'proposal' ? 'ready' : 'failed');
    expect(log.proposals).toHaveLength(mode === 'proposal' ? 1 : 0);
    expect(chosen).toEqual(original);
    expect(server.requests).toHaveLength(1);
    expect(server.failures).toEqual([]);
    expect(JSON.stringify(log.attempts)).not.toContain('PRIVATE_NOTICE');
  }
});
    ...
    const server = await fixture(async (_body, response) => {
      if (mode === 'unknown') await writeSse(response, toolComplete([tool('knowledge.read')]));
        ...
        await writeSse(
          ...
          toolComplete([
            tool(),
            { ...(tool('registration.propose', 'second') as object), index: 1 } as Json,
          ])
        );
        ...
        await writeSse(response, complete({ ...proposed(), ignored: 'x'.repeat(32000) }));
      else if (mode === 'enabled') {
        const plan = proposed();
        plan.connection.draft.enabled = true;
        await writeSse(response, complete(plan));
      } else {
        response.writeHead(200, { 'content-type': 'text/event-stream' });
        response.end(sse({ type: 'text_delta', delta: JSON.stringify(proposed()) }));
      }
    });
    const log = observe(server.origin);
    expect(
      (await runRegistrationAgent(target(server.origin), 'Prepare a proposal', log.hooks)).status
    ).toBe('failed');
...
test('initial disabled/preloaded/invalid requests and failed attempt persistence cannot send HTTP', async () => {
  ...
  expect((await runRegistrationAgent(chosen, 'Prepare', log.hooks)).error).toBe(
    'REGISTRATION_MODEL_DISABLED'
  );
  chosen.enabled = true;
  chosen.connection.enabled = false;
  expect((await runRegistrationAgent(chosen, 'Prepare', log.hooks)).error).toBe(
    'CONNECTION_NOT_AUTHORIZED'
  );
  chosen.connection.enabled = true;
  for (const request of ['', ' ', 'x'.repeat(6001)])
    expect((await runRegistrationAgent(chosen, request, log.hooks)).error).toBe(
      'REGISTRATION_REQUEST_INVALID'
    );
  expect(
    (await runRegistrationAgent(chosen, 'Prepare', { ...log.hooks, timeoutMs: 60001 })).error
  ).toBe('REGISTRATION_TIMEOUT_INVALID');
  expect(
    (
      await runRegistrationAgent(chosen, 'Prepare', {
        ...log.hooks,
        onAttemptStart: () => {
          throw new Error('PRIVATE_PERSISTENCE_ERROR');
        },
      })
    ).error
  ).toBe('REGISTRATION_ATTEMPT_START_FAILED');
  expect(server.requests).toHaveLength(0);
  expect(log.proposals).toHaveLength(0);
});
...
test('authority is checked again after journal admission and after the model response', async () => {
  for (const mode of ['before-fetch', 'after-response']) {
    ...
    const server = await fixture(async (_body, response) => {
      allowed = false;
      await writeSse(response, complete());
    });
    const log = observe(server.origin, {
      authorize: (connection) => ({ ...connection, enabled: allowed }),
    });
    if (mode === 'before-fetch') {
    ...
    expect(result.error).toBe('CONNECTION_NOT_AUTHORIZED');
    expect(log.proposals).toHaveLength(0);
    expect(log.attempts).toHaveLength(1);
    expect(server.requests).toHaveLength(mode === 'before-fetch' ? 0 : 1);
    expect(server.failures).toEqual([]);
  }
});
...
test('real HTTP timeout and user cancellation cannot produce a proposal or a replay', async () => {
  for (const mode of ['timeout', 'cancel']) {
    ...
    const server = await fixture((_body, response) => {
      response.writeHead(200, { 'content-type': 'text/event-stream' });
      response.write(sse({ type: 'text_delta', delta: '{' }));
      if (mode === 'cancel') controller.abort('PRIVATE_ABORT');
    });
    ...
    expect(server.requests).toHaveLength(1);
    expect(log.attempts).toHaveLength(1);
    expect(JSON.stringify(result)).not.toContain('PRIVATE_ABORT');
...
test('normalization and journal-finalization failures preserve one attempt but cannot return ready', async () => {
  for (const mode of ['normalize', 'journal']) {
    ...
    const log = observe(server.origin);
    if (mode === 'normalize')
      log.hooks.onProposal = () => {
        throw new Error('PRIVATE_VALIDATION_ERROR');
      };
    else
      log.hooks.onAttemptFinish = () => {
        throw new Error('PRIVATE_JOURNAL_ERROR');
      };
    const result = await runRegistrationAgent(target(server.origin), 'Prepare', log.hooks);
    expect(result.error).toBe(
      mode === 'normalize' ? 'REGISTRATION_PROPOSAL_INVALID' : 'REGISTRATION_ATTEMPT_FINISH_FAILED'
    );
