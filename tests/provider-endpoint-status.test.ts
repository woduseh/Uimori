...
beforeEach(() => {
  ...
  vi.spyOn(GoogleAuth.prototype, 'getClient').mockRejectedValue(
    new Error('Unexpected credential resolution')
  );
});
afterEach(async () => {
  ...
  for (const item of owned.splice(0).reverse()) {
    ...
    if (
      isAbsolute(within) ||
      within.startsWith('..') ||
      !basename(target).startsWith('uimori-endpoint-status-')
    )
...
const post = (app: App, payload: Record<string, unknown>, cookie?: string) =>
  injectWithFixtureBot(app, {
    method: 'POST',
    url: '/api/provider-management/endpoint-status',
    headers: { host: '127.0.0.1', ...(cookie ? { cookie } : {}) },
    payload,
  });
const settings = (app: App) =>
  app.store.db.prepare('SELECT * FROM provider_settings ORDER BY id').all();
...
test('official endpoints are approved without environment configuration or saving the draft', async () => {
  ...
  expect(app.store.db.prepare('SELECT COUNT(*) AS n FROM provider_connection_tests').get()).toEqual(
    { n: 0 }
  );
});
...
test('custom origins distinguish configured and needs-approval without mutating saved settings', async () => {
  ...
  const saved = app.store.product.connection({
    title: 'Existing custom connection',
    protocol: 'openai-chat-v1',
    endpoint: 'https://configured.synthetic.invalid/v1',
    enabled: true,
  }) as Connection;
  ...
  for (const [origin, status] of [
    ['https://configured.synthetic.invalid', 'configured'],
    ['https://unapproved.synthetic.invalid', 'needs-approval'],
  ]) {
    const response = await post(app, { protocol: 'openai-chat-v1', endpoint: `${origin}/v1` });
    expect(response.statusCode, response.body).toBe(200);
    expect(response.json()).toEqual({ status, origin });
  }
  expect(settings(app)).toEqual(before);
});
...
test('malformed and protocol-mismatched endpoints cannot report approval even for configured origins', async () => {
  ...
  for (const payload of [
    ...
    { protocol: 'vertex-gemini-v1', endpoint: 'https://aiplatform.googleapis.com/v1' },
    {
      protocol: 'vertex-gemini-v1',
      endpoint:
        'https://us-central1-aiplatform.googleapis.com/v1/projects/synthetic-project/locations/us-central1/publishers/google/models',
    },
  ]) {
    const response = await post(app, payload);
    expect(response.statusCode, response.body).toBe(200);
    expect(response.json()).toEqual({ status: 'invalid', origin: null });
  }
...
test('unsupported protocols and invalid request fields return 400', async () => {
  ...
  for (const payload of [
    {},
    { ...official[0], protocol: 'unsupported' },
    { ...official[0], endpoint: 42 },
    { ...official[0], endpoint: '' },
    { ...official[0], credentialEnv: 'UNREQUESTED_SECRET' },
  ]) {
...
test('the local Codex endpoint does not require an HTTP origin', async () => {
  const app = await setup();
  const response = await post(app, { protocol: 'codex-app-server-v1', endpoint: 'codex://local' });
  expect(response.statusCode).toBe(200);
  expect(response.json()).toEqual({ status: 'local', origin: null });
});
...
test('saved official connections report originApproved with an empty configured origin list', async () => {
  ...
  for (const payload of official) {
    ...
    const response = await injectWithFixtureBot(app, {
      method: 'GET',
      url: `/api/provider-management/connections/${connection.id}/readiness`,
      headers: { host: '127.0.0.1' },
    });
    expect(response.statusCode, response.body).toBe(200);
    expect(response.json()).toMatchObject({ originApproved: true });
    expect(settings(app)).toEqual(before);
  }
});

test('endpoint inspection requires a session when application authentication is enabled', async () => {
  const token = 'synthetic-endpoint-access-token-123456';
  ...
  expect(response.statusCode, response.body).toBe(200);
  expect(response.json().status).toBe('official');
  expect(settings(app)).toEqual([]);
});
