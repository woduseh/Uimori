...
async function fixture(handler: (body: Body, response: ServerResponse, requestNumber: number) => void | Promise<void>, settings: { timeoutMs?: number; maximumToolRounds?: number; maxCalls?: number } = {}) {
  ...
  const provider = await loopbackProvider(async (captured, response) => {
    try {
      expect(captured.url).toBe('/v1/responses'); expect(captured.headers.authorization).toBe(`Bearer ${bearer}`);
      ...
      expect(attempts).toHaveLength(provider.requests.length); expect(attempts.filter(attempt => attempt.status === 'running')).toHaveLength(1);
  ...
  const model = await api<ModelPreset>(app, '/api/model-presets', { title: 'Synthetic evaluated model', connectionId: connection.id, modelId: 'synthetic-evaluated-model', maxOutputTokens: 4096, temperature: null, timeoutMs: settings.timeoutMs ?? 4000, evaluationTools: { ...defaultEvaluationToolOptions(), maximumToolRounds: settings.maximumToolRounds ?? 8 } });
  const prior = await api<ChatProfile>(app, `/api/chats/${chat.id}/profile`);
  const profile = await api<ChatProfile>(app, `/api/chats/${chat.id}/profile`, { expectedRevision: prior.revision, attachments: [ref(lore)], creative: prior.creative, routes: { main: {id:model.id}, translation: {id:model.id}, status: null, image: null }, image: false }, 'PUT');
...
async function send(target: ServerResponse, output: Json[], json = false, status = 'completed') {
  ...
  if (json) { target.writeHead(200, { 'content-type': 'application/json' }); target.end(JSON.stringify(payload)); }
...
async function settled(state: Awaited<ReturnType<typeof fixture>>, id: string): Promise<Run> {
  await expect.poll(async () => (await api<Run>(state.app, `/api/runs/${id}`)).status, { timeout: 6000 }).not.toMatch(/^(queued|running)$/);
  expect(state.failures).toEqual([]); return api<Run>(state.app, `/api/runs/${id}`);
}
...
test('preset evaluation mixes permitted reads and local tools; buffered Responses translation keeps source/hash and per-request attempts', async () => {
  ...
  const before = await state.detail(); expect(before.jobs).toEqual([]); expect(state.provider.requests).toHaveLength(2);
  const source = before.sources[0]; expect(source).toMatchObject({ text: sourceText, hash: createHash('sha256').update(sourceText).digest('hex'), runId: run.id });
  await api(state.app, `/api/sources/${source.id}/translation`, {});
  await expect.poll(async () => (await state.detail()).jobs[0]?.status, { timeout: 6000 }).toBe('completed');
  const after = await state.detail(); expect(state.failures).toEqual([]); expect(state.provider.requests).toHaveLength(4);
...
test('current connection enabled flag and credential availability are rechecked between evaluation rounds', async () => {
  for (const revoke of ['enabled', 'credential']) {
    ...
    const run = await state.start(); await received.promise;
    if (revoke === 'enabled') await api(state.app, `/api/connections/${state.connection.id}`, { title: state.connection.title, protocol: state.connection.protocol, endpoint: state.connection.endpoint, credentialEnv, enabled: false, expectedRevision: state.connection.revision }, 'PUT');
    else vi.stubEnv(credentialEnv, '');
    gate.release(); const result = await settled(state, run.id);
    expect(result.status).toBe('failed'); expect(result.sourceRevision).toBeNull(); expect(state.provider.requests).toHaveLength(1);
...
test('HTTP failure and partial terminal output finish without provider replay or source commit', async () => {
  for (const mode of ['http', 'partial']) {
    const state = await fixture(async (body, target) => {
      if (mode === 'http') { target.writeHead(503, { 'content-type': 'application/json' }); target.end('{"error":"synthetic"}'); }
      else await send(target, [artifact(body, 'must not commit')], false, 'incomplete');
    });
    const result = await settled(state, (await state.start()).id); expect(result.status).not.toBe('completed');
    const detail = await state.detail(); expect(detail.sources).toEqual([]); expect(detail.attempts).toHaveLength(1); expect(state.provider.requests).toHaveLength(1);
...
test('both host maxCalls and evaluation maximumToolRounds bound actual HTTP requests', async () => {
  for (const settings of [{ maxCalls: 1, maximumToolRounds: 8 }, { maxCalls: 8, maximumToolRounds: 0 }]) {
    ...
    const result = await settled(state, (await state.start()).id);
    expect(result).toMatchObject({ status: 'failed', error: 'MODEL_CALL_BUDGET_EXHAUSTED', sourceRevision: null, usage: { modelCalls: 1 } });
    expect(state.provider.requests).toHaveLength(1); expect((await state.detail()).sources).toEqual([]);
  }
});
...
test('duplicate call IDs and terminal plus host calls cannot commit or dispatch a skipped host operation', async () => {
  for (const mode of ['duplicate', 'mixed']) {
    const state = await fixture(async (body, target) => {
      ...
      const other = mode === 'mixed' ? call(body, 'knowledge.read', { id: state.lore.id }, 'host') : { ...first as object, id: 'different-output-id' } as Json;
      await send(target, [first, other]);
    });
    const result = await settled(state, (await state.start()).id); expect(result.status).toBe('failed'); expect(result.toolEvents).toEqual([]);
    expect((await state.detail()).sources).toEqual([]); expect(state.provider.requests).toHaveLength(1); expect(JSON.stringify(await state.detail())).not.toContain(marker);
  }
});
...
test('source edit CAS fences an in-flight evaluated translation without mutating the original source or run snapshot', async () => {
  ...
  const state = await fixture(async (body, target) => {
    ...
    else { received.release(); await gate.promise; await send(target, [artifact(body, translated(body))], true); }
  });
  state.owner.release = gate.release;
  const run = await settled(state, (await state.start()).id); const before = await state.detail(); const original = before.sources[0];
  await api(state.app, `/api/sources/${original.id}/translation`, {}); await received.promise;
  const edited = await api<Source>(state.app, `/api/sources/${original.id}/text`, { text: 'Edited fixture source.', expectedRevision: 0 }, 'PUT');
  const stale = await state.app.inject({ method: 'PUT', url: `/api/sources/${original.id}/text`, headers: { host: '127.0.0.1' }, payload: { text: 'Stale overwrite', expectedRevision: 0 } }); expect(stale.statusCode).toBe(409);
  gate.release(); await expect.poll(async () => (await state.detail()).attempts!.filter(attempt => attempt.status === 'running').length).toBe(0);
  expect(sourceContent(state.app.store.sourceOriginal(original.id))).toEqual(sourceContent(original)); expect(state.app.store.run(run.id).snapshot).toEqual(run.snapshot);
  expect(edited.hash).not.toBe(original.hash); expect((await state.detail()).sources[0].text).toBe('Edited fixture source.');
  expect((await state.detail()).jobs.some(job => job.status === 'completed')).toBe(false); expect(state.provider.requests).toHaveLength(2); expect(state.failures).toEqual([]);
});
