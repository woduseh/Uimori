...
// Upstream sources: src/ts/cbs.ts:52-2504
// Copyright (c) Kwaroran and the RisuAI contributors. Licensed under GPL-3.0-only; see ./LICENSE.
// Modifications for Uimori:
//   - @ts-nocheck: upstream cbs.ts is written against RisuAI's non-strict settings (it indexes possibly
//     null `vars`/`matcherArg.var`, builds untyped object literals and narrows `character|groupChat` by
//     hand). Uimori's tsconfig is `strict`, and typing all ~150 callbacks would mean rewriting Risu code,
//     so the file is excluded from checking. The exported types below are still declared by hand and are
//     checked at every call site outside this file.
//   - Imports: `Database`/`character`/`loreBook`/`RisuModule`/`LLMModel` now come from ./types.js and
//     ./cbs-support.js, `CbsConditions` from ./cbs-parser.js. The `svelte/store` `get` import and the
...
//   - Node's `Buffer` in {{file}}, {{xor}} and {{xordecrypt}} is replaced by base64ToUtf8 /
//     bytesToBase64 / base64ToBytes from ./cbs-support.js. Same bytes, no Node built-in.
//   - No other evaluation semantics were changed: name normalisation, argument splitting, truthiness,
//     'null' fallbacks, the display (`matcherArg.displaying`) branches and every description string are
//     as upstream.
// @ts-nocheck

import type { RisuModule, character, loreBook } from './types.js';
import type { CbsConditions } from './cbs-parser.js';
import type { Database, LLMModel } from './cbs-support.js';
import { base64ToBytes, base64ToUtf8, bytesToBase64 } from './cbs-support.js';
...
export type CBSRegisterArg = {
    ...
    isMobile: boolean,
    appVer: string,
    // Uimori additions, see the file header.
    getTriggerId: () => string,
    now: () => number,
    random: () => number,
...
export function registerCBS(arg:CBSRegisterArg) {
    ...
    registerFunction({
        ...
        callback: (str, matcherArg, args, vars) => {
            ...
            return (Math.floor(randomValue() * (max - min + 1)) + min).toString()
        },
        alias: [],
    ...
    registerFunction({
        ...
        callback: (str, matcherArg, args, vars) => {
            ...
            let total = 0
            for(let i = 0; i < num; i++){
                total += Math.floor(randomValue() * sides) + 1
            }
    ...
    registerFunction({
        ...
        callback: (str, matcherArg, args, vars) => {
            ...
            switch(args[0].toLocaleLowerCase()){
                ...
                case 'browserlocale':
                case 'browserlang':{
                    return unsupported('browserlanguage')
                }
                case 'modelshortname':{
    ...
    registerFunction({
        name: 'xordecrypt',
        callback: (str, matcherArg, args, vars) => {
            const buf = base64ToBytes(args[0])
            for(let i = 0; i < buf.length; i++){
                buf[i] ^= 0xFF
            }
    ...
    registerFunction({
        name: 'random',
        callback: (str, matcherArg, args, vars) => {
            return randomPickImpl(str, matcherArg, args, randomValue())
        },
        alias: [],
